...
| **D19** | **Obsolete backward-compat code** that accumulates without a removal deadline. Three sub-cases: (a) `# DEAD CODE` comment markers on legacy branches that a typed pragma / @lowering / native pass supersedes — current count pinned, future markers MUST motivate; (b) **legacy `_lower_<op>` helper functions** in `lowerings/__init__.py` — count is strictly monotonically decreasing; B-PRs MUST NOT add new helpers (new per-op lowering code goes in `lowerings/lower_mir_<op>.py`); Layer 3 cleanup PRs decrement the cap atomically as they delete superseded helpers (a LOC ratchet was tried first but proved noisy — every B-PR bumped it ~10 LOC for dispatch wiring before any helper deletion); (c) the `USE_DECLARATIVE` ratchet coverage — non-strict monotonic floor today; once it reaches 100% chain-eligible MIR ops, the legacy fall-through `raise ValueError('unsupported inner op')` becomes unreachable and MUST be deleted in the same commit (gated by the completeness test). | Without a deadline-bound ratchet on transitional code, the "this gets deleted in Layer 3" intent silently becomes permanent debt — the original sin this redesign reverses. D18 covers mutation shims; D19 generalizes the same discipline to the OTHER transitional patterns (DEAD CODE markers + legacy helper count + USE_DECLARATIVE completeness). | `test_discipline_obsolete_code_ratchet.py` — three tests: (i) `test_dead_code_markers_pinned_exact` (strict ratchet, cap == count); (ii) `test_legacy_helper_function_count_strict_ratchet` (cap on `^def _lower\w*` count in the monolith, decrement-only); (iii) `test_use_declarative_chain_coverage_monitor` + `test_use_declarative_completeness_gate_layer3` (floor + Phase-B-done gate). |
