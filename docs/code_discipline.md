...
| **D19** | **Obsolete backward-compat code** that accumulates without a removal deadline. Three sub-cases: (a) `# DEAD CODE` comment markers on legacy branches that a typed pragma / @lowering / native pass supersedes — current count pinned, future markers MUST motivate; (b) the legacy `lowerings/__init__.py` monolith — LOC ratchet pinned, MUST shrink as Phase B migrations land; (c) the `USE_DECLARATIVE` ratchet coverage — non-strict monotonic floor today; once it reaches 100% chain-eligible MIR ops, the legacy fall-through `raise ValueError('unsupported inner op')` becomes unreachable and MUST be deleted in the same commit (gated by the completeness test). | Without a deadline-bound ratchet on transitional code, the "this gets deleted in Layer 3" intent silently becomes permanent debt — the original sin this redesign reverses. D18 covers mutation shims; D19 generalizes the same discipline to the OTHER transitional patterns (DEAD CODE markers + monolith LOC + USE_DECLARATIVE completeness). | `test_discipline_obsolete_code_ratchet.py` — three tests: (i) `test_dead_code_markers_pinned_exact` (strict ratchet, cap == count); (ii) `test_lowerings_monolith_loc_ratchet` (cap on file LOC); (iii) `test_use_declarative_chain_coverage_monitor` + `test_use_declarative_completeness_gate_layer3` (floor + Phase-B-done gate). |
