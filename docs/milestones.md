...
| **S4.7** | Discipline test pinning RawString count at 0 (or a documented small N for inherently-target-specific intrinsics that don't have a structured form yet). New RawString uses caught in CI. | ✅ — cap is now **0**. Cat J uses `iir.cf.UserCode`; Cat K uses `iir.cf.Bind(..., inline_comment=...)`. |
| **S4.8** | Now-tractable: formalize R1–R5 from `ir_lowering_semantics.md` §11 as `Rewrite` instances on `sorted_array.R` (S3A.5 from the original Stage 3A scope, deferred here). | **Deferred — needs Stage 3B prerequisite.** R1–R5 are MIR-level rewrites; MIR ops are still plain `@dataclass`, not framework `Op` subclasses, so `apply_rewrites_to_fixpoint` cannot walk them. R1–R5 are also currently implemented imperatively as scattered branches in `lower_scan_pipeline` — extracting them as pure rewrites is a major refactor. Pick up after Stage 3B promotes MIR onto `Op`. |
| **S4.9** | Implement `apply_rewrites_to_fixpoint` and `verify_renderability` in `PassDriver.run`, per the per-op contract in [`ir_dialect_contract.md`](./ir_dialect_contract.md). Promotes `@rewrite` from inert metadata to a real transform pass. | ✅ — driver now runs rewrites bottom-up + repeat to fixpoint; `verify_renderability` wired into `compile_kernel_body` so the contract self-enforces in CI; first consumer is S4.6b's `DedupTryInsert`. |
