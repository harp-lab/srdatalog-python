...
AddressOf = Relation("AddressOf", 2, column_types=(int, int,), input_file="addressOf.csv")
Assign = Relation("Assign", 2, column_types=(int, int,), input_file="assign.csv")
Load = Relation("Load", 2, column_types=(int, int,), input_file="load.csv")
Store = Relation("Store", 2, column_types=(int, int,), input_file="store.csv")
PointsTo = Relation("PointsTo", 2, column_types=(int, int,), print_size=True)
...
def build_andersendb_program() -> Program:
  ...
  return Program(
    ...
    rules=[
      ...
      (PointsTo(y, x) <= PointsTo(z, x) & Assign(y, z)).named('Assign').with_plan(var_order=['z', 'x', 'y']),
      (PointsTo(y, w) <= PointsTo(x, z) & Load(y, x) & PointsTo(z, w)).named('Load').with_plan(delta=0),
      (PointsTo(z, w) <= PointsTo(y, z) & Store(y, x) & PointsTo(x, w)).named('Store').with_plan(delta=0),
    ],
  )
