...
AssignInput = Relation("AssignInput", 2, column_types=(int, int,), input_file="Assign.csv")
DerefInput = Relation("DerefInput", 2, column_types=(int, int,), input_file="Dereference.csv")
Assign = Relation("Assign", 2, column_types=(int, int,))
Deref = Relation("Deref", 2, column_types=(int, int,))
ValueFlow = Relation("ValueFlow", 2, column_types=(int, int,), print_size=True)
ValueAlias = Relation("ValueAlias", 2, column_types=(int, int,), print_size=True)
MemoryAlias = Relation("MemoryAlias", 2, column_types=(int, int,), print_size=True)
...
def build_cspa_db_program() -> Program:
  ...
  return Program(
    ...
    rules=[
      ...
      (ValueFlow(x, y) <= ValueFlow(x, z) & ValueFlow(z, y)).named('VF_Trans').with_plan(var_order=['z', 'x', 'y']),
      (ValueFlow(x, y) <= Assign(x, z) & MemoryAlias(z, y)).named('VF_Assign_MA').with_plan(var_order=['z', 'x', 'y']),
      (ValueAlias(x, y) <= ValueFlow(z, x) & ValueFlow(z, y)).named('VA_VF').with_plan(var_order=['z', 'x', 'y']),
      (ValueAlias(x, y) <= ValueFlow(z, x) & MemoryAlias(z, w) & ValueFlow(w, y)).named('VA_VF_MA_VF').with_plan(var_order=['z', 'w', 'x', 'y']),
      (MemoryAlias(x, w) <= Deref(y, x) & ValueAlias(y, z) & Deref(z, w)).named('MA_Deref_VA').with_plan(var_order=['y', 'z', 'x', 'w']),
    ],
  )
