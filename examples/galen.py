...
PInput = Relation("PInput", 2, column_types=(int, int,), input_file="P.csv")
QInput = Relation("QInput", 3, column_types=(int, int, int,), input_file="Q.csv")
RInput = Relation("RInput", 3, column_types=(int, int, int,), input_file="R.csv")
CInput = Relation("CInput", 3, column_types=(int, int, int,), input_file="C.csv")
UInput = Relation("UInput", 3, column_types=(int, int, int,), input_file="U.csv")
SInput = Relation("SInput", 2, column_types=(int, int,), input_file="S.csv")
OutP = Relation("OutP", 2, column_types=(int, int,), print_size=True)
OutQ = Relation("OutQ", 3, column_types=(int, int, int,), print_size=True)
...
def build_galendb_program() -> Program:
  ...
  return Program(
    ...
    rules=[
      ...
      (OutP(x, z) <= OutP(y, w) & UInput(w, r, z) & OutQ(x, r, y)).named('Join3a').with_plan(var_order=['y', 'w', 'r', 'x', 'z']),
      (OutP(x, z) <= CInput(y, w, z) & OutP(x, w) & OutP(x, y)).named('Join3b'),
      (OutQ(x, q, z) <= OutQ(x, r, z) & SInput(r, q)).named('PropQS'),
      (OutQ(x, e, o) <= OutQ(x, y, z) & RInput(y, u, e) & OutQ(z, u, o)).named('Join3c').with_plan(var_order=['z', 'y', 'u', 'x', 'e', 'o']),
    ],
  )
