...
IsLocatedInInput = Relation("IsLocatedInInput", 2, column_types=(int, int,), input_file="Person_isLocatedIn_City.csv")
IsPartOfInput = Relation("IsPartOfInput", 2, column_types=(int, int,), input_file="City_isPartOf_Country.csv")
Knows = Relation("Knows", 2, column_types=(int, int,))
IsLocatedIn = Relation("IsLocatedIn", 2, column_types=(int, int,))
IsPartOf = Relation("IsPartOf", 2, column_types=(int, int,))
Triangle = Relation("Triangle", 3, column_types=(int, int, int,), print_size=True)
...
def build_lsqb_q3_db_program() -> Program:
  ...
  return Program(
    ...
    rules=[
      ...
      (Triangle(p1, p2, p3) <= Knows(p1, p2) & Knows(p2, p3) & Knows(p3, p1) & IsLocatedIn(p1, c1) & IsLocatedIn(p2, c2) & IsLocatedIn(p3, c3) & IsPartOf(c1, co) & IsPartOf(c2, co) & IsPartOf(c3, co)).named('LabeledTriangle').with_plan(var_order=['p1', 'p2', 'p3', 'c1', 'c2', 'c3', 'co']),
    ],
  )
