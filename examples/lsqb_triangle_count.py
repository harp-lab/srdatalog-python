...
Triangle = Relation("Triangle", 3, column_types=(int, int, int,))
...
def build_triangle_db_program() -> Program:
  ...
  return Program(
    ...
    rules=[
      ...
      (Triangle(a, b, c) <= Knows(a, b) & Knows(b, c) & Knows(a, c)).named('TriangleJoin').with_plan(var_order=['a', 'b', 'c']).with_count(),
    ],
  )
