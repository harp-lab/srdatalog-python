...
Arc = Relation("Arc", 2, column_types=(int, int,), input_file="Arc.csv")
Sg = Relation("Sg", 2, column_types=(int, int,), print_size=True, index_type="SRDatalog::GPU::Device2LevelIndex")
...
def build_sgdb_program() -> Program:
  ...
  return Program(
    ...
    rules=[
      (Sg(x, y) <= Arc(p, x) & Arc(p, y) & Filter(('x', 'y',), "return x != y;")).named('SGBase').with_plan(var_order=['p', 'x', 'y']),
      (Sg(x, y) <= Arc(p, x) & Sg(p, q) & Arc(q, y)).named('SGRec').with_plan(var_order=['p', 'q', 'x', 'y']),
    ],
  )
