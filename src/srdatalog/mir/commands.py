...
MISSING_HANDLE = -99  # inserted when nodes are converted to C++ but no MirInstructions is associated
ATTRIBUTES_CONTAINING_CHILDREN = ['instructions', 'body', 'sources', 'dests']  # for recursive programs
...
def generate_multi_vhm(sources=[], cursor=-1):
    ...
    if len(var_to_cursors) == 0:
      ...
      for v, handles in var_to_cursors.items():
        ...
        pairs.append( f"boost::hana::make_pair(boost::hana::type_c<SRDatalog::AST::Var<decltype(\"{v}\"_s)>>, {handleSeq}{{}})" )
      return "decltype(boost::hana::make_map(" + ", ".join(pairs) + "))"
...
class MirNode():
  ...
  def _request_cursor(self, qty=1):
    if self.program:
      ...
      print("cursor request could not be handled (this block of code is not part of a MirInstructions node)")
...
class MirInstructions:
  ...
  def cpp(self):  # synonym for str()
    ...
    for s_in in range(len(self.structure)):
      build += f"constexpr auto step_{s_in} = {str(self.structure[s_in])} \nusing step_{s_in}_t = decltype(step_{s_in});"
    return build
...
class InsertInto(FMirNode):
  ...
  def __str__(self):
    ...
    return f'insert_into<{self.fact.name}, {self.version.code}, decltype({index_str})>({variable_str})'
...
class ExecutePipeline(MirNode):
  ...
  def __str__(self):
    ...
    list_source_schemas = [n for n in dedup_source_schemas]  # should all be IndexSpecs, not 100% sure
...
class ColumnJoin(MirNode):
  ...
  def __str__(self):
    ...
    variable_str = ", ".join(f'"{v}"_v' for v in self.vars)
    var_to_handle_map = generate_multi_vhm(sources=self.sources, cursor=self.cursor)  # TODO, is this the correct arg LOL
...
class CartesianJoin(MirNode):
  ...
  def __str__(self):
    ...
    var_to_handle_map = generate_multi_vhm(sources=self.sources, cursor=self.cursor)  # TODO, this changes if there are any prefixes
