...
struct JitRunner_Triangle {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_SRel_0_1_2_FULL_VER = views[2];
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          hint_hi_4 = (hint_hi_4 <= view_RRel_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_RRel_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_RRel_0_1_FULL_VER.num_rows_;
          auto h_RRel_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_RRel_0_1_FULL_VER);
          if (!h_RRel_0_root.valid()) continue;
          auto h_TRel_1_root = HandleType(0, view_TRel_1_0_2_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_TRel_1_0_2_FULL_VER);
          if (!h_TRel_1_root.valid()) continue;
          auto x = root_val_2;
        ...
        auto h_RRel_2_12 = h_RRel_0_root;
        auto h_SRel_3_13 = HandleType(0, view_SRel_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_RRel_2_12.iterators(view_RRel_0_1_FULL_VER), h_SRel_3_13.iterators(view_SRel_0_1_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto y = it_15.value();
          auto positions = it_15.positions();
          auto ch_RRel_2_y = h_RRel_2_12.child_range(positions[0], y, tile, view_RRel_0_1_FULL_VER);
          auto ch_SRel_3_y = h_SRel_3_13.child_range(positions[1], y, tile, view_SRel_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'z' from 2 sources
        // MIR: (column-join :var z :sources ((SRel :handle 4 :prefix (y)) (TRel :handle 5 :prefix (x)) ))
        auto h_SRel_4_8 = ch_SRel_3_y;
        auto h_TRel_5_9 = h_TRel_1_root;
        auto intersect_10 = intersect_handles(tile, h_SRel_4_8.iterators(view_SRel_0_1_2_FULL_VER), h_TRel_5_9.iterators(view_TRel_1_0_2_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto z = it_11.value();
          auto positions = it_11.positions();
          auto ch_SRel_4_z = h_SRel_4_8.child_range(positions[0], z, tile, view_SRel_0_1_2_FULL_VER);
          auto ch_TRel_5_z = h_TRel_5_9.child_range(positions[1], z, tile, view_TRel_1_0_2_FULL_VER);
        // Nested CartesianJoin: bind h, f from 2 source(s)
        // MIR: (cartesian-join :vars (h f) :sources ((SRel :handle 6 :prefix (y z)) (TRel :handle 7 :prefix (x z)) ))
        ...
        if (!h_SRel_6_4.valid() || !h_TRel_7_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'y' from 2 sources
        // MIR: (column-join :var y :sources ((RRel :handle 2 :prefix (x)) (SRel :handle 3 :prefix ()) ))
        auto h_RRel_2_24 = h_RRel_0_root;
        auto h_SRel_3_25 = HandleType(0, view_SRel_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_26 = intersect_handles(tile, h_RRel_2_24.iterators(view_RRel_0_1_FULL_VER), h_SRel_3_25.iterators(view_SRel_0_1_2_FULL_VER));
        for (auto it_27 = intersect_26.begin(); it_27.valid(); it_27.next()) {
          auto y = it_27.value();
          auto positions = it_27.positions();
          auto ch_RRel_2_y = h_RRel_2_24.child_range(positions[0], y, tile, view_RRel_0_1_FULL_VER);
          auto ch_SRel_3_y = h_SRel_3_25.child_range(positions[1], y, tile, view_SRel_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'z' from 2 sources
        // MIR: (column-join :var z :sources ((SRel :handle 4 :prefix (y)) (TRel :handle 5 :prefix (x)) ))
        auto h_SRel_4_20 = ch_SRel_3_y;
        auto h_TRel_5_21 = h_TRel_1_root;
        auto intersect_22 = intersect_handles(tile, h_SRel_4_20.iterators(view_SRel_0_1_2_FULL_VER), h_TRel_5_21.iterators(view_TRel_1_0_2_FULL_VER));
        for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
          auto z = it_23.value();
          auto positions = it_23.positions();
          auto ch_SRel_4_z = h_SRel_4_20.child_range(positions[0], z, tile, view_SRel_0_1_2_FULL_VER);
          auto ch_TRel_5_z = h_TRel_5_21.child_range(positions[1], z, tile, view_TRel_1_0_2_FULL_VER);
        // Nested CartesianJoin: bind h, f from 2 source(s)
        // MIR: (cartesian-join :vars (h f) :sources ((SRel :handle 6 :prefix (y z)) (TRel :handle 7 :prefix (x z)) ))
        uint32_t lane_2 = tile.thread_rank();
        uint32_t group_size_3 = tile.size();

        auto h_SRel_6_5 = ch_SRel_4_z;  // reusing narrowed handle
        auto h_TRel_7_7 = ch_TRel_5_z;  // reusing narrowed handle

        if (!h_SRel_6_5.valid() || !h_TRel_7_7.valid()) continue;

        uint32_t degree_4 = h_SRel_6_5.degree();
        uint32_t degree_6 = h_TRel_7_7.degree();
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_SRel_0_1_2_FULL_VER.get_value(2, h_SRel_6_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_TRel_1_0_2_FULL_VER.get_value(2, h_TRel_7_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto h = tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
                auto f = tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
        // Emit: ZRel(x, y, z)
        {
          ...
          if (_tc_active > 0) {
            ...
            if (tc_valid_1) {
              ...
              output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = y;
              output_data_0[2 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = z;
            }
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto h = view_SRel_0_1_2_FULL_VER.get_value(2, h_SRel_6_5.begin() + idx0_18);
            auto f = view_TRel_1_0_2_FULL_VER.get_value(2, h_TRel_7_7.begin() + idx1_19);
        // Emit: ZRel(x, y, z)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
          uint32_t _tc_active = __popc(_tc_ballot);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_fused(
    ...
    SpecCtx_0 output_ctx_0{output_data_0, atomic_write_pos_0, overflow_flag,
        ...
        auto intersect_18 = intersect_handles(tile, h_RRel_2_16.iterators(view_RRel_0_1_FULL_VER), h_SRel_3_17.iterators(view_SRel_0_1_2_FULL_VER));
        ...
        auto intersect_14 = intersect_handles(tile, h_SRel_4_12.iterators(view_SRel_0_1_2_FULL_VER), h_TRel_5_13.iterators(view_TRel_1_0_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto z = it_15.value();
          auto positions = it_15.positions();
          auto ch_SRel_4_z = h_SRel_4_12.child_range(positions[0], z, tile, view_SRel_0_1_2_FULL_VER);
          auto ch_TRel_5_z = h_TRel_5_13.child_range(positions[1], z, tile, view_TRel_1_0_2_FULL_VER);
        // Nested CartesianJoin: bind h, f from 2 source(s)
        // MIR: (cartesian-join :vars (h f) :sources ((SRel :handle 6 :prefix (y z)) (TRel :handle 7 :prefix (x z)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
...
JitRunner_Triangle::LaunchParams JitRunner_Triangle::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: RRel version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<RRel, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: TRel version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<TRel, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0, 2}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: SRel version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<SRel, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<RRel, FULL_VER>(db);
...
uint32_t JitRunner_Triangle::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<ZRel, NEW_VER>(db);
...
void JitRunner_Triangle::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
