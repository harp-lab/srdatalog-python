// =============================================================
// JIT-Generated Complete Runner: Assign_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Assign_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Assign_1_0_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'z' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var z :sources ((PointsTo :handle 0) (Assign :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_Assign_1_root = HandleType(0, view_Assign_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Assign_1_0_FULL_VER);
          if (!h_Assign_1_root.valid()) continue;
          auto z = root_val_2;
        // Nested CartesianJoin: bind x, y from 2 source(s)
        // MIR: (cartesian-join :vars (x y) :sources ((PointsTo :handle 2 :prefix (z)) (Assign :handle 3 :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_PointsTo_2_4 = h_PointsTo_0_root;  // reusing narrowed handle
        auto h_Assign_3_6 = h_Assign_1_root;  // reusing narrowed handle

        if (!h_PointsTo_2_4.valid() || !h_Assign_3_6.valid()) continue;

        uint32_t degree_3 = h_PointsTo_2_4.degree();
        uint32_t degree_5 = h_Assign_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto x = view_PointsTo_0_1_DELTA_VER.get_value(1, h_PointsTo_2_4.begin() + idx0_9);
          auto y = view_Assign_1_0_FULL_VER.get_value(1, h_Assign_3_6.begin() + idx1_10);

        // Emit: PointsTo(y, x)
        output_ctx_0.emit_direct(y, x);
        }
        }
  }
...
JitRunner_Assign_D0::LaunchParams JitRunner_Assign_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: Assign version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Assign, FULL_VER>(db);
...
void JitRunner_Assign_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Assign_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Assign_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Assign_D0::read_total(LaunchParams& p) {
...
void JitRunner_Assign_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Assign_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Assign_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Assign_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Assign_D0");
...
void JitRunner_Assign_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
