...
struct JitRunner_Load_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_PointsTo_0_root.valid())
        continue;
      auto h_PointsTo_1_root = HandleType(0, view_PointsTo_1_0_FULL_VER.num_rows_, 0)
                                   .prefix(root_val_2, tile, view_PointsTo_1_0_FULL_VER);
      if (!h_PointsTo_1_root.valid())
        continue;
      auto z = root_val_2;
      ...
      auto intersect_10 =
          intersect_handles(tile, h_PointsTo_2_8.iterators(view_PointsTo_1_0_FULL_VER),
                            h_Load_3_9.iterators(view_Load_1_0_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        auto x = it_11.value();
        auto positions = it_11.positions();
        auto ch_PointsTo_2_x =
            h_PointsTo_2_8.child_range(positions[0], x, tile, view_PointsTo_1_0_FULL_VER);
        ...
        // MIR: (cartesian-join :vars (y w) :sources ((PointsTo :handle 4 :prefix (z)) (Load :handle
        // 5 :prefix (x)) ))
        ...
        if (!h_PointsTo_4_4.valid() || !h_Load_5_6.valid())
          continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 =
          intersect_handles(tile, h_PointsTo_2_12.iterators(view_PointsTo_1_0_FULL_VER),
                            h_Load_3_13.iterators(view_Load_1_0_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto x = it_15.value();
        auto positions = it_15.positions();
        auto ch_PointsTo_2_x =
            h_PointsTo_2_12.child_range(positions[0], x, tile, view_PointsTo_1_0_FULL_VER);
...
JitRunner_Load_D2::LaunchParams JitRunner_Load_D2::setup(DB& db, uint32_t iteration,
                                                         GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Load_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                           GPU_STREAM_T stream) {
