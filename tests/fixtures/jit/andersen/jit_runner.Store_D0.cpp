...
struct JitRunner_Store_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_PointsTo_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_PointsTo_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_PointsTo_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_PointsTo_0_1_DELTA_VER.num_rows_;
      auto h_PointsTo_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_PointsTo_0_1_DELTA_VER);
      if (!h_PointsTo_0_root.valid())
        continue;
      auto h_Store_1_root = HandleType(0, view_Store_0_1_FULL_VER.num_rows_, 0)
                                .prefix(root_val_2, tile, view_Store_0_1_FULL_VER);
      if (!h_Store_1_root.valid())
        continue;
      auto y = root_val_2;
      // Nested ColumnJoin (intersection): bind 'x' from 2 sources
      // MIR: (column-join :var x :sources ((Store :handle 2 :prefix (y)) (PointsTo :handle 3
      // :prefix ()) ))
      auto h_Store_2_8 = h_Store_1_root;
      auto h_PointsTo_3_9 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
      auto intersect_10 = intersect_handles(tile, h_Store_2_8.iterators(view_Store_0_1_FULL_VER),
                                            h_PointsTo_3_9.iterators(view_PointsTo_0_1_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        ...
        auto ch_PointsTo_3_x =
            h_PointsTo_3_9.child_range(positions[1], x, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind z, w from 2 source(s)
        // MIR: (cartesian-join :vars (z w) :sources ((PointsTo :handle 4 :prefix (y)) (PointsTo
        // :handle 5 :prefix (x)) ))
        ...
        if (!h_PointsTo_4_4.valid() || !h_PointsTo_5_6.valid())
          continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 = intersect_handles(tile, h_Store_2_12.iterators(view_Store_0_1_FULL_VER),
                                            h_PointsTo_3_13.iterators(view_PointsTo_0_1_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto x = it_15.value();
        auto positions = it_15.positions();
        auto ch_Store_2_x =
            h_Store_2_12.child_range(positions[0], x, tile, view_Store_0_1_FULL_VER);
        auto ch_PointsTo_3_x =
            h_PointsTo_3_13.child_range(positions[1], x, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind z, w from 2 source(s)
        // MIR: (cartesian-join :vars (z w) :sources ((PointsTo :handle 4 :prefix (y)) (PointsTo
        // :handle 5 :prefix (x)) ))
        uint32_t lane_1 = tile.thread_rank();
...
JitRunner_Store_D0::LaunchParams JitRunner_Store_D0::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<PointsTo, FULL_VER>(db)
                                   : get_relation_by_schema<PointsTo, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<PointsTo, FULL_VER>(db)
                                     : get_relation_by_schema<PointsTo, DELTA_VER>(db);
...
void JitRunner_Store_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
