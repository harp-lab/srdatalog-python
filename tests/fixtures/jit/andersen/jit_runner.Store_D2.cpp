...
struct JitRunner_Store_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_PointsTo_0_root.valid())
        continue;
      auto h_Store_1_root = HandleType(0, view_Store_1_0_FULL_VER.num_rows_, 0)
                                .prefix(root_val_2, tile, view_Store_1_0_FULL_VER);
      if (!h_Store_1_root.valid())
        continue;
      auto x = root_val_2;
      // Nested ColumnJoin (intersection): bind 'y' from 2 sources
      // MIR: (column-join :var y :sources ((Store :handle 2 :prefix (x)) (PointsTo :handle 3
      // :prefix ()) ))
      auto h_Store_2_8 = h_Store_1_root;
      auto h_PointsTo_3_9 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
      auto intersect_10 = intersect_handles(tile, h_Store_2_8.iterators(view_Store_1_0_FULL_VER),
      ...
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        ...
        auto ch_PointsTo_3_y =
            h_PointsTo_3_9.child_range(positions[1], y, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind w, z from 2 source(s)
        // MIR: (cartesian-join :vars (w z) :sources ((PointsTo :handle 4 :prefix (x)) (PointsTo
        // :handle 5 :prefix (y)) ))
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 = intersect_handles(tile, h_Store_2_12.iterators(view_Store_1_0_FULL_VER),
      ...
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto ch_Store_2_y =
            h_Store_2_12.child_range(positions[0], y, tile, view_Store_1_0_FULL_VER);
        auto ch_PointsTo_3_y =
            h_PointsTo_3_13.child_range(positions[1], y, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind w, z from 2 source(s)
        // MIR: (cartesian-join :vars (w z) :sources ((PointsTo :handle 4 :prefix (x)) (PointsTo
        // :handle 5 :prefix (y)) ))
        uint32_t lane_1 = tile.thread_rank();
...
JitRunner_Store_D2::LaunchParams JitRunner_Store_D2::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Store_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
