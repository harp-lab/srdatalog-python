...
struct JitRunner_CurrentValue {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Assign_0_1_2_3_4_FULL_VER = views[0];
        auto view_Remove_0_1_FULL_VER = views[1];

        // Root Scan: Assign binding IDCtr, IDN, ElemCtr, ElemN, Value
        // MIR: (scan :rel Assign :vars (IDCtr IDN ElemCtr ElemN Value) :handle 0)
        auto root_handle_1 = HandleType(0, view_Assign_0_1_2_3_4_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto IDCtr = view_Assign_0_1_2_3_4_FULL_VER.get_value(0, idx_3);
          auto IDN = view_Assign_0_1_2_3_4_FULL_VER.get_value(1, idx_3);
          auto ElemCtr = view_Assign_0_1_2_3_4_FULL_VER.get_value(2, idx_3);
          auto ElemN = view_Assign_0_1_2_3_4_FULL_VER.get_value(3, idx_3);
          auto Value = view_Assign_0_1_2_3_4_FULL_VER.get_value(4, idx_3);
        // Negation: NOT EXISTS in Remove
        // MIR: (negation :rel Remove :prefix (IDCtr IDN) :handle 1)
        auto h_Remove_neg_1_1 = HandleType(0, view_Remove_0_1_FULL_VER.num_rows_, 0).prefix(IDCtr, tile, view_Remove_0_1_FULL_VER).prefix(IDN, tile, view_Remove_0_1_FULL_VER);
        if (!h_Remove_neg_1_1.valid()) {
        // Emit: CurrentValue(ElemCtr, ElemN, Value)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_Remove_neg_1_1.valid()) {
        // Emit: CurrentValue(ElemCtr, ElemN, Value)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(ElemCtr, ElemN, Value);
        }
        }
  }
...
JitRunner_CurrentValue::LaunchParams JitRunner_CurrentValue::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Assign version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<Assign, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3, 4}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: Remove version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Remove, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<Assign, FULL_VER>(db);
  auto& first_idx = first_rel.get_index(SRDatalog::IndexSpec{{0, 1, 2, 3, 4}});
...
void JitRunner_CurrentValue::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_CurrentValue::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<CurrentValue, NEW_VER>(db);
...
void JitRunner_CurrentValue::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_CurrentValue::read_total(LaunchParams& p) {
...
void JitRunner_CurrentValue::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_CurrentValue::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_CurrentValue::read_fused_result(LaunchParams& p) {
...
void JitRunner_CurrentValue::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("CurrentValue");
...
void JitRunner_CurrentValue::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
