...
struct JitRunner_CurrentValue {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      auto h_Remove_neg_1_1 = HandleType(0, view_Remove_0_1_FULL_VER.num_rows_, 0)
                                  .prefix(IDCtr, tile, view_Remove_0_1_FULL_VER)
                                  .prefix(IDN, tile, view_Remove_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      if (!h_Remove_neg_1_1.valid()) {
        // Emit: CurrentValue(ElemCtr, ElemN, Value)
        if (tile.thread_rank() == 0)
          output_ctx_0.emit_direct(ElemCtr, ElemN, Value);
      }
    }
  }
...
JitRunner_CurrentValue::LaunchParams JitRunner_CurrentValue::setup(DB& db, uint32_t iteration,
                                                                   GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_CurrentValue::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                GPU_STREAM_T stream) {
