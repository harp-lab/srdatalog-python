...
struct JitRunner_FirstChild {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Insert_0_1_2_3_FULL_VER = views[0];
        auto view_LaterChild_2_3_0_1_FULL_VER = views[1];

        // Root Scan: Insert binding ChildCtr, ChildN, ParentCtr, ParentN
        // MIR: (scan :rel Insert :vars (ChildCtr ChildN ParentCtr ParentN) :handle 0)
        auto root_handle_1 = HandleType(0, view_Insert_0_1_2_3_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto ChildCtr = view_Insert_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto ChildN = view_Insert_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto ParentCtr = view_Insert_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto ParentN = view_Insert_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Negation: NOT EXISTS in LaterChild
        // MIR: (negation :rel LaterChild :prefix (ChildCtr ChildN ParentCtr ParentN) :handle 1)
        auto h_LaterChild_neg_1_1 = HandleType(0, view_LaterChild_2_3_0_1_FULL_VER.num_rows_, 0).prefix(ChildCtr, tile, view_LaterChild_2_3_0_1_FULL_VER).prefix(ChildN, tile, view_LaterChild_2_3_0_1_FULL_VER).prefix(ParentCtr, tile, view_LaterChild_2_3_0_1_FULL_VER).prefix(ParentN, tile, view_LaterChild_2_3_0_1_FULL_VER);
        if (!h_LaterChild_neg_1_1.valid()) {
        // Emit: FirstChild(ParentCtr, ParentN, ChildCtr, ChildN)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_LaterChild_neg_1_1.valid()) {
        // Emit: FirstChild(ParentCtr, ParentN, ChildCtr, ChildN)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(ParentCtr, ParentN, ChildCtr, ChildN);
        }
        }
  }
...
JitRunner_FirstChild::LaunchParams JitRunner_FirstChild::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: LaterChild version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<LaterChild, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{2, 3, 0, 1}}, true);
...
void JitRunner_FirstChild::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_FirstChild::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<FirstChild, NEW_VER>(db);
...
void JitRunner_FirstChild::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_FirstChild::read_total(LaunchParams& p) {
...
void JitRunner_FirstChild::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_FirstChild::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_FirstChild::read_fused_result(LaunchParams& p) {
...
void JitRunner_FirstChild::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("FirstChild");
...
void JitRunner_FirstChild::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
