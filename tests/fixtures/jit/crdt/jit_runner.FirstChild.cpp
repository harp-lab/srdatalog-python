...
struct JitRunner_FirstChild {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      auto h_LaterChild_neg_1_1 = HandleType(0, view_LaterChild_2_3_0_1_FULL_VER.num_rows_, 0)
                                      .prefix(ChildCtr, tile, view_LaterChild_2_3_0_1_FULL_VER)
                                      .prefix(ChildN, tile, view_LaterChild_2_3_0_1_FULL_VER)
                                      .prefix(ParentCtr, tile, view_LaterChild_2_3_0_1_FULL_VER)
                                      .prefix(ParentN, tile, view_LaterChild_2_3_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      if (!h_LaterChild_neg_1_1.valid()) {
        // Emit: FirstChild(ParentCtr, ParentN, ChildCtr, ChildN)
        if (tile.thread_rank() == 0)
          output_ctx_0.emit_direct(ParentCtr, ParentN, ChildCtr, ChildN);
      }
    }
  }
...
JitRunner_FirstChild::LaunchParams JitRunner_FirstChild::setup(DB& db, uint32_t iteration,
                                                               GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_FirstChild::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                              GPU_STREAM_T stream) {
