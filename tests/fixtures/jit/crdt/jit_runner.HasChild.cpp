...
struct JitRunner_HasChild {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: HasChild(ParentCtr, ParentN)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(ParentCtr, ParentN);
    }
  }
...
JitRunner_HasChild::LaunchParams JitRunner_HasChild::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_HasChild::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
