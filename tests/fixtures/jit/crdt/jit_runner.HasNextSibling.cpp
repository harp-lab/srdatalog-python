...
struct JitRunner_HasNextSibling {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: LaterSibling binding SibCtr1, SibN1, _gen1, _gen2
        // MIR: (scan :rel LaterSibling :vars (SibCtr1 SibN1 _gen1 _gen2) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto SibCtr1 = view_LaterSibling_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto SibN1 = view_LaterSibling_0_1_2_3_FULL_VER.get_value(1, idx_3);
        // Emit: HasNextSibling(SibCtr1, SibN1)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto SibCtr1 = view_LaterSibling_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto SibN1 = view_LaterSibling_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto _gen1 = view_LaterSibling_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto _gen2 = view_LaterSibling_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Emit: HasNextSibling(SibCtr1, SibN1)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(SibCtr1, SibN1);
        }
  }
...
JitRunner_HasNextSibling::LaunchParams JitRunner_HasNextSibling::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_HasNextSibling::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_HasNextSibling::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<HasNextSibling, NEW_VER>(db);
...
void JitRunner_HasNextSibling::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_HasNextSibling::read_total(LaunchParams& p) {
...
void JitRunner_HasNextSibling::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_HasNextSibling::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_HasNextSibling::read_fused_result(LaunchParams& p) {
...
void JitRunner_HasNextSibling::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("HasNextSibling");
...
void JitRunner_HasNextSibling::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
