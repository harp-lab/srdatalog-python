...
struct JitRunner_HasNextSibling {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: HasNextSibling(SibCtr1, SibN1)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(SibCtr1, SibN1);
    }
  }
...
JitRunner_HasNextSibling::LaunchParams JitRunner_HasNextSibling::setup(DB& db, uint32_t iteration,
                                                                       GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_HasNextSibling::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                  GPU_STREAM_T stream) {
