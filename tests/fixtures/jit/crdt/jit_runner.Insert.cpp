...
struct JitRunner_Insert {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: Insert(a, b, c, d)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(a, b, c, d);
    }
  }
...
JitRunner_Insert::LaunchParams JitRunner_Insert::setup(DB& db, uint32_t iteration,
                                                       GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Insert::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                          GPU_STREAM_T stream) {
