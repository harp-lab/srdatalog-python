...
struct JitRunner_LaterChild {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (cartesian-join :vars (Ctr1 N1 Ctr2 N2) :sources ((Insert :handle 4 :prefix (ParentCtr ParentN)) (Insert :handle 5 :prefix (ParentCtr ParentN)) ))
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if ((Ctr1 * 10 + N1) > (Ctr2 * 10 + N2)) {
        // Emit: LaterChild(ParentCtr, ParentN, Ctr2, N2)
        output_ctx_0.emit_direct(ParentCtr, ParentN, Ctr2, N2);
        }
        }
        }
        }
...
JitRunner_LaterChild::LaunchParams JitRunner_LaterChild::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_LaterChild::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LaterChild::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<LaterChild, NEW_VER>(db);
...
void JitRunner_LaterChild::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LaterChild::read_total(LaunchParams& p) {
...
void JitRunner_LaterChild::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LaterChild::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LaterChild::read_fused_result(LaunchParams& p) {
...
void JitRunner_LaterChild::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LaterChild");
...
void JitRunner_LaterChild::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
