...
struct JitRunner_LaterSibling {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: Sibling binding Ctr1, N1, Ctr2, N2
        // MIR: (scan :rel Sibling :vars (Ctr1 N1 Ctr2 N2) :handle 0)
        auto root_handle_1 = HandleType(0, view_Sibling_0_1_2_3_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto Ctr1 = view_Sibling_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto N1 = view_Sibling_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto Ctr2 = view_Sibling_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto N2 = view_Sibling_0_1_2_3_FULL_VER.get_value(3, idx_3);
        if ((Ctr1 * 10 + N1) > (Ctr2 * 10 + N2)) {
        // Emit: LaterSibling(Ctr1, N1, Ctr2, N2)
...
JitRunner_LaterSibling::LaunchParams JitRunner_LaterSibling::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_LaterSibling::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LaterSibling::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<LaterSibling, NEW_VER>(db);
...
void JitRunner_LaterSibling::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LaterSibling::read_total(LaunchParams& p) {
...
void JitRunner_LaterSibling::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LaterSibling::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LaterSibling::read_fused_result(LaunchParams& p) {
...
void JitRunner_LaterSibling::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LaterSibling");
...
void JitRunner_LaterSibling::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
