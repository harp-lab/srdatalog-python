// =============================================================
// JIT-Generated Complete Runner: LaterSibling2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_LaterSibling2 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = CRDTRules_DB_DeviceDB;
  using FirstSchema = Sibling;
  using DestSchema = LaterSibling2;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_Sibling_0_1_2_3_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_Sibling_0_1_2_3_FULL_VER.num_rows_) ? hint_hi_4 : view_Sibling_0_1_2_3_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Sibling_0_1_2_3_FULL_VER.num_rows_;
          auto h_Sibling_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Sibling_0_1_2_3_FULL_VER);
          if (!h_Sibling_0_root.valid()) continue;
          auto h_Sibling_1_root = HandleType(0, view_Sibling_0_1_2_3_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Sibling_0_1_2_3_FULL_VER);
          if (!h_Sibling_1_root.valid()) continue;
          auto Ctr1 = root_val_2;
        // Nested ColumnJoin (intersection): bind 'N1' from 2 sources
        // MIR: (column-join :var N1 :sources ((Sibling :handle 2 :prefix (Ctr1)) (Sibling :handle 3 :prefix (Ctr1)) ))
        auto h_Sibling_2_12 = h_Sibling_1_root;
        auto h_Sibling_3_13 = h_Sibling_1_root;
        auto intersect_14 = intersect_handles(tile, h_Sibling_2_12.iterators(view_Sibling_0_1_2_3_FULL_VER), h_Sibling_3_13.iterators(view_Sibling_0_1_2_3_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto N1 = it_15.value();
          auto positions = it_15.positions();
          auto ch_Sibling_2_N1 = h_Sibling_2_12.child_range(positions[0], N1, tile, view_Sibling_0_1_2_3_FULL_VER);
          auto ch_Sibling_3_N1 = h_Sibling_3_13.child_range(positions[1], N1, tile, view_Sibling_0_1_2_3_FULL_VER);
        // Nested CartesianJoin: bind Ctr2, N2, Ctr3, N3 from 2 source(s)
        // MIR: (cartesian-join :vars (Ctr2 N2 Ctr3 N3) :sources ((Sibling :handle 4 :prefix (Ctr1 N1)) (Sibling :handle 5 :prefix (Ctr1 N1)) ))
        ...
        if (!h_Sibling_4_4.valid() || !h_Sibling_5_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if ((Ctr2 * 10 + N2) > (Ctr3 * 10 + N3)) {
        // Emit: LaterSibling2(Ctr1, N1, Ctr3, N3)
        output_ctx_0.emit_direct(Ctr1, N1, Ctr3, N3);
        }
        }
        }
        }
...
JitRunner_LaterSibling2::LaunchParams JitRunner_LaterSibling2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Sibling version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<Sibling, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<Sibling, FULL_VER>(db);
...
void JitRunner_LaterSibling2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LaterSibling2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<LaterSibling2, NEW_VER>(db);
...
void JitRunner_LaterSibling2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LaterSibling2::read_total(LaunchParams& p) {
...
void JitRunner_LaterSibling2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LaterSibling2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LaterSibling2::read_fused_result(LaunchParams& p) {
...
void JitRunner_LaterSibling2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LaterSibling2");
...
void JitRunner_LaterSibling2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
