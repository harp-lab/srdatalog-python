...
struct JitRunner_NextElem2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      auto h_HasChild_neg_1_1 = HandleType(0, view_HasChild_0_1_FULL_VER.num_rows_, 0)
                                    .prefix(PrevCtr, tile, view_HasChild_0_1_FULL_VER)
                                    .prefix(PrevN, tile, view_HasChild_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      if (!h_HasChild_neg_1_1.valid()) {
        // Emit: NextElem(PrevCtr, PrevN, NextCtr, NextN)
        if (tile.thread_rank() == 0)
          output_ctx_0.emit_direct(PrevCtr, PrevN, NextCtr, NextN);
      }
    }
  }
...
JitRunner_NextElem2::LaunchParams JitRunner_NextElem2::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_NextElem2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
