...
struct JitRunner_NextElem2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_NextSiblingAnc_0_1_2_3_FULL_VER = views[0];
        auto view_HasChild_0_1_FULL_VER = views[1];

        // Root Scan: NextSiblingAnc binding PrevCtr, PrevN, NextCtr, NextN
        // MIR: (scan :rel NextSiblingAnc :vars (PrevCtr PrevN NextCtr NextN) :handle 0)
        auto root_handle_1 = HandleType(0, view_NextSiblingAnc_0_1_2_3_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto PrevCtr = view_NextSiblingAnc_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto PrevN = view_NextSiblingAnc_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto NextCtr = view_NextSiblingAnc_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto NextN = view_NextSiblingAnc_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Negation: NOT EXISTS in HasChild
        // MIR: (negation :rel HasChild :prefix (PrevCtr PrevN) :handle 1)
        auto h_HasChild_neg_1_1 = HandleType(0, view_HasChild_0_1_FULL_VER.num_rows_, 0).prefix(PrevCtr, tile, view_HasChild_0_1_FULL_VER).prefix(PrevN, tile, view_HasChild_0_1_FULL_VER);
        if (!h_HasChild_neg_1_1.valid()) {
        // Emit: NextElem(PrevCtr, PrevN, NextCtr, NextN)
...
JitRunner_NextElem2::LaunchParams JitRunner_NextElem2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: NextSiblingAnc version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<NextSiblingAnc, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: HasChild version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<HasChild, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<NextSiblingAnc, FULL_VER>(db);
...
void JitRunner_NextElem2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_NextElem2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<NextElem, NEW_VER>(db);
...
void JitRunner_NextElem2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_NextElem2::read_total(LaunchParams& p) {
...
void JitRunner_NextElem2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_NextElem2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NextElem2::read_fused_result(LaunchParams& p) {
...
void JitRunner_NextElem2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("NextElem2");
...
void JitRunner_NextElem2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
