// =============================================================
// JIT-Generated Complete Runner: NextSibling
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_NextSibling {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = CRDTRules_DB_DeviceDB;
  using FirstSchema = LaterSibling;
  using DestSchema = NextSibling;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto h_LaterSibling2_neg_1_1 = HandleType(0, view_LaterSibling2_0_1_2_3_FULL_VER.num_rows_, 0).prefix(Ctr1, tile, view_LaterSibling2_0_1_2_3_FULL_VER).prefix(N1, tile, view_LaterSibling2_0_1_2_3_FULL_VER).prefix(Ctr2, tile, view_LaterSibling2_0_1_2_3_FULL_VER).prefix(N2, tile, view_LaterSibling2_0_1_2_3_FULL_VER);
...
JitRunner_NextSibling::LaunchParams JitRunner_NextSibling::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: LaterSibling version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<LaterSibling, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: LaterSibling2 version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<LaterSibling2, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, true);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<LaterSibling, FULL_VER>(db);
...
void JitRunner_NextSibling::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_NextSibling::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<NextSibling, NEW_VER>(db);
...
void JitRunner_NextSibling::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_NextSibling::read_total(LaunchParams& p) {
...
void JitRunner_NextSibling::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_NextSibling::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NextSibling::read_fused_result(LaunchParams& p) {
...
void JitRunner_NextSibling::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("NextSibling");
...
void JitRunner_NextSibling::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
