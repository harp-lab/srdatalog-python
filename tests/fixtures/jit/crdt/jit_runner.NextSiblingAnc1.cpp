...
struct JitRunner_NextSiblingAnc1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_NextSibling_0_1_2_3_FULL_VER = views[0];

        // Root Scan: NextSibling binding StartCtr, StartN, NextCtr, NextN
        // MIR: (scan :rel NextSibling :vars (StartCtr StartN NextCtr NextN) :handle 0)
        auto root_handle_1 = HandleType(0, view_NextSibling_0_1_2_3_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto StartCtr = view_NextSibling_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto StartN = view_NextSibling_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto NextCtr = view_NextSibling_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto NextN = view_NextSibling_0_1_2_3_FULL_VER.get_value(3, idx_3);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          ...
          auto NextN = view_NextSibling_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Emit: NextSiblingAnc(StartCtr, StartN, NextCtr, NextN)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(StartCtr, StartN, NextCtr, NextN);
        }
  }
...
JitRunner_NextSiblingAnc1::LaunchParams JitRunner_NextSiblingAnc1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: NextSibling version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<NextSibling, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<NextSibling, FULL_VER>(db);
...
void JitRunner_NextSiblingAnc1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_NextSiblingAnc1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NextSiblingAnc1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_NextSiblingAnc1::read_total(LaunchParams& p) {
...
void JitRunner_NextSiblingAnc1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_NextSiblingAnc1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NextSiblingAnc1::read_fused_result(LaunchParams& p) {
...
void JitRunner_NextSiblingAnc1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("NextSiblingAnc1");
...
void JitRunner_NextSiblingAnc1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
