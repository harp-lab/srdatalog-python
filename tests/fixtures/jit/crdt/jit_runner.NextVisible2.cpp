// =============================================================
// JIT-Generated Complete Runner: NextVisible2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_NextVisible2 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = CRDTRules_DB_DeviceDB;
  using FirstSchema = HasValue;
  using DestSchema = NextVisible;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_HasValue_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_HasValue_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_HasValue_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_HasValue_0_1_FULL_VER.num_rows_;
          auto h_HasValue_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_HasValue_0_1_FULL_VER);
          if (!h_HasValue_0_root.valid()) continue;
          auto h_SkipBlank_1_root = HandleType(0, view_SkipBlank_0_1_2_3_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_SkipBlank_0_1_2_3_FULL_VER);
          if (!h_SkipBlank_1_root.valid()) continue;
          auto PrevCtr = root_val_2;
        // Nested ColumnJoin (intersection): bind 'PrevN' from 2 sources
        // MIR: (column-join :var PrevN :sources ((HasValue :handle 2 :prefix (PrevCtr)) (SkipBlank :handle 3 :prefix (PrevCtr)) ))
        auto h_HasValue_2_9 = h_HasValue_0_root;
        auto h_SkipBlank_3_10 = h_SkipBlank_1_root;
        auto intersect_11 = intersect_handles(tile, h_HasValue_2_9.iterators(view_HasValue_0_1_FULL_VER), h_SkipBlank_3_10.iterators(view_SkipBlank_0_1_2_3_FULL_VER));
        for (auto it_12 = intersect_11.begin(); it_12.valid(); it_12.next()) {
          auto PrevN = it_12.value();
          auto positions = it_12.positions();
          auto ch_HasValue_2_PrevN = h_HasValue_2_9.child_range(positions[0], PrevN, tile, view_HasValue_0_1_FULL_VER);
          auto ch_SkipBlank_3_PrevN = h_SkipBlank_3_10.child_range(positions[1], PrevN, tile, view_SkipBlank_0_1_2_3_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'NextCtr' from 2 sources
        // MIR: (column-join :var NextCtr :sources ((SkipBlank :handle 4 :prefix (PrevCtr PrevN)) (HasValue :handle 5 :prefix ()) ))
        auto h_SkipBlank_4_5 = ch_SkipBlank_3_PrevN;
        auto h_HasValue_5_6 = HandleType(0, view_HasValue_0_1_FULL_VER.num_rows_, 0);
        auto intersect_7 = intersect_handles(tile, h_SkipBlank_4_5.iterators(view_SkipBlank_0_1_2_3_FULL_VER), h_HasValue_5_6.iterators(view_HasValue_0_1_FULL_VER));
        for (auto it_8 = intersect_7.begin(); it_8.valid(); it_8.next()) {
          auto NextCtr = it_8.value();
          auto positions = it_8.positions();
          auto ch_SkipBlank_4_NextCtr = h_SkipBlank_4_5.child_range(positions[0], NextCtr, tile, view_SkipBlank_0_1_2_3_FULL_VER);
          auto ch_HasValue_5_NextCtr = h_HasValue_5_6.child_range(positions[1], NextCtr, tile, view_HasValue_0_1_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'NextN' from 2 sources
        // MIR: (column-join :var NextN :sources ((SkipBlank :handle 6 :prefix (PrevCtr PrevN NextCtr)) (HasValue :handle 7 :prefix (NextCtr)) ))
        auto h_SkipBlank_6_1 = ch_SkipBlank_4_NextCtr;
        auto h_HasValue_7_2 = ch_HasValue_5_NextCtr;
        auto intersect_3 = intersect_handles(tile, h_SkipBlank_6_1.iterators(view_SkipBlank_0_1_2_3_FULL_VER), h_HasValue_7_2.iterators(view_HasValue_0_1_FULL_VER));
        for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
          auto NextN = it_4.value();
          auto positions = it_4.positions();
          auto ch_SkipBlank_6_NextN = h_SkipBlank_6_1.child_range(positions[0], NextN, tile, view_SkipBlank_0_1_2_3_FULL_VER);
          auto ch_HasValue_7_NextN = h_HasValue_7_2.child_range(positions[1], NextN, tile, view_HasValue_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
          ...
          auto ch_HasValue_7_NextN = h_HasValue_7_2.child_range(positions[1], NextN, tile, view_HasValue_0_1_FULL_VER);
        // Emit: NextVisible(PrevCtr, PrevN, NextCtr, NextN)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(PrevCtr, PrevN, NextCtr, NextN);
        }
        }
        }
        }
...
JitRunner_NextVisible2::LaunchParams JitRunner_NextVisible2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: HasValue version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<HasValue, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: SkipBlank version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<SkipBlank, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<HasValue, FULL_VER>(db);
...
void JitRunner_NextVisible2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_NextVisible2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<NextVisible, NEW_VER>(db);
...
void JitRunner_NextVisible2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_NextVisible2::read_total(LaunchParams& p) {
...
void JitRunner_NextVisible2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_NextVisible2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NextVisible2::read_fused_result(LaunchParams& p) {
...
void JitRunner_NextVisible2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("NextVisible2");
...
void JitRunner_NextVisible2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
