...
struct JitRunner_NotHasValueNextElem {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      auto h_HasValue_neg_1_1 = HandleType(0, view_HasValue_0_1_FULL_VER.num_rows_, 0)
                                    .prefix(Ctr2, tile, view_HasValue_0_1_FULL_VER)
                                    .prefix(N2, tile, view_HasValue_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      if (!h_HasValue_neg_1_1.valid()) {
        // Emit: NotHasValueNextElem(Ctr1, N1, Ctr2, N2)
        if (tile.thread_rank() == 0)
          output_ctx_0.emit_direct(Ctr1, N1, Ctr2, N2);
      }
    }
  }
...
JitRunner_NotHasValueNextElem::LaunchParams JitRunner_NotHasValueNextElem::setup(
...
uint32_t JitRunner_NotHasValueNextElem::scan_and_resize(DB& db, LaunchParams& p,
                                                        GPU_STREAM_T stream) {
...
void JitRunner_NotHasValueNextElem::launch_materialize(DB& db, LaunchParams& p,
                                                       uint32_t total_count, GPU_STREAM_T stream) {
