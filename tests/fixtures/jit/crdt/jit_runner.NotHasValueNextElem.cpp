// =============================================================
// JIT-Generated Complete Runner: NotHasValueNextElem
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_NotHasValueNextElem {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = CRDTRules_DB_DeviceDB;
  using FirstSchema = NextElem;
  using DestSchema = NotHasValueNextElem;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_NextElem_0_1_2_3_FULL_VER = views[0];
        auto view_HasValue_0_1_FULL_VER = views[1];

        // Root Scan: NextElem binding Ctr1, N1, Ctr2, N2
        // MIR: (scan :rel NextElem :vars (Ctr1 N1 Ctr2 N2) :handle 0)
        auto root_handle_1 = HandleType(0, view_NextElem_0_1_2_3_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto Ctr1 = view_NextElem_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto N1 = view_NextElem_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto Ctr2 = view_NextElem_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto N2 = view_NextElem_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Negation: NOT EXISTS in HasValue
        // MIR: (negation :rel HasValue :prefix (Ctr2 N2) :handle 1)
        auto h_HasValue_neg_1_1 = HandleType(0, view_HasValue_0_1_FULL_VER.num_rows_, 0).prefix(Ctr2, tile, view_HasValue_0_1_FULL_VER).prefix(N2, tile, view_HasValue_0_1_FULL_VER);
        if (!h_HasValue_neg_1_1.valid()) {
        // Emit: NotHasValueNextElem(Ctr1, N1, Ctr2, N2)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_HasValue_neg_1_1.valid()) {
        // Emit: NotHasValueNextElem(Ctr1, N1, Ctr2, N2)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(Ctr1, N1, Ctr2, N2);
        }
        }
  }
...
JitRunner_NotHasValueNextElem::LaunchParams JitRunner_NotHasValueNextElem::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: NextElem version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<NextElem, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: HasValue version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<HasValue, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<NextElem, FULL_VER>(db);
...
void JitRunner_NotHasValueNextElem::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_NotHasValueNextElem::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<NotHasValueNextElem, NEW_VER>(db);
...
void JitRunner_NotHasValueNextElem::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_NotHasValueNextElem::read_total(LaunchParams& p) {
...
void JitRunner_NotHasValueNextElem::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_NotHasValueNextElem::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NotHasValueNextElem::read_fused_result(LaunchParams& p) {
...
void JitRunner_NotHasValueNextElem::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("NotHasValueNextElem");
...
void JitRunner_NotHasValueNextElem::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
