...
struct JitRunner_NotSiblingInsert {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_HasNextSibling_0_1_FULL_VER = views[1];

        // Root Scan: Insert binding Ctr1, N1, Ctr2, N2
        // MIR: (scan :rel Insert :vars (Ctr1 N1 Ctr2 N2) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto Ctr1 = view_Insert_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto N1 = view_Insert_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto Ctr2 = view_Insert_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto N2 = view_Insert_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Negation: NOT EXISTS in HasNextSibling
        // MIR: (negation :rel HasNextSibling :prefix (Ctr1 N1) :handle 1)
        auto h_HasNextSibling_neg_1_1 = HandleType(0, view_HasNextSibling_0_1_FULL_VER.num_rows_, 0).prefix(Ctr1, tile, view_HasNextSibling_0_1_FULL_VER).prefix(N1, tile, view_HasNextSibling_0_1_FULL_VER);
        if (!h_HasNextSibling_neg_1_1.valid()) {
        // Emit: NotSiblingInsert(Ctr1, N1, Ctr2, N2)
...
JitRunner_NotSiblingInsert::LaunchParams JitRunner_NotSiblingInsert::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: HasNextSibling version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<HasNextSibling, FULL_VER>(db);
...
void JitRunner_NotSiblingInsert::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_NotSiblingInsert::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<NotSiblingInsert, NEW_VER>(db);
...
void JitRunner_NotSiblingInsert::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_NotSiblingInsert::read_total(LaunchParams& p) {
...
void JitRunner_NotSiblingInsert::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_NotSiblingInsert::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_NotSiblingInsert::read_fused_result(LaunchParams& p) {
...
void JitRunner_NotSiblingInsert::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("NotSiblingInsert");
...
void JitRunner_NotSiblingInsert::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
