...
struct JitRunner_NotSiblingInsert {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      auto h_HasNextSibling_neg_1_1 = HandleType(0, view_HasNextSibling_0_1_FULL_VER.num_rows_, 0)
                                          .prefix(Ctr1, tile, view_HasNextSibling_0_1_FULL_VER)
                                          .prefix(N1, tile, view_HasNextSibling_0_1_FULL_VER);
...
JitRunner_NotSiblingInsert::LaunchParams JitRunner_NotSiblingInsert::setup(DB& db,
                                                                           uint32_t iteration,
                                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_NotSiblingInsert::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                    GPU_STREAM_T stream) {
