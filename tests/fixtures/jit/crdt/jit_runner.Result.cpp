...
struct JitRunner_Result {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_NextVisible_2_3_0_1_FULL_VER = views[0];
        auto view_CurrentValue_0_1_2_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'ctr2' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var ctr2 :sources ((NextVisible :handle 0) (CurrentValue :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_NextVisible_2_3_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_NextVisible_2_3_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_NextVisible_2_3_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_NextVisible_2_3_0_1_FULL_VER.num_rows_;
          auto h_NextVisible_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_NextVisible_2_3_0_1_FULL_VER);
          if (!h_NextVisible_0_root.valid()) continue;
          auto h_CurrentValue_1_root = HandleType(0, view_CurrentValue_0_1_2_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_CurrentValue_0_1_2_FULL_VER);
          if (!h_CurrentValue_1_root.valid()) continue;
          auto ctr2 = root_val_2;
        // Nested ColumnJoin (intersection): bind 'N2' from 2 sources
        // MIR: (column-join :var N2 :sources ((NextVisible :handle 2 :prefix (ctr2)) (CurrentValue :handle 3 :prefix (ctr2)) ))
        auto h_NextVisible_2_8 = h_NextVisible_0_root;
        auto h_CurrentValue_3_9 = h_CurrentValue_1_root;
        auto intersect_10 = intersect_handles(tile, h_NextVisible_2_8.iterators(view_NextVisible_2_3_0_1_FULL_VER), h_CurrentValue_3_9.iterators(view_CurrentValue_0_1_2_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto N2 = it_11.value();
          auto positions = it_11.positions();
          auto ch_NextVisible_2_N2 = h_NextVisible_2_8.child_range(positions[0], N2, tile, view_NextVisible_2_3_0_1_FULL_VER);
          auto ch_CurrentValue_3_N2 = h_CurrentValue_3_9.child_range(positions[1], N2, tile, view_CurrentValue_0_1_2_FULL_VER);
        // Nested CartesianJoin: bind ctr1, _gen8, value from 2 source(s)
        // MIR: (cartesian-join :vars (ctr1 _gen8 value) :sources ((NextVisible :handle 4 :prefix (ctr2 N2)) (CurrentValue :handle 5 :prefix (ctr2 N2)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_NextVisible_4_4 = ch_NextVisible_2_N2;  // reusing narrowed handle
        auto h_CurrentValue_5_6 = ch_CurrentValue_3_N2;  // reusing narrowed handle

        if (!h_NextVisible_4_4.valid() || !h_CurrentValue_5_6.valid()) continue;

        uint32_t degree_3 = h_NextVisible_4_4.degree();
        uint32_t degree_5 = h_CurrentValue_5_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'N2' from 2 sources
        // MIR: (column-join :var N2 :sources ((NextVisible :handle 2 :prefix (ctr2)) (CurrentValue :handle 3 :prefix (ctr2)) ))
        auto h_NextVisible_2_12 = h_NextVisible_0_root;
        auto h_CurrentValue_3_13 = h_CurrentValue_1_root;
        auto intersect_14 = intersect_handles(tile, h_NextVisible_2_12.iterators(view_NextVisible_2_3_0_1_FULL_VER), h_CurrentValue_3_13.iterators(view_CurrentValue_0_1_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto N2 = it_15.value();
          auto positions = it_15.positions();
          auto ch_NextVisible_2_N2 = h_NextVisible_2_12.child_range(positions[0], N2, tile, view_NextVisible_2_3_0_1_FULL_VER);
          auto ch_CurrentValue_3_N2 = h_CurrentValue_3_13.child_range(positions[1], N2, tile, view_CurrentValue_0_1_2_FULL_VER);
        // Nested CartesianJoin: bind ctr1, _gen8, value from 2 source(s)
        // MIR: (cartesian-join :vars (ctr1 _gen8 value) :sources ((NextVisible :handle 4 :prefix (ctr2 N2)) (CurrentValue :handle 5 :prefix (ctr2 N2)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto ctr1 = view_NextVisible_2_3_0_1_FULL_VER.get_value(2, h_NextVisible_4_4.begin() + idx0_9);
          auto _gen8 = view_NextVisible_2_3_0_1_FULL_VER.get_value(3, h_NextVisible_4_4.begin() + idx0_9);
          auto value = view_CurrentValue_0_1_2_FULL_VER.get_value(2, h_CurrentValue_5_6.begin() + idx1_10);

        // Emit: Result(ctr1, ctr2, value)
        output_ctx_0.emit_direct(ctr1, ctr2, value);
        }
        }
        }
  }
...
JitRunner_Result::LaunchParams JitRunner_Result::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: NextVisible version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<NextVisible, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{2, 3, 0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: CurrentValue version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<CurrentValue, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<NextVisible, FULL_VER>(db);
  auto& first_idx = first_rel.get_index(SRDatalog::IndexSpec{{2, 3, 0, 1}});
...
void JitRunner_Result::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Result::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Result, NEW_VER>(db);
...
void JitRunner_Result::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Result::read_total(LaunchParams& p) {
...
void JitRunner_Result::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Result::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Result::read_fused_result(LaunchParams& p) {
...
void JitRunner_Result::execute(DB& db, uint32_t iteration) {
...
void JitRunner_Result::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
