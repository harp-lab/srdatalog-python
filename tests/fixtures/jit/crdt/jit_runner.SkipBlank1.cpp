// =============================================================
// JIT-Generated Complete Runner: SkipBlank1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SkipBlank1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: NextElem binding FromCtr, FromN, ToCtr, ToN
        // MIR: (scan :rel NextElem :vars (FromCtr FromN ToCtr ToN) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto FromCtr = view_NextElem_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto FromN = view_NextElem_0_1_2_3_FULL_VER.get_value(1, idx_3);
          auto ToCtr = view_NextElem_0_1_2_3_FULL_VER.get_value(2, idx_3);
          auto ToN = view_NextElem_0_1_2_3_FULL_VER.get_value(3, idx_3);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          ...
          auto ToN = view_NextElem_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Emit: SkipBlank(FromCtr, FromN, ToCtr, ToN)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(FromCtr, FromN, ToCtr, ToN);
        }
  }
...
JitRunner_SkipBlank1::LaunchParams JitRunner_SkipBlank1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_SkipBlank1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SkipBlank1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SkipBlank1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SkipBlank1::read_total(LaunchParams& p) {
...
void JitRunner_SkipBlank1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SkipBlank1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SkipBlank1::read_fused_result(LaunchParams& p) {
...
void JitRunner_SkipBlank1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SkipBlank1");
...
void JitRunner_SkipBlank1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
