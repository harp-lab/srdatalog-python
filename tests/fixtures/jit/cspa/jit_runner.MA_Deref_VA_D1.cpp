...
struct JitRunner_MA_Deref_VA_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_ValueAlias_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_ValueAlias_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_ValueAlias_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ValueAlias_0_1_DELTA_VER.num_rows_;
      auto h_ValueAlias_0_root = HandleType(hint_lo_3, hint_hi_4, 0)
                                     .prefix(root_val_2, tile, view_ValueAlias_0_1_DELTA_VER);
      if (!h_ValueAlias_0_root.valid())
        continue;
      auto h_Deref_1_root = HandleType(0, view_Deref_0_1_FULL_VER.num_rows_, 0)
                                .prefix(root_val_2, tile, view_Deref_0_1_FULL_VER);
      if (!h_Deref_1_root.valid())
        continue;
      auto y = root_val_2;
      // Nested ColumnJoin (intersection): bind 'z' from 2 sources
      // MIR: (column-join :var z :sources ((ValueAlias :handle 2 :prefix (y)) (Deref :handle 3
      // :prefix ()) ))
      auto h_ValueAlias_2_8 = h_ValueAlias_0_root;
      auto h_Deref_3_9 = HandleType(0, view_Deref_0_1_FULL_VER.num_rows_, 0);
      auto intersect_10 =
          intersect_handles(tile, h_ValueAlias_2_8.iterators(view_ValueAlias_0_1_DELTA_VER),
                            h_Deref_3_9.iterators(view_Deref_0_1_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        auto z = it_11.value();
        auto positions = it_11.positions();
        auto ch_ValueAlias_2_z =
            h_ValueAlias_2_8.child_range(positions[0], z, tile, view_ValueAlias_0_1_DELTA_VER);
        ...
        // MIR: (cartesian-join :vars (x w) :sources ((Deref :handle 4 :prefix (y)) (Deref :handle 5
        // :prefix (z)) ))
        ...
        if (!h_Deref_4_4.valid() || !h_Deref_5_6.valid())
          continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_22 =
          intersect_handles(tile, h_ValueAlias_2_20.iterators(view_ValueAlias_0_1_DELTA_VER),
                            h_Deref_3_21.iterators(view_Deref_0_1_FULL_VER));
      for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
        auto z = it_23.value();
        auto positions = it_23.positions();
        auto ch_ValueAlias_2_z =
            h_ValueAlias_2_20.child_range(positions[0], z, tile, view_ValueAlias_0_1_DELTA_VER);
        auto ch_Deref_3_z =
            h_Deref_3_21.child_range(positions[1], z, tile, view_Deref_0_1_FULL_VER);
        // Nested CartesianJoin: bind x, w from 2 source(s)
        // MIR: (cartesian-join :vars (x w) :sources ((Deref :handle 4 :prefix (y)) (Deref :handle 5
        // :prefix (z)) ))
        ...
        if (!h_Deref_4_5.valid() || !h_Deref_5_7.valid())
          continue;
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] =
                  view_Deref_0_1_FULL_VER.get_value(1, h_Deref_4_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] =
                    view_Deref_0_1_FULL_VER.get_value(1, h_Deref_5_7.begin() + t1_base_11 + _ti);
              tile.sync();
              uint32_t tile_total_14 = t0_len_12 * t1_len_13;
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14;
                   tc_batch_15 += group_size_3) {
                uint32_t flat_idx_9 = tc_batch_15 + lane_2;
                bool tc_valid_1 = flat_idx_9 < tile_total_14;
                auto x =
                    tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
                auto w =
                    tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
                // Emit: MemoryAlias(x, w)
                {
                  ...
                  if (_tc_active > 0) {
                    ...
                    if (tc_valid_1) {
                      uint32_t _tc_pos_0 =
                          old_size_0 + warp_write_base + warp_local_count + _tc_off;
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            if (major_is_1_17) {
              idx0_18 = flat_idx_9 / degree_6;
              idx1_19 = flat_idx_9 % degree_6;
            } else {
              idx1_19 = flat_idx_9 / degree_4;
              idx0_18 = flat_idx_9 % degree_4;
            }
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 =
          intersect_handles(tile, h_ValueAlias_2_12.iterators(view_ValueAlias_0_1_DELTA_VER),
                            h_Deref_3_13.iterators(view_Deref_0_1_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto z = it_15.value();
        auto positions = it_15.positions();
        auto ch_ValueAlias_2_z =
            h_ValueAlias_2_12.child_range(positions[0], z, tile, view_ValueAlias_0_1_DELTA_VER);
        auto ch_Deref_3_z =
            h_Deref_3_13.child_range(positions[1], z, tile, view_Deref_0_1_FULL_VER);
        // Nested CartesianJoin: bind x, w from 2 source(s)
        // MIR: (cartesian-join :vars (x w) :sources ((Deref :handle 4 :prefix (y)) (Deref :handle 5
        // :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
...
JitRunner_MA_Deref_VA_D1::LaunchParams JitRunner_MA_Deref_VA_D1::setup(DB& db, uint32_t iteration,
                                                                       GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ValueAlias, FULL_VER>(db)
                                   : get_relation_by_schema<ValueAlias, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ValueAlias, FULL_VER>(db)
                                     : get_relation_by_schema<ValueAlias, DELTA_VER>(db);
...
void JitRunner_MA_Deref_VA_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                  GPU_STREAM_T stream) {
