// =============================================================
// JIT-Generated Complete Runner: MA_Refl1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_MA_Refl1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: Assign binding _gen3, x
        // MIR: (scan :rel Assign :vars (_gen3 x) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto x = view_Assign_0_1_FULL_VER.get_value(1, idx_3);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto _gen3 = view_Assign_0_1_FULL_VER.get_value(0, idx_3);
          auto x = view_Assign_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: MemoryAlias(x, x)
...
JitRunner_MA_Refl1::LaunchParams JitRunner_MA_Refl1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_MA_Refl1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_MA_Refl1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MA_Refl1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_MA_Refl1::read_total(LaunchParams& p) {
...
void JitRunner_MA_Refl1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_MA_Refl1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MA_Refl1::read_fused_result(LaunchParams& p) {
...
void JitRunner_MA_Refl1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("MA_Refl1");
...
void JitRunner_MA_Refl1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
