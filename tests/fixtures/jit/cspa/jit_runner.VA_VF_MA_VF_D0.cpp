// =============================================================
// JIT-Generated Complete Runner: VA_VF_MA_VF_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VA_VF_MA_VF_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_ValueFlow_0_root.valid())
        continue;
      auto h_MemoryAlias_1_root = HandleType(0, view_MemoryAlias_0_1_FULL_VER.num_rows_, 0)
                                      .prefix(root_val_2, tile, view_MemoryAlias_0_1_FULL_VER);
      if (!h_MemoryAlias_1_root.valid())
        continue;
      auto z = root_val_2;
      // Nested ColumnJoin (intersection): bind 'w' from 2 sources
      // MIR: (column-join :var w :sources ((MemoryAlias :handle 2 :prefix (z)) (ValueFlow :handle 3
      // :prefix ()) ))
      auto h_MemoryAlias_2_8 = h_MemoryAlias_1_root;
      auto h_ValueFlow_3_9 = HandleType(0, view_ValueFlow_0_1_FULL_VER.num_rows_, 0);
      auto intersect_10 =
          intersect_handles(tile, h_MemoryAlias_2_8.iterators(view_MemoryAlias_0_1_FULL_VER),
      ...
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        ...
        auto ch_MemoryAlias_2_w =
            h_MemoryAlias_2_8.child_range(positions[0], w, tile, view_MemoryAlias_0_1_FULL_VER);
        auto ch_ValueFlow_3_w =
            h_ValueFlow_3_9.child_range(positions[1], w, tile, view_ValueFlow_0_1_FULL_VER);
        // Nested CartesianJoin: bind x, y from 2 source(s)
        // MIR: (cartesian-join :vars (x y) :sources ((ValueFlow :handle 4 :prefix (z)) (ValueFlow
        // :handle 5 :prefix (w)) ))
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 =
          intersect_handles(tile, h_MemoryAlias_2_12.iterators(view_MemoryAlias_0_1_FULL_VER),
      ...
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto ch_MemoryAlias_2_w =
            h_MemoryAlias_2_12.child_range(positions[0], w, tile, view_MemoryAlias_0_1_FULL_VER);
        auto ch_ValueFlow_3_w =
            h_ValueFlow_3_13.child_range(positions[1], w, tile, view_ValueFlow_0_1_FULL_VER);
        // Nested CartesianJoin: bind x, y from 2 source(s)
        // MIR: (cartesian-join :vars (x y) :sources ((ValueFlow :handle 4 :prefix (z)) (ValueFlow
        // :handle 5 :prefix (w)) ))
        uint32_t lane_1 = tile.thread_rank();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto x = view_ValueFlow_0_1_DELTA_VER.get_value(1, h_ValueFlow_4_4.begin() + idx0_9);
...
JitRunner_VA_VF_MA_VF_D0::LaunchParams JitRunner_VA_VF_MA_VF_D0::setup(DB& db, uint32_t iteration,
                                                                       GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VA_VF_MA_VF_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_VA_VF_MA_VF_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VA_VF_MA_VF_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_VA_VF_MA_VF_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_VA_VF_MA_VF_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                  GPU_STREAM_T stream) {
...
void JitRunner_VA_VF_MA_VF_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_VA_VF_MA_VF_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_VA_VF_MA_VF_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VA_VF_MA_VF_D0");
...
void JitRunner_VA_VF_MA_VF_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
