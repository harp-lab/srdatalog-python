...
struct JitRunner_VA_VF_MA_VF_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_MemoryAlias_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_MemoryAlias_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_MemoryAlias_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_MemoryAlias_0_1_DELTA_VER.num_rows_;
      auto h_MemoryAlias_0_root = HandleType(hint_lo_3, hint_hi_4, 0)
                                      .prefix(root_val_2, tile, view_MemoryAlias_0_1_DELTA_VER);
      if (!h_MemoryAlias_0_root.valid())
        continue;
      auto h_ValueFlow_1_root = HandleType(0, view_ValueFlow_0_1_FULL_VER.num_rows_, 0)
                                    .prefix(root_val_2, tile, view_ValueFlow_0_1_FULL_VER);
      if (!h_ValueFlow_1_root.valid())
        continue;
      auto z = root_val_2;
      ...
      // :prefix ()) ))
      auto h_MemoryAlias_2_8 = h_MemoryAlias_0_root;
      auto h_ValueFlow_3_9 = HandleType(0, view_ValueFlow_0_1_FULL_VER.num_rows_, 0);
      auto intersect_10 =
          intersect_handles(tile, h_MemoryAlias_2_8.iterators(view_MemoryAlias_0_1_DELTA_VER),
      ...
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        ...
        auto ch_MemoryAlias_2_w =
            h_MemoryAlias_2_8.child_range(positions[0], w, tile, view_MemoryAlias_0_1_DELTA_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 =
          intersect_handles(tile, h_MemoryAlias_2_12.iterators(view_MemoryAlias_0_1_DELTA_VER),
      ...
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto ch_MemoryAlias_2_w =
            h_MemoryAlias_2_12.child_range(positions[0], w, tile, view_MemoryAlias_0_1_DELTA_VER);
        auto ch_ValueFlow_3_w =
...
JitRunner_VA_VF_MA_VF_D1::LaunchParams JitRunner_VA_VF_MA_VF_D1::setup(DB& db, uint32_t iteration,
                                                                       GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: MemoryAlias version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<MemoryAlias, FULL_VER>(db)
                                   : get_relation_by_schema<MemoryAlias, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: ValueFlow version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<ValueFlow, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<MemoryAlias, FULL_VER>(db)
                                     : get_relation_by_schema<MemoryAlias, DELTA_VER>(db);
...
void JitRunner_VA_VF_MA_VF_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_VA_VF_MA_VF_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VA_VF_MA_VF_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_VA_VF_MA_VF_D1::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_VA_VF_MA_VF_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                  GPU_STREAM_T stream) {
...
void JitRunner_VA_VF_MA_VF_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_VA_VF_MA_VF_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_VA_VF_MA_VF_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VA_VF_MA_VF_D1");
...
void JitRunner_VA_VF_MA_VF_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
