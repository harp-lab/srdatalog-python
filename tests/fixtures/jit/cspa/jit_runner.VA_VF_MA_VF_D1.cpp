...
struct JitRunner_VA_VF_MA_VF_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto intersect_10 = intersect_handles(tile, h_MemoryAlias_2_8.iterators(view_MemoryAlias_0_1_DELTA_VER), h_ValueFlow_3_9.iterators(view_ValueFlow_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto w = it_11.value();
          auto positions = it_11.positions();
          auto ch_MemoryAlias_2_w = h_MemoryAlias_2_8.child_range(positions[0], w, tile, view_MemoryAlias_0_1_DELTA_VER);
...
JitRunner_VA_VF_MA_VF_D1::LaunchParams JitRunner_VA_VF_MA_VF_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VA_VF_MA_VF_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
