...
struct JitRunner_VA_VF_MA_VF_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_ValueFlow_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_ValueFlow_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_ValueFlow_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ValueFlow_0_1_DELTA_VER.num_rows_;
      auto h_ValueFlow_0_root = HandleType(hint_lo_3, hint_hi_4, 0)
                                    .prefix(root_val_2, tile, view_ValueFlow_0_1_DELTA_VER);
      if (!h_ValueFlow_0_root.valid())
        continue;
      auto h_MemoryAlias_1_root = HandleType(0, view_MemoryAlias_1_0_FULL_VER.num_rows_, 0)
                                      .prefix(root_val_2, tile, view_MemoryAlias_1_0_FULL_VER);
      if (!h_MemoryAlias_1_root.valid())
        continue;
      auto w = root_val_2;
      // Nested ColumnJoin (intersection): bind 'z' from 2 sources
      // MIR: (column-join :var z :sources ((MemoryAlias :handle 2 :prefix (w)) (ValueFlow :handle 3
      // :prefix ()) ))
      auto h_MemoryAlias_2_8 = h_MemoryAlias_1_root;
      auto h_ValueFlow_3_9 = HandleType(0, view_ValueFlow_0_1_FULL_VER.num_rows_, 0);
      auto intersect_10 =
          intersect_handles(tile, h_MemoryAlias_2_8.iterators(view_MemoryAlias_1_0_FULL_VER),
                            h_ValueFlow_3_9.iterators(view_ValueFlow_0_1_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        auto z = it_11.value();
        auto positions = it_11.positions();
        auto ch_MemoryAlias_2_z =
            h_MemoryAlias_2_8.child_range(positions[0], z, tile, view_MemoryAlias_1_0_FULL_VER);
        auto ch_ValueFlow_3_z =
            h_ValueFlow_3_9.child_range(positions[1], z, tile, view_ValueFlow_0_1_FULL_VER);
        // Nested CartesianJoin: bind y, x from 2 source(s)
        // MIR: (cartesian-join :vars (y x) :sources ((ValueFlow :handle 4 :prefix (w)) (ValueFlow
        // :handle 5 :prefix (z)) ))
        ...
        if (!h_ValueFlow_4_4.valid() || !h_ValueFlow_5_6.valid())
          continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 =
          intersect_handles(tile, h_MemoryAlias_2_12.iterators(view_MemoryAlias_1_0_FULL_VER),
                            h_ValueFlow_3_13.iterators(view_ValueFlow_0_1_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto z = it_15.value();
        auto positions = it_15.positions();
        auto ch_MemoryAlias_2_z =
            h_MemoryAlias_2_12.child_range(positions[0], z, tile, view_MemoryAlias_1_0_FULL_VER);
        auto ch_ValueFlow_3_z =
            h_ValueFlow_3_13.child_range(positions[1], z, tile, view_ValueFlow_0_1_FULL_VER);
        // Nested CartesianJoin: bind y, x from 2 source(s)
        // MIR: (cartesian-join :vars (y x) :sources ((ValueFlow :handle 4 :prefix (w)) (ValueFlow
        // :handle 5 :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
...
JitRunner_VA_VF_MA_VF_D2::LaunchParams JitRunner_VA_VF_MA_VF_D2::setup(DB& db, uint32_t iteration,
                                                                       GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ValueFlow, FULL_VER>(db)
                                   : get_relation_by_schema<ValueFlow, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ValueFlow, FULL_VER>(db)
                                     : get_relation_by_schema<ValueFlow, DELTA_VER>(db);
...
void JitRunner_VA_VF_MA_VF_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                  GPU_STREAM_T stream) {
