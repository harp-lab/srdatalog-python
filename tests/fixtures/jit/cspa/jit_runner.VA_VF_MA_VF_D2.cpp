// =============================================================
// JIT-Generated Complete Runner: VA_VF_MA_VF_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VA_VF_MA_VF_D2 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = CSPA_Rules_DB_DeviceDB;
  using FirstSchema = ValueFlow;
  using DestSchema = ValueAlias;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_ValueFlow_0_1_DELTA_VER = views[0];
        auto view_MemoryAlias_1_0_FULL_VER = views[1];
        auto view_ValueFlow_0_1_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'w' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var w :sources ((ValueFlow :handle 0) (MemoryAlias :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_ValueFlow_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_ValueFlow_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_ValueFlow_0_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ValueFlow_0_1_DELTA_VER.num_rows_;
          auto h_ValueFlow_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_ValueFlow_0_1_DELTA_VER);
          if (!h_ValueFlow_0_root.valid()) continue;
          auto h_MemoryAlias_1_root = HandleType(0, view_MemoryAlias_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_MemoryAlias_1_0_FULL_VER);
          if (!h_MemoryAlias_1_root.valid()) continue;
          auto w = root_val_2;
        // Nested ColumnJoin (intersection): bind 'z' from 2 sources
        // MIR: (column-join :var z :sources ((MemoryAlias :handle 2 :prefix (w)) (ValueFlow :handle 3 :prefix ()) ))
        auto h_MemoryAlias_2_8 = h_MemoryAlias_1_root;
        auto h_ValueFlow_3_9 = HandleType(0, view_ValueFlow_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_MemoryAlias_2_8.iterators(view_MemoryAlias_1_0_FULL_VER), h_ValueFlow_3_9.iterators(view_ValueFlow_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto z = it_11.value();
          auto positions = it_11.positions();
          auto ch_MemoryAlias_2_z = h_MemoryAlias_2_8.child_range(positions[0], z, tile, view_MemoryAlias_1_0_FULL_VER);
          auto ch_ValueFlow_3_z = h_ValueFlow_3_9.child_range(positions[1], z, tile, view_ValueFlow_0_1_FULL_VER);
        // Nested CartesianJoin: bind y, x from 2 source(s)
        // MIR: (cartesian-join :vars (y x) :sources ((ValueFlow :handle 4 :prefix (w)) (ValueFlow :handle 5 :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_ValueFlow_4_4 = h_ValueFlow_0_root;  // reusing narrowed handle
        auto h_ValueFlow_5_6 = ch_ValueFlow_3_z;  // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'z' from 2 sources
        // MIR: (column-join :var z :sources ((MemoryAlias :handle 2 :prefix (w)) (ValueFlow :handle 3 :prefix ()) ))
        auto h_MemoryAlias_2_12 = h_MemoryAlias_1_root;
        auto h_ValueFlow_3_13 = HandleType(0, view_ValueFlow_0_1_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_MemoryAlias_2_12.iterators(view_MemoryAlias_1_0_FULL_VER), h_ValueFlow_3_13.iterators(view_ValueFlow_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto z = it_15.value();
          auto positions = it_15.positions();
          auto ch_MemoryAlias_2_z = h_MemoryAlias_2_12.child_range(positions[0], z, tile, view_MemoryAlias_1_0_FULL_VER);
          auto ch_ValueFlow_3_z = h_ValueFlow_3_13.child_range(positions[1], z, tile, view_ValueFlow_0_1_FULL_VER);
        // Nested CartesianJoin: bind y, x from 2 source(s)
        // MIR: (cartesian-join :vars (y x) :sources ((ValueFlow :handle 4 :prefix (w)) (ValueFlow :handle 5 :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto y = view_ValueFlow_0_1_DELTA_VER.get_value(1, h_ValueFlow_4_4.begin() + idx0_9);
          auto x = view_ValueFlow_0_1_FULL_VER.get_value(1, h_ValueFlow_5_6.begin() + idx1_10);

        // Emit: ValueAlias(x, y)
        output_ctx_0.emit_direct(x, y);
        }
        }
        }
  }
...
JitRunner_VA_VF_MA_VF_D2::LaunchParams JitRunner_VA_VF_MA_VF_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ValueFlow version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ValueFlow, FULL_VER>(db) : get_relation_by_schema<ValueFlow, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: MemoryAlias version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<MemoryAlias, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: ValueFlow version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<ValueFlow, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ValueFlow, FULL_VER>(db) : get_relation_by_schema<ValueFlow, DELTA_VER>(db);
...
void JitRunner_VA_VF_MA_VF_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VA_VF_MA_VF_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<ValueAlias, NEW_VER>(db);
...
void JitRunner_VA_VF_MA_VF_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VA_VF_MA_VF_D2::read_total(LaunchParams& p) {
...
void JitRunner_VA_VF_MA_VF_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VA_VF_MA_VF_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VA_VF_MA_VF_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_VA_VF_MA_VF_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VA_VF_MA_VF_D2");
...
void JitRunner_VA_VF_MA_VF_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
