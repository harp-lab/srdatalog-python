// =============================================================
// JIT-Generated Complete Runner: VF_Assign
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VF_Assign {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: Assign binding y, x
        // MIR: (scan :rel Assign :vars (y x) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto y = view_Assign_0_1_FULL_VER.get_value(0, idx_3);
...
JitRunner_VF_Assign::LaunchParams JitRunner_VF_Assign::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VF_Assign::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VF_Assign::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VF_Assign::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VF_Assign::read_total(LaunchParams& p) {
...
void JitRunner_VF_Assign::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VF_Assign::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VF_Assign::read_fused_result(LaunchParams& p) {
...
void JitRunner_VF_Assign::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VF_Assign");
...
void JitRunner_VF_Assign::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
