...
struct JitRunner_VF_Assign_MA_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (cartesian-join :vars (y x) :sources ((MemoryAlias :handle 2 :prefix (z)) (Assign :handle 3 :prefix (z)) ))
        ...
        if (!h_MemoryAlias_2_4.valid() || !h_Assign_3_6.valid()) continue;
...
JitRunner_VF_Assign_MA_D1::LaunchParams JitRunner_VF_Assign_MA_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: MemoryAlias version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<MemoryAlias, FULL_VER>(db) : get_relation_by_schema<MemoryAlias, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<MemoryAlias, FULL_VER>(db) : get_relation_by_schema<MemoryAlias, DELTA_VER>(db);
...
void JitRunner_VF_Assign_MA_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
