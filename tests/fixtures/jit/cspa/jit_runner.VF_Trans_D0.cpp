// =============================================================
// JIT-Generated Complete Runner: VF_Trans_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VF_Trans_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_ValueFlow_1_0_DELTA_VER = views[0];
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_ValueFlow_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_ValueFlow_1_0_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_ValueFlow_1_0_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ValueFlow_1_0_DELTA_VER.num_rows_;
      auto h_ValueFlow_0_root = HandleType(hint_lo_3, hint_hi_4, 0)
                                    .prefix(root_val_2, tile, view_ValueFlow_1_0_DELTA_VER);
      ...
      // MIR: (cartesian-join :vars (x y) :sources ((ValueFlow :handle 2 :prefix (z)) (ValueFlow
      // :handle 3 :prefix (z)) ))
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto x = view_ValueFlow_1_0_DELTA_VER.get_value(1, h_ValueFlow_2_4.begin() + idx0_9);
        auto y = view_ValueFlow_0_1_FULL_VER.get_value(1, h_ValueFlow_3_6.begin() + idx1_10);
...
JitRunner_VF_Trans_D0::LaunchParams JitRunner_VF_Trans_D0::setup(DB& db, uint32_t iteration,
                                                                 GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VF_Trans_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_VF_Trans_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VF_Trans_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_VF_Trans_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_VF_Trans_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                               GPU_STREAM_T stream) {
...
void JitRunner_VF_Trans_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_VF_Trans_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_VF_Trans_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VF_Trans_D0");
...
void JitRunner_VF_Trans_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
