  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_ValueFlow_0_root.valid())
        continue;
      auto h_ValueFlow_1_root = HandleType(0, view_ValueFlow_1_0_FULL_VER.num_rows_, 0)
                                    .prefix(root_val_2, tile, view_ValueFlow_1_0_FULL_VER);
      if (!h_ValueFlow_1_root.valid())
        continue;
      auto z = root_val_2;
      // Nested CartesianJoin: bind y, x from 2 source(s)
      // MIR: (cartesian-join :vars (y x) :sources ((ValueFlow :handle 2 :prefix (z)) (ValueFlow
      // :handle 3 :prefix (z)) ))
      ...
      if (!h_ValueFlow_2_4.valid() || !h_ValueFlow_3_6.valid())
        continue;
...
JitRunner_VF_Trans_D1::LaunchParams JitRunner_VF_Trans_D1::setup(DB& db, uint32_t iteration,
                                                                 GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VF_Trans_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                               GPU_STREAM_T stream) {
