// =============================================================
// JIT-Generated Complete Runner: DefUsedAddr2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_DefUsedAddr2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_RegDefUseDefUsed_2_1_0_3_FULL_VER = views[0];
        auto view_RegUsedFor_0_1_2_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'ea' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var ea :sources ((RegDefUseDefUsed :handle 0) (RegUsedFor :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_RegDefUseDefUsed_2_1_0_3_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_RegDefUseDefUsed_2_1_0_3_FULL_VER.num_rows_) ? hint_hi_4 : view_RegDefUseDefUsed_2_1_0_3_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_RegDefUseDefUsed_2_1_0_3_FULL_VER.num_rows_;
          auto h_RegDefUseDefUsed_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_RegDefUseDefUsed_2_1_0_3_FULL_VER);
          if (!h_RegDefUseDefUsed_0_root.valid()) continue;
          auto h_RegUsedFor_1_root = HandleType(0, view_RegUsedFor_0_1_2_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_RegUsedFor_0_1_2_FULL_VER);
          if (!h_RegUsedFor_1_root.valid()) continue;
          auto ea = root_val_2;
        // Nested ColumnJoin (intersection): bind 'reg' from 2 sources
        // MIR: (column-join :var reg :sources ((RegDefUseDefUsed :handle 2 :prefix (ea)) (RegUsedFor :handle 3 :prefix (ea)) ))
        auto h_RegDefUseDefUsed_2_8 = h_RegDefUseDefUsed_0_root;
        auto h_RegUsedFor_3_9 = h_RegUsedFor_1_root;
        auto intersect_10 = intersect_handles(tile, h_RegDefUseDefUsed_2_8.iterators(view_RegDefUseDefUsed_2_1_0_3_FULL_VER), h_RegUsedFor_3_9.iterators(view_RegUsedFor_0_1_2_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto reg = it_11.value();
          auto positions = it_11.positions();
          auto ch_RegDefUseDefUsed_2_reg = h_RegDefUseDefUsed_2_8.child_range(positions[0], reg, tile, view_RegDefUseDefUsed_2_1_0_3_FULL_VER);
          auto ch_RegUsedFor_3_reg = h_RegUsedFor_3_9.child_range(positions[1], reg, tile, view_RegUsedFor_0_1_2_FULL_VER);
        // Nested CartesianJoin: bind eaDef, mtype, _gen23 from 2 source(s)
        // MIR: (cartesian-join :vars (eaDef mtype _gen23) :sources ((RegDefUseDefUsed :handle 4 :prefix (ea reg)) (RegUsedFor :handle 5 :prefix (ea reg)) ))
        ...
        auto h_RegUsedFor_5_6 = ch_RegUsedFor_3_reg;  // reusing narrowed handle

        if (!h_RegDefUseDefUsed_4_4.valid() || !h_RegUsedFor_5_6.valid()) continue;

        uint32_t degree_3 = h_RegDefUseDefUsed_4_4.degree();
        uint32_t degree_5 = h_RegUsedFor_5_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_RegUsedFor_3_13 = h_RegUsedFor_1_root;
        auto intersect_14 = intersect_handles(tile, h_RegDefUseDefUsed_2_12.iterators(view_RegDefUseDefUsed_2_1_0_3_FULL_VER), h_RegUsedFor_3_13.iterators(view_RegUsedFor_0_1_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto reg = it_15.value();
          auto positions = it_15.positions();
          auto ch_RegDefUseDefUsed_2_reg = h_RegDefUseDefUsed_2_12.child_range(positions[0], reg, tile, view_RegDefUseDefUsed_2_1_0_3_FULL_VER);
          auto ch_RegUsedFor_3_reg = h_RegUsedFor_3_13.child_range(positions[1], reg, tile, view_RegUsedFor_0_1_2_FULL_VER);
        // Nested CartesianJoin: bind eaDef, mtype, _gen23 from 2 source(s)
        // MIR: (cartesian-join :vars (eaDef mtype _gen23) :sources ((RegDefUseDefUsed :handle 4 :prefix (ea reg)) (RegUsedFor :handle 5 :prefix (ea reg)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto eaDef = view_RegDefUseDefUsed_2_1_0_3_FULL_VER.get_value(2, h_RegDefUseDefUsed_4_4.begin() + idx0_9);
          auto _gen23 = view_RegDefUseDefUsed_2_1_0_3_FULL_VER.get_value(3, h_RegDefUseDefUsed_4_4.begin() + idx0_9);
          auto mtype = view_RegUsedFor_0_1_2_FULL_VER.get_value(2, h_RegUsedFor_5_6.begin() + idx1_10);

        // Emit: DefUsedForAddress(eaDef, reg, mtype)
        output_ctx_0.emit_direct(eaDef, reg, mtype);
        }
        }
        }
  }
...
JitRunner_DefUsedAddr2::LaunchParams JitRunner_DefUsedAddr2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: RegDefUseDefUsed version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<RegDefUseDefUsed, FULL_VER>(db);
  ...
  // Source 1: RegUsedFor version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<RegUsedFor, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<RegDefUseDefUsed, FULL_VER>(db);
...
void JitRunner_DefUsedAddr2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_DefUsedAddr2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_DefUsedAddr2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_DefUsedAddr2::read_total(LaunchParams& p) {
...
void JitRunner_DefUsedAddr2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_DefUsedAddr2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_DefUsedAddr2::read_fused_result(LaunchParams& p) {
...
void JitRunner_DefUsedAddr2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("DefUsedAddr2");
...
void JitRunner_DefUsedAddr2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
