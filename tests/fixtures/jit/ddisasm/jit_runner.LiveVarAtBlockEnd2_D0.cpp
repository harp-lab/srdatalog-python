// =============================================================
// JIT-Generated Complete Runner: LiveVarAtBlockEnd2_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_LiveVarAtBlockEnd2_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_RegDefUseRefInBlock_0_1_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'blk' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var blk :sources ((RegDefUseLiveVarAtBlockEnd :handle 0) (BlockNext :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto blk = root_val_2;
        // Nested CartesianJoin: bind blockUsed, mvar, prevBlock, _gen19 from 2 source(s)
        // MIR: (cartesian-join :vars (blockUsed mvar prevBlock _gen19) :sources ((RegDefUseLiveVarAtBlockEnd :handle 2 :prefix (blk)) (BlockNext :handle 3 :prefix (blk)) ))
        uint32_t lane_3 = tile.thread_rank();
        uint32_t group_size_4 = tile.size();

        auto h_RegDefUseLiveVarAtBlockEnd_2_6 = h_RegDefUseLiveVarAtBlockEnd_0_root;  // reusing narrowed handle
        auto h_BlockNext_3_8 = h_BlockNext_1_root;  // reusing narrowed handle

        if (!h_RegDefUseLiveVarAtBlockEnd_2_6.valid() || !h_BlockNext_3_8.valid()) continue;
        ...
        auto h_RegDefUseRefInBlock_neg_pre_1 = HandleType(0, view_RegDefUseRefInBlock_0_1_FULL_VER.num_rows_, 0).prefix(blk, tile, view_RegDefUseRefInBlock_0_1_FULL_VER);
        ...
        for (uint32_t flat_idx_10 = lane_3; flat_idx_10 < total_9; flat_idx_10 += group_size_4) {
          ...
          auto blockUsed = view_RegDefUseLiveVarAtBlockEnd_0_1_2_DELTA_VER.get_value(1, h_RegDefUseLiveVarAtBlockEnd_2_6.begin() + idx0_11);
          auto mvar = view_RegDefUseLiveVarAtBlockEnd_0_1_2_DELTA_VER.get_value(2, h_RegDefUseLiveVarAtBlockEnd_2_6.begin() + idx0_11);
        ...
        auto h_RegDefUseRefInBlock_neg_4_2 = h_RegDefUseRefInBlock_neg_pre_1.prefix_seq(mvar, view_RegDefUseRefInBlock_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_10 = lane_3; flat_idx_10 < total_9; flat_idx_10 += group_size_4) {
          ...
          auto mvar = view_RegDefUseLiveVarAtBlockEnd_0_1_2_DELTA_VER.get_value(2, h_RegDefUseLiveVarAtBlockEnd_2_6.begin() + idx0_11);
          auto prevBlock = view_BlockNext_2_0_1_FULL_VER.get_value(1, h_BlockNext_3_8.begin() + idx1_12);
          auto _gen19 = view_BlockNext_2_0_1_FULL_VER.get_value(2, h_BlockNext_3_8.begin() + idx1_12);
        ...
        if (!h_RegDefUseRefInBlock_neg_4_2.valid()) {
        // Emit: RegDefUseLiveVarAtBlockEnd(prevBlock, blockUsed, mvar)
        output_ctx_0.emit_direct(prevBlock, blockUsed, mvar);
        }
        }
        }
  }
...
JitRunner_LiveVarAtBlockEnd2_D0::LaunchParams JitRunner_LiveVarAtBlockEnd2_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 4: RegDefUseRefInBlock version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<RegDefUseRefInBlock, FULL_VER>(db);
...
void JitRunner_LiveVarAtBlockEnd2_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LiveVarAtBlockEnd2_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtBlockEnd2_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LiveVarAtBlockEnd2_D0::read_total(LaunchParams& p) {
...
void JitRunner_LiveVarAtBlockEnd2_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtBlockEnd2_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtBlockEnd2_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_LiveVarAtBlockEnd2_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LiveVarAtBlockEnd2_D0");
...
void JitRunner_LiveVarAtBlockEnd2_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
