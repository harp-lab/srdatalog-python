// =============================================================
// JIT-Generated Complete Runner: LiveVarAtPriorUsed_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_LiveVarAtPriorUsed_D0 {
  ...
  using DestSchema = RegDefUseLiveVarAtPriorUsed;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_RegDefUseDefinedInBlock_0_1_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'blk' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var blk :sources ((RegDefUseLiveVarAtBlockEnd :handle 0) (RegDefUseUsedInBlock :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER.num_rows_;
          auto h_RegDefUseLiveVarAtBlockEnd_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER);
          if (!h_RegDefUseLiveVarAtBlockEnd_0_root.valid()) continue;
          auto h_RegDefUseUsedInBlock_1_root = HandleType(0, view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER);
          if (!h_RegDefUseUsedInBlock_1_root.valid()) continue;
          auto blk = root_val_2;
        // Nested ColumnJoin (intersection): bind 'mvar' from 2 sources
        // MIR: (column-join :var mvar :sources ((RegDefUseLiveVarAtBlockEnd :handle 2 :prefix (blk)) (RegDefUseUsedInBlock :handle 3 :prefix (blk)) ))
        ...
        auto intersect_15 = intersect_handles(tile, h_RegDefUseLiveVarAtBlockEnd_2_13.iterators(view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER), h_RegDefUseUsedInBlock_3_14.iterators(view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER));
        for (auto it_16 = intersect_15.begin(); it_16.valid(); it_16.next()) {
          auto mvar = it_16.value();
          auto positions = it_16.positions();
          auto ch_RegDefUseLiveVarAtBlockEnd_2_mvar = h_RegDefUseLiveVarAtBlockEnd_2_13.child_range(positions[0], mvar, tile, view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER);
          auto ch_RegDefUseUsedInBlock_3_mvar = h_RegDefUseUsedInBlock_3_14.child_range(positions[1], mvar, tile, view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER);
        // Nested CartesianJoin: bind blockUsed, eaUsed, _gen15 from 2 source(s)
        // MIR: (cartesian-join :vars (blockUsed eaUsed _gen15) :sources ((RegDefUseLiveVarAtBlockEnd :handle 4 :prefix (blk mvar)) (RegDefUseUsedInBlock :handle 5 :prefix (blk mvar)) ))
        uint32_t lane_2 = tile.thread_rank();
        uint32_t group_size_3 = tile.size();

        auto h_RegDefUseLiveVarAtBlockEnd_4_5 = ch_RegDefUseLiveVarAtBlockEnd_2_mvar;  // reusing narrowed handle
        auto h_RegDefUseUsedInBlock_5_7 = ch_RegDefUseUsedInBlock_3_mvar;  // reusing narrowed handle

        if (!h_RegDefUseLiveVarAtBlockEnd_4_5.valid() || !h_RegDefUseUsedInBlock_5_7.valid()) continue;
        ...
        auto h_RegDefUseDefinedInBlock_neg_pre_1 = HandleType(0, view_RegDefUseDefinedInBlock_0_1_FULL_VER.num_rows_, 0).prefix(blk, tile, view_RegDefUseDefinedInBlock_0_1_FULL_VER).prefix(mvar, tile, view_RegDefUseDefinedInBlock_0_1_FULL_VER);
        ...
        for (uint32_t flat_idx_9 = lane_2; flat_idx_9 < total_8; flat_idx_9 += group_size_3) {
          ...
          auto blockUsed = view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER.get_value(2, h_RegDefUseLiveVarAtBlockEnd_4_5.begin() + idx0_10);
          auto eaUsed = view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER.get_value(2, h_RegDefUseUsedInBlock_5_7.begin() + idx1_11);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_9 = lane_2; flat_idx_9 < total_8; flat_idx_9 += group_size_3) {
          ...
          auto blockUsed = view_RegDefUseLiveVarAtBlockEnd_0_2_1_DELTA_VER.get_value(2, h_RegDefUseLiveVarAtBlockEnd_4_5.begin() + idx0_10);
          auto eaUsed = view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER.get_value(2, h_RegDefUseUsedInBlock_5_7.begin() + idx1_11);
          auto _gen15 = view_RegDefUseUsedInBlock_0_2_1_3_FULL_VER.get_value(3, h_RegDefUseUsedInBlock_5_7.begin() + idx1_11);
        ...
        if (!h_RegDefUseDefinedInBlock_neg_pre_1.valid()) {
        // Emit: RegDefUseLiveVarAtPriorUsed(eaUsed, blockUsed, mvar)
        output_ctx_0.emit_direct(eaUsed, blockUsed, mvar);
        }
        }
        }
        }
...
JitRunner_LiveVarAtPriorUsed_D0::LaunchParams JitRunner_LiveVarAtPriorUsed_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: RegDefUseLiveVarAtBlockEnd version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<RegDefUseLiveVarAtBlockEnd, FULL_VER>(db) : get_relation_by_schema<RegDefUseLiveVarAtBlockEnd, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 2, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: RegDefUseUsedInBlock version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<RegDefUseUsedInBlock, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 2, 1, 3}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 6: RegDefUseDefinedInBlock version FULL_VER
  {
    auto& rel_6 = get_relation_by_schema<RegDefUseDefinedInBlock, FULL_VER>(db);
    auto& idx_6 = rel_6.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_6.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<RegDefUseLiveVarAtBlockEnd, FULL_VER>(db) : get_relation_by_schema<RegDefUseLiveVarAtBlockEnd, DELTA_VER>(db);
...
void JitRunner_LiveVarAtPriorUsed_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LiveVarAtPriorUsed_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<RegDefUseLiveVarAtPriorUsed, NEW_VER>(db);
...
void JitRunner_LiveVarAtPriorUsed_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LiveVarAtPriorUsed_D0::read_total(LaunchParams& p) {
...
void JitRunner_LiveVarAtPriorUsed_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtPriorUsed_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtPriorUsed_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_LiveVarAtPriorUsed_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LiveVarAtPriorUsed_D0");
...
void JitRunner_LiveVarAtPriorUsed_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
