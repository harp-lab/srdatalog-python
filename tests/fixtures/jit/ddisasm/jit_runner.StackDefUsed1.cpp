...
struct JitRunner_StackDefUsed1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_StackDefUseUsed_0_1_2_3_FULL_VER = views[0];
        auto view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'eaUsed' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var eaUsed :sources ((StackDefUseUsed :handle 0) (StackDefUseBlockLastDef :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_StackDefUseUsed_0_1_2_3_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_StackDefUseUsed_0_1_2_3_FULL_VER.num_rows_) ? hint_hi_4 : view_StackDefUseUsed_0_1_2_3_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_StackDefUseUsed_0_1_2_3_FULL_VER.num_rows_;
          auto h_StackDefUseUsed_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_StackDefUseUsed_0_1_2_3_FULL_VER);
          if (!h_StackDefUseUsed_0_root.valid()) continue;
          auto h_StackDefUseBlockLastDef_1_root = HandleType(0, view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER);
          if (!h_StackDefUseBlockLastDef_1_root.valid()) continue;
        ...
        auto h_StackDefUseUsed_2_12 = h_StackDefUseUsed_0_root;
        auto h_StackDefUseBlockLastDef_3_13 = h_StackDefUseBlockLastDef_1_root;
        auto intersect_14 = intersect_handles(tile, h_StackDefUseUsed_2_12.iterators(view_StackDefUseUsed_0_1_2_3_FULL_VER), h_StackDefUseBlockLastDef_3_13.iterators(view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto varr = it_15.value();
          auto positions = it_15.positions();
          auto ch_StackDefUseUsed_2_varr = h_StackDefUseUsed_2_12.child_range(positions[0], varr, tile, view_StackDefUseUsed_0_1_2_3_FULL_VER);
          auto ch_StackDefUseBlockLastDef_3_varr = h_StackDefUseBlockLastDef_3_13.child_range(positions[1], varr, tile, view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER);
        ...
        auto h_StackDefUseUsed_4_8 = ch_StackDefUseUsed_2_varr;
        auto h_StackDefUseBlockLastDef_5_9 = ch_StackDefUseBlockLastDef_3_varr;
        auto intersect_10 = intersect_handles(tile, h_StackDefUseUsed_4_8.iterators(view_StackDefUseUsed_0_1_2_3_FULL_VER), h_StackDefUseBlockLastDef_5_9.iterators(view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto varp = it_11.value();
          auto positions = it_11.positions();
          auto ch_StackDefUseUsed_4_varp = h_StackDefUseUsed_4_8.child_range(positions[0], varp, tile, view_StackDefUseUsed_0_1_2_3_FULL_VER);
          auto ch_StackDefUseBlockLastDef_5_varp = h_StackDefUseBlockLastDef_5_9.child_range(positions[1], varp, tile, view_StackDefUseBlockLastDef_0_2_3_1_FULL_VER);
...
JitRunner_StackDefUsed1::LaunchParams JitRunner_StackDefUsed1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: StackDefUseUsed version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<StackDefUseUsed, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: StackDefUseBlockLastDef version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<StackDefUseBlockLastDef, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<StackDefUseUsed, FULL_VER>(db);
...
uint32_t JitRunner_StackDefUsed1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_StackDefUsed1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_StackDefUsed1::read_total(LaunchParams& p) {
...
void JitRunner_StackDefUsed1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_StackDefUsed1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_StackDefUsed1::read_fused_result(LaunchParams& p) {
...
void JitRunner_StackDefUsed1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("StackDefUsed1");
...
void JitRunner_StackDefUsed1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
