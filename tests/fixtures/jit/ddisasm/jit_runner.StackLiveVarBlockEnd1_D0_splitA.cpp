// =============================================================
// JIT-Generated Complete Runner: StackLiveVarBlockEnd1_D0_splitA
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_StackLiveVarBlockEnd1_D0_splitA {
  ...
  using DestSchema = _temp_StackLiveVarBlockEnd1;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_StackDefUseLiveVarAtBlockEnd_0_2_3_1_DELTA_VER = views[0];
        auto view_StackDefUseRefInBlock_0_1_2_FULL_VER = views[1];
        auto view_RegDefUseDefinedInBlock_0_1_FULL_VER = views[2];

        // Root Scan: StackDefUseLiveVarAtBlockEnd binding blk, varr, varp, blockUsed
        // MIR: (scan :rel StackDefUseLiveVarAtBlockEnd :vars (blk varr varp blockUsed) :handle 0)
        auto root_handle_1 = HandleType(0, view_StackDefUseLiveVarAtBlockEnd_0_2_3_1_DELTA_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto blk = view_StackDefUseLiveVarAtBlockEnd_0_2_3_1_DELTA_VER.get_value(0, idx_3);
          auto varr = view_StackDefUseLiveVarAtBlockEnd_0_2_3_1_DELTA_VER.get_value(1, idx_3);
          auto varp = view_StackDefUseLiveVarAtBlockEnd_0_2_3_1_DELTA_VER.get_value(2, idx_3);
          auto blockUsed = view_StackDefUseLiveVarAtBlockEnd_0_2_3_1_DELTA_VER.get_value(3, idx_3);
        // Negation: NOT EXISTS in StackDefUseRefInBlock
        // MIR: (negation :rel StackDefUseRefInBlock :prefix (blk varr varp) :handle 1)
        auto h_StackDefUseRefInBlock_neg_1_2 = HandleType(0, view_StackDefUseRefInBlock_0_1_2_FULL_VER.num_rows_, 0).prefix(blk, tile, view_StackDefUseRefInBlock_0_1_2_FULL_VER).prefix(varr, tile, view_StackDefUseRefInBlock_0_1_2_FULL_VER).prefix(varp, tile, view_StackDefUseRefInBlock_0_1_2_FULL_VER);
        if (!h_StackDefUseRefInBlock_neg_1_2.valid()) {
        // Negation: NOT EXISTS in RegDefUseDefinedInBlock
        // MIR: (negation :rel RegDefUseDefinedInBlock :prefix (blk varr) :handle 2)
        auto h_RegDefUseDefinedInBlock_neg_2_1 = HandleType(0, view_RegDefUseDefinedInBlock_0_1_FULL_VER.num_rows_, 0).prefix(blk, tile, view_RegDefUseDefinedInBlock_0_1_FULL_VER).prefix(varr, tile, view_RegDefUseDefinedInBlock_0_1_FULL_VER);
        if (!h_RegDefUseDefinedInBlock_neg_2_1.valid()) {
        // Emit: _temp_StackLiveVarBlockEnd1(blk, blockUsed, varr, varp)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_RegDefUseDefinedInBlock_neg_2_1.valid()) {
        // Emit: _temp_StackLiveVarBlockEnd1(blk, blockUsed, varr, varp)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(blk, blockUsed, varr, varp);
        }
        }
        }
  }
...
JitRunner_StackLiveVarBlockEnd1_D0_splitA::LaunchParams JitRunner_StackLiveVarBlockEnd1_D0_splitA::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: StackDefUseLiveVarAtBlockEnd version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<StackDefUseLiveVarAtBlockEnd, FULL_VER>(db) : get_relation_by_schema<StackDefUseLiveVarAtBlockEnd, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 2, 3, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: StackDefUseRefInBlock version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<StackDefUseRefInBlock, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, true);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 2: RegDefUseDefinedInBlock version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<RegDefUseDefinedInBlock, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_2.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<StackDefUseLiveVarAtBlockEnd, FULL_VER>(db) : get_relation_by_schema<StackDefUseLiveVarAtBlockEnd, DELTA_VER>(db);
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_StackLiveVarBlockEnd1_D0_splitA::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<_temp_StackLiveVarBlockEnd1, NEW_VER>(db);
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_StackLiveVarBlockEnd1_D0_splitA::read_total(LaunchParams& p) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::read_fused_result(LaunchParams& p) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("StackLiveVarBlockEnd1_D0_splitA");
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitA::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
