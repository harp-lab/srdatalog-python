...
struct JitRunner_StackLiveVarBlockEnd1_D0_splitB {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER = views[0];
        auto view_BlockNext_2_0_1_FULL_VER = views[1];

        // Root Scan: _temp_StackLiveVarBlockEnd1 binding blk, blockUsed, varr, varp
        // MIR: (scan :rel _temp_StackLiveVarBlockEnd1 :vars (blk blockUsed varr varp) :handle 0)
        auto root_handle_1 = HandleType(0, view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto blk = view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER.get_value(0, idx_3);
        // Nested CartesianJoin: bind prevBlock from 1 source(s)
        // MIR: (cartesian-join :vars (prevBlock) :sources ((BlockNext :handle 1 :prefix (blk)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_BlockNext_1_4 = HandleType(0, view_BlockNext_2_0_1_FULL_VER.num_rows_, 0).prefix(blk, tile, view_BlockNext_2_0_1_FULL_VER);

        if (!h_BlockNext_1_4.valid()) continue;

        uint32_t degree_3 = h_BlockNext_1_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto blk = view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER.get_value(0, idx_3);
          auto blockUsed = view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER.get_value(1, idx_3);
          auto varr = view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER.get_value(2, idx_3);
          auto varp = view__temp_StackLiveVarBlockEnd1_0_1_2_3_NEW_VER.get_value(3, idx_3);
        // Nested CartesianJoin: bind prevBlock from 1 source(s)
        // MIR: (cartesian-join :vars (prevBlock) :sources ((BlockNext :handle 1 :prefix (blk)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto prevBlock = view_BlockNext_2_0_1_FULL_VER.get_value(1, h_BlockNext_1_4.begin() + idx0_7);

        // Emit: StackDefUseLiveVarAtBlockEnd(prevBlock, blockUsed, varr, varp)
        output_ctx_0.emit_direct(prevBlock, blockUsed, varr, varp);
        }
        }
  }
...
JitRunner_StackLiveVarBlockEnd1_D0_splitB::LaunchParams JitRunner_StackLiveVarBlockEnd1_D0_splitB::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: _temp_StackLiveVarBlockEnd1 version NEW_VER
  {
    auto& rel_0 = get_relation_by_schema<_temp_StackLiveVarBlockEnd1, NEW_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<_temp_StackLiveVarBlockEnd1, NEW_VER>(db);
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_StackLiveVarBlockEnd1_D0_splitB::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_StackLiveVarBlockEnd1_D0_splitB::read_total(LaunchParams& p) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::read_fused_result(LaunchParams& p) {
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("StackLiveVarBlockEnd1_D0_splitB");
...
void JitRunner_StackLiveVarBlockEnd1_D0_splitB::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
