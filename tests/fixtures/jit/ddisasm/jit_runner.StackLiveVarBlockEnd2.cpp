// =============================================================
// JIT-Generated Complete Runner: StackLiveVarBlockEnd2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_StackLiveVarBlockEnd2 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DdisasmPlan_DB_DeviceDB;
  using FirstSchema = BlockNext;
  using DestSchema = StackDefUseLiveVarAtBlockEnd;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_BlockNext_2_0_1_FULL_VER = views[0];
        auto view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'blk' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var blk :sources ((BlockNext :handle 0) (StackDefUseLiveVarUsedEDB :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_BlockNext_2_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_BlockNext_2_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_BlockNext_2_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_BlockNext_2_0_1_FULL_VER.num_rows_;
          auto h_BlockNext_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_BlockNext_2_0_1_FULL_VER);
          if (!h_BlockNext_0_root.valid()) continue;
          auto h_StackDefUseLiveVarUsedEDB_1_root = HandleType(0, view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER);
          if (!h_StackDefUseLiveVarUsedEDB_1_root.valid()) continue;
          auto blk = root_val_2;
        // Nested CartesianJoin: bind prevBlock, _gen35, varr, varp, _gen36, _gen37, _gen38, _gen39, _gen40 from 2 source(s)
        // MIR: (cartesian-join :vars (prevBlock _gen35 varr varp _gen36 _gen37 _gen38 _gen39 _gen40) :sources ((BlockNext :handle 2 :prefix (blk)) (StackDefUseLiveVarUsedEDB :handle 3 :prefix (blk)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_BlockNext_2_4 = h_BlockNext_0_root;  // reusing narrowed handle
        auto h_StackDefUseLiveVarUsedEDB_3_6 = h_StackDefUseLiveVarUsedEDB_1_root;  // reusing narrowed handle

        if (!h_BlockNext_2_4.valid() || !h_StackDefUseLiveVarUsedEDB_3_6.valid()) continue;

        uint32_t degree_3 = h_BlockNext_2_4.degree();
        uint32_t degree_5 = h_StackDefUseLiveVarUsedEDB_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto prevBlock = view_BlockNext_2_0_1_FULL_VER.get_value(1, h_BlockNext_2_4.begin() + idx0_9);
          auto _gen35 = view_BlockNext_2_0_1_FULL_VER.get_value(2, h_BlockNext_2_4.begin() + idx0_9);
          auto varr = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(1, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);
          auto varp = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(2, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);
          auto _gen36 = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(3, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);
          auto _gen37 = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(4, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);
          auto _gen38 = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(5, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);
          auto _gen39 = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(6, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);
          auto _gen40 = view_StackDefUseLiveVarUsedEDB_0_1_2_3_4_5_6_7_FULL_VER.get_value(7, h_StackDefUseLiveVarUsedEDB_3_6.begin() + idx1_10);

        // Emit: StackDefUseLiveVarAtBlockEnd(prevBlock, blk, varr, varp)
        output_ctx_0.emit_direct(prevBlock, blk, varr, varp);
        }
        }
  }
...
JitRunner_StackLiveVarBlockEnd2::LaunchParams JitRunner_StackLiveVarBlockEnd2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: BlockNext version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<BlockNext, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{2, 0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: StackDefUseLiveVarUsedEDB version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<StackDefUseLiveVarUsedEDB, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3, 4, 5, 6, 7}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<BlockNext, FULL_VER>(db);
  auto& first_idx = first_rel.get_index(SRDatalog::IndexSpec{{2, 0, 1}});
...
void JitRunner_StackLiveVarBlockEnd2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_StackLiveVarBlockEnd2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<StackDefUseLiveVarAtBlockEnd, NEW_VER>(db);
...
void JitRunner_StackLiveVarBlockEnd2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_StackLiveVarBlockEnd2::read_total(LaunchParams& p) {
...
void JitRunner_StackLiveVarBlockEnd2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_StackLiveVarBlockEnd2::read_fused_result(LaunchParams& p) {
...
void JitRunner_StackLiveVarBlockEnd2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("StackLiveVarBlockEnd2");
...
void JitRunner_StackLiveVarBlockEnd2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
