...
static void step_0(DB& db, std::size_t max_iterations) {
  ...
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<TRel, std::integer_sequence<int, 1, 0>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<RRel, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<SRel, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<SRel, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<TRel, std::integer_sequence<int, 1, 0>, FULL_VER>>(db, 0);
  if (_tail_mode)
    JitRunner_TriangleDebug::execute_fused(db, 0);
  else
    JitRunner_TriangleDebug::execute(db, 0);
