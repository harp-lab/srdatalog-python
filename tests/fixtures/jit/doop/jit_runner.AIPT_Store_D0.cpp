...
struct JitRunner_AIPT_Store_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_HeapAllocSuperType_1_0_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'frm' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var frm :sources ((ReachableSortedIndex :handle 0) (VarPointsTo :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_ReachableSortedIndex_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_ReachableSortedIndex_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_ReachableSortedIndex_0_1_DELTA_VER.num_rows_;
        ...
        auto h_ReachableSortedIndex_2_9 = h_ReachableSortedIndex_0_root;
        auto h_VarPointsTo_3_10 = HandleType(0, view_VarPointsTo_1_0_FULL_VER.num_rows_, 0);
        auto intersect_11 = intersect_handles(tile, h_ReachableSortedIndex_2_9.iterators(view_ReachableSortedIndex_0_1_DELTA_VER), h_VarPointsTo_3_10.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_12 = intersect_11.begin(); it_12.valid(); it_12.next()) {
          auto base = it_12.value();
          auto positions = it_12.positions();
          auto ch_ReachableSortedIndex_2_base = h_ReachableSortedIndex_2_9.child_range(positions[0], base, tile, view_ReachableSortedIndex_0_1_DELTA_VER);
          auto ch_VarPointsTo_3_base = h_VarPointsTo_3_10.child_range(positions[1], base, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'baseheap' from 2 sources
        // MIR: (column-join :var baseheap :sources ((VarPointsTo :handle 4 :prefix (base)) (HeapAllocSuperType :handle 5 :prefix ()) ))
        auto h_VarPointsTo_4_5 = ch_VarPointsTo_3_base;
        auto h_HeapAllocSuperType_5_6 = HandleType(0, view_HeapAllocSuperType_1_0_FULL_VER.num_rows_, 0);
        auto intersect_7 = intersect_handles(tile, h_VarPointsTo_4_5.iterators(view_VarPointsTo_1_0_FULL_VER), h_HeapAllocSuperType_5_6.iterators(view_HeapAllocSuperType_1_0_FULL_VER));
        for (auto it_8 = intersect_7.begin(); it_8.valid(); it_8.next()) {
          auto baseheap = it_8.value();
          auto positions = it_8.positions();
          auto ch_VarPointsTo_4_baseheap = h_VarPointsTo_4_5.child_range(positions[0], baseheap, tile, view_VarPointsTo_1_0_FULL_VER);
          auto ch_HeapAllocSuperType_5_baseheap = h_HeapAllocSuperType_5_6.child_range(positions[1], baseheap, tile, view_HeapAllocSuperType_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'heap' from 2 sources
        // MIR: (column-join :var heap :sources ((VarPointsTo :handle 6 :prefix (frm)) (HeapAllocSuperType :handle 7 :prefix (baseheap)) ))
        auto h_VarPointsTo_6_1 = h_VarPointsTo_1_root;
        auto h_HeapAllocSuperType_7_2 = ch_HeapAllocSuperType_5_baseheap;
        auto intersect_3 = intersect_handles(tile, h_VarPointsTo_6_1.iterators(view_VarPointsTo_1_0_FULL_VER), h_HeapAllocSuperType_7_2.iterators(view_HeapAllocSuperType_1_0_FULL_VER));
        for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
          auto heap = it_4.value();
          auto positions = it_4.positions();
          auto ch_VarPointsTo_6_heap = h_VarPointsTo_6_1.child_range(positions[0], heap, tile, view_VarPointsTo_1_0_FULL_VER);
          auto ch_HeapAllocSuperType_7_heap = h_HeapAllocSuperType_7_2.child_range(positions[1], heap, tile, view_HeapAllocSuperType_1_0_FULL_VER);
...
JitRunner_AIPT_Store_D0::LaunchParams JitRunner_AIPT_Store_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ReachableSortedIndex version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ReachableSortedIndex, FULL_VER>(db) : get_relation_by_schema<ReachableSortedIndex, DELTA_VER>(db);
  ...
  {
    auto& rel_5 = get_relation_by_schema<HeapAllocSuperType, FULL_VER>(db);
    auto& idx_5 = rel_5.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_5.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ReachableSortedIndex, FULL_VER>(db) : get_relation_by_schema<ReachableSortedIndex, DELTA_VER>(db);
...
void JitRunner_AIPT_Store_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_AIPT_Store_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_AIPT_Store_D0::read_total(LaunchParams& p) {
...
void JitRunner_AIPT_Store_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_AIPT_Store_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("AIPT_Store_D0");
...
void JitRunner_AIPT_Store_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
