// =============================================================
// JIT-Generated Complete Runner: AIPT_Store_D1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_AIPT_Store_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (auto it_12 = intersect_11.begin(); it_12.valid(); it_12.next()) {
          ...
          auto ch_VarPointsTo_3_frm = h_VarPointsTo_3_10.child_range(positions[1], frm, tile, view_VarPointsTo_1_0_FULL_VER);
        ...
        auto h_VarPointsTo_4_5 = h_VarPointsTo_0_root;
        auto h_HeapAllocSuperType_5_6 = HandleType(0, view_HeapAllocSuperType_1_0_FULL_VER.num_rows_, 0);
        auto intersect_7 = intersect_handles(tile, h_VarPointsTo_4_5.iterators(view_VarPointsTo_1_0_DELTA_VER), h_HeapAllocSuperType_5_6.iterators(view_HeapAllocSuperType_1_0_FULL_VER));
        for (auto it_8 = intersect_7.begin(); it_8.valid(); it_8.next()) {
          auto baseheap = it_8.value();
          auto positions = it_8.positions();
          auto ch_VarPointsTo_4_baseheap = h_VarPointsTo_4_5.child_range(positions[0], baseheap, tile, view_VarPointsTo_1_0_DELTA_VER);
        ...
        auto h_VarPointsTo_6_1 = ch_VarPointsTo_3_frm;
...
JitRunner_AIPT_Store_D1::LaunchParams JitRunner_AIPT_Store_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_AIPT_Store_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_AIPT_Store_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_AIPT_Store_D1::read_total(LaunchParams& p) {
...
void JitRunner_AIPT_Store_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_AIPT_Store_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("AIPT_Store_D1");
...
void JitRunner_AIPT_Store_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
