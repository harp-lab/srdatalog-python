// =============================================================
// JIT-Generated Complete Runner: AIPT_Store_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_AIPT_Store_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_ReachableSortedIndex_0_1_FULL_VER = views[2];
...
JitRunner_AIPT_Store_D2::LaunchParams JitRunner_AIPT_Store_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: HeapAllocSuperType version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<HeapAllocSuperType, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: ReachableSortedIndex version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<ReachableSortedIndex, FULL_VER>(db);
...
void JitRunner_AIPT_Store_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_AIPT_Store_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_AIPT_Store_D2::read_total(LaunchParams& p) {
...
void JitRunner_AIPT_Store_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_AIPT_Store_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("AIPT_Store_D2");
...
void JitRunner_AIPT_Store_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
