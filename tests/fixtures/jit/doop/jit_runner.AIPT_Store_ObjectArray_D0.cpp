// =============================================================
// JIT-Generated Complete Runner: AIPT_Store_ObjectArray_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_AIPT_Store_ObjectArray_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto h_VarPointsTo_6_4 = h_VarPointsTo_1_root;  // reusing narrowed handle
...
JitRunner_AIPT_Store_ObjectArray_D0::LaunchParams JitRunner_AIPT_Store_ObjectArray_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_AIPT_Store_ObjectArray_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_AIPT_Store_ObjectArray_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_ObjectArray_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_AIPT_Store_ObjectArray_D0::read_total(LaunchParams& p) {
...
void JitRunner_AIPT_Store_ObjectArray_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_ObjectArray_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_AIPT_Store_ObjectArray_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_AIPT_Store_ObjectArray_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("AIPT_Store_ObjectArray_D0");
...
void JitRunner_AIPT_Store_ObjectArray_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
