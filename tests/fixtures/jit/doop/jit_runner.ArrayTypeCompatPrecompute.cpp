// =============================================================
// JIT-Generated Complete Runner: ArrayTypeCompatPrecompute
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_ArrayTypeCompatPrecompute {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = HeapAllocation_Type;
  using DestSchema = ArrayTypeCompat;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_ComponentType_0_1_FULL_VER = views[1];
        auto view_SupertypeOf_1_0_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'baseheaptype' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var baseheaptype :sources ((HeapAllocation_Type :handle 0) (ComponentType :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_ComponentType_1_root = HandleType(0, view_ComponentType_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_ComponentType_0_1_FULL_VER);
          if (!h_ComponentType_1_root.valid()) continue;
          auto baseheaptype = root_val_2;
        // Nested ColumnJoin (intersection): bind 'basecomptype' from 2 sources
        // MIR: (column-join :var basecomptype :sources ((ComponentType :handle 2 :prefix (baseheaptype)) (SupertypeOf :handle 3 :prefix ()) ))
        auto h_ComponentType_2_8 = h_ComponentType_1_root;
        auto h_SupertypeOf_3_9 = HandleType(0, view_SupertypeOf_1_0_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_ComponentType_2_8.iterators(view_ComponentType_0_1_FULL_VER), h_SupertypeOf_3_9.iterators(view_SupertypeOf_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto basecomptype = it_11.value();
          auto positions = it_11.positions();
          auto ch_ComponentType_2_basecomptype = h_ComponentType_2_8.child_range(positions[0], basecomptype, tile, view_ComponentType_0_1_FULL_VER);
          auto ch_SupertypeOf_3_basecomptype = h_SupertypeOf_3_9.child_range(positions[1], basecomptype, tile, view_SupertypeOf_1_0_FULL_VER);
        // Nested CartesianJoin: bind vtype, baseheap from 2 source(s)
        // MIR: (cartesian-join :vars (vtype baseheap) :sources ((HeapAllocation_Type :handle 4 :prefix (baseheaptype)) (SupertypeOf :handle 5 :prefix (basecomptype)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_HeapAllocation_Type_4_4 = h_HeapAllocation_Type_0_root;  // reusing narrowed handle
        auto h_SupertypeOf_5_6 = ch_SupertypeOf_3_basecomptype;  // reusing narrowed handle

        if (!h_HeapAllocation_Type_4_4.valid() || !h_SupertypeOf_5_6.valid()) continue;

        uint32_t degree_3 = h_HeapAllocation_Type_4_4.degree();
        uint32_t degree_5 = h_SupertypeOf_5_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'basecomptype' from 2 sources
        // MIR: (column-join :var basecomptype :sources ((ComponentType :handle 2 :prefix (baseheaptype)) (SupertypeOf :handle 3 :prefix ()) ))
        auto h_ComponentType_2_12 = h_ComponentType_1_root;
        auto h_SupertypeOf_3_13 = HandleType(0, view_SupertypeOf_1_0_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_ComponentType_2_12.iterators(view_ComponentType_0_1_FULL_VER), h_SupertypeOf_3_13.iterators(view_SupertypeOf_1_0_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto basecomptype = it_15.value();
          auto positions = it_15.positions();
          auto ch_ComponentType_2_basecomptype = h_ComponentType_2_12.child_range(positions[0], basecomptype, tile, view_ComponentType_0_1_FULL_VER);
          auto ch_SupertypeOf_3_basecomptype = h_SupertypeOf_3_13.child_range(positions[1], basecomptype, tile, view_SupertypeOf_1_0_FULL_VER);
        // Nested CartesianJoin: bind vtype, baseheap from 2 source(s)
        // MIR: (cartesian-join :vars (vtype baseheap) :sources ((HeapAllocation_Type :handle 4 :prefix (baseheaptype)) (SupertypeOf :handle 5 :prefix (basecomptype)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto baseheap = view_HeapAllocation_Type_1_0_FULL_VER.get_value(1, h_HeapAllocation_Type_4_4.begin() + idx0_9);
          auto vtype = view_SupertypeOf_1_0_FULL_VER.get_value(1, h_SupertypeOf_5_6.begin() + idx1_10);

        // Emit: ArrayTypeCompat(baseheap, vtype)
        output_ctx_0.emit_direct(baseheap, vtype);
        }
        }
        }
  }
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      uint32_t bg_hhi = view_HeapAllocation_Type_1_0_FULL_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_HeapAllocation_Type_1_0_FULL_VER.num_rows_) ? bg_hhi : view_HeapAllocation_Type_1_0_FULL_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_HeapAllocation_Type_1_0_FULL_VER.num_rows_;
      auto h_HeapAllocation_Type_0_root = HandleType(bg_hlo, bg_hhi, 0).prefix(bg_hist_root_val, tile, view_HeapAllocation_Type_1_0_FULL_VER);
      if (!h_HeapAllocation_Type_0_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      auto h_ComponentType_1_root = HandleType(0, view_ComponentType_0_1_FULL_VER.num_rows_, 0).prefix(bg_hist_root_val, tile, view_ComponentType_0_1_FULL_VER);
      if (!h_ComponentType_1_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      uint64_t bg_deg = 1;
      bg_deg *= h_HeapAllocation_Type_0_root.degree();
      bg_deg *= h_ComponentType_1_root.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root ColumnJoin (BLOCK-GROUP): bind 'baseheaptype' from 2 sources
        ...
        for (uint32_t bg_key_idx_1 = bg_key_lo; bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end; bg_key_idx_1++) {
          ...
          uint32_t hint_hi_4 = view_HeapAllocation_Type_1_0_FULL_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_HeapAllocation_Type_1_0_FULL_VER.num_rows_) ? hint_hi_4 : view_HeapAllocation_Type_1_0_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_HeapAllocation_Type_1_0_FULL_VER.num_rows_;
          auto h_HeapAllocation_Type_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_HeapAllocation_Type_1_0_FULL_VER);
          if (!h_HeapAllocation_Type_0_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          auto h_ComponentType_1_root = HandleType(0, view_ComponentType_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_ComponentType_0_1_FULL_VER);
          if (!h_ComponentType_1_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          ...
          uint32_t bg_deg_first = (uint32_t)(h_HeapAllocation_Type_0_root.end() - h_HeapAllocation_Type_0_root.begin());
          ...
          {
            auto bg_narrow_begin = h_HeapAllocation_Type_0_root.begin() + bg_warp_row_begin;
            auto bg_narrow_end = h_HeapAllocation_Type_0_root.begin() + bg_warp_row_end;
            h_HeapAllocation_Type_0_root = HandleType(bg_narrow_begin, bg_narrow_end, h_HeapAllocation_Type_0_root.depth());
          }
...
JitRunner_ArrayTypeCompatPrecompute::LaunchParams JitRunner_ArrayTypeCompatPrecompute::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: HeapAllocation_Type version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<HeapAllocation_Type, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: ComponentType version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<ComponentType, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: SupertypeOf version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<SupertypeOf, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<HeapAllocation_Type, FULL_VER>(db);
...
void JitRunner_ArrayTypeCompatPrecompute::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_ArrayTypeCompatPrecompute::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<ArrayTypeCompat, NEW_VER>(db);
...
void JitRunner_ArrayTypeCompatPrecompute::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_ArrayTypeCompatPrecompute::read_total(LaunchParams& p) {
...
void JitRunner_ArrayTypeCompatPrecompute::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_ArrayTypeCompatPrecompute::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ArrayTypeCompatPrecompute::read_fused_result(LaunchParams& p) {
...
void JitRunner_ArrayTypeCompatPrecompute::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("ArrayTypeCompatPrecompute");
  ...
  auto& bg_dest_0 = get_relation_by_schema<ArrayTypeCompat, NEW_VER>(db);
...
void JitRunner_ArrayTypeCompatPrecompute::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
