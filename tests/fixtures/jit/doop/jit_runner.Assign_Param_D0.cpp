// =============================================================
// JIT-Generated Complete Runner: Assign_Param_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Assign_Param_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_FormalParam_1_0_2_FULL_VER = views[2];
        ...
        auto h_FormalParam_3_13 = HandleType(0, view_FormalParam_1_0_2_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_CallGraphEdge_2_12.iterators(view_CallGraphEdge_0_1_DELTA_VER), h_FormalParam_3_13.iterators(view_FormalParam_1_0_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          ...
          auto ch_FormalParam_3_meth = h_FormalParam_3_13.child_range(positions[1], meth, tile, view_FormalParam_1_0_2_FULL_VER);
        ...
        auto h_FormalParam_4_8 = ch_FormalParam_3_meth;
        auto h_ActualParam_5_9 = h_ActualParam_1_root;
        auto intersect_10 = intersect_handles(tile, h_FormalParam_4_8.iterators(view_FormalParam_1_0_2_FULL_VER), h_ActualParam_5_9.iterators(view_ActualParam_1_0_2_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto idx = it_11.value();
          auto positions = it_11.positions();
          auto ch_FormalParam_4_idx = h_FormalParam_4_8.child_range(positions[0], idx, tile, view_FormalParam_1_0_2_FULL_VER);
          auto ch_ActualParam_5_idx = h_ActualParam_5_9.child_range(positions[1], idx, tile, view_ActualParam_1_0_2_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        output_ctx_0.emit_direct(actual, formal);
        }
        }
        }
        }
...
JitRunner_Assign_Param_D0::LaunchParams JitRunner_Assign_Param_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: ActualParam version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<ActualParam, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0, 2}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: FormalParam version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<FormalParam, FULL_VER>(db);
...
void JitRunner_Assign_Param_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Assign_Param_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Assign_Param_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Assign_Param_D0::read_total(LaunchParams& p) {
...
void JitRunner_Assign_Param_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Assign_Param_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Assign_Param_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Assign_Param_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Assign_Param_D0");
...
void JitRunner_Assign_Param_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
