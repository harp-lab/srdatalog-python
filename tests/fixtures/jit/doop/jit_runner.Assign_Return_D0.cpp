// =============================================================
// JIT-Generated Complete Runner: Assign_Return_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Assign_Return_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = CallGraphEdge;
  using DestSchema = Assign;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_AssignReturnValue_0_1_FULL_VER = views[1];
        auto view_ReturnVar_1_0_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'invocation' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var invocation :sources ((CallGraphEdge :handle 0) (AssignReturnValue :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_AssignReturnValue_1_root = HandleType(0, view_AssignReturnValue_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_AssignReturnValue_0_1_FULL_VER);
          if (!h_AssignReturnValue_1_root.valid()) continue;
          auto invocation = root_val_2;
        // Nested ColumnJoin (intersection): bind 'meth' from 2 sources
        // MIR: (column-join :var meth :sources ((CallGraphEdge :handle 2 :prefix (invocation)) (ReturnVar :handle 3 :prefix ()) ))
        auto h_CallGraphEdge_2_8 = h_CallGraphEdge_0_root;
        auto h_ReturnVar_3_9 = HandleType(0, view_ReturnVar_1_0_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_CallGraphEdge_2_8.iterators(view_CallGraphEdge_0_1_DELTA_VER), h_ReturnVar_3_9.iterators(view_ReturnVar_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto meth = it_11.value();
          auto positions = it_11.positions();
          auto ch_CallGraphEdge_2_meth = h_CallGraphEdge_2_8.child_range(positions[0], meth, tile, view_CallGraphEdge_0_1_DELTA_VER);
          auto ch_ReturnVar_3_meth = h_ReturnVar_3_9.child_range(positions[1], meth, tile, view_ReturnVar_1_0_FULL_VER);
        // Nested CartesianJoin: bind ret, local from 2 source(s)
        // MIR: (cartesian-join :vars (ret local) :sources ((ReturnVar :handle 4 :prefix (meth)) (AssignReturnValue :handle 5 :prefix (invocation)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_ReturnVar_4_4 = ch_ReturnVar_3_meth;  // reusing narrowed handle
        auto h_AssignReturnValue_5_6 = h_AssignReturnValue_1_root;  // reusing narrowed handle

        if (!h_ReturnVar_4_4.valid() || !h_AssignReturnValue_5_6.valid()) continue;

        uint32_t degree_3 = h_ReturnVar_4_4.degree();
        uint32_t degree_5 = h_AssignReturnValue_5_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'meth' from 2 sources
        // MIR: (column-join :var meth :sources ((CallGraphEdge :handle 2 :prefix (invocation)) (ReturnVar :handle 3 :prefix ()) ))
        auto h_CallGraphEdge_2_12 = h_CallGraphEdge_0_root;
        auto h_ReturnVar_3_13 = HandleType(0, view_ReturnVar_1_0_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_CallGraphEdge_2_12.iterators(view_CallGraphEdge_0_1_DELTA_VER), h_ReturnVar_3_13.iterators(view_ReturnVar_1_0_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto meth = it_15.value();
          auto positions = it_15.positions();
          auto ch_CallGraphEdge_2_meth = h_CallGraphEdge_2_12.child_range(positions[0], meth, tile, view_CallGraphEdge_0_1_DELTA_VER);
          auto ch_ReturnVar_3_meth = h_ReturnVar_3_13.child_range(positions[1], meth, tile, view_ReturnVar_1_0_FULL_VER);
        // Nested CartesianJoin: bind ret, local from 2 source(s)
        // MIR: (cartesian-join :vars (ret local) :sources ((ReturnVar :handle 4 :prefix (meth)) (AssignReturnValue :handle 5 :prefix (invocation)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto ret = view_ReturnVar_1_0_FULL_VER.get_value(1, h_ReturnVar_4_4.begin() + idx0_9);
          auto local = view_AssignReturnValue_0_1_FULL_VER.get_value(1, h_AssignReturnValue_5_6.begin() + idx1_10);

        // Emit: Assign(ret, local)
        output_ctx_0.emit_direct(ret, local);
        }
        }
        }
  }
...
JitRunner_Assign_Return_D0::LaunchParams JitRunner_Assign_Return_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: CallGraphEdge version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<CallGraphEdge, FULL_VER>(db) : get_relation_by_schema<CallGraphEdge, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: AssignReturnValue version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<AssignReturnValue, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: ReturnVar version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<ReturnVar, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<CallGraphEdge, FULL_VER>(db) : get_relation_by_schema<CallGraphEdge, DELTA_VER>(db);
...
void JitRunner_Assign_Return_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Assign_Return_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Assign, NEW_VER>(db);
...
void JitRunner_Assign_Return_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Assign_Return_D0::read_total(LaunchParams& p) {
...
void JitRunner_Assign_Return_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Assign_Return_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Assign_Return_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Assign_Return_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Assign_Return_D0");
...
void JitRunner_Assign_Return_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
