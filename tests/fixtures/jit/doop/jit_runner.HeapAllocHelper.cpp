...
struct JitRunner_HeapAllocHelper {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_ComponentType_1_0_FULL_VER = views[2];
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (baseheaptype != 1016) {
        // Emit: HeapAllocSuperType(heap, baseheap)
        output_ctx_0.emit_direct(heap, baseheap);
        }
        }
        }
        }
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      auto h_SupertypeOf_1_root = HandleType(0, view_SupertypeOf_1_0_FULL_VER.num_rows_, 0).prefix(bg_hist_root_val, tile, view_SupertypeOf_1_0_FULL_VER);
...
JitRunner_HeapAllocHelper::LaunchParams JitRunner_HeapAllocHelper::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 3: ComponentType version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<ComponentType, FULL_VER>(db);
...
void JitRunner_HeapAllocHelper::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_HeapAllocHelper::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<HeapAllocSuperType, NEW_VER>(db);
...
void JitRunner_HeapAllocHelper::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_HeapAllocHelper::read_total(LaunchParams& p) {
...
void JitRunner_HeapAllocHelper::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_HeapAllocHelper::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_HeapAllocHelper::read_fused_result(LaunchParams& p) {
...
void JitRunner_HeapAllocHelper::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("HeapAllocHelper");
  ...
  auto& bg_dest_0 = get_relation_by_schema<HeapAllocSuperType, NEW_VER>(db);
...
void JitRunner_HeapAllocHelper::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
