// =============================================================
// JIT-Generated Complete Runner: IFPT_Store_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_IFPT_Store_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto h_VarPointsTo_3_15 = HandleType(0, view_VarPointsTo_1_0_FULL_VER.num_rows_, 0);
        auto intersect_16 = intersect_handles(tile, h_StoreInstanceField_2_14.iterators(view_StoreInstanceField_3_0_1_2_FULL_VER), h_VarPointsTo_3_15.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto frm = it_17.value();
          auto positions = it_17.positions();
          auto ch_StoreInstanceField_2_frm = h_StoreInstanceField_2_14.child_range(positions[0], frm, tile, view_StoreInstanceField_3_0_1_2_FULL_VER);
          auto ch_VarPointsTo_3_frm = h_VarPointsTo_3_15.child_range(positions[1], frm, tile, view_VarPointsTo_1_0_FULL_VER);
        ...
        auto h_StoreInstanceField_4_10 = ch_StoreInstanceField_2_frm;
        auto h_VarPointsTo_5_11 = HandleType(0, view_VarPointsTo_1_0_FULL_VER.num_rows_, 0);
        auto intersect_12 = intersect_handles(tile, h_StoreInstanceField_4_10.iterators(view_StoreInstanceField_3_0_1_2_FULL_VER), h_VarPointsTo_5_11.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto base = it_13.value();
          auto positions = it_13.positions();
          auto ch_StoreInstanceField_4_base = h_StoreInstanceField_4_10.child_range(positions[0], base, tile, view_StoreInstanceField_3_0_1_2_FULL_VER);
...
JitRunner_IFPT_Store_D0::LaunchParams JitRunner_IFPT_Store_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_IFPT_Store_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_IFPT_Store_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_IFPT_Store_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_IFPT_Store_D0::read_total(LaunchParams& p) {
...
void JitRunner_IFPT_Store_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_IFPT_Store_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_IFPT_Store_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_IFPT_Store_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("IFPT_Store_D0");
...
void JitRunner_IFPT_Store_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
