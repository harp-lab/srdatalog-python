...
struct JitRunner_IFPT_Store_D3 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_StoreInstanceField_1_3_0_2_FULL_VER = views[1];
        auto view_Reachable_0_FULL_VER = views[2];
        auto view_VarPointsTo_1_0_FULL_VER = views[3];

        // Root ColumnJoin (multi-source intersection): bind 'base' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var base :sources ((VarPointsTo :handle 0) (StoreInstanceField :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_StoreInstanceField_1_root = HandleType(0, view_StoreInstanceField_1_3_0_2_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_StoreInstanceField_1_3_0_2_FULL_VER);
        ...
        // MIR: (column-join :var inmeth :sources ((StoreInstanceField :handle 2 :prefix (base)) (Reachable :handle 3 :prefix ()) ))
        auto h_StoreInstanceField_2_14 = h_StoreInstanceField_1_root;
        auto h_Reachable_3_15 = HandleType(0, view_Reachable_0_FULL_VER.num_rows_, 0);
        auto intersect_16 = intersect_handles(tile, h_StoreInstanceField_2_14.iterators(view_StoreInstanceField_1_3_0_2_FULL_VER), h_Reachable_3_15.iterators(view_Reachable_0_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto inmeth = it_17.value();
          auto positions = it_17.positions();
          auto ch_StoreInstanceField_2_inmeth = h_StoreInstanceField_2_14.child_range(positions[0], inmeth, tile, view_StoreInstanceField_1_3_0_2_FULL_VER);
          auto ch_Reachable_3_inmeth = h_Reachable_3_15.child_range(positions[1], inmeth, tile, view_Reachable_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'frm' from 2 sources
        // MIR: (column-join :var frm :sources ((StoreInstanceField :handle 4 :prefix (base inmeth)) (VarPointsTo :handle 5 :prefix ()) ))
        for (int _nseg_1 = 0; _nseg_1 < 2; _nseg_1++) {
          view_VarPointsTo_1_0_FULL_VER = views[3 + _nseg_1];
          auto h_StoreInstanceField_4_10 = ch_StoreInstanceField_2_inmeth;
          auto h_VarPointsTo_5_11 = HandleType(0, view_VarPointsTo_1_0_FULL_VER.num_rows_, 0);
          auto intersect_12 = intersect_handles(tile, h_StoreInstanceField_4_10.iterators(view_StoreInstanceField_1_3_0_2_FULL_VER), h_VarPointsTo_5_11.iterators(view_VarPointsTo_1_0_FULL_VER));
          for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto frm = it_13.value();
          auto positions = it_13.positions();
          auto ch_StoreInstanceField_4_frm = h_StoreInstanceField_4_10.child_range(positions[0], frm, tile, view_StoreInstanceField_1_3_0_2_FULL_VER);
          auto ch_VarPointsTo_5_frm = h_VarPointsTo_5_11.child_range(positions[1], frm, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested CartesianJoin: bind heap, fld, baseheap from 3 source(s)
        // MIR: (cartesian-join :vars (heap fld baseheap) :sources ((VarPointsTo :handle 6 :prefix (base)) (StoreInstanceField :handle 7 :prefix (base inmeth frm)) (VarPointsTo :handle 8 :prefix (frm)) ))
        ...
        auto h_StoreInstanceField_7_6 = ch_StoreInstanceField_4_frm;  // reusing narrowed handle
        auto h_VarPointsTo_8_8 = ch_VarPointsTo_5_frm;  // reusing narrowed handle

        if (!h_VarPointsTo_6_4.valid() || !h_StoreInstanceField_7_6.valid() || !h_VarPointsTo_8_8.valid()) continue;

        uint32_t degree_3 = h_VarPointsTo_6_4.degree();
        uint32_t degree_5 = h_StoreInstanceField_7_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_Reachable_3_19 = HandleType(0, view_Reachable_0_FULL_VER.num_rows_, 0);
        auto intersect_20 = intersect_handles(tile, h_StoreInstanceField_2_18.iterators(view_StoreInstanceField_1_3_0_2_FULL_VER), h_Reachable_3_19.iterators(view_Reachable_0_FULL_VER));
        for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
          auto inmeth = it_21.value();
          auto positions = it_21.positions();
          auto ch_StoreInstanceField_2_inmeth = h_StoreInstanceField_2_18.child_range(positions[0], inmeth, tile, view_StoreInstanceField_1_3_0_2_FULL_VER);
          auto ch_Reachable_3_inmeth = h_Reachable_3_19.child_range(positions[1], inmeth, tile, view_Reachable_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'frm' from 2 sources
        // MIR: (column-join :var frm :sources ((StoreInstanceField :handle 4 :prefix (base inmeth)) (VarPointsTo :handle 5 :prefix ()) ))
        for (int _nseg_1 = 0; _nseg_1 < 2; _nseg_1++) {
          view_VarPointsTo_1_0_FULL_VER = views[3 + _nseg_1];
          auto h_StoreInstanceField_4_14 = ch_StoreInstanceField_2_inmeth;
          auto h_VarPointsTo_5_15 = HandleType(0, view_VarPointsTo_1_0_FULL_VER.num_rows_, 0);
          auto intersect_16 = intersect_handles(tile, h_StoreInstanceField_4_14.iterators(view_StoreInstanceField_1_3_0_2_FULL_VER), h_VarPointsTo_5_15.iterators(view_VarPointsTo_1_0_FULL_VER));
          for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto frm = it_17.value();
          auto positions = it_17.positions();
          auto ch_StoreInstanceField_4_frm = h_StoreInstanceField_4_14.child_range(positions[0], frm, tile, view_StoreInstanceField_1_3_0_2_FULL_VER);
          auto ch_VarPointsTo_5_frm = h_VarPointsTo_5_15.child_range(positions[1], frm, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested CartesianJoin: bind heap, fld, baseheap from 3 source(s)
        // MIR: (cartesian-join :vars (heap fld baseheap) :sources ((VarPointsTo :handle 6 :prefix (base)) (StoreInstanceField :handle 7 :prefix (base inmeth frm)) (VarPointsTo :handle 8 :prefix (frm)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_10 = lane_1; flat_idx_10 < total_9; flat_idx_10 += group_size_2) {
          ...
          auto baseheap = view_VarPointsTo_1_0_DELTA_VER.get_value(1, h_VarPointsTo_6_4.begin() + idx0_11);
          auto fld = view_StoreInstanceField_1_3_0_2_FULL_VER.get_value(3, h_StoreInstanceField_7_6.begin() + idx1_12);
          auto heap = view_VarPointsTo_1_0_FULL_VER.get_value(1, h_VarPointsTo_8_8.begin() + idx2_13);

        // Emit: InstanceFieldPointsTo(heap, fld, baseheap)
        output_ctx_0.emit_direct(heap, fld, baseheap);
        }
          }
        }
        }
...
JitRunner_IFPT_Store_D3::LaunchParams JitRunner_IFPT_Store_D3::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: StoreInstanceField version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<StoreInstanceField, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 3, 0, 2}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: Reachable version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<Reachable, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // Source 5: VarPointsTo version FULL_VER
  {
...
void JitRunner_IFPT_Store_D3::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_IFPT_Store_D3::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<InstanceFieldPointsTo, NEW_VER>(db);
...
void JitRunner_IFPT_Store_D3::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_IFPT_Store_D3::read_total(LaunchParams& p) {
...
void JitRunner_IFPT_Store_D3::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_IFPT_Store_D3::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_IFPT_Store_D3::read_fused_result(LaunchParams& p) {
...
void JitRunner_IFPT_Store_D3::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("IFPT_Store_D3");
...
void JitRunner_IFPT_Store_D3::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
