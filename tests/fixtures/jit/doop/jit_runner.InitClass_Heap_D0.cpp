// =============================================================
// JIT-Generated Complete Runner: InitClass_Heap_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_InitClass_Heap_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = Reachable;
  using DestSchema = InitializedClass;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_AssignHeapAllocation_2_0_1_FULL_VER = views[1];
        auto view_HeapAllocation_Type_0_1_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'inmeth' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var inmeth :sources ((Reachable :handle 0) (AssignHeapAllocation :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_AssignHeapAllocation_1_root = HandleType(0, view_AssignHeapAllocation_2_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_AssignHeapAllocation_2_0_1_FULL_VER);
          if (!h_AssignHeapAllocation_1_root.valid()) continue;
          auto inmeth = root_val_2;
        // Nested ColumnJoin (intersection): bind 'heap' from 2 sources
        // MIR: (column-join :var heap :sources ((AssignHeapAllocation :handle 2 :prefix (inmeth)) (HeapAllocation_Type :handle 3 :prefix ()) ))
        auto h_AssignHeapAllocation_2_8 = h_AssignHeapAllocation_1_root;
        auto h_HeapAllocation_Type_3_9 = HandleType(0, view_HeapAllocation_Type_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_AssignHeapAllocation_2_8.iterators(view_AssignHeapAllocation_2_0_1_FULL_VER), h_HeapAllocation_Type_3_9.iterators(view_HeapAllocation_Type_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto heap = it_11.value();
          auto positions = it_11.positions();
          auto ch_AssignHeapAllocation_2_heap = h_AssignHeapAllocation_2_8.child_range(positions[0], heap, tile, view_AssignHeapAllocation_2_0_1_FULL_VER);
          auto ch_HeapAllocation_Type_3_heap = h_HeapAllocation_Type_3_9.child_range(positions[1], heap, tile, view_HeapAllocation_Type_0_1_FULL_VER);
        // Nested CartesianJoin: bind _gen3, classD from 2 source(s)
        // MIR: (cartesian-join :vars (_gen3 classD) :sources ((AssignHeapAllocation :handle 4 :prefix (inmeth heap)) (HeapAllocation_Type :handle 5 :prefix (heap)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_AssignHeapAllocation_4_4 = ch_AssignHeapAllocation_2_heap;  // reusing narrowed handle
        auto h_HeapAllocation_Type_5_6 = ch_HeapAllocation_Type_3_heap;  // reusing narrowed handle

        if (!h_AssignHeapAllocation_4_4.valid() || !h_HeapAllocation_Type_5_6.valid()) continue;

        uint32_t degree_3 = h_AssignHeapAllocation_4_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'heap' from 2 sources
        // MIR: (column-join :var heap :sources ((AssignHeapAllocation :handle 2 :prefix (inmeth)) (HeapAllocation_Type :handle 3 :prefix ()) ))
        auto h_AssignHeapAllocation_2_12 = h_AssignHeapAllocation_1_root;
        auto h_HeapAllocation_Type_3_13 = HandleType(0, view_HeapAllocation_Type_0_1_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_AssignHeapAllocation_2_12.iterators(view_AssignHeapAllocation_2_0_1_FULL_VER), h_HeapAllocation_Type_3_13.iterators(view_HeapAllocation_Type_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto heap = it_15.value();
          auto positions = it_15.positions();
          auto ch_AssignHeapAllocation_2_heap = h_AssignHeapAllocation_2_12.child_range(positions[0], heap, tile, view_AssignHeapAllocation_2_0_1_FULL_VER);
          auto ch_HeapAllocation_Type_3_heap = h_HeapAllocation_Type_3_13.child_range(positions[1], heap, tile, view_HeapAllocation_Type_0_1_FULL_VER);
        // Nested CartesianJoin: bind _gen3, classD from 2 source(s)
        // MIR: (cartesian-join :vars (_gen3 classD) :sources ((AssignHeapAllocation :handle 4 :prefix (inmeth heap)) (HeapAllocation_Type :handle 5 :prefix (heap)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto _gen3 = view_AssignHeapAllocation_2_0_1_FULL_VER.get_value(2, h_AssignHeapAllocation_4_4.begin() + idx0_9);
          auto classD = view_HeapAllocation_Type_0_1_FULL_VER.get_value(1, h_HeapAllocation_Type_5_6.begin() + idx1_10);

        // Emit: InitializedClass(classD)
        output_ctx_0.emit_direct(classD);
        }
        }
        }
  }
...
JitRunner_InitClass_Heap_D0::LaunchParams JitRunner_InitClass_Heap_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Reachable version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<Reachable, FULL_VER>(db) : get_relation_by_schema<Reachable, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: AssignHeapAllocation version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<AssignHeapAllocation, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{2, 0, 1}}, false);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<Reachable, FULL_VER>(db) : get_relation_by_schema<Reachable, DELTA_VER>(db);
...
void JitRunner_InitClass_Heap_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_InitClass_Heap_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<InitializedClass, NEW_VER>(db);
...
void JitRunner_InitClass_Heap_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_InitClass_Heap_D0::read_total(LaunchParams& p) {
...
void JitRunner_InitClass_Heap_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_Heap_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_Heap_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_InitClass_Heap_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("InitClass_Heap_D0");
...
void JitRunner_InitClass_Heap_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
