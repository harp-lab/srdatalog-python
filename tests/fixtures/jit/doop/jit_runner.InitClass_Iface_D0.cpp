...
struct JitRunner_InitClass_Iface_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_DirectSuperinterface_0_1_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'classOrIface' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var classOrIface :sources ((InitializedClass :handle 0) (DirectSuperinterface :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_DirectSuperinterface_1_root = HandleType(0, view_DirectSuperinterface_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_DirectSuperinterface_0_1_FULL_VER);
          if (!h_DirectSuperinterface_1_root.valid()) continue;
          auto classOrIface = root_val_2;
        // Nested CartesianJoin: bind superiface from 1 source(s)
        // MIR: (cartesian-join :vars (superiface) :sources ((DirectSuperinterface :handle 2 :prefix (classOrIface)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_DirectSuperinterface_2_4 = h_DirectSuperinterface_1_root;  // reusing narrowed handle

        if (!h_DirectSuperinterface_2_4.valid()) continue;

        uint32_t degree_3 = h_DirectSuperinterface_2_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto superiface = view_DirectSuperinterface_0_1_FULL_VER.get_value(1, h_DirectSuperinterface_2_4.begin() + idx0_7);

        // Emit: InitializedClass(superiface)
        output_ctx_0.emit_direct(superiface);
        }
        }
  }
...
JitRunner_InitClass_Iface_D0::LaunchParams JitRunner_InitClass_Iface_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: InitializedClass version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<InitializedClass, FULL_VER>(db) : get_relation_by_schema<InitializedClass, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<InitializedClass, FULL_VER>(db) : get_relation_by_schema<InitializedClass, DELTA_VER>(db);
...
void JitRunner_InitClass_Iface_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_InitClass_Iface_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_Iface_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_InitClass_Iface_D0::read_total(LaunchParams& p) {
...
void JitRunner_InitClass_Iface_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_Iface_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_Iface_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_InitClass_Iface_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("InitClass_Iface_D0");
...
void JitRunner_InitClass_Iface_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
