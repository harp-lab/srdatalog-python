// =============================================================
// JIT-Generated Complete Runner: InitClass_LoadStatic_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_InitClass_LoadStatic_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var sig :sources ((LoadStaticField :handle 2 :prefix (inmeth)) (Field_DeclaringType :handle 3 :prefix ()) ))
        auto h_LoadStaticField_2_8 = h_LoadStaticField_1_root;
        auto h_Field_DeclaringType_3_9 = HandleType(0, view_Field_DeclaringType_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_LoadStaticField_2_8.iterators(view_LoadStaticField_2_0_1_FULL_VER), h_Field_DeclaringType_3_9.iterators(view_Field_DeclaringType_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto sig = it_11.value();
          auto positions = it_11.positions();
          auto ch_LoadStaticField_2_sig = h_LoadStaticField_2_8.child_range(positions[0], sig, tile, view_LoadStaticField_2_0_1_FULL_VER);
        ...
        // Nested CartesianJoin: bind _gen5, classOrIface from 2 source(s)
        // MIR: (cartesian-join :vars (_gen5 classOrIface) :sources ((LoadStaticField :handle 4 :prefix (inmeth sig)) (Field_DeclaringType :handle 5 :prefix (sig)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_LoadStaticField_4_4 = ch_LoadStaticField_2_sig;  // reusing narrowed handle
        auto h_Field_DeclaringType_5_6 = ch_Field_DeclaringType_3_sig;  // reusing narrowed handle

        if (!h_LoadStaticField_4_4.valid() || !h_Field_DeclaringType_5_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto intersect_14 = intersect_handles(tile, h_LoadStaticField_2_12.iterators(view_LoadStaticField_2_0_1_FULL_VER), h_Field_DeclaringType_3_13.iterators(view_Field_DeclaringType_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto sig = it_15.value();
          auto positions = it_15.positions();
          auto ch_LoadStaticField_2_sig = h_LoadStaticField_2_12.child_range(positions[0], sig, tile, view_LoadStaticField_2_0_1_FULL_VER);
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto _gen5 = view_LoadStaticField_2_0_1_FULL_VER.get_value(2, h_LoadStaticField_4_4.begin() + idx0_9);
...
JitRunner_InitClass_LoadStatic_D0::LaunchParams JitRunner_InitClass_LoadStatic_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: LoadStaticField version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<LoadStaticField, FULL_VER>(db);
...
void JitRunner_InitClass_LoadStatic_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_InitClass_LoadStatic_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_LoadStatic_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_InitClass_LoadStatic_D0::read_total(LaunchParams& p) {
...
void JitRunner_InitClass_LoadStatic_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_LoadStatic_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_LoadStatic_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_InitClass_LoadStatic_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("InitClass_LoadStatic_D0");
...
void JitRunner_InitClass_LoadStatic_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
