// =============================================================
// JIT-Generated Complete Runner: InitClass_StaticInvoke_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_InitClass_StaticInvoke_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_isStaticMethodInvocation_Insn_0_FULL_VER = views[3];
        auto view_Method_DeclaringType_0_1_FULL_VER = views[4];
        ...
        auto h_Instruction_Method_2_10 = h_Instruction_Method_1_root;
        auto h_MethodInvocation_Method_3_11 = HandleType(0, view_MethodInvocation_Method_0_1_FULL_VER.num_rows_, 0);
        auto h_isStaticMethodInvocation_Insn_4_12 = HandleType(0, view_isStaticMethodInvocation_Insn_0_FULL_VER.num_rows_, 0);
        auto intersect_13 = intersect_handles(tile, h_Instruction_Method_2_10.iterators(view_Instruction_Method_1_0_FULL_VER), h_MethodInvocation_Method_3_11.iterators(view_MethodInvocation_Method_0_1_FULL_VER), h_isStaticMethodInvocation_Insn_4_12.iterators(view_isStaticMethodInvocation_Insn_0_FULL_VER));
        for (auto it_14 = intersect_13.begin(); it_14.valid(); it_14.next()) {
          auto invocation = it_14.value();
          auto positions = it_14.positions();
          auto ch_Instruction_Method_2_invocation = h_Instruction_Method_2_10.child_range(positions[0], invocation, tile, view_Instruction_Method_1_0_FULL_VER);
          auto ch_MethodInvocation_Method_3_invocation = h_MethodInvocation_Method_3_11.child_range(positions[1], invocation, tile, view_MethodInvocation_Method_0_1_FULL_VER);
          auto ch_isStaticMethodInvocation_Insn_4_invocation = h_isStaticMethodInvocation_Insn_4_12.child_range(positions[2], invocation, tile, view_isStaticMethodInvocation_Insn_0_FULL_VER);
        ...
        auto h_MethodInvocation_Method_5_6 = ch_MethodInvocation_Method_3_invocation;
        auto h_Method_DeclaringType_6_7 = HandleType(0, view_Method_DeclaringType_0_1_FULL_VER.num_rows_, 0);
        auto intersect_8 = intersect_handles(tile, h_MethodInvocation_Method_5_6.iterators(view_MethodInvocation_Method_0_1_FULL_VER), h_Method_DeclaringType_6_7.iterators(view_Method_DeclaringType_0_1_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto sig = it_9.value();
          auto positions = it_9.positions();
          auto ch_MethodInvocation_Method_5_sig = h_MethodInvocation_Method_5_6.child_range(positions[0], sig, tile, view_MethodInvocation_Method_0_1_FULL_VER);
          auto ch_Method_DeclaringType_6_sig = h_Method_DeclaringType_6_7.child_range(positions[1], sig, tile, view_Method_DeclaringType_0_1_FULL_VER);
...
JitRunner_InitClass_StaticInvoke_D0::LaunchParams JitRunner_InitClass_StaticInvoke_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: Instruction_Method version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Instruction_Method, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: MethodInvocation_Method version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<MethodInvocation_Method, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // Source 4: isStaticMethodInvocation_Insn version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<isStaticMethodInvocation_Insn, FULL_VER>(db);
    auto& idx_4 = rel_4.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_4.view());
  }

  // Source 6: Method_DeclaringType version FULL_VER
  {
    auto& rel_6 = get_relation_by_schema<Method_DeclaringType, FULL_VER>(db);
...
void JitRunner_InitClass_StaticInvoke_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_InitClass_StaticInvoke_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_StaticInvoke_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_InitClass_StaticInvoke_D0::read_total(LaunchParams& p) {
...
void JitRunner_InitClass_StaticInvoke_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_StaticInvoke_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_InitClass_StaticInvoke_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_InitClass_StaticInvoke_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("InitClass_StaticInvoke_D0");
...
void JitRunner_InitClass_StaticInvoke_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
