// =============================================================
// JIT-Generated Complete Runner: IsCastableToString_rule
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_IsCastableToString_rule {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = SupertypeOf;
  using DestSchema = IsCastableToString;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_SupertypeOf_0_1_FULL_VER = views[0];

        // Root Scan: SupertypeOf binding casttype, _c8
        // MIR: (scan :rel SupertypeOf :vars (casttype _c8) :handle 0)
        auto root_handle_1 = HandleType(0, view_SupertypeOf_0_1_FULL_VER.num_rows_, 0);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (_c8 == 434817) {
        // Emit: IsCastableToString(casttype)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(casttype);
        }
        }
  }
...
JitRunner_IsCastableToString_rule::LaunchParams JitRunner_IsCastableToString_rule::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: SupertypeOf version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<SupertypeOf, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<SupertypeOf, FULL_VER>(db);
...
void JitRunner_IsCastableToString_rule::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_IsCastableToString_rule::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<IsCastableToString, NEW_VER>(db);
...
void JitRunner_IsCastableToString_rule::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_IsCastableToString_rule::read_total(LaunchParams& p) {
...
void JitRunner_IsCastableToString_rule::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_IsCastableToString_rule::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_IsCastableToString_rule::read_fused_result(LaunchParams& p) {
...
void JitRunner_IsCastableToString_rule::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("IsCastableToString_rule");
...
void JitRunner_IsCastableToString_rule::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
