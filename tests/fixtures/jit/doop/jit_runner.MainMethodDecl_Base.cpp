// =============================================================
// JIT-Generated Complete Runner: MainMethodDecl_Base
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_MainMethodDecl_Base {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = MainClass;
  using DestSchema = MainMethodDeclaration;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Method_SimpleName_0_1_FULL_VER = views[2];
        auto view_Method_Descriptor_0_1_FULL_VER = views[3];
        auto view_Method_Modifier_1_0_FULL_VER = views[4];
...
JitRunner_MainMethodDecl_Base::LaunchParams JitRunner_MainMethodDecl_Base::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: MainClass version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<MainClass, FULL_VER>(db);
  ...
  // Source 3: Method_SimpleName version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<Method_SimpleName, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // Source 4: Method_Descriptor version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<Method_Descriptor, FULL_VER>(db);
    auto& idx_4 = rel_4.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_4.view());
  }

  // Source 5: Method_Modifier version FULL_VER
  {
    auto& rel_5 = get_relation_by_schema<Method_Modifier, FULL_VER>(db);
    auto& idx_5 = rel_5.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_5.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<MainClass, FULL_VER>(db);
...
void JitRunner_MainMethodDecl_Base::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_MainMethodDecl_Base::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<MainMethodDeclaration, NEW_VER>(db);
...
void JitRunner_MainMethodDecl_Base::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_MainMethodDecl_Base::read_total(LaunchParams& p) {
...
void JitRunner_MainMethodDecl_Base::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_MainMethodDecl_Base::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MainMethodDecl_Base::read_fused_result(LaunchParams& p) {
...
void JitRunner_MainMethodDecl_Base::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("MainMethodDecl_Base");
...
void JitRunner_MainMethodDecl_Base::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
