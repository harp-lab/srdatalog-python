// =============================================================
// JIT-Generated Complete Runner: MethodImplemented
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_MethodImplemented {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = Method_SimpleName;
  using DestSchema = MethodImplemented;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Method_Modifier_0_1_FULL_VER = views[3];
...
JitRunner_MethodImplemented::LaunchParams JitRunner_MethodImplemented::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Method_SimpleName version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<Method_SimpleName, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: Method_Descriptor version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Method_Descriptor, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 2: Method_DeclaringType version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<Method_DeclaringType, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_2.view());
  }

  // Source 6: Method_Modifier version FULL_VER
  {
    auto& rel_6 = get_relation_by_schema<Method_Modifier, FULL_VER>(db);
    auto& idx_6 = rel_6.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_6.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<Method_SimpleName, FULL_VER>(db);
...
void JitRunner_MethodImplemented::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_MethodImplemented::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<MethodImplemented, NEW_VER>(db);
...
void JitRunner_MethodImplemented::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_MethodImplemented::read_total(LaunchParams& p) {
...
void JitRunner_MethodImplemented::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_MethodImplemented::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MethodImplemented::read_fused_result(LaunchParams& p) {
...
void JitRunner_MethodImplemented::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("MethodImplemented");
...
void JitRunner_MethodImplemented::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
