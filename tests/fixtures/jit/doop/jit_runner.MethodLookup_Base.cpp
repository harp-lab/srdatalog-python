...
struct JitRunner_MethodLookup_Base {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_MethodImplemented_0_1_2_3_FULL_VER = views[0];

        // Root Scan: MethodImplemented binding simplename, descriptor, mtype, meth
        // MIR: (scan :rel MethodImplemented :vars (simplename descriptor mtype meth) :handle 0)
        auto root_handle_1 = HandleType(0, view_MethodImplemented_0_1_2_3_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto simplename = view_MethodImplemented_0_1_2_3_FULL_VER.get_value(0, idx_3);
          auto descriptor = view_MethodImplemented_0_1_2_3_FULL_VER.get_value(1, idx_3);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          ...
          auto meth = view_MethodImplemented_0_1_2_3_FULL_VER.get_value(3, idx_3);
        // Emit: MethodLookup(simplename, descriptor, mtype, meth)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(simplename, descriptor, mtype, meth);
        }
  }
...
JitRunner_MethodLookup_Base::LaunchParams JitRunner_MethodLookup_Base::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: MethodImplemented version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<MethodImplemented, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<MethodImplemented, FULL_VER>(db);
...
void JitRunner_MethodLookup_Base::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_MethodLookup_Base::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MethodLookup_Base::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_MethodLookup_Base::read_total(LaunchParams& p) {
...
void JitRunner_MethodLookup_Base::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_MethodLookup_Base::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MethodLookup_Base::read_fused_result(LaunchParams& p) {
...
void JitRunner_MethodLookup_Base::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("MethodLookup_Base");
...
void JitRunner_MethodLookup_Base::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
