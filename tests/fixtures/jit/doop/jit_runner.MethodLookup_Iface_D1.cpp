...
struct JitRunner_MethodLookup_Iface_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_MethodImplemented_0_1_2_3_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'supertype' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var supertype :sources ((MethodLookup :handle 0) (DirectSuperinterface :handle 1) ))
        ...
        // MIR: (cartesian-join :vars (simplename descriptor meth mtype) :sources ((MethodLookup :handle 2 :prefix (supertype)) (DirectSuperinterface :handle 3 :prefix (supertype)) ))
        ...
        auto h_DirectSuperinterface_3_7 = h_DirectSuperinterface_1_root;  // reusing narrowed handle

        if (!h_MethodLookup_2_5.valid() || !h_DirectSuperinterface_3_7.valid()) continue;

        uint32_t degree_4 = h_MethodLookup_2_5.degree();
        uint32_t degree_6 = h_DirectSuperinterface_3_7.degree();
        ...
        for (uint32_t flat_idx_9 = lane_2; flat_idx_9 < total_8; flat_idx_9 += group_size_3) {
          ...
          auto mtype = view_DirectSuperinterface_1_0_FULL_VER.get_value(1, h_DirectSuperinterface_3_7.begin() + idx1_11);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_MethodImplemented_neg_4_1.valid()) {
        // Emit: MethodLookup(simplename, descriptor, mtype, meth)
        output_ctx_0.emit_direct(simplename, descriptor, mtype, meth);
        }
        }
        }
  }
...
JitRunner_MethodLookup_Iface_D1::LaunchParams JitRunner_MethodLookup_Iface_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: MethodLookup version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<MethodLookup, FULL_VER>(db) : get_relation_by_schema<MethodLookup, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{2, 0, 1, 3}}, false);
  ...
  // Source 4: MethodImplemented version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<MethodImplemented, FULL_VER>(db);
    auto& idx_4 = rel_4.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, true);
    p.views_vec.push_back(idx_4.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<MethodLookup, FULL_VER>(db) : get_relation_by_schema<MethodLookup, DELTA_VER>(db);
  auto& first_idx = first_rel.get_index(SRDatalog::IndexSpec{{2, 0, 1, 3}});
...
void JitRunner_MethodLookup_Iface_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_MethodLookup_Iface_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<MethodLookup, NEW_VER>(db);
...
void JitRunner_MethodLookup_Iface_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_MethodLookup_Iface_D1::read_total(LaunchParams& p) {
...
void JitRunner_MethodLookup_Iface_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_MethodLookup_Iface_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_MethodLookup_Iface_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_MethodLookup_Iface_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("MethodLookup_Iface_D1");
...
void JitRunner_MethodLookup_Iface_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
