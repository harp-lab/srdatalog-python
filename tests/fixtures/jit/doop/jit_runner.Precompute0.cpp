...
struct JitRunner_Precompute0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_AssignCast_0_1_2_3_FULL_VER = views[0];
        auto view_SupertypeOf_0_1_FULL_VER = views[1];
        auto view_HeapAllocation_Type_1_0_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'casttype' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var casttype :sources ((AssignCast :handle 0) (SupertypeOf :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_AssignCast_0_1_2_3_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_AssignCast_0_1_2_3_FULL_VER.num_rows_) ? hint_hi_4 : view_AssignCast_0_1_2_3_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_AssignCast_0_1_2_3_FULL_VER.num_rows_;
          auto h_AssignCast_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_AssignCast_0_1_2_3_FULL_VER);
          if (!h_AssignCast_0_root.valid()) continue;
          auto h_SupertypeOf_1_root = HandleType(0, view_SupertypeOf_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_SupertypeOf_0_1_FULL_VER);
          if (!h_SupertypeOf_1_root.valid()) continue;
          auto casttype = root_val_2;
        // Nested ColumnJoin (intersection): bind 'heaptype' from 2 sources
        // MIR: (column-join :var heaptype :sources ((SupertypeOf :handle 2 :prefix (casttype)) (HeapAllocation_Type :handle 3 :prefix ()) ))
        auto h_SupertypeOf_2_12 = h_SupertypeOf_1_root;
        auto h_HeapAllocation_Type_3_13 = HandleType(0, view_HeapAllocation_Type_1_0_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_SupertypeOf_2_12.iterators(view_SupertypeOf_0_1_FULL_VER), h_HeapAllocation_Type_3_13.iterators(view_HeapAllocation_Type_1_0_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto heaptype = it_15.value();
          auto positions = it_15.positions();
          auto ch_SupertypeOf_2_heaptype = h_SupertypeOf_2_12.child_range(positions[0], heaptype, tile, view_SupertypeOf_0_1_FULL_VER);
          auto ch_HeapAllocation_Type_3_heaptype = h_HeapAllocation_Type_3_13.child_range(positions[1], heaptype, tile, view_HeapAllocation_Type_1_0_FULL_VER);
        // Nested CartesianJoin: bind frm, to, inmeth, heap from 2 source(s)
        // MIR: (cartesian-join :vars (frm to inmeth heap) :sources ((AssignCast :handle 4 :prefix (casttype)) (HeapAllocation_Type :handle 5 :prefix (heaptype)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_AssignCast_4_4 = h_AssignCast_0_root;  // reusing narrowed handle
        auto h_HeapAllocation_Type_5_6 = ch_HeapAllocation_Type_3_heaptype;  // reusing narrowed handle

        if (!h_AssignCast_4_4.valid() || !h_HeapAllocation_Type_5_6.valid()) continue;

        uint32_t degree_3 = h_AssignCast_4_4.degree();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto frm = view_AssignCast_0_1_2_3_FULL_VER.get_value(1, h_AssignCast_4_4.begin() + idx0_9);
          auto to = view_AssignCast_0_1_2_3_FULL_VER.get_value(2, h_AssignCast_4_4.begin() + idx0_9);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      uint32_t bg_hhi = view_AssignCast_0_1_2_3_FULL_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_AssignCast_0_1_2_3_FULL_VER.num_rows_) ? bg_hhi : view_AssignCast_0_1_2_3_FULL_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_AssignCast_0_1_2_3_FULL_VER.num_rows_;
      auto h_AssignCast_0_root = HandleType(bg_hlo, bg_hhi, 0).prefix(bg_hist_root_val, tile, view_AssignCast_0_1_2_3_FULL_VER);
      if (!h_AssignCast_0_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      auto h_SupertypeOf_1_root = HandleType(0, view_SupertypeOf_0_1_FULL_VER.num_rows_, 0).prefix(bg_hist_root_val, tile, view_SupertypeOf_0_1_FULL_VER);
      if (!h_SupertypeOf_1_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      uint64_t bg_deg = 1;
      bg_deg *= h_AssignCast_0_root.degree();
      bg_deg *= h_SupertypeOf_1_root.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root ColumnJoin (BLOCK-GROUP): bind 'casttype' from 2 sources
        ...
        for (uint32_t bg_key_idx_1 = bg_key_lo; bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end; bg_key_idx_1++) {
          ...
          uint32_t hint_hi_4 = view_AssignCast_0_1_2_3_FULL_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_AssignCast_0_1_2_3_FULL_VER.num_rows_) ? hint_hi_4 : view_AssignCast_0_1_2_3_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_AssignCast_0_1_2_3_FULL_VER.num_rows_;
          auto h_AssignCast_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_AssignCast_0_1_2_3_FULL_VER);
          if (!h_AssignCast_0_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          auto h_SupertypeOf_1_root = HandleType(0, view_SupertypeOf_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_SupertypeOf_0_1_FULL_VER);
          if (!h_SupertypeOf_1_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          ...
          uint32_t bg_deg_first = (uint32_t)(h_AssignCast_0_root.end() - h_AssignCast_0_root.begin());
          ...
          {
            auto bg_narrow_begin = h_AssignCast_0_root.begin() + bg_warp_row_begin;
            auto bg_narrow_end = h_AssignCast_0_root.begin() + bg_warp_row_end;
            h_AssignCast_0_root = HandleType(bg_narrow_begin, bg_narrow_end, h_AssignCast_0_root.depth());
          }
...
JitRunner_Precompute0::LaunchParams JitRunner_Precompute0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: AssignCast version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<AssignCast, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: SupertypeOf version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<SupertypeOf, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: HeapAllocation_Type version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<HeapAllocation_Type, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<AssignCast, FULL_VER>(db);
...
void JitRunner_Precompute0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Precompute0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<CastTo, NEW_VER>(db);
...
void JitRunner_Precompute0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Precompute0::read_total(LaunchParams& p) {
...
void JitRunner_Precompute0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Precompute0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Precompute0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Precompute0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Precompute0");
  ...
  auto& bg_dest_0 = get_relation_by_schema<CastTo, NEW_VER>(db);
...
void JitRunner_Precompute0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
