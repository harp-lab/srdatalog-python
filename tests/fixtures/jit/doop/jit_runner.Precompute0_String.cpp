// =============================================================
// JIT-Generated Complete Runner: Precompute0_String
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Precompute0_String {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_IsCastableToString_0_FULL_VER = views[1];
        auto view_IsStringHeap_0_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'casttype' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var casttype :sources ((AssignCast :handle 0) (IsCastableToString :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_IsCastableToString_1_root = HandleType(0, view_IsCastableToString_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_IsCastableToString_0_FULL_VER);
          if (!h_IsCastableToString_1_root.valid()) continue;
          auto casttype = root_val_2;
        // Nested CartesianJoin: bind frm, to, inmeth, heap from 2 source(s)
        // MIR: (cartesian-join :vars (frm to inmeth heap) :sources ((AssignCast :handle 2 :prefix (casttype)) (IsStringHeap :handle 3 :prefix ()) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_AssignCast_2_4 = h_AssignCast_0_root;  // reusing narrowed handle
        auto h_IsStringHeap_3_6 = HandleType(0, view_IsStringHeap_0_FULL_VER.num_rows_, 0);

        if (!h_AssignCast_2_4.valid() || !h_IsStringHeap_3_6.valid()) continue;

        uint32_t degree_3 = h_AssignCast_2_4.degree();
        uint32_t degree_5 = h_IsStringHeap_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto frm = view_AssignCast_0_1_2_3_FULL_VER.get_value(1, h_AssignCast_2_4.begin() + idx0_9);
          auto to = view_AssignCast_0_1_2_3_FULL_VER.get_value(2, h_AssignCast_2_4.begin() + idx0_9);
          auto inmeth = view_AssignCast_0_1_2_3_FULL_VER.get_value(3, h_AssignCast_2_4.begin() + idx0_9);
          auto heap = view_IsStringHeap_0_FULL_VER.get_value(0, h_IsStringHeap_3_6.begin() + idx1_10);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      auto h_IsCastableToString_1_root = HandleType(0, view_IsCastableToString_0_FULL_VER.num_rows_, 0).prefix(bg_hist_root_val, tile, view_IsCastableToString_0_FULL_VER);
      if (!h_IsCastableToString_1_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      uint64_t bg_deg = 1;
      bg_deg *= h_AssignCast_0_root.degree();
      bg_deg *= h_IsCastableToString_1_root.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t bg_key_idx_1 = bg_key_lo; bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end; bg_key_idx_1++) {
          ...
          auto h_IsCastableToString_1_root = HandleType(0, view_IsCastableToString_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_IsCastableToString_0_FULL_VER);
          if (!h_IsCastableToString_1_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
...
JitRunner_Precompute0_String::LaunchParams JitRunner_Precompute0_String::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: IsCastableToString version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<IsCastableToString, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: IsStringHeap version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<IsStringHeap, FULL_VER>(db);
...
void JitRunner_Precompute0_String::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Precompute0_String::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Precompute0_String::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Precompute0_String::read_total(LaunchParams& p) {
...
void JitRunner_Precompute0_String::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Precompute0_String::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Precompute0_String::read_fused_result(LaunchParams& p) {
...
void JitRunner_Precompute0_String::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Precompute0_String");
...
void JitRunner_Precompute0_String::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
