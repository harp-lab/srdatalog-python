...
struct JitRunner_Precompute1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_ThisVar_0_1_FULL_VER = views[2];
        ...
        auto h_MethodLookup_2_10 = h_MethodLookup_0_root;
        auto h_ThisVar_3_11 = HandleType(0, view_ThisVar_0_1_FULL_VER.num_rows_, 0);
        auto intersect_12 = intersect_handles(tile, h_MethodLookup_2_10.iterators(view_MethodLookup_2_3_0_1_FULL_VER), h_ThisVar_3_11.iterators(view_ThisVar_0_1_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto toMeth = it_13.value();
          auto positions = it_13.positions();
          auto ch_MethodLookup_2_toMeth = h_MethodLookup_2_10.child_range(positions[0], toMeth, tile, view_MethodLookup_2_3_0_1_FULL_VER);
          auto ch_ThisVar_3_toMeth = h_ThisVar_3_11.child_range(positions[1], toMeth, tile, view_ThisVar_0_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        output_ctx_0.emit_direct(simplename, descriptor, toMeth, thisP, heap);
        }
        }
        }
  }
...
JitRunner_Precompute1::LaunchParams JitRunner_Precompute1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 3: ThisVar version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<ThisVar, FULL_VER>(db);
...
void JitRunner_Precompute1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Precompute1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<HeapHelper, NEW_VER>(db);
...
void JitRunner_Precompute1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Precompute1::read_total(LaunchParams& p) {
...
void JitRunner_Precompute1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Precompute1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Precompute1::read_fused_result(LaunchParams& p) {
...
void JitRunner_Precompute1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Precompute1");
  ...
  auto& bg_dest_0 = get_relation_by_schema<HeapHelper, NEW_VER>(db);
...
void JitRunner_Precompute1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
