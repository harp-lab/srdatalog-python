// =============================================================
// JIT-Generated Complete Runner: Precompute1b
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Precompute1b {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = MethodLookup;
  using DestSchema = HeapHelperNoThis;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Nested CartesianJoin: bind toMeth, simplename, descriptor, heap from 2 source(s)
        // MIR: (cartesian-join :vars (toMeth simplename descriptor heap) :sources ((MethodLookup :handle 2 :prefix (heaptype)) (HeapAllocation_Type :handle 3 :prefix (heaptype)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_MethodLookup_2_4 = h_MethodLookup_0_root;  // reusing narrowed handle
        auto h_HeapAllocation_Type_3_6 = h_HeapAllocation_Type_1_root;  // reusing narrowed handle

        if (!h_MethodLookup_2_4.valid() || !h_HeapAllocation_Type_3_6.valid()) continue;

        uint32_t degree_3 = h_MethodLookup_2_4.degree();
        uint32_t degree_5 = h_HeapAllocation_Type_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto toMeth = view_MethodLookup_2_3_0_1_FULL_VER.get_value(1, h_MethodLookup_2_4.begin() + idx0_9);
          auto simplename = view_MethodLookup_2_3_0_1_FULL_VER.get_value(2, h_MethodLookup_2_4.begin() + idx0_9);
          auto descriptor = view_MethodLookup_2_3_0_1_FULL_VER.get_value(3, h_MethodLookup_2_4.begin() + idx0_9);
          auto heap = view_HeapAllocation_Type_1_0_FULL_VER.get_value(1, h_HeapAllocation_Type_3_6.begin() + idx1_10);

        // Emit: HeapHelperNoThis(simplename, descriptor, toMeth, heap)
        output_ctx_0.emit_direct(simplename, descriptor, toMeth, heap);
        }
        }
  }
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      uint32_t bg_hhi = view_MethodLookup_2_3_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_MethodLookup_2_3_0_1_FULL_VER.num_rows_) ? bg_hhi : view_MethodLookup_2_3_0_1_FULL_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_MethodLookup_2_3_0_1_FULL_VER.num_rows_;
      auto h_MethodLookup_0_root = HandleType(bg_hlo, bg_hhi, 0).prefix(bg_hist_root_val, tile, view_MethodLookup_2_3_0_1_FULL_VER);
      if (!h_MethodLookup_0_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      auto h_HeapAllocation_Type_1_root = HandleType(0, view_HeapAllocation_Type_1_0_FULL_VER.num_rows_, 0).prefix(bg_hist_root_val, tile, view_HeapAllocation_Type_1_0_FULL_VER);
      if (!h_HeapAllocation_Type_1_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      uint64_t bg_deg = 1;
      bg_deg *= h_MethodLookup_0_root.degree();
      bg_deg *= h_HeapAllocation_Type_1_root.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root ColumnJoin (BLOCK-GROUP): bind 'heaptype' from 2 sources
        ...
        for (uint32_t bg_key_idx_1 = bg_key_lo; bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end; bg_key_idx_1++) {
          ...
          uint32_t hint_hi_4 = view_MethodLookup_2_3_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_MethodLookup_2_3_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_MethodLookup_2_3_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_MethodLookup_2_3_0_1_FULL_VER.num_rows_;
          auto h_MethodLookup_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_MethodLookup_2_3_0_1_FULL_VER);
          if (!h_MethodLookup_0_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          auto h_HeapAllocation_Type_1_root = HandleType(0, view_HeapAllocation_Type_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_HeapAllocation_Type_1_0_FULL_VER);
          if (!h_HeapAllocation_Type_1_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          ...
          uint32_t bg_deg_first = (uint32_t)(h_MethodLookup_0_root.end() - h_MethodLookup_0_root.begin());
          ...
          {
            auto bg_narrow_begin = h_MethodLookup_0_root.begin() + bg_warp_row_begin;
            auto bg_narrow_end = h_MethodLookup_0_root.begin() + bg_warp_row_end;
            h_MethodLookup_0_root = HandleType(bg_narrow_begin, bg_narrow_end, h_MethodLookup_0_root.depth());
          }
...
JitRunner_Precompute1b::LaunchParams JitRunner_Precompute1b::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: MethodLookup version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<MethodLookup, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{2, 3, 0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: HeapAllocation_Type version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<HeapAllocation_Type, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<MethodLookup, FULL_VER>(db);
...
void JitRunner_Precompute1b::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Precompute1b::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<HeapHelperNoThis, NEW_VER>(db);
...
void JitRunner_Precompute1b::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Precompute1b::read_total(LaunchParams& p) {
...
void JitRunner_Precompute1b::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Precompute1b::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Precompute1b::read_fused_result(LaunchParams& p) {
...
void JitRunner_Precompute1b::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Precompute1b");
  ...
  auto& bg_dest_0 = get_relation_by_schema<HeapHelperNoThis, NEW_VER>(db);
...
void JitRunner_Precompute1b::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
