// =============================================================
// JIT-Generated Complete Runner: Precompute2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Precompute2 {
  ...
  using DestSchema = VirtualMethodInvocation;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        output_ctx_0.emit_direct(invocation, base, simplename, descriptor);
        }
        }
  }
...
JitRunner_Precompute2::LaunchParams JitRunner_Precompute2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: VirtualMethodInvocation_SimpleName version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<VirtualMethodInvocation_SimpleName, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 2: VirtualMethodInvocation_Descriptor version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<VirtualMethodInvocation_Descriptor, FULL_VER>(db);
...
void JitRunner_Precompute2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Precompute2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<VirtualMethodInvocation, NEW_VER>(db);
...
void JitRunner_Precompute2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Precompute2::read_total(LaunchParams& p) {
...
void JitRunner_Precompute2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Precompute2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Precompute2::read_fused_result(LaunchParams& p) {
...
void JitRunner_Precompute2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Precompute2");
...
void JitRunner_Precompute2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
