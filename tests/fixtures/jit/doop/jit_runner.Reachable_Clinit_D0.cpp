// =============================================================
// JIT-Generated Complete Runner: Reachable_Clinit_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Reachable_Clinit_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        output_ctx_0.emit_direct(clinit);
        }
        }
  }
...
JitRunner_Reachable_Clinit_D0::LaunchParams JitRunner_Reachable_Clinit_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: ClassInitializer version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<ClassInitializer, FULL_VER>(db);
...
void JitRunner_Reachable_Clinit_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Reachable_Clinit_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Reachable_Clinit_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Reachable_Clinit_D0::read_total(LaunchParams& p) {
...
void JitRunner_Reachable_Clinit_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Reachable_Clinit_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Reachable_Clinit_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Reachable_Clinit_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Reachable_Clinit_D0");
...
void JitRunner_Reachable_Clinit_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
