// =============================================================
// JIT-Generated Complete Runner: Reachable_Main
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Reachable_Main {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: MainMethodDeclaration binding meth
        // MIR: (scan :rel MainMethodDeclaration :vars (meth) :handle 0)
        auto root_handle_1 = HandleType(0, view_MainMethodDeclaration_0_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto meth = view_MainMethodDeclaration_0_FULL_VER.get_value(0, idx_3);
        // Emit: Reachable(meth)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto meth = view_MainMethodDeclaration_0_FULL_VER.get_value(0, idx_3);
        // Emit: Reachable(meth)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(meth);
        }
  }
...
JitRunner_Reachable_Main::LaunchParams JitRunner_Reachable_Main::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Reachable_Main::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Reachable_Main::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Reachable_Main::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Reachable_Main::read_total(LaunchParams& p) {
...
void JitRunner_Reachable_Main::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Reachable_Main::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Reachable_Main::read_fused_result(LaunchParams& p) {
...
void JitRunner_Reachable_Main::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Reachable_Main");
...
void JitRunner_Reachable_Main::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
