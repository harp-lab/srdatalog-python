// =============================================================
// JIT-Generated Complete Runner: SFPT_Store_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SFPT_Store_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_StoreStaticField_2_0_1_FULL_VER = views[1];
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_StoreStaticField_1_root = HandleType(0, view_StoreStaticField_2_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_StoreStaticField_2_0_1_FULL_VER);
        ...
        // Nested CartesianJoin: bind fld, heap from 2 source(s)
        // MIR: (cartesian-join :vars (fld heap) :sources ((StoreStaticField :handle 4 :prefix (inmeth frm)) (VarPointsTo :handle 5 :prefix (frm)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_StoreStaticField_4_4 = ch_StoreStaticField_2_frm;  // reusing narrowed handle
        auto h_VarPointsTo_5_6 = ch_VarPointsTo_3_frm;  // reusing narrowed handle

        if (!h_StoreStaticField_4_4.valid() || !h_VarPointsTo_5_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto fld = view_StoreStaticField_2_0_1_FULL_VER.get_value(2, h_StoreStaticField_4_4.begin() + idx0_9);
...
JitRunner_SFPT_Store_D0::LaunchParams JitRunner_SFPT_Store_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_SFPT_Store_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SFPT_Store_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SFPT_Store_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SFPT_Store_D0::read_total(LaunchParams& p) {
...
void JitRunner_SFPT_Store_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SFPT_Store_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SFPT_Store_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_SFPT_Store_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SFPT_Store_D0");
...
void JitRunner_SFPT_Store_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
