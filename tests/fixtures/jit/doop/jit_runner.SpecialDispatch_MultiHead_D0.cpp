// =============================================================
// JIT-Generated Complete Runner: SpecialDispatch_MultiHead_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SpecialDispatch_MultiHead_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var invocation :sources ((ReachableInstruction :handle 0) (SpecialMethodInvocation_Base :handle 1) (MethodInvocation_Method :handle 2) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_SpecialMethodInvocation_Base_1_root = HandleType(0, view_SpecialMethodInvocation_Base_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_SpecialMethodInvocation_Base_0_1_FULL_VER);
          if (!h_SpecialMethodInvocation_Base_1_root.valid()) continue;
          auto h_MethodInvocation_Method_2_root = HandleType(0, view_MethodInvocation_Method_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_MethodInvocation_Method_0_1_FULL_VER);
          if (!h_MethodInvocation_Method_2_root.valid()) continue;
        ...
        for (int _nseg_1 = 0; _nseg_1 < 2; _nseg_1++) {
          ...
          auto ch_VarPointsTo_4_base = h_VarPointsTo_4_13.child_range(positions[1], base, tile, view_VarPointsTo_1_0_FULL_VER);
        ...
        auto h_MethodInvocation_Method_5_8 = h_MethodInvocation_Method_2_root;
        ...
        auto h_VarPointsTo_7_4 = ch_VarPointsTo_4_base;  // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_2 output_ctx_2{output_data_2, output_prov_2, output_stride_2, old_size_2 + thread_offset};
        ...
        for (int _nseg_1 = 0; _nseg_1 < 2; _nseg_1++) {
          ...
          auto ch_VarPointsTo_4_base = h_VarPointsTo_4_25.child_range(positions[1], base, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'tometh' from 2 sources
        // MIR: (column-join :var tometh :sources ((MethodInvocation_Method :handle 5 :prefix (invocation)) (ThisVar :handle 6 :prefix ()) ))
        auto h_MethodInvocation_Method_5_20 = h_MethodInvocation_Method_2_root;
        ...
        auto h_VarPointsTo_7_5 = ch_VarPointsTo_4_base;  // reusing narrowed handle
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_VarPointsTo_1_0_FULL_VER.get_value(1, h_VarPointsTo_7_5.begin() + t0_base_10 + _ti);
        ...
        // Emit: Reachable(tometh)
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto heap = view_VarPointsTo_1_0_FULL_VER.get_value(1, h_VarPointsTo_7_5.begin() + idx0_18);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_fused(
    ...
    SpecCtx_2 output_ctx_2{output_data_2, atomic_write_pos_2, overflow_flag,
        ...
        for (int _nseg_1 = 0; _nseg_1 < 2; _nseg_1++) {
          ...
          auto ch_VarPointsTo_4_base = h_VarPointsTo_4_17.child_range(positions[1], base, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'tometh' from 2 sources
        // MIR: (column-join :var tometh :sources ((MethodInvocation_Method :handle 5 :prefix (invocation)) (ThisVar :handle 6 :prefix ()) ))
        auto h_MethodInvocation_Method_5_12 = h_MethodInvocation_Method_2_root;
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto heap = view_VarPointsTo_1_0_FULL_VER.get_value(1, h_VarPointsTo_7_4.begin() + idx0_9);
...
JitRunner_SpecialDispatch_MultiHead_D0::LaunchParams JitRunner_SpecialDispatch_MultiHead_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 2: MethodInvocation_Method version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<MethodInvocation_Method, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_2.view());
  }

  // Source 4: VarPointsTo version FULL_VER
  {
...
void JitRunner_SpecialDispatch_MultiHead_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SpecialDispatch_MultiHead_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SpecialDispatch_MultiHead_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SpecialDispatch_MultiHead_D0::read_total(LaunchParams& p) {
...
void JitRunner_SpecialDispatch_MultiHead_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SpecialDispatch_MultiHead_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SpecialDispatch_MultiHead_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_SpecialDispatch_MultiHead_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SpecialDispatch_MultiHead_D0");
...
void JitRunner_SpecialDispatch_MultiHead_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
