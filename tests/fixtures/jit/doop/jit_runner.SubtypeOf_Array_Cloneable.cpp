// =============================================================
// JIT-Generated Complete Runner: SubtypeOf_Array_Cloneable
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SubtypeOf_Array_Cloneable {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto _hc2 = 1007;
        // Emit: SubtypeOf(s, _hc2)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(s, _hc2);
        }
  }
...
JitRunner_SubtypeOf_Array_Cloneable::LaunchParams JitRunner_SubtypeOf_Array_Cloneable::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_SubtypeOf_Array_Cloneable::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Array_Cloneable::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_Cloneable::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Array_Cloneable::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Array_Cloneable::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_Cloneable::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_Cloneable::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Array_Cloneable::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Array_Cloneable");
...
void JitRunner_SubtypeOf_Array_Cloneable::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
