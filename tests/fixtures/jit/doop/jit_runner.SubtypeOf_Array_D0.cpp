// =============================================================
// JIT-Generated Complete Runner: SubtypeOf_Array_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SubtypeOf_Array_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = SubtypeOf;
  using DestSchema = SubtypeOf;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto h_SubtypeOf_3_8 = h_SubtypeOf_0_root;
        auto h_ComponentType_4_9 = HandleType(0, view_ComponentType_1_0_FULL_VER.num_rows_, 0);
        auto h_isReferenceType_5_10 = HandleType(0, view_isReferenceType_0_FULL_VER.num_rows_, 0);
        auto intersect_11 = intersect_handles(tile, h_SubtypeOf_3_8.iterators(view_SubtypeOf_0_1_DELTA_VER), h_ComponentType_4_9.iterators(view_ComponentType_1_0_FULL_VER), h_isReferenceType_5_10.iterators(view_isReferenceType_0_FULL_VER));
        for (auto it_12 = intersect_11.begin(); it_12.valid(); it_12.next()) {
          auto tc = it_12.value();
          auto positions = it_12.positions();
          auto ch_SubtypeOf_3_tc = h_SubtypeOf_3_8.child_range(positions[0], tc, tile, view_SubtypeOf_0_1_DELTA_VER);
          auto ch_ComponentType_4_tc = h_ComponentType_4_9.child_range(positions[1], tc, tile, view_ComponentType_1_0_FULL_VER);
          auto ch_isReferenceType_5_tc = h_isReferenceType_5_10.child_range(positions[2], tc, tile, view_isReferenceType_0_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'tc' from 3 sources
        // MIR: (column-join :var tc :sources ((SubtypeOf :handle 3 :prefix (sc)) (ComponentType :handle 4 :prefix ()) (isReferenceType :handle 5 :prefix ()) ))
        auto h_SubtypeOf_3_20 = h_SubtypeOf_0_root;
        auto h_ComponentType_4_21 = HandleType(0, view_ComponentType_1_0_FULL_VER.num_rows_, 0);
        auto h_isReferenceType_5_22 = HandleType(0, view_isReferenceType_0_FULL_VER.num_rows_, 0);
        auto intersect_23 = intersect_handles(tile, h_SubtypeOf_3_20.iterators(view_SubtypeOf_0_1_DELTA_VER), h_ComponentType_4_21.iterators(view_ComponentType_1_0_FULL_VER), h_isReferenceType_5_22.iterators(view_isReferenceType_0_FULL_VER));
        for (auto it_24 = intersect_23.begin(); it_24.valid(); it_24.next()) {
          auto tc = it_24.value();
          auto positions = it_24.positions();
          auto ch_SubtypeOf_3_tc = h_SubtypeOf_3_20.child_range(positions[0], tc, tile, view_SubtypeOf_0_1_DELTA_VER);
          auto ch_ComponentType_4_tc = h_ComponentType_4_21.child_range(positions[1], tc, tile, view_ComponentType_1_0_FULL_VER);
          auto ch_isReferenceType_5_tc = h_isReferenceType_5_22.child_range(positions[2], tc, tile, view_isReferenceType_0_FULL_VER);
        // Nested CartesianJoin: bind s, t from 2 source(s)
        // MIR: (cartesian-join :vars (s t) :sources ((ComponentType :handle 6 :prefix (sc)) (ComponentType :handle 7 :prefix (tc)) ))
        uint32_t lane_2 = tile.thread_rank();
        uint32_t group_size_3 = tile.size();

        auto h_ComponentType_6_5 = h_ComponentType_1_root;  // reusing narrowed handle
        auto h_ComponentType_7_7 = ch_ComponentType_4_tc;  // reusing narrowed handle

        if (!h_ComponentType_6_5.valid() || !h_ComponentType_7_7.valid()) continue;

        uint32_t degree_4 = h_ComponentType_6_5.degree();
        uint32_t degree_6 = h_ComponentType_7_7.degree();
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_ComponentType_1_0_FULL_VER.get_value(1, h_ComponentType_6_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_ComponentType_1_0_FULL_VER.get_value(1, h_ComponentType_7_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto s = tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
                auto t = tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
        // Emit: SubtypeOf(s, t)
        {
          ...
          if (_tc_active > 0) {
            ...
            if (tc_valid_1) {
              ...
              output_data_0[0 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = s;
              output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = t;
            }
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto s = view_ComponentType_1_0_FULL_VER.get_value(1, h_ComponentType_6_5.begin() + idx0_18);
            auto t = view_ComponentType_1_0_FULL_VER.get_value(1, h_ComponentType_7_7.begin() + idx1_19);
        // Emit: SubtypeOf(s, t)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
          uint32_t _tc_active = __popc(_tc_ballot);
...
JitRunner_SubtypeOf_Array_D0::LaunchParams JitRunner_SubtypeOf_Array_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: SubtypeOf version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<SubtypeOf, FULL_VER>(db) : get_relation_by_schema<SubtypeOf, DELTA_VER>(db);
  ...
  // Source 2: isReferenceType version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<isReferenceType, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_2.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<SubtypeOf, FULL_VER>(db) : get_relation_by_schema<SubtypeOf, DELTA_VER>(db);
...
void JitRunner_SubtypeOf_Array_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Array_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<SubtypeOf, NEW_VER>(db);
...
void JitRunner_SubtypeOf_Array_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Array_D0::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Array_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Array_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Array_D0");
...
void JitRunner_SubtypeOf_Array_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
