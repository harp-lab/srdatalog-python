// =============================================================
// JIT-Generated Complete Runner: SubtypeOf_Array_Serializable
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SubtypeOf_Array_Serializable {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = isArrayType_IDB;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto _hc3 = 1008;
        // Emit: SubtypeOf(s, _hc3)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(s, _hc3);
        }
  }
...
JitRunner_SubtypeOf_Array_Serializable::LaunchParams JitRunner_SubtypeOf_Array_Serializable::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: isArrayType_IDB version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<isArrayType_IDB, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<isArrayType_IDB, FULL_VER>(db);
...
void JitRunner_SubtypeOf_Array_Serializable::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Array_Serializable::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_Serializable::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Array_Serializable::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Array_Serializable::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_Serializable::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Array_Serializable::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Array_Serializable::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Array_Serializable");
...
void JitRunner_SubtypeOf_Array_Serializable::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
