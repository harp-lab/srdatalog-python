...
struct JitRunner_SubtypeOf_Class_Iface {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_isClassType_IDB_0_FULL_VER = views[0];
        auto view_Superinterface_1_0_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 's' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var s :sources ((isClassType_IDB :handle 0) (Superinterface :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_isClassType_IDB_0_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_isClassType_IDB_0_FULL_VER.num_rows_) ? hint_hi_4 : view_isClassType_IDB_0_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_isClassType_IDB_0_FULL_VER.num_rows_;
          auto h_isClassType_IDB_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_isClassType_IDB_0_FULL_VER);
          if (!h_isClassType_IDB_0_root.valid()) continue;
...
JitRunner_SubtypeOf_Class_Iface::LaunchParams JitRunner_SubtypeOf_Class_Iface::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: isClassType_IDB version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<isClassType_IDB, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<isClassType_IDB, FULL_VER>(db);
...
void JitRunner_SubtypeOf_Class_Iface::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Class_Iface::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Class_Iface::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Class_Iface::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Class_Iface::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Class_Iface::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Class_Iface::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Class_Iface::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Class_Iface");
...
void JitRunner_SubtypeOf_Class_Iface::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
