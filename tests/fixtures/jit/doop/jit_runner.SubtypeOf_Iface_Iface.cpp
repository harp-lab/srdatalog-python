// =============================================================
// JIT-Generated Complete Runner: SubtypeOf_Iface_Iface
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SubtypeOf_Iface_Iface {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = isInterfaceType_IDB;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_isInterfaceType_IDB_0_FULL_VER = views[0];
        auto view_Superinterface_1_0_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 's' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var s :sources ((isInterfaceType_IDB :handle 0) (Superinterface :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_isInterfaceType_IDB_0_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_isInterfaceType_IDB_0_FULL_VER.num_rows_) ? hint_hi_4 : view_isInterfaceType_IDB_0_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_isInterfaceType_IDB_0_FULL_VER.num_rows_;
          auto h_isInterfaceType_IDB_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_isInterfaceType_IDB_0_FULL_VER);
          if (!h_isInterfaceType_IDB_0_root.valid()) continue;
          auto h_Superinterface_1_root = HandleType(0, view_Superinterface_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Superinterface_1_0_FULL_VER);
          if (!h_Superinterface_1_root.valid()) continue;
          auto s = root_val_2;
        // Nested CartesianJoin: bind t from 1 source(s)
        // MIR: (cartesian-join :vars (t) :sources ((Superinterface :handle 2 :prefix (s)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_Superinterface_2_4 = h_Superinterface_1_root;  // reusing narrowed handle

        if (!h_Superinterface_2_4.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto t = view_Superinterface_1_0_FULL_VER.get_value(1, h_Superinterface_2_4.begin() + idx0_7);
...
JitRunner_SubtypeOf_Iface_Iface::LaunchParams JitRunner_SubtypeOf_Iface_Iface::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: isInterfaceType_IDB version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<isInterfaceType_IDB, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: Superinterface version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Superinterface, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<isInterfaceType_IDB, FULL_VER>(db);
...
void JitRunner_SubtypeOf_Iface_Iface::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Iface_Iface::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Iface_Iface::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Iface_Iface::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Iface_Iface::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Iface_Iface::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Iface_Iface::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Iface_Iface::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Iface_Iface");
...
void JitRunner_SubtypeOf_Iface_Iface::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
