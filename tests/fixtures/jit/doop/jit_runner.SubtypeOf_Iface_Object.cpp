// =============================================================
// JIT-Generated Complete Runner: SubtypeOf_Iface_Object
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SubtypeOf_Iface_Object {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto _hc0 = 427039;
        // Emit: SubtypeOf(s, _hc0)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(s, _hc0);
        }
  }
...
JitRunner_SubtypeOf_Iface_Object::LaunchParams JitRunner_SubtypeOf_Iface_Object::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_SubtypeOf_Iface_Object::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Iface_Object::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Iface_Object::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Iface_Object::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Iface_Object::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Iface_Object::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Iface_Object::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Iface_Object::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Iface_Object");
...
void JitRunner_SubtypeOf_Iface_Object::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
