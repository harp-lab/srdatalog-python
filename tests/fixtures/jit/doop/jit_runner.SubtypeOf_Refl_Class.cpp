// =============================================================
// JIT-Generated Complete Runner: SubtypeOf_Refl_Class
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SubtypeOf_Refl_Class {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: isClassType_IDB binding s
        // MIR: (scan :rel isClassType_IDB :vars (s) :handle 0)
        auto root_handle_1 = HandleType(0, view_isClassType_IDB_0_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto s = view_isClassType_IDB_0_FULL_VER.get_value(0, idx_3);
        // Emit: SubtypeOf(s, s)
...
JitRunner_SubtypeOf_Refl_Class::LaunchParams JitRunner_SubtypeOf_Refl_Class::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_SubtypeOf_Refl_Class::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SubtypeOf_Refl_Class::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Refl_Class::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SubtypeOf_Refl_Class::read_total(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Refl_Class::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Refl_Class::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SubtypeOf_Refl_Class::read_fused_result(LaunchParams& p) {
...
void JitRunner_SubtypeOf_Refl_Class::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SubtypeOf_Refl_Class");
...
void JitRunner_SubtypeOf_Refl_Class::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
