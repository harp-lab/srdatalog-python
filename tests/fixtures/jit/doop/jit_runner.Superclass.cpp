...
struct JitRunner_Superclass {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: Subclass binding a, c
        // MIR: (scan :rel Subclass :vars (a c) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto a = view_Subclass_0_1_FULL_VER.get_value(0, idx_3);
          auto c = view_Subclass_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: Superclass(c, a)
...
JitRunner_Superclass::LaunchParams JitRunner_Superclass::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Superclass::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Superclass::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Superclass, NEW_VER>(db);
...
void JitRunner_Superclass::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Superclass::read_total(LaunchParams& p) {
...
void JitRunner_Superclass::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Superclass::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Superclass::read_fused_result(LaunchParams& p) {
...
void JitRunner_Superclass::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Superclass");
...
void JitRunner_Superclass::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
