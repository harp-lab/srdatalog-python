// =============================================================
// JIT-Generated Complete Runner: Superinterface_Inherit_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Superinterface_Inherit_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var b :sources ((Superinterface :handle 0) (DirectSuperclass :handle 1) ))
        ...
        // MIR: (cartesian-join :vars (k c) :sources ((Superinterface :handle 2 :prefix (b)) (DirectSuperclass :handle 3 :prefix (b)) ))
        ...
        auto h_DirectSuperclass_3_6 = h_DirectSuperclass_1_root;  // reusing narrowed handle

        if (!h_Superinterface_2_4.valid() || !h_DirectSuperclass_3_6.valid()) continue;

        uint32_t degree_3 = h_Superinterface_2_4.degree();
        uint32_t degree_5 = h_DirectSuperclass_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto c = view_DirectSuperclass_1_0_FULL_VER.get_value(1, h_DirectSuperclass_3_6.begin() + idx1_10);
...
JitRunner_Superinterface_Inherit_D0::LaunchParams JitRunner_Superinterface_Inherit_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: DirectSuperclass version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<DirectSuperclass, FULL_VER>(db);
...
void JitRunner_Superinterface_Inherit_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Superinterface_Inherit_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Superinterface_Inherit_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Superinterface_Inherit_D0::read_total(LaunchParams& p) {
...
void JitRunner_Superinterface_Inherit_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Superinterface_Inherit_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Superinterface_Inherit_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Superinterface_Inherit_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Superinterface_Inherit_D0");
...
void JitRunner_Superinterface_Inherit_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
