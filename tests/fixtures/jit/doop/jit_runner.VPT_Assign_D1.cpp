// =============================================================
// JIT-Generated Complete Runner: VPT_Assign_D1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_Assign_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Assign_0_1_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'frm' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var frm :sources ((VarPointsTo :handle 0) (Assign :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_Assign_1_root = HandleType(0, view_Assign_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Assign_0_1_FULL_VER);
          if (!h_Assign_1_root.valid()) continue;
          auto frm = root_val_2;
        // Nested CartesianJoin: bind heap, to from 2 source(s)
        // MIR: (cartesian-join :vars (heap to) :sources ((VarPointsTo :handle 2 :prefix (frm)) (Assign :handle 3 :prefix (frm)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_VarPointsTo_2_4 = h_VarPointsTo_0_root;  // reusing narrowed handle
        auto h_Assign_3_6 = h_Assign_1_root;  // reusing narrowed handle

        if (!h_VarPointsTo_2_4.valid() || !h_Assign_3_6.valid()) continue;

        uint32_t degree_3 = h_VarPointsTo_2_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto heap = view_VarPointsTo_1_0_DELTA_VER.get_value(1, h_VarPointsTo_2_4.begin() + idx0_9);
          auto to = view_Assign_0_1_FULL_VER.get_value(1, h_Assign_3_6.begin() + idx1_10);
...
JitRunner_VPT_Assign_D1::LaunchParams JitRunner_VPT_Assign_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VPT_Assign_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_Assign_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Assign_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_Assign_D1::read_total(LaunchParams& p) {
...
void JitRunner_VPT_Assign_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Assign_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Assign_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_Assign_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_Assign_D1");
...
void JitRunner_VPT_Assign_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
