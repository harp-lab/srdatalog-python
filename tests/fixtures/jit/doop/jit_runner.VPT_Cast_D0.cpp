// =============================================================
// JIT-Generated Complete Runner: VPT_Cast_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_Cast_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_CastTo_1_root = HandleType(0, view_CastTo_2_0_3_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_CastTo_2_0_3_1_FULL_VER);
        ...
        // MIR: (column-join :var heap :sources ((CastTo :handle 4 :prefix (inmeth frm)) (VarPointsTo :handle 5 :prefix (frm)) ))
        auto h_CastTo_4_6 = ch_CastTo_2_frm;
        auto h_VarPointsTo_5_7 = ch_VarPointsTo_3_frm;
        auto intersect_8 = intersect_handles(tile, h_CastTo_4_6.iterators(view_CastTo_2_0_3_1_FULL_VER), h_VarPointsTo_5_7.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto heap = it_9.value();
          auto positions = it_9.positions();
          auto ch_CastTo_4_heap = h_CastTo_4_6.child_range(positions[0], heap, tile, view_CastTo_2_0_3_1_FULL_VER);
          auto ch_VarPointsTo_5_heap = h_VarPointsTo_5_7.child_range(positions[1], heap, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested CartesianJoin: bind to from 1 source(s)
        // MIR: (cartesian-join :vars (to) :sources ((CastTo :handle 6 :prefix (inmeth frm heap)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_CastTo_6_4 = ch_CastTo_4_heap;  // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'heap' from 2 sources
        // MIR: (column-join :var heap :sources ((CastTo :handle 4 :prefix (inmeth frm)) (VarPointsTo :handle 5 :prefix (frm)) ))
        auto h_CastTo_4_8 = ch_CastTo_2_frm;
        auto h_VarPointsTo_5_9 = ch_VarPointsTo_3_frm;
        auto intersect_10 = intersect_handles(tile, h_CastTo_4_8.iterators(view_CastTo_2_0_3_1_FULL_VER), h_VarPointsTo_5_9.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto heap = it_11.value();
          auto positions = it_11.positions();
          auto ch_CastTo_4_heap = h_CastTo_4_8.child_range(positions[0], heap, tile, view_CastTo_2_0_3_1_FULL_VER);
          auto ch_VarPointsTo_5_heap = h_VarPointsTo_5_9.child_range(positions[1], heap, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested CartesianJoin: bind to from 1 source(s)
        // MIR: (cartesian-join :vars (to) :sources ((CastTo :handle 6 :prefix (inmeth frm heap)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto to = view_CastTo_2_0_3_1_FULL_VER.get_value(3, h_CastTo_6_4.begin() + idx0_7);
...
JitRunner_VPT_Cast_D0::LaunchParams JitRunner_VPT_Cast_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: CastTo version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<CastTo, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{2, 0, 3, 1}}, false);
...
void JitRunner_VPT_Cast_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_Cast_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Cast_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_Cast_D0::read_total(LaunchParams& p) {
...
void JitRunner_VPT_Cast_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Cast_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Cast_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_Cast_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_Cast_D0");
...
void JitRunner_VPT_Cast_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
