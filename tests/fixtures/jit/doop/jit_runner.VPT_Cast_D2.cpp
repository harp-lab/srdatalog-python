// =============================================================
// JIT-Generated Complete Runner: VPT_Cast_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_Cast_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto h_VarPointsTo_2_10 = h_VarPointsTo_0_root;
        auto h_CastTo_3_11 = h_CastTo_1_root;
        auto intersect_12 = intersect_handles(tile, h_VarPointsTo_2_10.iterators(view_VarPointsTo_0_1_DELTA_VER), h_CastTo_3_11.iterators(view_CastTo_3_0_2_1_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto frm = it_13.value();
          auto positions = it_13.positions();
          auto ch_VarPointsTo_2_frm = h_VarPointsTo_2_10.child_range(positions[0], frm, tile, view_VarPointsTo_0_1_DELTA_VER);
          auto ch_CastTo_3_frm = h_CastTo_3_11.child_range(positions[1], frm, tile, view_CastTo_3_0_2_1_FULL_VER);
        ...
        auto h_CastTo_4_6 = ch_CastTo_3_frm;
        auto h_Reachable_5_7 = HandleType(0, view_Reachable_0_FULL_VER.num_rows_, 0);
        auto intersect_8 = intersect_handles(tile, h_CastTo_4_6.iterators(view_CastTo_3_0_2_1_FULL_VER), h_Reachable_5_7.iterators(view_Reachable_0_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto inmeth = it_9.value();
          auto positions = it_9.positions();
          auto ch_CastTo_4_inmeth = h_CastTo_4_6.child_range(positions[0], inmeth, tile, view_CastTo_3_0_2_1_FULL_VER);
          auto ch_Reachable_5_inmeth = h_Reachable_5_7.child_range(positions[1], inmeth, tile, view_Reachable_0_FULL_VER);
...
JitRunner_VPT_Cast_D2::LaunchParams JitRunner_VPT_Cast_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: CastTo version FULL_VER
  {
    ...
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{3, 0, 2, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 5: Reachable version FULL_VER
  {
    auto& rel_5 = get_relation_by_schema<Reachable, FULL_VER>(db);
...
void JitRunner_VPT_Cast_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_Cast_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Cast_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_Cast_D2::read_total(LaunchParams& p) {
...
void JitRunner_VPT_Cast_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Cast_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_Cast_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_Cast_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_Cast_D2");
...
void JitRunner_VPT_Cast_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
