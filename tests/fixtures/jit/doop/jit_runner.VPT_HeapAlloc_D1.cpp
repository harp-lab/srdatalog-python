// =============================================================
// JIT-Generated Complete Runner: VPT_HeapAlloc_D1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_HeapAlloc_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Nested CartesianJoin: bind heap, vr from 1 source(s)
        // MIR: (cartesian-join :vars (heap vr) :sources ((AssignHeapAllocation :handle 2 :prefix (inMeth)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_AssignHeapAllocation_2_4 = h_AssignHeapAllocation_1_root;  // reusing narrowed handle

        if (!h_AssignHeapAllocation_2_4.valid()) continue;

        uint32_t degree_3 = h_AssignHeapAllocation_2_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto heap = view_AssignHeapAllocation_2_0_1_FULL_VER.get_value(1, h_AssignHeapAllocation_2_4.begin() + idx0_7);
          auto vr = view_AssignHeapAllocation_2_0_1_FULL_VER.get_value(2, h_AssignHeapAllocation_2_4.begin() + idx0_7);

        // Emit: VarPointsTo(heap, vr)
        output_ctx_0.emit_direct(heap, vr);
        }
        }
  }
...
JitRunner_VPT_HeapAlloc_D1::LaunchParams JitRunner_VPT_HeapAlloc_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_VPT_HeapAlloc_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_HeapAlloc_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_HeapAlloc_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_HeapAlloc_D1::read_total(LaunchParams& p) {
...
void JitRunner_VPT_HeapAlloc_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_HeapAlloc_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_HeapAlloc_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_HeapAlloc_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_HeapAlloc_D1");
...
void JitRunner_VPT_HeapAlloc_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
