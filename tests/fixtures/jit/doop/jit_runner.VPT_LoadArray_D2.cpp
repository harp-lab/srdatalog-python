// =============================================================
// JIT-Generated Complete Runner: VPT_LoadArray_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_LoadArray_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_ArrayIndexPointsTo_0_1_FULL_VER = views[1];
        ...
        // MIR: (column-join :var baseheap :sources ((VarPointsTo :handle 0) (ArrayIndexPointsTo :handle 1) (ArrayTypeCompat :handle 2) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_ArrayIndexPointsTo_1_root = HandleType(0, view_ArrayIndexPointsTo_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_ArrayIndexPointsTo_0_1_FULL_VER);
          if (!h_ArrayIndexPointsTo_1_root.valid()) continue;
        ...
        auto h_VarPointsTo_3_18 = h_VarPointsTo_0_root;
        auto h_LoadArrayIndex_4_19 = HandleType(0, view_LoadArrayIndex_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_20 = intersect_handles(tile, h_VarPointsTo_3_18.iterators(view_VarPointsTo_0_1_DELTA_VER), h_LoadArrayIndex_4_19.iterators(view_LoadArrayIndex_0_1_2_FULL_VER));
        for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
          auto base = it_21.value();
          auto positions = it_21.positions();
          auto ch_VarPointsTo_3_base = h_VarPointsTo_3_18.child_range(positions[0], base, tile, view_VarPointsTo_0_1_DELTA_VER);
          auto ch_LoadArrayIndex_4_base = h_LoadArrayIndex_4_19.child_range(positions[1], base, tile, view_LoadArrayIndex_0_1_2_FULL_VER);
        ...
        auto h_LoadArrayIndex_5_14 = ch_LoadArrayIndex_4_base;
        auto h_Var_Type_6_15 = HandleType(0, view_Var_Type_0_1_FULL_VER.num_rows_, 0);
        auto intersect_16 = intersect_handles(tile, h_LoadArrayIndex_5_14.iterators(view_LoadArrayIndex_0_1_2_FULL_VER), h_Var_Type_6_15.iterators(view_Var_Type_0_1_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto to = it_17.value();
          auto positions = it_17.positions();
          auto ch_LoadArrayIndex_5_to = h_LoadArrayIndex_5_14.child_range(positions[0], to, tile, view_LoadArrayIndex_0_1_2_FULL_VER);
          auto ch_Var_Type_6_to = h_Var_Type_6_15.child_range(positions[1], to, tile, view_Var_Type_0_1_FULL_VER);
        ...
        auto h_LoadArrayIndex_7_10 = ch_LoadArrayIndex_5_to;
        auto h_Reachable_8_11 = HandleType(0, view_Reachable_0_FULL_VER.num_rows_, 0);
        auto intersect_12 = intersect_handles(tile, h_LoadArrayIndex_7_10.iterators(view_LoadArrayIndex_0_1_2_FULL_VER), h_Reachable_8_11.iterators(view_Reachable_0_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto inmeth = it_13.value();
          auto positions = it_13.positions();
          auto ch_LoadArrayIndex_7_inmeth = h_LoadArrayIndex_7_10.child_range(positions[0], inmeth, tile, view_LoadArrayIndex_0_1_2_FULL_VER);
          auto ch_Reachable_8_inmeth = h_Reachable_8_11.child_range(positions[1], inmeth, tile, view_Reachable_0_FULL_VER);
        ...
        auto h_Var_Type_9_6 = ch_Var_Type_6_to;
        auto h_ArrayTypeCompat_10_7 = h_ArrayTypeCompat_2_root;
        auto intersect_8 = intersect_handles(tile, h_Var_Type_9_6.iterators(view_Var_Type_0_1_FULL_VER), h_ArrayTypeCompat_10_7.iterators(view_ArrayTypeCompat_0_1_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto vtype = it_9.value();
          auto positions = it_9.positions();
          auto ch_Var_Type_9_vtype = h_Var_Type_9_6.child_range(positions[0], vtype, tile, view_Var_Type_0_1_FULL_VER);
          auto ch_ArrayTypeCompat_10_vtype = h_ArrayTypeCompat_10_7.child_range(positions[1], vtype, tile, view_ArrayTypeCompat_0_1_FULL_VER);
        ...
        auto h_ArrayIndexPointsTo_11_4 = h_ArrayIndexPointsTo_1_root;  // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'base' from 2 sources
        // MIR: (column-join :var base :sources ((VarPointsTo :handle 3 :prefix (baseheap)) (LoadArrayIndex :handle 4 :prefix ()) ))
        auto h_VarPointsTo_3_20 = h_VarPointsTo_0_root;
        auto h_LoadArrayIndex_4_21 = HandleType(0, view_LoadArrayIndex_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_22 = intersect_handles(tile, h_VarPointsTo_3_20.iterators(view_VarPointsTo_0_1_DELTA_VER), h_LoadArrayIndex_4_21.iterators(view_LoadArrayIndex_0_1_2_FULL_VER));
        for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
          auto base = it_23.value();
          auto positions = it_23.positions();
          auto ch_VarPointsTo_3_base = h_VarPointsTo_3_20.child_range(positions[0], base, tile, view_VarPointsTo_0_1_DELTA_VER);
...
JitRunner_VPT_LoadArray_D2::LaunchParams JitRunner_VPT_LoadArray_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: ArrayIndexPointsTo version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<ArrayIndexPointsTo, FULL_VER>(db);
  ...
  {
    auto& rel_2 = get_relation_by_schema<ArrayTypeCompat, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_2.view());
  }

  // Source 4: LoadArrayIndex version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<LoadArrayIndex, FULL_VER>(db);
    auto& idx_4 = rel_4.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_4.view());
  }

  // Source 6: Var_Type version FULL_VER
  {
    auto& rel_6 = get_relation_by_schema<Var_Type, FULL_VER>(db);
    auto& idx_6 = rel_6.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_6.view());
  }

  // Source 8: Reachable version FULL_VER
  {
    auto& rel_8 = get_relation_by_schema<Reachable, FULL_VER>(db);
    auto& idx_8 = rel_8.ensure_index(SRDatalog::IndexSpec{{0}}, false);
...
void JitRunner_VPT_LoadArray_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadArray_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadArray_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadArray_D2::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadArray_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadArray_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadArray_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadArray_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadArray_D2");
...
void JitRunner_VPT_LoadArray_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
