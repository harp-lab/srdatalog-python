...
struct JitRunner_VPT_LoadArray_D3 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto intersect_20 = intersect_handles(tile, h_VarPointsTo_3_18.iterators(view_VarPointsTo_0_1_FULL_VER), h_LoadArrayIndex_4_19.iterators(view_LoadArrayIndex_0_1_2_FULL_VER));
        for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
          auto base = it_21.value();
          auto positions = it_21.positions();
          auto ch_VarPointsTo_3_base = h_VarPointsTo_3_18.child_range(positions[0], base, tile, view_VarPointsTo_0_1_FULL_VER);
...
JitRunner_VPT_LoadArray_D3::LaunchParams JitRunner_VPT_LoadArray_D3::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ArrayIndexPointsTo version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ArrayIndexPointsTo, FULL_VER>(db) : get_relation_by_schema<ArrayIndexPointsTo, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ArrayIndexPointsTo, FULL_VER>(db) : get_relation_by_schema<ArrayIndexPointsTo, DELTA_VER>(db);
...
void JitRunner_VPT_LoadArray_D3::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadArray_D3::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadArray_D3::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadArray_D3::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadArray_D3::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadArray_D3::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadArray_D3::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadArray_D3::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadArray_D3");
...
void JitRunner_VPT_LoadArray_D3::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
