  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (int _nseg_1 = 0; _nseg_1 < 2; _nseg_1++) {
          ...
          auto ch_InstanceFieldPointsTo_3_sig = h_InstanceFieldPointsTo_3_13.child_range(positions[1], sig, tile, view_InstanceFieldPointsTo_1_2_0_FULL_VER);
        ...
        auto h_VarPointsTo_4_8 = h_VarPointsTo_1_root;
        auto h_InstanceFieldPointsTo_5_9 = ch_InstanceFieldPointsTo_3_sig;
        auto intersect_10 = intersect_handles(tile, h_VarPointsTo_4_8.iterators(view_VarPointsTo_1_0_FULL_VER), h_InstanceFieldPointsTo_5_9.iterators(view_InstanceFieldPointsTo_1_2_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          ...
          auto ch_InstanceFieldPointsTo_5_baseheap = h_InstanceFieldPointsTo_5_9.child_range(positions[1], baseheap, tile, view_InstanceFieldPointsTo_1_2_0_FULL_VER);
...
JitRunner_VPT_LoadField_D0::LaunchParams JitRunner_VPT_LoadField_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ReachableLoadInstanceField version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ReachableLoadInstanceField, FULL_VER>(db) : get_relation_by_schema<ReachableLoadInstanceField, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ReachableLoadInstanceField, FULL_VER>(db) : get_relation_by_schema<ReachableLoadInstanceField, DELTA_VER>(db);
...
void JitRunner_VPT_LoadField_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadField_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadField_D0::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadField_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadField_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadField_D0");
...
void JitRunner_VPT_LoadField_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
