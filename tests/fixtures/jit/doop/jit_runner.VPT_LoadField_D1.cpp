// =============================================================
// JIT-Generated Complete Runner: VPT_LoadField_D1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_LoadField_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_InstanceFieldPointsTo_2_1_0_FULL_VER = views[1];
        auto view_ReachableLoadInstanceField_0_1_2_FULL_VER = views[3];

        // Root ColumnJoin (multi-source intersection): bind 'baseheap' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var baseheap :sources ((VarPointsTo :handle 0) (InstanceFieldPointsTo :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          // Segment loop: InstanceFieldPointsTo FULL_VER has 2 segments (FULL + HEAD)
          for (int _seg_1 = 0; _seg_1 < 2; _seg_1++) {
            auto view_InstanceFieldPointsTo_1 = views[1 + _seg_1];
            view_InstanceFieldPointsTo_2_1_0_FULL_VER = view_InstanceFieldPointsTo_1;
            auto h_InstanceFieldPointsTo_1_root = HandleType(0, view_InstanceFieldPointsTo_1.num_rows_, 0).prefix(root_val_2, tile, view_InstanceFieldPointsTo_1);
            if (!h_InstanceFieldPointsTo_1_root.valid()) continue;
            auto baseheap = root_val_2;
        // Nested ColumnJoin (intersection): bind 'base' from 2 sources
        // MIR: (column-join :var base :sources ((VarPointsTo :handle 2 :prefix (baseheap)) (ReachableLoadInstanceField :handle 3 :prefix ()) ))
        auto h_VarPointsTo_2_12 = h_VarPointsTo_0_root;
        auto h_ReachableLoadInstanceField_3_13 = HandleType(0, view_ReachableLoadInstanceField_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_VarPointsTo_2_12.iterators(view_VarPointsTo_0_1_DELTA_VER), h_ReachableLoadInstanceField_3_13.iterators(view_ReachableLoadInstanceField_0_1_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto base = it_15.value();
          auto positions = it_15.positions();
          auto ch_VarPointsTo_2_base = h_VarPointsTo_2_12.child_range(positions[0], base, tile, view_VarPointsTo_0_1_DELTA_VER);
          auto ch_ReachableLoadInstanceField_3_base = h_ReachableLoadInstanceField_3_13.child_range(positions[1], base, tile, view_ReachableLoadInstanceField_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'sig' from 2 sources
        // MIR: (column-join :var sig :sources ((ReachableLoadInstanceField :handle 4 :prefix (base)) (InstanceFieldPointsTo :handle 5 :prefix (baseheap)) ))
        auto h_ReachableLoadInstanceField_4_8 = ch_ReachableLoadInstanceField_3_base;
        auto h_InstanceFieldPointsTo_5_9 = h_InstanceFieldPointsTo_1_root;
        auto intersect_10 = intersect_handles(tile, h_ReachableLoadInstanceField_4_8.iterators(view_ReachableLoadInstanceField_0_1_2_FULL_VER), h_InstanceFieldPointsTo_5_9.iterators(view_InstanceFieldPointsTo_2_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto sig = it_11.value();
          auto positions = it_11.positions();
          auto ch_ReachableLoadInstanceField_4_sig = h_ReachableLoadInstanceField_4_8.child_range(positions[0], sig, tile, view_ReachableLoadInstanceField_0_1_2_FULL_VER);
          auto ch_InstanceFieldPointsTo_5_sig = h_InstanceFieldPointsTo_5_9.child_range(positions[1], sig, tile, view_InstanceFieldPointsTo_2_1_0_FULL_VER);
        // Nested CartesianJoin: bind to, heap from 2 source(s)
        // MIR: (cartesian-join :vars (to heap) :sources ((ReachableLoadInstanceField :handle 6 :prefix (base sig)) (InstanceFieldPointsTo :handle 7 :prefix (baseheap sig)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_ReachableLoadInstanceField_6_4 = ch_ReachableLoadInstanceField_4_sig;  // reusing narrowed handle
        auto h_InstanceFieldPointsTo_7_6 = ch_InstanceFieldPointsTo_5_sig;  // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'base' from 2 sources
        // MIR: (column-join :var base :sources ((VarPointsTo :handle 2 :prefix (baseheap)) (ReachableLoadInstanceField :handle 3 :prefix ()) ))
        auto h_VarPointsTo_2_16 = h_VarPointsTo_0_root;
        auto h_ReachableLoadInstanceField_3_17 = HandleType(0, view_ReachableLoadInstanceField_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_18 = intersect_handles(tile, h_VarPointsTo_2_16.iterators(view_VarPointsTo_0_1_DELTA_VER), h_ReachableLoadInstanceField_3_17.iterators(view_ReachableLoadInstanceField_0_1_2_FULL_VER));
        for (auto it_19 = intersect_18.begin(); it_19.valid(); it_19.next()) {
          auto base = it_19.value();
          auto positions = it_19.positions();
          auto ch_VarPointsTo_2_base = h_VarPointsTo_2_16.child_range(positions[0], base, tile, view_VarPointsTo_0_1_DELTA_VER);
          auto ch_ReachableLoadInstanceField_3_base = h_ReachableLoadInstanceField_3_17.child_range(positions[1], base, tile, view_ReachableLoadInstanceField_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'sig' from 2 sources
        // MIR: (column-join :var sig :sources ((ReachableLoadInstanceField :handle 4 :prefix (base)) (InstanceFieldPointsTo :handle 5 :prefix (baseheap)) ))
        auto h_ReachableLoadInstanceField_4_12 = ch_ReachableLoadInstanceField_3_base;
        auto h_InstanceFieldPointsTo_5_13 = h_InstanceFieldPointsTo_1_root;
        auto intersect_14 = intersect_handles(tile, h_ReachableLoadInstanceField_4_12.iterators(view_ReachableLoadInstanceField_0_1_2_FULL_VER), h_InstanceFieldPointsTo_5_13.iterators(view_InstanceFieldPointsTo_2_1_0_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto sig = it_15.value();
          auto positions = it_15.positions();
          auto ch_ReachableLoadInstanceField_4_sig = h_ReachableLoadInstanceField_4_12.child_range(positions[0], sig, tile, view_ReachableLoadInstanceField_0_1_2_FULL_VER);
          auto ch_InstanceFieldPointsTo_5_sig = h_InstanceFieldPointsTo_5_13.child_range(positions[1], sig, tile, view_InstanceFieldPointsTo_2_1_0_FULL_VER);
        // Nested CartesianJoin: bind to, heap from 2 source(s)
        // MIR: (cartesian-join :vars (to heap) :sources ((ReachableLoadInstanceField :handle 6 :prefix (base sig)) (InstanceFieldPointsTo :handle 7 :prefix (baseheap sig)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto to = view_ReachableLoadInstanceField_0_1_2_FULL_VER.get_value(2, h_ReachableLoadInstanceField_6_4.begin() + idx0_9);
          auto heap = view_InstanceFieldPointsTo_2_1_0_FULL_VER.get_value(2, h_InstanceFieldPointsTo_7_6.begin() + idx1_10);
...
JitRunner_VPT_LoadField_D1::LaunchParams JitRunner_VPT_LoadField_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: InstanceFieldPointsTo version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<InstanceFieldPointsTo, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{2, 1, 0}}, false);
  ...
  // Source 3: ReachableLoadInstanceField version FULL_VER
  {
    ...
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
...
void JitRunner_VPT_LoadField_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadField_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadField_D1::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadField_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadField_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadField_D1");
...
void JitRunner_VPT_LoadField_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
