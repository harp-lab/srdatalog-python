...
struct JitRunner_VPT_LoadField_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var baseheap :sources ((InstanceFieldPointsTo :handle 0) (VarPointsTo :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_;
        ...
        auto h_ReachableLoadInstanceField_3_13 = HandleType(0, view_ReachableLoadInstanceField_1_0_2_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_InstanceFieldPointsTo_2_12.iterators(view_InstanceFieldPointsTo_2_1_0_DELTA_VER), h_ReachableLoadInstanceField_3_13.iterators(view_ReachableLoadInstanceField_1_0_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto sig = it_15.value();
          auto positions = it_15.positions();
          auto ch_InstanceFieldPointsTo_2_sig = h_InstanceFieldPointsTo_2_12.child_range(positions[0], sig, tile, view_InstanceFieldPointsTo_2_1_0_DELTA_VER);
          auto ch_ReachableLoadInstanceField_3_sig = h_ReachableLoadInstanceField_3_13.child_range(positions[1], sig, tile, view_ReachableLoadInstanceField_1_0_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'base' from 2 sources
        // MIR: (column-join :var base :sources ((ReachableLoadInstanceField :handle 4 :prefix (sig)) (VarPointsTo :handle 5 :prefix (baseheap)) ))
        auto h_ReachableLoadInstanceField_4_8 = ch_ReachableLoadInstanceField_3_sig;
        auto h_VarPointsTo_5_9 = h_VarPointsTo_1_root;
        auto intersect_10 = intersect_handles(tile, h_ReachableLoadInstanceField_4_8.iterators(view_ReachableLoadInstanceField_1_0_2_FULL_VER), h_VarPointsTo_5_9.iterators(view_VarPointsTo_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto base = it_11.value();
          auto positions = it_11.positions();
          auto ch_ReachableLoadInstanceField_4_base = h_ReachableLoadInstanceField_4_8.child_range(positions[0], base, tile, view_ReachableLoadInstanceField_1_0_2_FULL_VER);
          auto ch_VarPointsTo_5_base = h_VarPointsTo_5_9.child_range(positions[1], base, tile, view_VarPointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind heap, to from 2 source(s)
        // MIR: (cartesian-join :vars (heap to) :sources ((InstanceFieldPointsTo :handle 6 :prefix (baseheap sig)) (ReachableLoadInstanceField :handle 7 :prefix (sig base)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_InstanceFieldPointsTo_6_4 = ch_InstanceFieldPointsTo_2_sig;  // reusing narrowed handle
        auto h_ReachableLoadInstanceField_7_6 = ch_ReachableLoadInstanceField_4_base;  // reusing narrowed handle

        if (!h_InstanceFieldPointsTo_6_4.valid() || !h_ReachableLoadInstanceField_7_6.valid()) continue;

        uint32_t degree_3 = h_InstanceFieldPointsTo_6_4.degree();
        uint32_t degree_5 = h_ReachableLoadInstanceField_7_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_ReachableLoadInstanceField_3_17 = HandleType(0, view_ReachableLoadInstanceField_1_0_2_FULL_VER.num_rows_, 0);
        auto intersect_18 = intersect_handles(tile, h_InstanceFieldPointsTo_2_16.iterators(view_InstanceFieldPointsTo_2_1_0_DELTA_VER), h_ReachableLoadInstanceField_3_17.iterators(view_ReachableLoadInstanceField_1_0_2_FULL_VER));
        for (auto it_19 = intersect_18.begin(); it_19.valid(); it_19.next()) {
          auto sig = it_19.value();
          auto positions = it_19.positions();
          auto ch_InstanceFieldPointsTo_2_sig = h_InstanceFieldPointsTo_2_16.child_range(positions[0], sig, tile, view_InstanceFieldPointsTo_2_1_0_DELTA_VER);
          auto ch_ReachableLoadInstanceField_3_sig = h_ReachableLoadInstanceField_3_17.child_range(positions[1], sig, tile, view_ReachableLoadInstanceField_1_0_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'base' from 2 sources
        // MIR: (column-join :var base :sources ((ReachableLoadInstanceField :handle 4 :prefix (sig)) (VarPointsTo :handle 5 :prefix (baseheap)) ))
        auto h_ReachableLoadInstanceField_4_12 = ch_ReachableLoadInstanceField_3_sig;
        auto h_VarPointsTo_5_13 = h_VarPointsTo_1_root;
        auto intersect_14 = intersect_handles(tile, h_ReachableLoadInstanceField_4_12.iterators(view_ReachableLoadInstanceField_1_0_2_FULL_VER), h_VarPointsTo_5_13.iterators(view_VarPointsTo_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto base = it_15.value();
          auto positions = it_15.positions();
          auto ch_ReachableLoadInstanceField_4_base = h_ReachableLoadInstanceField_4_12.child_range(positions[0], base, tile, view_ReachableLoadInstanceField_1_0_2_FULL_VER);
          auto ch_VarPointsTo_5_base = h_VarPointsTo_5_13.child_range(positions[1], base, tile, view_VarPointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind heap, to from 2 source(s)
        // MIR: (cartesian-join :vars (heap to) :sources ((InstanceFieldPointsTo :handle 6 :prefix (baseheap sig)) (ReachableLoadInstanceField :handle 7 :prefix (sig base)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto heap = view_InstanceFieldPointsTo_2_1_0_DELTA_VER.get_value(2, h_InstanceFieldPointsTo_6_4.begin() + idx0_9);
          auto to = view_ReachableLoadInstanceField_1_0_2_FULL_VER.get_value(2, h_ReachableLoadInstanceField_7_6.begin() + idx1_10);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      uint32_t bg_hhi = view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_) ? bg_hhi : view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t bg_key_idx_1 = bg_key_lo; bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end; bg_key_idx_1++) {
          ...
          uint32_t hint_hi_4 = view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_InstanceFieldPointsTo_2_1_0_DELTA_VER.num_rows_;
          auto h_InstanceFieldPointsTo_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_InstanceFieldPointsTo_2_1_0_DELTA_VER);
          ...
          uint32_t bg_deg_first = (uint32_t)(h_InstanceFieldPointsTo_0_root.end() - h_InstanceFieldPointsTo_0_root.begin());
          ...
          {
            auto bg_narrow_begin = h_InstanceFieldPointsTo_0_root.begin() + bg_warp_row_begin;
            auto bg_narrow_end = h_InstanceFieldPointsTo_0_root.begin() + bg_warp_row_end;
...
JitRunner_VPT_LoadField_D2::LaunchParams JitRunner_VPT_LoadField_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: InstanceFieldPointsTo version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<InstanceFieldPointsTo, FULL_VER>(db) : get_relation_by_schema<InstanceFieldPointsTo, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{2, 1, 0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: VarPointsTo version FULL_VER
  {
  ...
  {
    auto& rel_3 = get_relation_by_schema<ReachableLoadInstanceField, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{1, 0, 2}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<InstanceFieldPointsTo, FULL_VER>(db) : get_relation_by_schema<InstanceFieldPointsTo, DELTA_VER>(db);
  auto& first_idx = first_rel.get_index(SRDatalog::IndexSpec{{2, 1, 0}});
...
void JitRunner_VPT_LoadField_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadField_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadField_D2::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadField_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadField_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadField_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadField_D2");
...
void JitRunner_VPT_LoadField_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
