// =============================================================
// JIT-Generated Complete Runner: VPT_LoadStatic_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VPT_LoadStatic_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_StaticFieldPointsTo_1_0_FULL_VER = views[2];
        ...
        auto h_StaticFieldPointsTo_3_9 = HandleType(0, view_StaticFieldPointsTo_1_0_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_LoadStaticField_2_8.iterators(view_LoadStaticField_2_0_1_FULL_VER), h_StaticFieldPointsTo_3_9.iterators(view_StaticFieldPointsTo_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto fld = it_11.value();
          auto positions = it_11.positions();
          auto ch_LoadStaticField_2_fld = h_LoadStaticField_2_8.child_range(positions[0], fld, tile, view_LoadStaticField_2_0_1_FULL_VER);
          auto ch_StaticFieldPointsTo_3_fld = h_StaticFieldPointsTo_3_9.child_range(positions[1], fld, tile, view_StaticFieldPointsTo_1_0_FULL_VER);
...
JitRunner_VPT_LoadStatic_D0::LaunchParams JitRunner_VPT_LoadStatic_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 3: StaticFieldPointsTo version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<StaticFieldPointsTo, FULL_VER>(db);
...
void JitRunner_VPT_LoadStatic_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadStatic_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadStatic_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadStatic_D0::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadStatic_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadStatic_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadStatic_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadStatic_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadStatic_D0");
...
void JitRunner_VPT_LoadStatic_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
