...
struct JitRunner_VPT_LoadStatic_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto intersect_10 = intersect_handles(tile, h_LoadStaticField_2_8.iterators(view_LoadStaticField_0_2_1_FULL_VER), h_Reachable_3_9.iterators(view_Reachable_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto inmeth = it_11.value();
          auto positions = it_11.positions();
          auto ch_LoadStaticField_2_inmeth = h_LoadStaticField_2_8.child_range(positions[0], inmeth, tile, view_LoadStaticField_0_2_1_FULL_VER);
...
JitRunner_VPT_LoadStatic_D2::LaunchParams JitRunner_VPT_LoadStatic_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: StaticFieldPointsTo version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<StaticFieldPointsTo, FULL_VER>(db) : get_relation_by_schema<StaticFieldPointsTo, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<StaticFieldPointsTo, FULL_VER>(db) : get_relation_by_schema<StaticFieldPointsTo, DELTA_VER>(db);
...
void JitRunner_VPT_LoadStatic_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VPT_LoadStatic_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadStatic_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VPT_LoadStatic_D2::read_total(LaunchParams& p) {
...
void JitRunner_VPT_LoadStatic_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadStatic_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VPT_LoadStatic_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_VPT_LoadStatic_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VPT_LoadStatic_D2");
...
void JitRunner_VPT_LoadStatic_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
