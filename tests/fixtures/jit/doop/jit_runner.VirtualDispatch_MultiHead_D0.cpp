// =============================================================
// JIT-Generated Complete Runner: VirtualDispatch_MultiHead_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_VirtualDispatch_MultiHead_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = DoopRules_DB_DeviceDB;
  using FirstSchema = ReachableInstruction;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_HeapHelper_0_1_4_2_3_FULL_VER = views[3];

        // Root ColumnJoin (multi-source intersection): bind 'invocation' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var invocation :sources ((ReachableInstruction :handle 0) (VirtualMethodInvocation :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_ReachableInstruction_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_ReachableInstruction_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_ReachableInstruction_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ReachableInstruction_0_DELTA_VER.num_rows_;
          auto h_ReachableInstruction_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_ReachableInstruction_0_DELTA_VER);
          if (!h_ReachableInstruction_0_root.valid()) continue;
          auto h_VirtualMethodInvocation_1_root = HandleType(0, view_VirtualMethodInvocation_0_1_2_3_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_VirtualMethodInvocation_0_1_2_3_FULL_VER);
        ...
        for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
          ...
          auto ch_VarPointsTo_3_base = h_VarPointsTo_3_19.child_range(positions[1], base, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'simplename' from 2 sources
        // MIR: (column-join :var simplename :sources ((VirtualMethodInvocation :handle 4 :prefix (invocation base)) (HeapHelper :handle 5 :prefix ()) ))
        auto h_VirtualMethodInvocation_4_14 = ch_VirtualMethodInvocation_2_base;
        auto h_HeapHelper_5_15 = HandleType(0, view_HeapHelper_0_1_4_2_3_FULL_VER.num_rows_, 0);
        auto intersect_16 = intersect_handles(tile, h_VirtualMethodInvocation_4_14.iterators(view_VirtualMethodInvocation_0_1_2_3_FULL_VER), h_HeapHelper_5_15.iterators(view_HeapHelper_0_1_4_2_3_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto simplename = it_17.value();
          auto positions = it_17.positions();
          auto ch_VirtualMethodInvocation_4_simplename = h_VirtualMethodInvocation_4_14.child_range(positions[0], simplename, tile, view_VirtualMethodInvocation_0_1_2_3_FULL_VER);
          auto ch_HeapHelper_5_simplename = h_HeapHelper_5_15.child_range(positions[1], simplename, tile, view_HeapHelper_0_1_4_2_3_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'descriptor' from 2 sources
        // MIR: (column-join :var descriptor :sources ((VirtualMethodInvocation :handle 6 :prefix (invocation base simplename)) (HeapHelper :handle 7 :prefix (simplename)) ))
        auto h_VirtualMethodInvocation_6_10 = ch_VirtualMethodInvocation_4_simplename;
        auto h_HeapHelper_7_11 = ch_HeapHelper_5_simplename;
        auto intersect_12 = intersect_handles(tile, h_VirtualMethodInvocation_6_10.iterators(view_VirtualMethodInvocation_0_1_2_3_FULL_VER), h_HeapHelper_7_11.iterators(view_HeapHelper_0_1_4_2_3_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto descriptor = it_13.value();
          auto positions = it_13.positions();
          auto ch_VirtualMethodInvocation_6_descriptor = h_VirtualMethodInvocation_6_10.child_range(positions[0], descriptor, tile, view_VirtualMethodInvocation_0_1_2_3_FULL_VER);
          auto ch_HeapHelper_7_descriptor = h_HeapHelper_7_11.child_range(positions[1], descriptor, tile, view_HeapHelper_0_1_4_2_3_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'heap' from 2 sources
        // MIR: (column-join :var heap :sources ((HeapHelper :handle 8 :prefix (simplename descriptor)) (VarPointsTo :handle 9 :prefix (base)) ))
        auto h_HeapHelper_8_6 = ch_HeapHelper_7_descriptor;
        auto h_VarPointsTo_9_7 = ch_VarPointsTo_3_base;
        auto intersect_8 = intersect_handles(tile, h_HeapHelper_8_6.iterators(view_HeapHelper_0_1_4_2_3_FULL_VER), h_VarPointsTo_9_7.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto heap = it_9.value();
          auto positions = it_9.positions();
          auto ch_HeapHelper_8_heap = h_HeapHelper_8_6.child_range(positions[0], heap, tile, view_HeapHelper_0_1_4_2_3_FULL_VER);
          auto ch_VarPointsTo_9_heap = h_VarPointsTo_9_7.child_range(positions[1], heap, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested CartesianJoin: bind toMeth, thisP from 1 source(s)
        // MIR: (cartesian-join :vars (toMeth thisP) :sources ((HeapHelper :handle 10 :prefix (simplename descriptor heap)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_HeapHelper_10_4 = ch_HeapHelper_8_heap;  // reusing narrowed handle

        if (!h_HeapHelper_10_4.valid()) continue;

        uint32_t degree_3 = h_HeapHelper_10_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_2 output_ctx_2{output_data_2, output_prov_2, output_stride_2, old_size_2 + thread_offset};
        ...
        for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
          ...
          auto ch_VarPointsTo_3_base = h_VarPointsTo_3_21.child_range(positions[1], base, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'simplename' from 2 sources
        // MIR: (column-join :var simplename :sources ((VirtualMethodInvocation :handle 4 :prefix (invocation base)) (HeapHelper :handle 5 :prefix ()) ))
        auto h_VirtualMethodInvocation_4_16 = ch_VirtualMethodInvocation_2_base;
        auto h_HeapHelper_5_17 = HandleType(0, view_HeapHelper_0_1_4_2_3_FULL_VER.num_rows_, 0);
        auto intersect_18 = intersect_handles(tile, h_VirtualMethodInvocation_4_16.iterators(view_VirtualMethodInvocation_0_1_2_3_FULL_VER), h_HeapHelper_5_17.iterators(view_HeapHelper_0_1_4_2_3_FULL_VER));
        for (auto it_19 = intersect_18.begin(); it_19.valid(); it_19.next()) {
          auto simplename = it_19.value();
          auto positions = it_19.positions();
          auto ch_VirtualMethodInvocation_4_simplename = h_VirtualMethodInvocation_4_16.child_range(positions[0], simplename, tile, view_VirtualMethodInvocation_0_1_2_3_FULL_VER);
          auto ch_HeapHelper_5_simplename = h_HeapHelper_5_17.child_range(positions[1], simplename, tile, view_HeapHelper_0_1_4_2_3_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'descriptor' from 2 sources
        // MIR: (column-join :var descriptor :sources ((VirtualMethodInvocation :handle 6 :prefix (invocation base simplename)) (HeapHelper :handle 7 :prefix (simplename)) ))
        auto h_VirtualMethodInvocation_6_12 = ch_VirtualMethodInvocation_4_simplename;
        auto h_HeapHelper_7_13 = ch_HeapHelper_5_simplename;
        auto intersect_14 = intersect_handles(tile, h_VirtualMethodInvocation_6_12.iterators(view_VirtualMethodInvocation_0_1_2_3_FULL_VER), h_HeapHelper_7_13.iterators(view_HeapHelper_0_1_4_2_3_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto descriptor = it_15.value();
          auto positions = it_15.positions();
          auto ch_VirtualMethodInvocation_6_descriptor = h_VirtualMethodInvocation_6_12.child_range(positions[0], descriptor, tile, view_VirtualMethodInvocation_0_1_2_3_FULL_VER);
          auto ch_HeapHelper_7_descriptor = h_HeapHelper_7_13.child_range(positions[1], descriptor, tile, view_HeapHelper_0_1_4_2_3_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'heap' from 2 sources
        // MIR: (column-join :var heap :sources ((HeapHelper :handle 8 :prefix (simplename descriptor)) (VarPointsTo :handle 9 :prefix (base)) ))
        auto h_HeapHelper_8_8 = ch_HeapHelper_7_descriptor;
        auto h_VarPointsTo_9_9 = ch_VarPointsTo_3_base;
        auto intersect_10 = intersect_handles(tile, h_HeapHelper_8_8.iterators(view_HeapHelper_0_1_4_2_3_FULL_VER), h_VarPointsTo_9_9.iterators(view_VarPointsTo_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto heap = it_11.value();
          auto positions = it_11.positions();
          auto ch_HeapHelper_8_heap = h_HeapHelper_8_8.child_range(positions[0], heap, tile, view_HeapHelper_0_1_4_2_3_FULL_VER);
          auto ch_VarPointsTo_9_heap = h_VarPointsTo_9_9.child_range(positions[1], heap, tile, view_VarPointsTo_1_0_FULL_VER);
        // Nested CartesianJoin: bind toMeth, thisP from 1 source(s)
        // MIR: (cartesian-join :vars (toMeth thisP) :sources ((HeapHelper :handle 10 :prefix (simplename descriptor heap)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto toMeth = view_HeapHelper_0_1_4_2_3_FULL_VER.get_value(3, h_HeapHelper_10_4.begin() + idx0_7);
          auto thisP = view_HeapHelper_0_1_4_2_3_FULL_VER.get_value(4, h_HeapHelper_10_4.begin() + idx0_7);
...
JitRunner_VirtualDispatch_MultiHead_D0::LaunchParams JitRunner_VirtualDispatch_MultiHead_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ReachableInstruction version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ReachableInstruction, FULL_VER>(db) : get_relation_by_schema<ReachableInstruction, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: VirtualMethodInvocation version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<VirtualMethodInvocation, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1, 2, 3}}, false);
  ...
  // Source 5: HeapHelper version FULL_VER
  {
    auto& rel_5 = get_relation_by_schema<HeapHelper, FULL_VER>(db);
    auto& idx_5 = rel_5.ensure_index(SRDatalog::IndexSpec{{0, 1, 4, 2, 3}}, false);
    p.views_vec.push_back(idx_5.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ReachableInstruction, FULL_VER>(db) : get_relation_by_schema<ReachableInstruction, DELTA_VER>(db);
  auto& first_idx = first_rel.get_index(SRDatalog::IndexSpec{{0}});
...
void JitRunner_VirtualDispatch_MultiHead_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_VirtualDispatch_MultiHead_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VirtualDispatch_MultiHead_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_VirtualDispatch_MultiHead_D0::read_total(LaunchParams& p) {
...
void JitRunner_VirtualDispatch_MultiHead_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_VirtualDispatch_MultiHead_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_VirtualDispatch_MultiHead_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_VirtualDispatch_MultiHead_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("VirtualDispatch_MultiHead_D0");
...
void JitRunner_VirtualDispatch_MultiHead_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
