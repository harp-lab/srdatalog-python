...
JitRunner_isClassType::LaunchParams JitRunner_isClassType::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_isClassType::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_isClassType::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<isClassType_IDB, NEW_VER>(db);
...
void JitRunner_isClassType::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_isClassType::read_total(LaunchParams& p) {
...
void JitRunner_isClassType::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_isClassType::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_isClassType::read_fused_result(LaunchParams& p) {
...
void JitRunner_isClassType::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("isClassType");
...
void JitRunner_isClassType::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
