...
struct JitRunner_isInterfaceType {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Emit: isInterfaceType_IDB(iface)
...
JitRunner_isInterfaceType::LaunchParams JitRunner_isInterfaceType::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_isInterfaceType::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_isInterfaceType::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<isInterfaceType_IDB, NEW_VER>(db);
...
void JitRunner_isInterfaceType::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_isInterfaceType::read_total(LaunchParams& p) {
...
void JitRunner_isInterfaceType::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_isInterfaceType::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_isInterfaceType::read_fused_result(LaunchParams& p) {
...
void JitRunner_isInterfaceType::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("isInterfaceType");
...
void JitRunner_isInterfaceType::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
