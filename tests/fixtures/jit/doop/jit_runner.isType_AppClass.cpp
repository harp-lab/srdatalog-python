// =============================================================
// JIT-Generated Complete Runner: isType_AppClass
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_isType_AppClass {
...
JitRunner_isType_AppClass::LaunchParams JitRunner_isType_AppClass::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_isType_AppClass::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_isType_AppClass::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_isType_AppClass::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_isType_AppClass::read_total(LaunchParams& p) {
...
void JitRunner_isType_AppClass::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_isType_AppClass::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_isType_AppClass::read_fused_result(LaunchParams& p) {
...
void JitRunner_isType_AppClass::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("isType_AppClass");
...
void JitRunner_isType_AppClass::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
