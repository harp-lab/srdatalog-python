...
struct JitRunner_Join3a_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_UInput_0_1_2_FULL_VER = views[2];

    // Root ColumnJoin (multi-source intersection): bind 'y' from 2 sources
    // Uses root_unique_values + prefix() pattern (like TMP)
    // MIR: (column-join :var y :sources ((OutP :handle 0) (OutQ :handle 1) ))
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      hint_hi_4 = (hint_hi_4 <= view_OutP_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_OutP_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_OutP_0_1_DELTA_VER.num_rows_;
      auto h_OutP_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_OutP_0_1_DELTA_VER);
      if (!h_OutP_0_root.valid())
        continue;
      ...
      auto h_UInput_3_13 = HandleType(0, view_UInput_0_1_2_FULL_VER.num_rows_, 0);
      auto intersect_14 = intersect_handles(tile, h_OutP_2_12.iterators(view_OutP_0_1_DELTA_VER),
                                            h_UInput_3_13.iterators(view_UInput_0_1_2_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto ch_UInput_3_w =
            h_UInput_3_13.child_range(positions[1], w, tile, view_UInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'r' from 2 sources
        // MIR: (column-join :var r :sources ((UInput :handle 4 :prefix (w)) (OutQ :handle 5 :prefix
        // (y)) ))
        auto h_UInput_4_8 = ch_UInput_3_w;
        auto h_OutQ_5_9 = h_OutQ_1_root;
        auto intersect_10 =
            intersect_handles(tile, h_UInput_4_8.iterators(view_UInput_0_1_2_FULL_VER),
                              h_OutQ_5_9.iterators(view_OutQ_2_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto r = it_11.value();
          auto positions = it_11.positions();
          auto ch_UInput_4_r =
              h_UInput_4_8.child_range(positions[0], r, tile, view_UInput_0_1_2_FULL_VER);
          auto ch_OutQ_5_r =
              h_OutQ_5_9.child_range(positions[1], r, tile, view_OutQ_2_1_0_FULL_VER);
          // Nested CartesianJoin: bind z, x from 2 source(s)
          // MIR: (cartesian-join :vars (z x) :sources ((UInput :handle 6 :prefix (w r)) (OutQ
          // :handle 7 :prefix (y r)) ))
          uint32_t lane_1 = tile.thread_rank();
          uint32_t group_size_2 = tile.size();

          auto h_UInput_6_4 = ch_UInput_4_r;  // reusing narrowed handle
          auto h_OutQ_7_6 = ch_OutQ_5_r;      // reusing narrowed handle

          if (!h_UInput_6_4.valid() || !h_OutQ_7_6.valid())
            continue;

          uint32_t degree_3 = h_UInput_6_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      // MIR: (column-join :var w :sources ((OutP :handle 2 :prefix (y)) (UInput :handle 3 :prefix
      // ()) ))
      auto h_OutP_2_16 = h_OutP_0_root;
      auto h_UInput_3_17 = HandleType(0, view_UInput_0_1_2_FULL_VER.num_rows_, 0);
      auto intersect_18 = intersect_handles(tile, h_OutP_2_16.iterators(view_OutP_0_1_DELTA_VER),
                                            h_UInput_3_17.iterators(view_UInput_0_1_2_FULL_VER));
      for (auto it_19 = intersect_18.begin(); it_19.valid(); it_19.next()) {
        auto w = it_19.value();
        auto positions = it_19.positions();
        auto ch_OutP_2_w = h_OutP_2_16.child_range(positions[0], w, tile, view_OutP_0_1_DELTA_VER);
        auto ch_UInput_3_w =
            h_UInput_3_17.child_range(positions[1], w, tile, view_UInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'r' from 2 sources
        // MIR: (column-join :var r :sources ((UInput :handle 4 :prefix (w)) (OutQ :handle 5 :prefix
        // (y)) ))
        auto h_UInput_4_12 = ch_UInput_3_w;
        auto h_OutQ_5_13 = h_OutQ_1_root;
        auto intersect_14 =
            intersect_handles(tile, h_UInput_4_12.iterators(view_UInput_0_1_2_FULL_VER),
                              h_OutQ_5_13.iterators(view_OutQ_2_1_0_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto r = it_15.value();
          auto positions = it_15.positions();
          auto ch_UInput_4_r =
              h_UInput_4_12.child_range(positions[0], r, tile, view_UInput_0_1_2_FULL_VER);
          auto ch_OutQ_5_r =
              h_OutQ_5_13.child_range(positions[1], r, tile, view_OutQ_2_1_0_FULL_VER);
          // Nested CartesianJoin: bind z, x from 2 source(s)
          // MIR: (cartesian-join :vars (z x) :sources ((UInput :handle 6 :prefix (w r)) (OutQ
          // :handle 7 :prefix (y r)) ))
          uint32_t lane_1 = tile.thread_rank();
          ...
          for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
            ...
            auto z = view_UInput_0_1_2_FULL_VER.get_value(2, h_UInput_6_4.begin() + idx0_9);
            auto x = view_OutQ_2_1_0_FULL_VER.get_value(2, h_OutQ_7_6.begin() + idx1_10);
...
JitRunner_Join3a_D0::LaunchParams JitRunner_Join3a_D0::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: OutP version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<OutP, FULL_VER>(db)
                                   : get_relation_by_schema<OutP, DELTA_VER>(db);
  ...
  // Source 3: UInput version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<UInput, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<OutP, FULL_VER>(db)
                                     : get_relation_by_schema<OutP, DELTA_VER>(db);
...
void JitRunner_Join3a_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_Join3a_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<OutP, NEW_VER>(db);
...
void JitRunner_Join3a_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_Join3a_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_Join3a_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
...
void JitRunner_Join3a_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_Join3a_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Join3a_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Join3a_D0");
...
void JitRunner_Join3a_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
