...
struct JitRunner_Join3a_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto intersect_14 = intersect_handles(tile, h_OutP_2_12.iterators(view_OutP_0_1_DELTA_VER), h_UInput_3_13.iterators(view_UInput_0_1_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          ...
          auto ch_UInput_3_w = h_UInput_3_13.child_range(positions[1], w, tile, view_UInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'r' from 2 sources
        // MIR: (column-join :var r :sources ((UInput :handle 4 :prefix (w)) (OutQ :handle 5 :prefix (y)) ))
        auto h_UInput_4_8 = ch_UInput_3_w;
        auto h_OutQ_5_9 = h_OutQ_1_root;
        auto intersect_10 = intersect_handles(tile, h_UInput_4_8.iterators(view_UInput_0_1_2_FULL_VER), h_OutQ_5_9.iterators(view_OutQ_2_1_0_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto r = it_11.value();
          auto positions = it_11.positions();
          auto ch_UInput_4_r = h_UInput_4_8.child_range(positions[0], r, tile, view_UInput_0_1_2_FULL_VER);
          auto ch_OutQ_5_r = h_OutQ_5_9.child_range(positions[1], r, tile, view_OutQ_2_1_0_FULL_VER);
        // Nested CartesianJoin: bind z, x from 2 source(s)
        // MIR: (cartesian-join :vars (z x) :sources ((UInput :handle 6 :prefix (w r)) (OutQ :handle 7 :prefix (y r)) ))
        ...
        if (!h_UInput_6_4.valid() || !h_OutQ_7_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto intersect_18 = intersect_handles(tile, h_OutP_2_16.iterators(view_OutP_0_1_DELTA_VER), h_UInput_3_17.iterators(view_UInput_0_1_2_FULL_VER));
        ...
        auto intersect_14 = intersect_handles(tile, h_UInput_4_12.iterators(view_UInput_0_1_2_FULL_VER), h_OutQ_5_13.iterators(view_OutQ_2_1_0_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto r = it_15.value();
          auto positions = it_15.positions();
          auto ch_UInput_4_r = h_UInput_4_12.child_range(positions[0], r, tile, view_UInput_0_1_2_FULL_VER);
          auto ch_OutQ_5_r = h_OutQ_5_13.child_range(positions[1], r, tile, view_OutQ_2_1_0_FULL_VER);
        // Nested CartesianJoin: bind z, x from 2 source(s)
        // MIR: (cartesian-join :vars (z x) :sources ((UInput :handle 6 :prefix (w r)) (OutQ :handle 7 :prefix (y r)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
...
JitRunner_Join3a_D0::LaunchParams JitRunner_Join3a_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: OutP version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<OutP, FULL_VER>(db) : get_relation_by_schema<OutP, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<OutP, FULL_VER>(db) : get_relation_by_schema<OutP, DELTA_VER>(db);
...
void JitRunner_Join3a_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
