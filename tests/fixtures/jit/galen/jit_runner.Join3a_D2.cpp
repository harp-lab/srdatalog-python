// =============================================================
// JIT-Generated Complete Runner: Join3a_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Join3a_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      // ()) ))
      auto h_OutP_2_12 = h_OutP_1_root;
      auto h_UInput_3_13 = HandleType(0, view_UInput_0_1_2_FULL_VER.num_rows_, 0);
      auto intersect_14 = intersect_handles(tile, h_OutP_2_12.iterators(view_OutP_0_1_FULL_VER),
      ...
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto ch_OutP_2_w = h_OutP_2_12.child_range(positions[0], w, tile, view_OutP_0_1_FULL_VER);
        auto ch_UInput_3_w =
        ...
        // MIR: (column-join :var r :sources ((OutQ :handle 4 :prefix (y)) (UInput :handle 5 :prefix
        // (w)) ))
        auto h_OutQ_4_8 = h_OutQ_0_root;
        auto h_UInput_5_9 = ch_UInput_3_w;
        auto intersect_10 = intersect_handles(tile, h_OutQ_4_8.iterators(view_OutQ_2_1_0_DELTA_VER),
                                              h_UInput_5_9.iterators(view_UInput_0_1_2_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto r = it_11.value();
          auto positions = it_11.positions();
          auto ch_OutQ_4_r =
              h_OutQ_4_8.child_range(positions[0], r, tile, view_OutQ_2_1_0_DELTA_VER);
          auto ch_UInput_5_r =
              h_UInput_5_9.child_range(positions[1], r, tile, view_UInput_0_1_2_FULL_VER);
          // Nested CartesianJoin: bind x, z from 2 source(s)
          // MIR: (cartesian-join :vars (x z) :sources ((OutQ :handle 6 :prefix (y r)) (UInput
          // :handle 7 :prefix (w r)) ))
          ...
          if (!h_OutQ_6_4.valid() || !h_UInput_7_6.valid())
            continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_18 = intersect_handles(tile, h_OutP_2_16.iterators(view_OutP_0_1_FULL_VER),
      ...
      for (auto it_19 = intersect_18.begin(); it_19.valid(); it_19.next()) {
        ...
        auto intersect_14 =
            intersect_handles(tile, h_OutQ_4_12.iterators(view_OutQ_2_1_0_DELTA_VER),
                              h_UInput_5_13.iterators(view_UInput_0_1_2_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto r = it_15.value();
          auto positions = it_15.positions();
          auto ch_OutQ_4_r =
              h_OutQ_4_12.child_range(positions[0], r, tile, view_OutQ_2_1_0_DELTA_VER);
          auto ch_UInput_5_r =
              h_UInput_5_13.child_range(positions[1], r, tile, view_UInput_0_1_2_FULL_VER);
          // Nested CartesianJoin: bind x, z from 2 source(s)
          // MIR: (cartesian-join :vars (x z) :sources ((OutQ :handle 6 :prefix (y r)) (UInput
          // :handle 7 :prefix (w r)) ))
          uint32_t lane_1 = tile.thread_rank();
...
JitRunner_Join3a_D2::LaunchParams JitRunner_Join3a_D2::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Join3a_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_Join3a_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Join3a_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_Join3a_D2::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_Join3a_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
...
void JitRunner_Join3a_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_Join3a_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_Join3a_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Join3a_D2");
...
void JitRunner_Join3a_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
