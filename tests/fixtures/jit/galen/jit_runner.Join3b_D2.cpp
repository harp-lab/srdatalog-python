// =============================================================
// JIT-Generated Complete Runner: Join3b_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Join3b_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_CInput_0_1_2_FULL_VER = views[2];
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_OutP_0_root.valid())
        continue;
      auto h_OutP_1_root = HandleType(0, view_OutP_0_1_FULL_VER.num_rows_, 0)
                               .prefix(root_val_2, tile, view_OutP_0_1_FULL_VER);
      if (!h_OutP_1_root.valid())
        continue;
      auto x = root_val_2;
      // Nested ColumnJoin (intersection): bind 'y' from 2 sources
      // MIR: (column-join :var y :sources ((OutP :handle 2 :prefix (x)) (CInput :handle 3 :prefix
      // ()) ))
      auto h_OutP_2_10 = h_OutP_0_root;
      auto h_CInput_3_11 = HandleType(0, view_CInput_0_1_2_FULL_VER.num_rows_, 0);
      auto intersect_12 = intersect_handles(tile, h_OutP_2_10.iterators(view_OutP_0_1_DELTA_VER),
                                            h_CInput_3_11.iterators(view_CInput_0_1_2_FULL_VER));
      for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
        auto y = it_13.value();
        auto positions = it_13.positions();
        auto ch_OutP_2_y = h_OutP_2_10.child_range(positions[0], y, tile, view_OutP_0_1_DELTA_VER);
        auto ch_CInput_3_y =
            h_CInput_3_11.child_range(positions[1], y, tile, view_CInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'w' from 2 sources
        // MIR: (column-join :var w :sources ((CInput :handle 4 :prefix (y)) (OutP :handle 5 :prefix
        // (x)) ))
        auto h_CInput_4_6 = ch_CInput_3_y;
        auto h_OutP_5_7 = h_OutP_1_root;
        auto intersect_8 =
            intersect_handles(tile, h_CInput_4_6.iterators(view_CInput_0_1_2_FULL_VER),
                              h_OutP_5_7.iterators(view_OutP_0_1_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto w = it_9.value();
          auto positions = it_9.positions();
          auto ch_CInput_4_w =
              h_CInput_4_6.child_range(positions[0], w, tile, view_CInput_0_1_2_FULL_VER);
          auto ch_OutP_5_w = h_OutP_5_7.child_range(positions[1], w, tile, view_OutP_0_1_FULL_VER);
          // Nested CartesianJoin: bind z from 1 source(s)
          // MIR: (cartesian-join :vars (z) :sources ((CInput :handle 6 :prefix (y w)) ))
          uint32_t lane_1 = tile.thread_rank();
          uint32_t group_size_2 = tile.size();

          auto h_CInput_6_4 = ch_CInput_4_w;  // reusing narrowed handle

          if (!h_CInput_6_4.valid())
            continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto h_CInput_3_13 = HandleType(0, view_CInput_0_1_2_FULL_VER.num_rows_, 0);
      auto intersect_14 = intersect_handles(tile, h_OutP_2_12.iterators(view_OutP_0_1_DELTA_VER),
                                            h_CInput_3_13.iterators(view_CInput_0_1_2_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto y = it_15.value();
        auto positions = it_15.positions();
        auto ch_OutP_2_y = h_OutP_2_12.child_range(positions[0], y, tile, view_OutP_0_1_DELTA_VER);
        auto ch_CInput_3_y =
            h_CInput_3_13.child_range(positions[1], y, tile, view_CInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'w' from 2 sources
        // MIR: (column-join :var w :sources ((CInput :handle 4 :prefix (y)) (OutP :handle 5 :prefix
        // (x)) ))
        auto h_CInput_4_8 = ch_CInput_3_y;
        auto h_OutP_5_9 = h_OutP_1_root;
        auto intersect_10 =
            intersect_handles(tile, h_CInput_4_8.iterators(view_CInput_0_1_2_FULL_VER),
                              h_OutP_5_9.iterators(view_OutP_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto w = it_11.value();
          auto positions = it_11.positions();
          auto ch_CInput_4_w =
              h_CInput_4_8.child_range(positions[0], w, tile, view_CInput_0_1_2_FULL_VER);
          auto ch_OutP_5_w = h_OutP_5_9.child_range(positions[1], w, tile, view_OutP_0_1_FULL_VER);
          // Nested CartesianJoin: bind z from 1 source(s)
          // MIR: (cartesian-join :vars (z) :sources ((CInput :handle 6 :prefix (y w)) ))
          uint32_t lane_1 = tile.thread_rank();
          uint32_t group_size_2 = tile.size();
          ...
          for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
            ...
            auto z = view_CInput_0_1_2_FULL_VER.get_value(2, h_CInput_6_4.begin() + idx0_7);
...
JitRunner_Join3b_D2::LaunchParams JitRunner_Join3b_D2::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: OutP version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<OutP, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: CInput version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<CInput, FULL_VER>(db);
...
void JitRunner_Join3b_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_Join3b_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Join3b_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_Join3b_D2::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_Join3b_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
...
void JitRunner_Join3b_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_Join3b_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_Join3b_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Join3b_D2");
...
void JitRunner_Join3b_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
