// =============================================================
// JIT-Generated Complete Runner: Join3c_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Join3c_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_RInput_0_1_2_FULL_VER = views[2];
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_OutQ_1_root = HandleType(0, view_OutQ_0_1_2_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_OutQ_0_1_2_FULL_VER);
        ...
        auto h_RInput_3_15 = HandleType(0, view_RInput_0_1_2_FULL_VER.num_rows_, 0);
        auto intersect_16 = intersect_handles(tile, h_OutQ_2_14.iterators(view_OutQ_2_1_0_DELTA_VER), h_RInput_3_15.iterators(view_RInput_0_1_2_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto y = it_17.value();
          auto positions = it_17.positions();
          auto ch_OutQ_2_y = h_OutQ_2_14.child_range(positions[0], y, tile, view_OutQ_2_1_0_DELTA_VER);
          auto ch_RInput_3_y = h_RInput_3_15.child_range(positions[1], y, tile, view_RInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'u' from 2 sources
        // MIR: (column-join :var u :sources ((RInput :handle 4 :prefix (y)) (OutQ :handle 5 :prefix (z)) ))
        auto h_RInput_4_10 = ch_RInput_3_y;
        auto h_OutQ_5_11 = h_OutQ_1_root;
        auto intersect_12 = intersect_handles(tile, h_RInput_4_10.iterators(view_RInput_0_1_2_FULL_VER), h_OutQ_5_11.iterators(view_OutQ_0_1_2_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto u = it_13.value();
          auto positions = it_13.positions();
          auto ch_RInput_4_u = h_RInput_4_10.child_range(positions[0], u, tile, view_RInput_0_1_2_FULL_VER);
          auto ch_OutQ_5_u = h_OutQ_5_11.child_range(positions[1], u, tile, view_OutQ_0_1_2_FULL_VER);
        // Nested CartesianJoin: bind x, e, o from 3 source(s)
        // MIR: (cartesian-join :vars (x e o) :sources ((OutQ :handle 6 :prefix (z y)) (RInput :handle 7 :prefix (y u)) (OutQ :handle 8 :prefix (z u)) ))
        ...
        if (!h_OutQ_6_4.valid() || !h_RInput_7_6.valid() || !h_OutQ_8_8.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto intersect_20 = intersect_handles(tile, h_OutQ_2_18.iterators(view_OutQ_2_1_0_DELTA_VER), h_RInput_3_19.iterators(view_RInput_0_1_2_FULL_VER));
        for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
          auto y = it_21.value();
          auto positions = it_21.positions();
          auto ch_OutQ_2_y = h_OutQ_2_18.child_range(positions[0], y, tile, view_OutQ_2_1_0_DELTA_VER);
          auto ch_RInput_3_y = h_RInput_3_19.child_range(positions[1], y, tile, view_RInput_0_1_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'u' from 2 sources
        // MIR: (column-join :var u :sources ((RInput :handle 4 :prefix (y)) (OutQ :handle 5 :prefix (z)) ))
        auto h_RInput_4_14 = ch_RInput_3_y;
        auto h_OutQ_5_15 = h_OutQ_1_root;
        auto intersect_16 = intersect_handles(tile, h_RInput_4_14.iterators(view_RInput_0_1_2_FULL_VER), h_OutQ_5_15.iterators(view_OutQ_0_1_2_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto u = it_17.value();
          auto positions = it_17.positions();
          auto ch_RInput_4_u = h_RInput_4_14.child_range(positions[0], u, tile, view_RInput_0_1_2_FULL_VER);
          auto ch_OutQ_5_u = h_OutQ_5_15.child_range(positions[1], u, tile, view_OutQ_0_1_2_FULL_VER);
        // Nested CartesianJoin: bind x, e, o from 3 source(s)
        // MIR: (cartesian-join :vars (x e o) :sources ((OutQ :handle 6 :prefix (z y)) (RInput :handle 7 :prefix (y u)) (OutQ :handle 8 :prefix (z u)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
...
JitRunner_Join3c_D0::LaunchParams JitRunner_Join3c_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Join3c_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Join3c_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Join3c_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Join3c_D0::read_total(LaunchParams& p) {
...
void JitRunner_Join3c_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Join3c_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Join3c_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Join3c_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Join3c_D0");
...
void JitRunner_Join3c_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
