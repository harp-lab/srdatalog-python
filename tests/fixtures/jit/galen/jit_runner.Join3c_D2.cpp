...
struct JitRunner_Join3c_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_OutQ_0_1_2_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_OutQ_0_1_2_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_OutQ_0_1_2_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_OutQ_0_1_2_DELTA_VER.num_rows_;
      auto h_OutQ_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_OutQ_0_1_2_DELTA_VER);
      if (!h_OutQ_0_root.valid())
        continue;
      auto h_OutQ_1_root = HandleType(0, view_OutQ_2_1_0_FULL_VER.num_rows_, 0)
                               .prefix(root_val_2, tile, view_OutQ_2_1_0_FULL_VER);
      ...
      auto intersect_16 = intersect_handles(tile, h_OutQ_2_14.iterators(view_OutQ_0_1_2_DELTA_VER),
                                            h_RInput_3_15.iterators(view_RInput_1_0_2_FULL_VER));
      for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
        auto u = it_17.value();
        auto positions = it_17.positions();
        auto ch_OutQ_2_u =
            h_OutQ_2_14.child_range(positions[0], u, tile, view_OutQ_0_1_2_DELTA_VER);
        auto ch_RInput_3_u =
            h_RInput_3_15.child_range(positions[1], u, tile, view_RInput_1_0_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'y' from 2 sources
        // MIR: (column-join :var y :sources ((OutQ :handle 4 :prefix (z)) (RInput :handle 5 :prefix
        // (u)) ))
        auto h_OutQ_4_10 = h_OutQ_1_root;
        auto h_RInput_5_11 = ch_RInput_3_u;
        auto intersect_12 = intersect_handles(tile, h_OutQ_4_10.iterators(view_OutQ_2_1_0_FULL_VER),
                                              h_RInput_5_11.iterators(view_RInput_1_0_2_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto y = it_13.value();
          auto positions = it_13.positions();
          auto ch_OutQ_4_y =
              h_OutQ_4_10.child_range(positions[0], y, tile, view_OutQ_2_1_0_FULL_VER);
          auto ch_RInput_5_y =
              h_RInput_5_11.child_range(positions[1], y, tile, view_RInput_1_0_2_FULL_VER);
          // Nested CartesianJoin: bind o, x, e from 3 source(s)
          // MIR: (cartesian-join :vars (o x e) :sources ((OutQ :handle 6 :prefix (z u)) (OutQ
          // :handle 7 :prefix (z y)) (RInput :handle 8 :prefix (u y)) ))
          ...
          if (!h_OutQ_6_4.valid() || !h_OutQ_7_6.valid() || !h_RInput_8_8.valid())
            continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_20 = intersect_handles(tile, h_OutQ_2_18.iterators(view_OutQ_0_1_2_DELTA_VER),
                                            h_RInput_3_19.iterators(view_RInput_1_0_2_FULL_VER));
      for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
        auto u = it_21.value();
        auto positions = it_21.positions();
        auto ch_OutQ_2_u =
            h_OutQ_2_18.child_range(positions[0], u, tile, view_OutQ_0_1_2_DELTA_VER);
        auto ch_RInput_3_u =
            h_RInput_3_19.child_range(positions[1], u, tile, view_RInput_1_0_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'y' from 2 sources
        // MIR: (column-join :var y :sources ((OutQ :handle 4 :prefix (z)) (RInput :handle 5 :prefix
        // (u)) ))
        auto h_OutQ_4_14 = h_OutQ_1_root;
        auto h_RInput_5_15 = ch_RInput_3_u;
        auto intersect_16 = intersect_handles(tile, h_OutQ_4_14.iterators(view_OutQ_2_1_0_FULL_VER),
                                              h_RInput_5_15.iterators(view_RInput_1_0_2_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto y = it_17.value();
          auto positions = it_17.positions();
          auto ch_OutQ_4_y =
              h_OutQ_4_14.child_range(positions[0], y, tile, view_OutQ_2_1_0_FULL_VER);
          auto ch_RInput_5_y =
              h_RInput_5_15.child_range(positions[1], y, tile, view_RInput_1_0_2_FULL_VER);
          // Nested CartesianJoin: bind o, x, e from 3 source(s)
          // MIR: (cartesian-join :vars (o x e) :sources ((OutQ :handle 6 :prefix (z u)) (OutQ
          // :handle 7 :prefix (z y)) (RInput :handle 8 :prefix (u y)) ))
          uint32_t lane_1 = tile.thread_rank();
...
JitRunner_Join3c_D2::LaunchParams JitRunner_Join3c_D2::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<OutQ, FULL_VER>(db)
                                   : get_relation_by_schema<OutQ, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<OutQ, FULL_VER>(db)
                                     : get_relation_by_schema<OutQ, DELTA_VER>(db);
...
void JitRunner_Join3c_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
