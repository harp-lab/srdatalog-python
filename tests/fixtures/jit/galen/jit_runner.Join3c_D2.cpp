// =============================================================
// JIT-Generated Complete Runner: Join3c_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Join3c_D2 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = GalenRules_DB_DeviceDB;
  using FirstSchema = OutQ;
  using DestSchema = OutQ;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_OutQ_0_1_2_DELTA_VER = views[0];
        auto view_OutQ_2_1_0_FULL_VER = views[1];
        auto view_RInput_1_0_2_FULL_VER = views[2];
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_OutQ_0_1_2_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_OutQ_0_1_2_DELTA_VER.num_rows_) ? hint_hi_4 : view_OutQ_0_1_2_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_OutQ_0_1_2_DELTA_VER.num_rows_;
          auto h_OutQ_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_OutQ_0_1_2_DELTA_VER);
          if (!h_OutQ_0_root.valid()) continue;
          auto h_OutQ_1_root = HandleType(0, view_OutQ_2_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_OutQ_2_1_0_FULL_VER);
          if (!h_OutQ_1_root.valid()) continue;
          auto z = root_val_2;
        // Nested ColumnJoin (intersection): bind 'u' from 2 sources
        // MIR: (column-join :var u :sources ((OutQ :handle 2 :prefix (z)) (RInput :handle 3 :prefix ()) ))
        auto h_OutQ_2_14 = h_OutQ_0_root;
        auto h_RInput_3_15 = HandleType(0, view_RInput_1_0_2_FULL_VER.num_rows_, 0);
        auto intersect_16 = intersect_handles(tile, h_OutQ_2_14.iterators(view_OutQ_0_1_2_DELTA_VER), h_RInput_3_15.iterators(view_RInput_1_0_2_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto u = it_17.value();
          auto positions = it_17.positions();
          auto ch_OutQ_2_u = h_OutQ_2_14.child_range(positions[0], u, tile, view_OutQ_0_1_2_DELTA_VER);
          auto ch_RInput_3_u = h_RInput_3_15.child_range(positions[1], u, tile, view_RInput_1_0_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'y' from 2 sources
        // MIR: (column-join :var y :sources ((OutQ :handle 4 :prefix (z)) (RInput :handle 5 :prefix (u)) ))
        auto h_OutQ_4_10 = h_OutQ_1_root;
        auto h_RInput_5_11 = ch_RInput_3_u;
        auto intersect_12 = intersect_handles(tile, h_OutQ_4_10.iterators(view_OutQ_2_1_0_FULL_VER), h_RInput_5_11.iterators(view_RInput_1_0_2_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto y = it_13.value();
          auto positions = it_13.positions();
          auto ch_OutQ_4_y = h_OutQ_4_10.child_range(positions[0], y, tile, view_OutQ_2_1_0_FULL_VER);
          auto ch_RInput_5_y = h_RInput_5_11.child_range(positions[1], y, tile, view_RInput_1_0_2_FULL_VER);
        // Nested CartesianJoin: bind o, x, e from 3 source(s)
        // MIR: (cartesian-join :vars (o x e) :sources ((OutQ :handle 6 :prefix (z u)) (OutQ :handle 7 :prefix (z y)) (RInput :handle 8 :prefix (u y)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_OutQ_6_4 = ch_OutQ_2_u;  // reusing narrowed handle
        auto h_OutQ_7_6 = ch_OutQ_4_y;  // reusing narrowed handle
        auto h_RInput_8_8 = ch_RInput_5_y;  // reusing narrowed handle

        if (!h_OutQ_6_4.valid() || !h_OutQ_7_6.valid() || !h_RInput_8_8.valid()) continue;

        uint32_t degree_3 = h_OutQ_6_4.degree();
        uint32_t degree_5 = h_OutQ_7_6.degree();
        uint32_t degree_7 = h_RInput_8_8.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_RInput_3_19 = HandleType(0, view_RInput_1_0_2_FULL_VER.num_rows_, 0);
        auto intersect_20 = intersect_handles(tile, h_OutQ_2_18.iterators(view_OutQ_0_1_2_DELTA_VER), h_RInput_3_19.iterators(view_RInput_1_0_2_FULL_VER));
        for (auto it_21 = intersect_20.begin(); it_21.valid(); it_21.next()) {
          auto u = it_21.value();
          auto positions = it_21.positions();
          auto ch_OutQ_2_u = h_OutQ_2_18.child_range(positions[0], u, tile, view_OutQ_0_1_2_DELTA_VER);
          auto ch_RInput_3_u = h_RInput_3_19.child_range(positions[1], u, tile, view_RInput_1_0_2_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'y' from 2 sources
        // MIR: (column-join :var y :sources ((OutQ :handle 4 :prefix (z)) (RInput :handle 5 :prefix (u)) ))
        auto h_OutQ_4_14 = h_OutQ_1_root;
        auto h_RInput_5_15 = ch_RInput_3_u;
        auto intersect_16 = intersect_handles(tile, h_OutQ_4_14.iterators(view_OutQ_2_1_0_FULL_VER), h_RInput_5_15.iterators(view_RInput_1_0_2_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto y = it_17.value();
          auto positions = it_17.positions();
          auto ch_OutQ_4_y = h_OutQ_4_14.child_range(positions[0], y, tile, view_OutQ_2_1_0_FULL_VER);
          auto ch_RInput_5_y = h_RInput_5_15.child_range(positions[1], y, tile, view_RInput_1_0_2_FULL_VER);
        // Nested CartesianJoin: bind o, x, e from 3 source(s)
        // MIR: (cartesian-join :vars (o x e) :sources ((OutQ :handle 6 :prefix (z u)) (OutQ :handle 7 :prefix (z y)) (RInput :handle 8 :prefix (u y)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_10 = lane_1; flat_idx_10 < total_9; flat_idx_10 += group_size_2) {
          ...
          auto o = view_OutQ_0_1_2_DELTA_VER.get_value(2, h_OutQ_6_4.begin() + idx0_11);
          auto x = view_OutQ_2_1_0_FULL_VER.get_value(2, h_OutQ_7_6.begin() + idx1_12);
          auto e = view_RInput_1_0_2_FULL_VER.get_value(2, h_RInput_8_8.begin() + idx2_13);

        // Emit: OutQ(x, e, o)
        output_ctx_0.emit_direct(x, e, o);
        }
        }
        }
        }
...
JitRunner_Join3c_D2::LaunchParams JitRunner_Join3c_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: OutQ version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<OutQ, FULL_VER>(db) : get_relation_by_schema<OutQ, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: OutQ version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<OutQ, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{2, 1, 0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: RInput version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<RInput, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{1, 0, 2}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<OutQ, FULL_VER>(db) : get_relation_by_schema<OutQ, DELTA_VER>(db);
...
void JitRunner_Join3c_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Join3c_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<OutQ, NEW_VER>(db);
...
void JitRunner_Join3c_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Join3c_D2::read_total(LaunchParams& p) {
...
void JitRunner_Join3c_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Join3c_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Join3c_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_Join3c_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Join3c_D2");
...
void JitRunner_Join3c_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
