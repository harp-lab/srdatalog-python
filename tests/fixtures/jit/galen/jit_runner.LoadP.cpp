...
struct JitRunner_LoadP {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_PInput_0_1_FULL_VER = views[0];

        // Root Scan: PInput binding x, z
        // MIR: (scan :rel PInput :vars (x z) :handle 0)
        auto root_handle_1 = HandleType(0, view_PInput_0_1_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto x = view_PInput_0_1_FULL_VER.get_value(0, idx_3);
          auto z = view_PInput_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: OutP(x, z)
...
JitRunner_LoadP::LaunchParams JitRunner_LoadP::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: PInput version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<PInput, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<PInput, FULL_VER>(db);
...
void JitRunner_LoadP::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LoadP::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LoadP::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LoadP::read_total(LaunchParams& p) {
...
void JitRunner_LoadP::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LoadP::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LoadP::read_fused_result(LaunchParams& p) {
...
void JitRunner_LoadP::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LoadP");
...
void JitRunner_LoadP::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
