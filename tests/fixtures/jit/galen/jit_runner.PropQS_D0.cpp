...
struct JitRunner_PropQS_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_OutQ_1_0_2_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_OutQ_1_0_2_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_OutQ_1_0_2_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_OutQ_1_0_2_DELTA_VER.num_rows_;
      auto h_OutQ_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_OutQ_1_0_2_DELTA_VER);
      if (!h_OutQ_0_root.valid())
        continue;
      auto h_SInput_1_root = HandleType(0, view_SInput_0_1_FULL_VER.num_rows_, 0)
                                 .prefix(root_val_2, tile, view_SInput_0_1_FULL_VER);
      if (!h_SInput_1_root.valid())
        continue;
      auto r = root_val_2;
      // Nested CartesianJoin: bind x, z, q from 2 source(s)
      // MIR: (cartesian-join :vars (x z q) :sources ((OutQ :handle 2 :prefix (r)) (SInput :handle 3
      // :prefix (r)) ))
      ...
      if (!h_OutQ_2_4.valid() || !h_SInput_3_6.valid())
        continue;
...
JitRunner_PropQS_D0::LaunchParams JitRunner_PropQS_D0::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_PropQS_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
