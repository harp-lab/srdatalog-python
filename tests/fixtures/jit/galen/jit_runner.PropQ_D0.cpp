// =============================================================
// JIT-Generated Complete Runner: PropQ_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_PropQ_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_OutP_1_0_DELTA_VER = views[0];
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_OutP_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_OutP_1_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_OutP_1_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_OutP_1_0_DELTA_VER.num_rows_;
          auto h_OutP_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_OutP_1_0_DELTA_VER);
          ...
          auto y = root_val_2;
        // Nested CartesianJoin: bind x, r, z from 2 source(s)
        // MIR: (cartesian-join :vars (x r z) :sources ((OutP :handle 2 :prefix (y)) (OutQ :handle 3 :prefix (y)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_OutP_2_4 = h_OutP_0_root;  // reusing narrowed handle
        auto h_OutQ_3_6 = h_OutQ_1_root;  // reusing narrowed handle

        if (!h_OutP_2_4.valid() || !h_OutQ_3_6.valid()) continue;

        uint32_t degree_3 = h_OutP_2_4.degree();
        uint32_t degree_5 = h_OutQ_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto x = view_OutP_1_0_DELTA_VER.get_value(1, h_OutP_2_4.begin() + idx0_9);
          auto r = view_OutQ_0_1_2_FULL_VER.get_value(1, h_OutQ_3_6.begin() + idx1_10);
          auto z = view_OutQ_0_1_2_FULL_VER.get_value(2, h_OutQ_3_6.begin() + idx1_10);

        // Emit: OutQ(x, r, z)
        output_ctx_0.emit_direct(x, r, z);
        }
        }
  }
...
JitRunner_PropQ_D0::LaunchParams JitRunner_PropQ_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_PropQ_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_PropQ_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_PropQ_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_PropQ_D0::read_total(LaunchParams& p) {
...
void JitRunner_PropQ_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_PropQ_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_PropQ_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_PropQ_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("PropQ_D0");
...
void JitRunner_PropQ_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
