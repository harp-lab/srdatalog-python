...
struct JitRunner_PropQ_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      hint_hi_4 = (hint_hi_4 <= view_OutP_1_0_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_OutP_1_0_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_OutP_1_0_DELTA_VER.num_rows_;
      auto h_OutP_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_OutP_1_0_DELTA_VER);
      ...
      // MIR: (cartesian-join :vars (x r z) :sources ((OutP :handle 2 :prefix (y)) (OutQ :handle 3
      // :prefix (y)) ))
      ...
      if (!h_OutP_2_4.valid() || !h_OutQ_3_6.valid())
        continue;
...
JitRunner_PropQ_D0::LaunchParams JitRunner_PropQ_D0::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_PropQ_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
