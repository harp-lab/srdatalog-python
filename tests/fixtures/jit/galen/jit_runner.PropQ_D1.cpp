  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_OutQ_0_root.valid())
        continue;
      auto h_OutP_1_root = HandleType(0, view_OutP_1_0_FULL_VER.num_rows_, 0)
                               .prefix(root_val_2, tile, view_OutP_1_0_FULL_VER);
      if (!h_OutP_1_root.valid())
        continue;
      auto y = root_val_2;
      // Nested CartesianJoin: bind r, z, x from 2 source(s)
      // MIR: (cartesian-join :vars (r z x) :sources ((OutQ :handle 2 :prefix (y)) (OutP :handle 3
      // :prefix (y)) ))
      ...
      if (!h_OutQ_2_4.valid() || !h_OutP_3_6.valid())
        continue;
...
JitRunner_PropQ_D1::LaunchParams JitRunner_PropQ_D1::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_PropQ_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
