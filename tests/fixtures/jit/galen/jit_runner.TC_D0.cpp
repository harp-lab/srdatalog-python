...
struct JitRunner_TC_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (cartesian-join :vars (x z) :sources ((OutP :handle 2 :prefix (y)) (OutP :handle 3 :prefix (y)) ))
        ...
        if (!h_OutP_2_4.valid() || !h_OutP_3_6.valid()) continue;
...
JitRunner_TC_D0::LaunchParams JitRunner_TC_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_TC_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
