...
struct JitRunner_TC_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (cartesian-join :vars (z x) :sources ((OutP :handle 2 :prefix (y)) (OutP :handle 3 :prefix (y)) ))
...
JitRunner_TC_D1::LaunchParams JitRunner_TC_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_TC_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
