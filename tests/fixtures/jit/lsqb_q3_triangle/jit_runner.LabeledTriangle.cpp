// =============================================================
// JIT-Generated Complete Runner: LabeledTriangle
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_LabeledTriangle {
...
JitRunner_LabeledTriangle::LaunchParams JitRunner_LabeledTriangle::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  ...
  // Source 1: IsLocatedIn version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<IsLocatedIn, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 2: Knows version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<Knows, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_2.view());
  }

  // Source 10: IsPartOf version FULL_VER
  {
    auto& rel_10 = get_relation_by_schema<IsPartOf, FULL_VER>(db);
    auto& idx_10 = rel_10.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
...
void JitRunner_LabeledTriangle::read_fused_result(LaunchParams& p) {
...
void JitRunner_LabeledTriangle::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LabeledTriangle");
