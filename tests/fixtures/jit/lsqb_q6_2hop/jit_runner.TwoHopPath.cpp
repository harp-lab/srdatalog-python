...
struct JitRunner_TwoHopPath {
  ...
  using DB = LSQB_Q6_Rules_DB_DeviceDB;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_HasInterest_0_1_FULL_VER = views[1];
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto h_Knows_4_4 = h_Knows_1_root;             // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      // MIR: (column-join :var p3 :sources ((Knows :handle 2 :prefix (p2)) (HasInterest :handle 3
      // :prefix ()) ))
      auto h_Knows_2_20 = h_Knows_1_root;
      auto h_HasInterest_3_21 = HandleType(0, view_HasInterest_0_1_FULL_VER.num_rows_, 0);
      auto intersect_22 =
          intersect_handles(tile, h_Knows_2_20.iterators(view_Knows_0_1_FULL_VER),
                            h_HasInterest_3_21.iterators(view_HasInterest_0_1_FULL_VER));
      for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
        auto p3 = it_23.value();
        auto positions = it_23.positions();
        auto ch_Knows_2_p3 =
            h_Knows_2_20.child_range(positions[0], p3, tile, view_Knows_0_1_FULL_VER);
        auto ch_HasInterest_3_p3 =
            h_HasInterest_3_21.child_range(positions[1], p3, tile, view_HasInterest_0_1_FULL_VER);
        // Nested CartesianJoin: bind p1, t from 2 source(s)
        // MIR: (cartesian-join :vars (p1 t) :sources ((Knows :handle 4 :prefix (p2)) (HasInterest
        // :handle 5 :prefix (p3)) ))
        ...
        auto h_Knows_4_5 = h_Knows_1_root;             // reusing narrowed handle
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] =
                  view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_HasInterest_0_1_FULL_VER.get_value(
                    1, h_HasInterest_5_7.begin() + t1_base_11 + _ti);
              tile.sync();
                ...
                // Emit: Path(p1, p2, p3, t)
                {
                  ...
                  if (_tc_active > 0) {
                    ...
                    if (tc_valid_1) {
                      ...
                      output_data_0[0 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = p1;
                      output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = p2;
                      output_data_0[2 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = p3;
                      output_data_0[3 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = t;
                    }
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto p1 = view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_5.begin() + idx0_18);
            auto t =
                view_HasInterest_0_1_FULL_VER.get_value(1, h_HasInterest_5_7.begin() + idx1_19);
            tc_valid_1 = tc_valid_1 && (p1 != p3);
            // Emit: Path(p1, p2, p3, t)
            {
              uint32_t _tc_ballot = tile.ballot(tc_valid_1);
...
void JitRunner_TwoHopPath::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_TwoHopPath::read_fused_result(LaunchParams& p) {
...
void JitRunner_TwoHopPath::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
