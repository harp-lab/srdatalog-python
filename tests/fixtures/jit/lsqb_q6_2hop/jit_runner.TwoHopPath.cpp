...
struct JitRunner_TwoHopPath {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto intersect_22 = intersect_handles(tile, h_Knows_2_20.iterators(view_Knows_0_1_FULL_VER), h_HasInterest_3_21.iterators(view_HasInterest_0_1_FULL_VER));
        for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
          auto p3 = it_23.value();
          auto positions = it_23.positions();
          auto ch_Knows_2_p3 = h_Knows_2_20.child_range(positions[0], p3, tile, view_Knows_0_1_FULL_VER);
          auto ch_HasInterest_3_p3 = h_HasInterest_3_21.child_range(positions[1], p3, tile, view_HasInterest_0_1_FULL_VER);
        // Nested CartesianJoin: bind p1, t from 2 source(s)
        // MIR: (cartesian-join :vars (p1 t) :sources ((Knows :handle 4 :prefix (p2)) (HasInterest :handle 5 :prefix (p3)) ))
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_HasInterest_0_1_FULL_VER.get_value(1, h_HasInterest_5_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto p1 = tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
        ...
        // Emit: Path(p1, p2, p3, t)
        {
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto t = view_HasInterest_0_1_FULL_VER.get_value(1, h_HasInterest_5_7.begin() + idx1_19);
        tc_valid_1 = tc_valid_1 && (p1 != p3);
        // Emit: Path(p1, p2, p3, t)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
