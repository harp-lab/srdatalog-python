// =============================================================
// JIT-Generated Complete Runner: TwoHopBG
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_TwoHopBG {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = LSQB_Q6_BG_Rules_DB_DeviceDB;
  using FirstSchema = Knows;
  using DestSchema = PathBG;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys;
      ...
      uint32_t bg_hhi =
          view_Knows_1_0_FULL_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_Knows_1_0_FULL_VER.num_rows_) ? bg_hhi
                                                             : view_Knows_1_0_FULL_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_Knows_1_0_FULL_VER.num_rows_;
      auto h_Knows_0_root =
          HandleType(bg_hlo, bg_hhi, 0).prefix(bg_hist_root_val, tile, view_Knows_1_0_FULL_VER);
      if (!h_Knows_0_root.valid()) {
        bg_work_per_key[bg_hist_key] = 0;
        continue;
      }
      auto h_Knows2_1_root = HandleType(0, view_Knows2_0_1_FULL_VER.num_rows_, 0)
                                 .prefix(bg_hist_root_val, tile, view_Knows2_0_1_FULL_VER);
      if (!h_Knows2_1_root.valid()) {
        bg_work_per_key[bg_hist_key] = 0;
        continue;
      }
      uint64_t bg_deg = 1;
      bg_deg *= h_Knows_0_root.degree();
      bg_deg *= h_Knows2_1_root.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // Root ColumnJoin (BLOCK-GROUP): bind 'p2' from 2 sources
    ...
    for (uint32_t bg_key_idx_1 = bg_key_lo;
      ...
      uint32_t hint_hi_4 =
          view_Knows_1_0_FULL_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_Knows_1_0_FULL_VER.num_rows_)
                      ? hint_hi_4
                      : view_Knows_1_0_FULL_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Knows_1_0_FULL_VER.num_rows_;
      ...
      uint32_t bg_deg_first = (uint32_t)(h_Knows_0_root.end() - h_Knows_0_root.begin());
      ...
      {
        auto bg_narrow_begin = h_Knows_0_root.begin() + bg_warp_row_begin;
        auto bg_narrow_end = h_Knows_0_root.begin() + bg_warp_row_end;
        h_Knows_0_root = HandleType(bg_narrow_begin, bg_narrow_end, h_Knows_0_root.depth());
      }
...
JitRunner_TwoHopBG::LaunchParams JitRunner_TwoHopBG::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_TwoHopBG::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_TwoHopBG::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<PathBG, NEW_VER>(db);
...
void JitRunner_TwoHopBG::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_TwoHopBG::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_TwoHopBG::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
...
void JitRunner_TwoHopBG::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_TwoHopBG::read_fused_result(LaunchParams& p) {
...
void JitRunner_TwoHopBG::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("TwoHopBG");
  ...
  auto& bg_dest_0 = get_relation_by_schema<PathBG, NEW_VER>(db);
...
void JitRunner_TwoHopBG::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
