...
struct JitRunner_KnowsLoadRev {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_KnowsInput_0_1_FULL_VER = views[0];

        // Root Scan: KnowsInput binding x, y
        // MIR: (scan :rel KnowsInput :vars (x y) :handle 0)
        auto root_handle_1 = HandleType(0, view_KnowsInput_0_1_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto x = view_KnowsInput_0_1_FULL_VER.get_value(0, idx_3);
          auto y = view_KnowsInput_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: Knows(y, x)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          ...
          auto y = view_KnowsInput_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: Knows(y, x)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(y, x);
        }
  }
...
JitRunner_KnowsLoadRev::LaunchParams JitRunner_KnowsLoadRev::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: KnowsInput version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<KnowsInput, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<KnowsInput, FULL_VER>(db);
...
void JitRunner_KnowsLoadRev::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_KnowsLoadRev::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Knows, NEW_VER>(db);
...
void JitRunner_KnowsLoadRev::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_KnowsLoadRev::read_total(LaunchParams& p) {
...
void JitRunner_KnowsLoadRev::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_KnowsLoadRev::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_KnowsLoadRev::read_fused_result(LaunchParams& p) {
...
void JitRunner_KnowsLoadRev::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("KnowsLoadRev");
...
void JitRunner_KnowsLoadRev::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
