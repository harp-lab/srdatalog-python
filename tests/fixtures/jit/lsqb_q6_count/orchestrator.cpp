...
static void step_0(DB& db, std::size_t max_iterations) {
  ...
  if (_tail_mode) {
    ...
    // === ParallelGroup (stream-parallel, 3 rules, shared dests) ===
    {
      ...
      thrust::exclusive_scan(rmm::exec_policy(_stream_pool.get(0)), shared_buf_Knows.data(),
                             shared_buf_Knows.data() + shared_buf_Knows_size,
                             shared_buf_Knows.data(), 0, thrust::plus<uint32_t>());
      ...
      GPU_MEMCPY_ASYNC(&bnd_Knows[0], shared_buf_Knows.data() + off_Knows_0, sizeof(uint32_t),
                       GPU_DEVICE_TO_HOST, 0);
      GPU_MEMCPY_ASYNC(&bnd_Knows[1], shared_buf_Knows.data() + off_Knows_1, sizeof(uint32_t),
                       GPU_DEVICE_TO_HOST, 0);
      GPU_MEMCPY_ASYNC(&bnd_Knows[2], shared_buf_Knows.data() + shared_buf_Knows_size - 1,
      ...
      if (total_1 > 0)
        JitRunner_KnowsLoadRev::launch_materialize(db, p_1, total_1, _stream_pool.get(1));
      if (total_2 > 0)
