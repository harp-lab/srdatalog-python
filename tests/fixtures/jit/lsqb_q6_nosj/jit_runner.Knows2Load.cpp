...
JitRunner_Knows2Load::LaunchParams JitRunner_Knows2Load::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: KnowsInput2 version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<KnowsInput2, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<KnowsInput2, FULL_VER>(db);
...
void JitRunner_Knows2Load::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Knows2Load::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Knows2, NEW_VER>(db);
...
void JitRunner_Knows2Load::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Knows2Load::read_total(LaunchParams& p) {
...
void JitRunner_Knows2Load::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Knows2Load::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Knows2Load::read_fused_result(LaunchParams& p) {
...
void JitRunner_Knows2Load::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Knows2Load");
...
void JitRunner_Knows2Load::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
