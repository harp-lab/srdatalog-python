// =============================================================
// JIT-Generated Complete Runner: TwoHopPath
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_TwoHopPath {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = LSQB_Q6_NoSJ_Rules_DB_DeviceDB;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto p1 = view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_4.begin() + idx0_9);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (p1 != p3) {
        // Emit: Path(p1, p2, p3, t)
        output_ctx_0.emit_direct(p1, p2, p3, t);
        }
        }
        }
        }
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count_ws(
      const ViewType* __restrict__ views,
      const ValueType* __restrict__ root_unique_values,
      uint32_t num_unique_root_keys,
      uint32_t num_root_keys,
    ...
    // Phase 1: Main traversal with inline WS splitting (count mode)
        ...
        if (h_Knows2_2_12.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
          uint32_t _ws_total_deg = h_Knows2_2_12.degree();
          ...
          if (_ws_K > 0) {
            ...
            for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
              ...
              uint32_t _ws_cb = h_Knows2_2_12.begin() + (_ws_ti + 1) * ws::WCOJ_SPLIT_THRESHOLD;
              uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
              if (_ws_ce > h_Knows2_2_12.end()) _ws_ce = h_Knows2_2_12.end();
              ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
              task.split_level = 2;
              task.num_handles = 4;
              task.handle_begin[0] = _ws_cb;
              task.handle_end[0] = _ws_ce;
              task.handle_depth[0] = h_Knows2_2_12.depth();
              task.handle_view_slot[0] = 1;
              task.handle_begin[1] = h_HasInterest_3_13.begin();
              task.handle_end[1] = h_HasInterest_3_13.end();
              task.handle_depth[1] = h_HasInterest_3_13.depth();
              task.handle_view_slot[1] = 2;
              task.handle_begin[2] = h_Knows_0_root.begin();
              task.handle_end[2] = h_Knows_0_root.end();
              task.handle_depth[2] = h_Knows_0_root.depth();
              task.handle_view_slot[2] = 0;
              task.handle_begin[3] = h_Knows2_1_root.begin();
              task.handle_end[3] = h_Knows2_1_root.end();
              task.handle_depth[3] = h_Knows2_1_root.depth();
              task.handle_view_slot[3] = 1;
              task.split_handle_idx = 0;
              task.split_depth = h_Knows2_2_12.depth();
              task.num_bound_vars = 0;
              wcoj_queue->ready_flags[_ws_slot].store(1, cuda::memory_order_release);
            }
            h_Knows2_2_12 = HandleType(h_Knows2_2_12.begin(), h_Knows2_2_12.begin() + ws::WCOJ_SPLIT_THRESHOLD, h_Knows2_2_12.depth());
          }
        }
        auto intersect_14 = intersect_handles(tile, h_Knows2_2_12.iterators(view_Knows2_0_1_FULL_VER), h_HasInterest_3_13.iterators(view_HasInterest_0_1_FULL_VER));
        ...
        if (total_7 > ws::CARTESIAN_SPLIT_THRESHOLD) {
          ...
          if (_ws_cart_K > 0) {
            ...
            if (tile.thread_rank() == 0) {
              if (wcoj_queue->tail.load(cuda::memory_order_relaxed) + _ws_cart_K <= wcoj_queue->capacity)
            ...
            for (uint32_t _ws_ci = tile.thread_rank(); _ws_ci < _ws_cart_K; _ws_ci += tile.size()) {
              ...
              task.split_level = 3;
              task.num_handles = 2;
              task.handle_begin[0] = h_Knows_4_4.begin();
              task.handle_end[0] = h_Knows_4_4.end();
              task.handle_depth[0] = h_Knows_4_4.depth();
              task.handle_view_slot[0] = 0;
              task.handle_begin[1] = h_HasInterest_5_6.begin();
              task.handle_end[1] = h_HasInterest_5_6.end();
              task.handle_depth[1] = h_HasInterest_5_6.depth();
              task.handle_view_slot[1] = 2;
              task.split_handle_idx = 0xFF;
              uint32_t _ws_cs = (_ws_ci + 1) * _ws_cart_chunk;
              uint32_t _ws_ce = _ws_cs + _ws_cart_chunk;
              if (_ws_ce > total_7) _ws_ce = total_7;
              task.split_begin = _ws_cs;
              task.split_end = _ws_ce;
              task.split_depth = 0;
              task.num_bound_vars = 2;
              task.bound_vars[0] = p2;
              task.bound_vars[1] = p3;
    ...
    while (true) {
      ws::WCOJTask ws_task;
      bool has_work = false;
      if (tile.thread_rank() == 0) has_work = wcoj_queue->pop(ws_task);
      ...
      if (has_work) {
        ...
        if (ws_task.split_handle_idx == 0) {
          {
        auto h_Knows2_2_2 = HandleType(ws_task.handle_begin[0], ws_task.handle_end[0], ws_task.handle_depth[0]);
        auto h_HasInterest_3_2 = HandleType(ws_task.handle_begin[1], ws_task.handle_end[1], ws_task.handle_depth[1]);
        auto h_Knows_0_root = HandleType(ws_task.handle_begin[2], ws_task.handle_end[2], ws_task.handle_depth[2]);
        auto h_Knows2_1_root = HandleType(ws_task.handle_begin[3], ws_task.handle_end[3], ws_task.handle_depth[3]);
        auto view_Knows2_0_1_FULL_VER = views[ws_task.handle_view_slot[0]];
        auto view_HasInterest_0_1_FULL_VER = views[ws_task.handle_view_slot[1]];
        auto view_Knows_0_1_FULL_VER = views[ws_task.handle_view_slot[2]];
        auto p2 = view_Knows2_0_1_FULL_VER.get_value(0, h_Knows2_2_2.begin());
                // Nested ColumnJoin (intersection): bind 'p3' from 2 sources
                // MIR: (column-join :var p3 :sources ((Knows2 :handle 2 :prefix (p2)) (HasInterest :handle 3 :prefix ()) ))
                auto h_Knows2_2_29 = h_Knows2_2_2;
                auto h_HasInterest_3_30 = h_HasInterest_3_2;
                // WS: bulk-donate CJ chunks to task queue
                if (h_Knows2_2_29.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
                  uint32_t _ws_total_deg = h_Knows2_2_29.degree();
                  uint32_t _ws_K = (_ws_total_deg - 1) / ws::WCOJ_SPLIT_THRESHOLD;
                  if (_ws_K > 0) {
                    uint32_t _ws_base_slot = 0;
                    if (tile.thread_rank() == 0) {
                    ...
                    for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
                      uint32_t _ws_slot = _ws_base_slot + _ws_ti;
                      uint32_t _ws_cb = h_Knows2_2_29.begin() + (_ws_ti + 1) * ws::WCOJ_SPLIT_THRESHOLD;
                      uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
                      if (_ws_ce > h_Knows2_2_29.end()) _ws_ce = h_Knows2_2_29.end();
                      ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
                      task.split_level = 2;
                      task.num_handles = 4;
                      task.handle_begin[0] = _ws_cb;
                      task.handle_end[0] = _ws_ce;
                      task.handle_depth[0] = h_Knows2_2_29.depth();
                      task.handle_view_slot[0] = 1;
                      task.handle_begin[1] = h_HasInterest_3_30.begin();
                      task.handle_end[1] = h_HasInterest_3_30.end();
                      task.handle_depth[1] = h_HasInterest_3_30.depth();
                      task.handle_view_slot[1] = 2;
                      task.handle_begin[2] = h_Knows_0_root.begin();
                      task.handle_end[2] = h_Knows_0_root.end();
                      task.handle_depth[2] = h_Knows_0_root.depth();
                      ...
                      task.split_handle_idx = 0;
                      task.split_depth = h_Knows2_2_29.depth();
                      task.num_bound_vars = 1;
                      task.bound_vars[0] = p2;
                      wcoj_queue->ready_flags[_ws_slot].store(1, cuda::memory_order_release);
                    }
                    h_Knows2_2_29 = HandleType(h_Knows2_2_29.begin(), h_Knows2_2_29.begin() + ws::WCOJ_SPLIT_THRESHOLD, h_Knows2_2_29.depth());
                  }
                }
                auto intersect_31 = intersect_handles(tile, h_Knows2_2_29.iterators(view_Knows2_0_1_FULL_VER), h_HasInterest_3_30.iterators(view_HasInterest_0_1_FULL_VER));
                uint32_t ws_cumulative_tuples = 0;
                constexpr uint32_t WS_CUM_WORK_THRESHOLD = 4096;
                for (auto it_32 = intersect_31.begin(); it_32.valid(); it_32.next()) {
                  auto p3 = it_32.value();
                  auto positions = it_32.positions();
                  auto ch_Knows2_2_p3 = h_Knows2_2_29.child_range(positions[0], p3, tile, view_Knows2_0_1_FULL_VER);
                  auto ch_HasInterest_3_p3 = h_HasInterest_3_30.child_range(positions[1], p3, tile, view_HasInterest_0_1_FULL_VER);
                // Nested CartesianJoin: bind p1, t from 2 source(s)
                // MIR: (cartesian-join :vars (p1 t) :sources ((Knows :handle 4 :prefix (p2)) (HasInterest :handle 5 :prefix (p3)) ))
                uint32_t lane_18 = tile.thread_rank();
                uint32_t group_size_19 = tile.size();

                auto h_Knows_4_21 = h_Knows_0_root;  // reusing narrowed handle
                auto h_HasInterest_5_23 = ch_HasInterest_3_p3;  // reusing narrowed handle

                if (!h_Knows_4_21.valid() || !h_HasInterest_5_23.valid()) continue;

                uint32_t degree_20 = h_Knows_4_21.degree();
                uint32_t degree_22 = h_HasInterest_5_23.degree();
                uint32_t total_24 = degree_20 * degree_22;
                if (total_24 == 0) continue;
                ...
                if (total_24 > ws::CARTESIAN_SPLIT_THRESHOLD) {
                  ...
                  if (_ws_cart_K > 0) {
                    ...
                    for (uint32_t _ws_ci = tile.thread_rank(); _ws_ci < _ws_cart_K; _ws_ci += tile.size()) {
                      ...
                      task.split_level = 3;
                      task.num_handles = 2;
                      task.handle_begin[0] = h_Knows_4_21.begin();
                      task.handle_end[0] = h_Knows_4_21.end();
                      task.handle_depth[0] = h_Knows_4_21.depth();
                      task.handle_view_slot[0] = 0;
                      task.handle_begin[1] = h_HasInterest_5_23.begin();
                      task.handle_end[1] = h_HasInterest_5_23.end();
                      task.handle_depth[1] = h_HasInterest_5_23.depth();
                      task.handle_view_slot[1] = 2;
                      task.split_handle_idx = 0xFF;
                      uint32_t _ws_cs = (_ws_ci + 1) * _ws_cart_chunk;
                      uint32_t _ws_ce = _ws_cs + _ws_cart_chunk;
                      if (_ws_ce > total_24) _ws_ce = total_24;
                      task.split_begin = _ws_cs;
                      task.split_end = _ws_ce;
                      task.split_depth = 0;
                      task.num_bound_vars = 2;
                ...
                for (uint32_t flat_idx_25 = lane_18; flat_idx_25 < total_24; flat_idx_25 += group_size_19) {
                  ...
                  auto p1 = view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_21.begin() + idx0_26);
                  auto t = view_HasInterest_0_1_FULL_VER.get_value(1, h_HasInterest_5_23.begin() + idx1_27);
          ...
          {
        auto ws_ct_h0 = HandleType(ws_task.handle_begin[0], ws_task.handle_end[0], ws_task.handle_depth[0]);
        auto ws_ct_h1 = HandleType(ws_task.handle_begin[1], ws_task.handle_end[1], ws_task.handle_depth[1]);
        auto view_Knows_0_1_FULL_VER = views[ws_task.handle_view_slot[0]];
        auto view_HasInterest_0_1_FULL_VER = views[ws_task.handle_view_slot[1]];
        auto p2 = static_cast<ValueType>(ws_task.bound_vars[0]);
        auto p3 = static_cast<ValueType>(ws_task.bound_vars[1]);
        ...
        if ((ws_ct_flat_end - ws_ct_flat_start) > ws::CARTESIAN_SPLIT_THRESHOLD) {
          ...
          if (tile.thread_rank() == 0) {
            for (uint32_t ws_ct_s = ws_ct_flat_start + ws_ct_chunk; ws_ct_s < ws_ct_flat_end; ws_ct_s += ws_ct_chunk) {
              ws::WCOJTask subtask;
              ...
              if (ws_ct_e > ws_ct_flat_end) ws_ct_e = ws_ct_flat_end;
              ...
              subtask.bound_vars[1] = ws_task.bound_vars[1];
        ...
        for (uint32_t ws_ct_flat = ws_ct_flat_start + ws_ct_lane; ws_ct_flat < ws_ct_flat_end; ws_ct_flat += ws_ct_gs) {
          const bool ws_ct_major = (ws_ct_deg1 >= ws_ct_deg0);
          uint32_t ws_ct_idx0, ws_ct_idx1;
          if (ws_ct_major) { ws_ct_idx0 = ws_ct_flat / ws_ct_deg1; ws_ct_idx1 = ws_ct_flat % ws_ct_deg1; }
          else { ws_ct_idx1 = ws_ct_flat / ws_ct_deg0; ws_ct_idx0 = ws_ct_flat % ws_ct_deg0; }
          auto p1 = view_Knows_0_1_FULL_VER.get_value(1, ws_ct_h0.begin() + ws_ct_idx0);
          auto t = view_HasInterest_0_1_FULL_VER.get_value(1, ws_ct_h1.begin() + ws_ct_idx1);
          if (p1 != p3) {
            local_count++;
          }
        }
        ...
        if (active == 0 && wcoj_queue->size() == 0) break;
      }
    }
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize_ws(
    ...
    SRDatalog::GPU::JIT::WS::ChunkedOutputContext<ValueType, 4, 16> output_ctx_0{output_data_0, global_write_pos, static_cast<uint32_t>(output_stride_0), old_size_0};
    ...
    // Phase 1: Main traversal with inline WS splitting
        ...
        // Nested ColumnJoin (intersection): bind 'p3' from 2 sources
        // MIR: (column-join :var p3 :sources ((Knows2 :handle 2 :prefix (p2)) (HasInterest :handle 3 :prefix ()) ))
        auto h_Knows2_2_14 = h_Knows2_1_root;
        auto h_HasInterest_3_15 = HandleType(0, view_HasInterest_0_1_FULL_VER.num_rows_, 0);
        // WS: bulk-donate CJ chunks to task queue
        if (h_Knows2_2_14.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
          uint32_t _ws_total_deg = h_Knows2_2_14.degree();
          uint32_t _ws_K = (_ws_total_deg - 1) / ws::WCOJ_SPLIT_THRESHOLD;
          if (_ws_K > 0) {
            uint32_t _ws_base_slot = 0;
            if (tile.thread_rank() == 0) {
            ...
            for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
              uint32_t _ws_slot = _ws_base_slot + _ws_ti;
              uint32_t _ws_cb = h_Knows2_2_14.begin() + (_ws_ti + 1) * ws::WCOJ_SPLIT_THRESHOLD;
              uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
              if (_ws_ce > h_Knows2_2_14.end()) _ws_ce = h_Knows2_2_14.end();
              ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
              task.split_level = 2;
              task.num_handles = 4;
              task.handle_begin[0] = _ws_cb;
              task.handle_end[0] = _ws_ce;
              task.handle_depth[0] = h_Knows2_2_14.depth();
              task.handle_view_slot[0] = 1;
              task.handle_begin[1] = h_HasInterest_3_15.begin();
              task.handle_end[1] = h_HasInterest_3_15.end();
              task.handle_depth[1] = h_HasInterest_3_15.depth();
              task.handle_view_slot[1] = 2;
              task.handle_begin[2] = h_Knows_0_root.begin();
              task.handle_end[2] = h_Knows_0_root.end();
              task.handle_depth[2] = h_Knows_0_root.depth();
              ...
              task.handle_view_slot[3] = 1;
              task.split_handle_idx = 0;
              task.split_depth = h_Knows2_2_14.depth();
              task.num_bound_vars = 0;
              wcoj_queue->ready_flags[_ws_slot].store(1, cuda::memory_order_release);
            }
            h_Knows2_2_14 = HandleType(h_Knows2_2_14.begin(), h_Knows2_2_14.begin() + ws::WCOJ_SPLIT_THRESHOLD, h_Knows2_2_14.depth());
          }
        }
        auto intersect_16 = intersect_handles(tile, h_Knows2_2_14.iterators(view_Knows2_0_1_FULL_VER), h_HasInterest_3_15.iterators(view_HasInterest_0_1_FULL_VER));
        uint32_t ws_cumulative_tuples = 0;
        constexpr uint32_t WS_CUM_WORK_THRESHOLD = 4096;
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto p3 = it_17.value();
          auto positions = it_17.positions();
          auto ch_Knows2_2_p3 = h_Knows2_2_14.child_range(positions[0], p3, tile, view_Knows2_0_1_FULL_VER);
          auto ch_HasInterest_3_p3 = h_HasInterest_3_15.child_range(positions[1], p3, tile, view_HasInterest_0_1_FULL_VER);
        // Nested CartesianJoin: bind p1, t from 2 source(s)
        // MIR: (cartesian-join :vars (p1 t) :sources ((Knows :handle 4 :prefix (p2)) (HasInterest :handle 5 :prefix (p3)) ))
        uint32_t lane_2 = tile.thread_rank();
        uint32_t group_size_3 = tile.size();

        auto h_Knows_4_5 = h_Knows_0_root;  // reusing narrowed handle
        auto h_HasInterest_5_7 = ch_HasInterest_3_p3;  // reusing narrowed handle

        if (!h_Knows_4_5.valid() || !h_HasInterest_5_7.valid()) continue;

        uint32_t degree_4 = h_Knows_4_5.degree();
        uint32_t degree_6 = h_HasInterest_5_7.degree();
        ...
        if (total_8 > ws::CARTESIAN_SPLIT_THRESHOLD) {
          ...
          if (_ws_cart_K > 0) {
            ...
            for (uint32_t _ws_ci = tile.thread_rank(); _ws_ci < _ws_cart_K; _ws_ci += tile.size()) {
              ...
              task.split_level = 3;
              task.num_handles = 2;
              task.handle_begin[0] = h_Knows_4_5.begin();
              task.handle_end[0] = h_Knows_4_5.end();
              task.handle_depth[0] = h_Knows_4_5.depth();
              task.handle_view_slot[0] = 0;
              task.handle_begin[1] = h_HasInterest_5_7.begin();
              task.handle_end[1] = h_HasInterest_5_7.end();
              task.handle_depth[1] = h_HasInterest_5_7.depth();
              task.handle_view_slot[1] = 2;
              task.split_handle_idx = 0xFF;
              uint32_t _ws_cs = (_ws_ci + 1) * _ws_cart_chunk;
              uint32_t _ws_ce = _ws_cs + _ws_cart_chunk;
              if (_ws_ce > total_8) _ws_ce = total_8;
              task.split_begin = _ws_cs;
              task.split_end = _ws_ce;
              task.split_depth = 0;
              task.num_bound_vars = 2;
        ...
        for (uint32_t ws_batch_10 = 0; ws_batch_10 < total_8; ws_batch_10 += group_size_3) {
          ...
          auto p1 = ws_valid_1 ? view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_5.begin() + idx0_11) : ValueType{0};
          auto t = ws_valid_1 ? view_HasInterest_0_1_FULL_VER.get_value(1, h_HasInterest_5_7.begin() + idx1_12) : ValueType{0};
        ws_valid_1 = ws_valid_1 && (p1 != p3);
        // Emit: Path(p1, p2, p3, t)
        output_ctx_0.emit_warp_coalesced(tile, ws_valid_1, p1, p2, p3, t);
        }
        }
        }
    if (tile.thread_rank() == 0) {
    ...
    while (true) {
      ...
      if (has_work) {
        ...
        auto p2 = view_Knows2_0_1_FULL_VER.get_value(0, h_Knows2_2_2.begin());
                // Nested ColumnJoin (intersection): bind 'p3' from 2 sources
                // MIR: (column-join :var p3 :sources ((Knows2 :handle 2 :prefix (p2)) (HasInterest :handle 3 :prefix ()) ))
                auto h_Knows2_2_14 = h_Knows2_2_2;
                auto h_HasInterest_3_15 = h_HasInterest_3_2;
                // WS: bulk-donate CJ chunks to task queue
                if (h_Knows2_2_14.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
                  uint32_t _ws_total_deg = h_Knows2_2_14.degree();
                  uint32_t _ws_K = (_ws_total_deg - 1) / ws::WCOJ_SPLIT_THRESHOLD;
...
JitRunner_TwoHopPath::LaunchParams JitRunner_TwoHopPath::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  ...
  // Source 1: Knows2 version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Knows2, FULL_VER>(db);
...
void JitRunner_TwoHopPath::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  ...
  if (p.num_root_keys < kAdaptiveWSThreshold) {
    ...
    if (s_ws_global_count == nullptr) {
      s_ws_global_count = static_cast<uint32_t*>(SRDatalog::GPU::JIT::WS::ws_rmm_alloc(sizeof(uint32_t), stream));
    }
    ...
    if (s_ws_count_queue == nullptr) {
      ...
      SRDatalog::GPU::JIT::WS::reset_task_queue(s_ws_count_queue, SRDatalog::GPU::JIT::WS::WCOJ_QUEUE_CAPACITY, stream);
    }
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_blocks_per_sm, kernel_count_ws, kBlockSize, 0);
    int num_sms = 0;
    cudaDeviceGetAttribute(&num_sms, cudaDevAttrMultiProcessorCount, 0);
    int ws_num_blocks = std::min((int)p.num_blocks, max_blocks_per_sm * num_sms);
    kernel_count_ws<<<ws_num_blocks, kBlockSize, 0, stream>>>(p.d_views.data(), p.root_unique_values_ptr, p.num_unique_root_keys, p.num_root_keys, s_ws_global_count, s_ws_count_queue, s_ws_count_range_board);
...
uint32_t JitRunner_TwoHopPath::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  if (p.used_ws) {
    ...
    GPU_MEMCPY_ASYNC(&total_count, p.thread_counts_ptr, sizeof(uint32_t), GPU_DEVICE_TO_HOST, stream);
...
void JitRunner_TwoHopPath::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
  ...
  if (p.used_ws) {
    ...
    if (s_global_write_pos == nullptr) {
      s_global_write_pos = static_cast<uint32_t*>(SRDatalog::GPU::JIT::WS::ws_rmm_alloc(sizeof(uint32_t), stream));
    }
    ...
    if (s_wcoj_queue == nullptr) {
      ...
      SRDatalog::GPU::JIT::WS::reset_task_queue(s_wcoj_queue, SRDatalog::GPU::JIT::WS::WCOJ_QUEUE_CAPACITY, stream);
    }
    ...
    int max_blocks_per_sm = 0;
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_blocks_per_sm, kernel_materialize_ws, kBlockSize, 0);
    int num_sms = 0;
    cudaDeviceGetAttribute(&num_sms, cudaDevAttrMultiProcessorCount, 0);
    uint32_t ws_num_blocks = std::min(p.num_blocks, static_cast<uint32_t>(max_blocks_per_sm) * static_cast<uint32_t>(num_sms));
...
void JitRunner_TwoHopPath::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("TwoHopPath");
