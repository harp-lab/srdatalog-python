...
struct JitRunner_TwoHopPath {
      ...
      if (h_Knows2_2_12.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
        ...
        if (_ws_K > 0) {
          ...
          for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
            ...
            if (_ws_ce > h_Knows2_2_12.end())
              _ws_ce = h_Knows2_2_12.end();
          ...
          h_Knows2_2_12 =
              HandleType(h_Knows2_2_12.begin(), h_Knows2_2_12.begin() + ws::WCOJ_SPLIT_THRESHOLD,
                         h_Knows2_2_12.depth());
        }
      }
      auto intersect_14 =
        ...
        if (ws_task.split_handle_idx == 0) {
          {
            ...
            // Nested ColumnJoin (intersection): bind 'p3' from 2 sources
            // MIR: (column-join :var p3 :sources ((Knows2 :handle 2 :prefix (p2)) (HasInterest
            // :handle 3 :prefix ()) ))
            ...
            if (h_Knows2_2_29.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
              ...
              if (_ws_K > 0) {
                ...
                for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
                  ...
                  uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
                  if (_ws_ce > h_Knows2_2_29.end())
                    _ws_ce = h_Knows2_2_29.end();
                  ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
                  task.split_level = 2;
                  task.num_handles = 4;
                  task.handle_begin[0] = _ws_cb;
                ...
                h_Knows2_2_29 = HandleType(h_Knows2_2_29.begin(),
                                           h_Knows2_2_29.begin() + ws::WCOJ_SPLIT_THRESHOLD,
                                           h_Knows2_2_29.depth());
              }
            }
            auto intersect_31 =
                intersect_handles(tile, h_Knows2_2_29.iterators(view_Knows2_0_1_FULL_VER),
                                  h_HasInterest_3_30.iterators(view_HasInterest_0_1_FULL_VER));
            uint32_t ws_cumulative_tuples = 0;
            constexpr uint32_t WS_CUM_WORK_THRESHOLD = 4096;
            for (auto it_32 = intersect_31.begin(); it_32.valid(); it_32.next()) {
              ...
              auto ch_Knows2_2_p3 =
                  h_Knows2_2_29.child_range(positions[0], p3, tile, view_Knows2_0_1_FULL_VER);
              auto ch_HasInterest_3_p3 = h_HasInterest_3_30.child_range(
                  positions[1], p3, tile, view_HasInterest_0_1_FULL_VER);
              // Nested CartesianJoin: bind p1, t from 2 source(s)
              // MIR: (cartesian-join :vars (p1 t) :sources ((Knows :handle 4 :prefix (p2))
              // (HasInterest :handle 5 :prefix (p3)) ))
              ...
              if (!h_Knows_4_21.valid() || !h_HasInterest_5_23.valid())
                continue;
              ...
              for (uint32_t flat_idx_25 = lane_18; flat_idx_25 < total_24;
                ...
                auto t = view_HasInterest_0_1_FULL_VER.get_value(
                    1, h_HasInterest_5_23.begin() + idx1_27);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize_ws(
    ...
    SRDatalog::GPU::JIT::WS::ChunkedOutputContext<ValueType, 4, 16> output_ctx_0{
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (h_Knows2_2_14.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
        ...
        if (_ws_K > 0) {
          ...
          for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
            ...
            uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
            if (_ws_ce > h_Knows2_2_14.end())
              _ws_ce = h_Knows2_2_14.end();
            ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
            task.split_level = 2;
            task.num_handles = 4;
            task.handle_begin[0] = _ws_cb;
          ...
          h_Knows2_2_14 =
              HandleType(h_Knows2_2_14.begin(), h_Knows2_2_14.begin() + ws::WCOJ_SPLIT_THRESHOLD,
                         h_Knows2_2_14.depth());
        }
      }
      auto intersect_16 =
          intersect_handles(tile, h_Knows2_2_14.iterators(view_Knows2_0_1_FULL_VER),
      ...
      for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
        ...
        auto ch_Knows2_2_p3 =
            h_Knows2_2_14.child_range(positions[0], p3, tile, view_Knows2_0_1_FULL_VER);
        auto ch_HasInterest_3_p3 =
        ...
        if (!h_Knows_4_5.valid() || !h_HasInterest_5_7.valid())
          continue;
        ...
        for (uint32_t ws_batch_10 = 0; ws_batch_10 < total_8; ws_batch_10 += group_size_3) {
          ...
          auto p1 = ws_valid_1 ? view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_5.begin() + idx0_11)
                               : ValueType{0};
          auto t = ws_valid_1 ? view_HasInterest_0_1_FULL_VER.get_value(
                                    1, h_HasInterest_5_7.begin() + idx1_12)
                              : ValueType{0};
        ...
        if (ws_task.split_handle_idx == 0) {
          {
            ...
            if (h_Knows2_2_14.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
              ...
              if (_ws_K > 0) {
                ...
                h_Knows2_2_14 = HandleType(h_Knows2_2_14.begin(),
                                           h_Knows2_2_14.begin() + ws::WCOJ_SPLIT_THRESHOLD,
                                           h_Knows2_2_14.depth());
              }
            }
            auto intersect_16 =
            ...
            for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
              ...
              auto ch_Knows2_2_p3 =
                  h_Knows2_2_14.child_range(positions[0], p3, tile, view_Knows2_0_1_FULL_VER);
              auto ch_HasInterest_3_p3 = h_HasInterest_3_15.child_range(
                  positions[1], p3, tile, view_HasInterest_0_1_FULL_VER);
              // Nested CartesianJoin: bind p1, t from 2 source(s)
              // MIR: (cartesian-join :vars (p1 t) :sources ((Knows :handle 4 :prefix (p2))
              // (HasInterest :handle 5 :prefix (p3)) ))
              ...
              for (uint32_t ws_batch_10 = 0; ws_batch_10 < total_8; ws_batch_10 += group_size_3) {
                ...
                auto p1 = ws_valid_1
                              ? view_Knows_0_1_FULL_VER.get_value(1, h_Knows_4_5.begin() + idx0_11)
                              : ValueType{0};
                auto t = ws_valid_1 ? view_HasInterest_0_1_FULL_VER.get_value(
                                          1, h_HasInterest_5_7.begin() + idx1_12)
                                    : ValueType{0};
...
void JitRunner_TwoHopPath::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                              GPU_STREAM_T stream) {
  if (total_count == 0)
    return;
