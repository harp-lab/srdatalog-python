...
struct JitRunner_Case1Both {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        {
          uint64_t cap_total = (uint64_t)degree_3 * (uint64_t)degree_5 * (uint64_t)degree_7 * (uint64_t)degree_9;
...
JitRunner_Case1Both::LaunchParams JitRunner_Case1Both::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
