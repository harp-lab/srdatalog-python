...
struct JitRunner_Case1Both {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var m :sources ((HasTag :handle 0) (ReplyOf :handle 1) (Likes :handle 2)
    // (HasCreator :handle 3) )) WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_ReplyOf_1_root.valid())
        continue;
      auto h_Likes_2_root = HandleType(0, view_Likes_1_0_FULL_VER.num_rows_, 0)
                                .prefix(root_val_2, tile, view_Likes_1_0_FULL_VER);
      if (!h_Likes_2_root.valid())
        continue;
      auto h_HasCreator_3_root = HandleType(0, view_HasCreator_0_1_FULL_VER.num_rows_, 0)
                                     .prefix(root_val_2, tile, view_HasCreator_0_1_FULL_VER);
      if (!h_HasCreator_3_root.valid())
        continue;
      auto m = root_val_2;
      // Nested CartesianJoin: bind comment, liker, t, c from 4 source(s)
      // MIR: (cartesian-join :vars (comment liker t c) :sources ((HasTag :handle 4 :prefix (m))
      // (ReplyOf :handle 5 :prefix (m)) (Likes :handle 6 :prefix (m)) (HasCreator :handle 7 :prefix
      // (m)) ))
      ...
      if (!h_HasTag_4_4.valid() || !h_ReplyOf_5_6.valid() || !h_Likes_6_8.valid() ||
          !h_HasCreator_7_10.valid())
        continue;
      ...
      {
        uint64_t cap_total =
            (uint64_t)degree_3 * (uint64_t)degree_5 * (uint64_t)degree_7 * (uint64_t)degree_9;
...
JitRunner_Case1Both::LaunchParams JitRunner_Case1Both::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 2: Likes version FULL_VER
  {
    auto& rel_2 = get_relation_by_schema<Likes, FULL_VER>(db);
    auto& idx_2 = rel_2.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_2.view());
  }

  // Source 3: HasCreator version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<HasCreator, FULL_VER>(db);
...
void JitRunner_Case1Both::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_Case1Both::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Case1, NEW_VER>(db);
...
void JitRunner_Case1Both::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_Case1Both::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_Case1Both::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Case1Both");
