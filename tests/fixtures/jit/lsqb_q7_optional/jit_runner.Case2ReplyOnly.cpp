...
struct JitRunner_Case2ReplyOnly {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var m :sources ((HasTag :handle 0) (ReplyOf :handle 1) (HasCreator :handle 2) ))
        ...
        // MIR: (cartesian-join :vars (comment t c) :sources ((HasTag :handle 3 :prefix (m)) (ReplyOf :handle 4 :prefix (m)) (HasCreator :handle 5 :prefix (m)) ))
        ...
        if (!h_HasTag_3_5.valid() || !h_ReplyOf_4_7.valid() || !h_HasCreator_5_9.valid()) continue;
        ...
        auto h_HasLiker_neg_pre_1 = HandleType(0, view_HasLiker_0_FULL_VER.num_rows_, 0).prefix(m, tile, view_HasLiker_0_FULL_VER);
...
JitRunner_Case2ReplyOnly::LaunchParams JitRunner_Case2ReplyOnly::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
