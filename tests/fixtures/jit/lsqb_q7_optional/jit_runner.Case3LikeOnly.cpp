...
struct JitRunner_Case3LikeOnly {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var m :sources ((HasTag :handle 0) (Likes :handle 1) (HasCreator :handle
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_HasTag_0_root.valid())
        continue;
      auto h_Likes_1_root = HandleType(0, view_Likes_1_0_FULL_VER.num_rows_, 0)
                                .prefix(root_val_2, tile, view_Likes_1_0_FULL_VER);
      if (!h_Likes_1_root.valid())
        continue;
      ...
      // MIR: (cartesian-join :vars (liker t c) :sources ((HasTag :handle 3 :prefix (m)) (Likes
      ...
      if (!h_HasTag_3_5.valid() || !h_Likes_4_7.valid() || !h_HasCreator_5_9.valid())
        continue;
...
JitRunner_Case3LikeOnly::LaunchParams JitRunner_Case3LikeOnly::setup(DB& db, uint32_t iteration,
                                                                     GPU_STREAM_T stream) {
  LaunchParams p;
