...
struct JitRunner_Case3LikeOnly {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Likes_1_0_FULL_VER = views[1];
        auto view_HasCreator_0_1_FULL_VER = views[2];
        auto view_HasReply_0_FULL_VER = views[3];

        // Root ColumnJoin (multi-source intersection): bind 'm' from 3 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var m :sources ((HasTag :handle 0) (Likes :handle 1) (HasCreator :handle 2) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_Likes_1_root = HandleType(0, view_Likes_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Likes_1_0_FULL_VER);
          if (!h_Likes_1_root.valid()) continue;
          auto h_HasCreator_2_root = HandleType(0, view_HasCreator_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_HasCreator_0_1_FULL_VER);
          if (!h_HasCreator_2_root.valid()) continue;
          auto m = root_val_2;
        // Nested CartesianJoin: bind liker, t, c from 3 source(s)
        // MIR: (cartesian-join :vars (liker t c) :sources ((HasTag :handle 3 :prefix (m)) (Likes :handle 4 :prefix (m)) (HasCreator :handle 5 :prefix (m)) ))
        uint32_t lane_2 = tile.thread_rank();
        uint32_t group_size_3 = tile.size();

        auto h_HasTag_3_5 = h_HasTag_0_root;  // reusing narrowed handle
        auto h_Likes_4_7 = h_Likes_1_root;  // reusing narrowed handle
        auto h_HasCreator_5_9 = h_HasCreator_2_root;  // reusing narrowed handle

        if (!h_HasTag_3_5.valid() || !h_Likes_4_7.valid() || !h_HasCreator_5_9.valid()) continue;

        uint32_t degree_4 = h_HasTag_3_5.degree();
        uint32_t degree_6 = h_Likes_4_7.degree();
        ...
        auto h_HasReply_neg_pre_1 = HandleType(0, view_HasReply_0_FULL_VER.num_rows_, 0).prefix(m, tile, view_HasReply_0_FULL_VER);
...
JitRunner_Case3LikeOnly::LaunchParams JitRunner_Case3LikeOnly::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: Likes version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Likes, FULL_VER>(db);
  ...
  // Source 6: HasReply version FULL_VER
  {
    auto& rel_6 = get_relation_by_schema<HasReply, FULL_VER>(db);
...
void JitRunner_Case3LikeOnly::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Case3LikeOnly::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Case3, NEW_VER>(db);
...
void JitRunner_Case3LikeOnly::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Case3LikeOnly::read_total(LaunchParams& p) {
...
void JitRunner_Case3LikeOnly::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Case3LikeOnly");
