...
struct JitRunner_NegPath {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto intersect_16 = intersect_handles(tile, h_Knows_2_14.iterators(view_Knows_0_1_FULL_VER), h_HasInterest_3_15.iterators(view_HasInterest_0_1_FULL_VER));
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto p3 = it_17.value();
          auto positions = it_17.positions();
          auto ch_Knows_2_p3 = h_Knows_2_14.child_range(positions[0], p3, tile, view_Knows_0_1_FULL_VER);
        ...
        if (!h_Knows_4_6.valid() || !h_HasInterest_5_8.valid()) continue;
        ...
        auto h_Knows_neg_pre_1 = HandleType(0, view_Knows_1_0_FULL_VER.num_rows_, 0).prefix(p3, tile, view_Knows_1_0_FULL_VER);
...
JitRunner_NegPath::LaunchParams JitRunner_NegPath::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
