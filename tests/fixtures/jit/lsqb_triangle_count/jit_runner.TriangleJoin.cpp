...
struct JitRunner_TriangleJoin {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_7 = intersect_handles(tile, h_Knows_2_5.iterators(view_Knows_0_1_FULL_VER),
                                           h_Knows_3_6.iterators(view_Knows_0_1_FULL_VER));
      for (auto it_8 = intersect_7.begin(); it_8.valid(); it_8.next()) {
        ...
        // MIR: (column-join :var c :sources ((Knows :handle 4 :prefix (b)) (Knows :handle 5 :prefix
        // (a)) ))
        auto h_Knows_4_1 = ch_Knows_3_b;
        auto h_Knows_5_2 = h_Knows_1_root;
        auto intersect_3 = intersect_handles(tile, h_Knows_4_1.iterators(view_Knows_0_1_FULL_VER),
                                             h_Knows_5_2.iterators(view_Knows_0_1_FULL_VER));
        for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
          auto c = it_4.value();
          auto positions = it_4.positions();
          auto ch_Knows_4_c =
              h_Knows_4_1.child_range(positions[0], c, tile, view_Knows_0_1_FULL_VER);
          auto ch_Knows_5_c =
              h_Knows_5_2.child_range(positions[1], c, tile, view_Knows_0_1_FULL_VER);
          // Emit: Triangle(a, b, c)
...
JitRunner_TriangleJoin::LaunchParams JitRunner_TriangleJoin::setup(DB& db, uint32_t iteration,
                                                                   GPU_STREAM_T stream) {
  LaunchParams p;
