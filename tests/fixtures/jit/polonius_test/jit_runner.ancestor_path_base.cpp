...
struct JitRunner_ancestor_path_base {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_child_path_0_1_FULL_VER = views[0];

        // Root Scan: child_path binding x, y
        // MIR: (scan :rel child_path :vars (x y) :handle 0)
        auto root_handle_1 = HandleType(0, view_child_path_0_1_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto x = view_child_path_0_1_FULL_VER.get_value(0, idx_3);
          auto y = view_child_path_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: ancestor_path(x, y)
...
JitRunner_ancestor_path_base::LaunchParams JitRunner_ancestor_path_base::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: child_path version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<child_path, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<child_path, FULL_VER>(db);
...
void JitRunner_ancestor_path_base::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_ancestor_path_base::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ancestor_path_base::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_ancestor_path_base::read_total(LaunchParams& p) {
...
void JitRunner_ancestor_path_base::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_ancestor_path_base::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ancestor_path_base::read_fused_result(LaunchParams& p) {
...
void JitRunner_ancestor_path_base::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("ancestor_path_base");
...
void JitRunner_ancestor_path_base::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
