// =============================================================
// JIT-Generated Complete Runner: ancestor_path_trans_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_ancestor_path_trans_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = PoloniusRules_DB_DeviceDB;
  using FirstSchema = ancestor_path;
  using DestSchema = ancestor_path;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_ancestor_path_0_1_DELTA_VER = views[0];
    auto view_child_path_0_1_FULL_VER = views[1];

    // Root ColumnJoin (multi-source intersection): bind 'parent' from 2 sources
    // Uses root_unique_values + prefix() pattern (like TMP)
    // MIR: (column-join :var parent :sources ((ancestor_path :handle 0) (child_path :handle 1) ))
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_ancestor_path_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_ancestor_path_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_ancestor_path_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ancestor_path_0_1_DELTA_VER.num_rows_;
      auto h_ancestor_path_0_root = HandleType(hint_lo_3, hint_hi_4, 0)
                                        .prefix(root_val_2, tile, view_ancestor_path_0_1_DELTA_VER);
      if (!h_ancestor_path_0_root.valid())
        continue;
      auto h_child_path_1_root = HandleType(0, view_child_path_0_1_FULL_VER.num_rows_, 0)
                                     .prefix(root_val_2, tile, view_child_path_0_1_FULL_VER);
      if (!h_child_path_1_root.valid())
        continue;
      auto parent = root_val_2;
      // Nested CartesianJoin: bind child, grandparent from 2 source(s)
      // MIR: (cartesian-join :vars (child grandparent) :sources ((ancestor_path :handle 2 :prefix
      // (parent)) (child_path :handle 3 :prefix (parent)) ))
      uint32_t lane_1 = tile.thread_rank();
      uint32_t group_size_2 = tile.size();

      auto h_ancestor_path_2_4 = h_ancestor_path_0_root;  // reusing narrowed handle
      auto h_child_path_3_6 = h_child_path_1_root;        // reusing narrowed handle

      if (!h_ancestor_path_2_4.valid() || !h_child_path_3_6.valid())
        continue;

      uint32_t degree_3 = h_ancestor_path_2_4.degree();
      uint32_t degree_5 = h_child_path_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto h_ancestor_path_2_5 = h_ancestor_path_0_root;  // reusing narrowed handle
      auto h_child_path_3_7 = h_child_path_1_root;        // reusing narrowed handle

      if (!h_ancestor_path_2_5.valid() || !h_child_path_3_7.valid())
        continue;

      uint32_t degree_4 = h_ancestor_path_2_5.degree();
      uint32_t degree_6 = h_child_path_3_7.degree();
      ...
      if (total_8 > 32) {
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] = view_ancestor_path_0_1_DELTA_VER.get_value(
                1, h_ancestor_path_2_5.begin() + t0_base_10 + _ti);
          for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
              s_cart[warp_in_block][1][_ti] = view_child_path_0_1_FULL_VER.get_value(
                  1, h_child_path_3_7.begin() + t1_base_11 + _ti);
            tile.sync();
            ...
            for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14;
              ...
              // Emit: ancestor_path(grandparent, child)
              {
                ...
                if (_tc_active > 0) {
                  ...
                  if (tc_valid_1) {
                    ...
                    output_data_0[0 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] =
                        grandparent;
                    output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = child;
                  }
        ...
        for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
          ...
          auto child =
              view_ancestor_path_0_1_DELTA_VER.get_value(1, h_ancestor_path_2_5.begin() + idx0_18);
          auto grandparent =
              view_child_path_0_1_FULL_VER.get_value(1, h_child_path_3_7.begin() + idx1_19);
          // Emit: ancestor_path(grandparent, child)
          {
            uint32_t _tc_ballot = tile.ballot(tc_valid_1);
            uint32_t _tc_active = __popc(_tc_ballot);
            if (_tc_active > 0) {
              ...
              if (tc_valid_1) {
                uint32_t _tc_pos_0 = old_size_0 + warp_write_base + warp_local_count + _tc_off;
                output_data_0[0 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = grandparent;
                output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = child;
              }
              warp_local_count += _tc_active;
            }
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto child =
            view_ancestor_path_0_1_DELTA_VER.get_value(1, h_ancestor_path_2_4.begin() + idx0_9);
        auto grandparent =
            view_child_path_0_1_FULL_VER.get_value(1, h_child_path_3_6.begin() + idx1_10);

        // Emit: ancestor_path(grandparent, child)
        output_ctx_0.emit_direct(grandparent, child);
      }
    }
    output_ctx_0.flush();
  }
...
JitRunner_ancestor_path_trans_D0::LaunchParams JitRunner_ancestor_path_trans_D0::setup(
    DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ancestor_path version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ancestor_path, FULL_VER>(db)
                                   : get_relation_by_schema<ancestor_path, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: child_path version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<child_path, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ancestor_path, FULL_VER>(db)
                                     : get_relation_by_schema<ancestor_path, DELTA_VER>(db);
...
void JitRunner_ancestor_path_trans_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_ancestor_path_trans_D0::scan_and_resize(DB& db, LaunchParams& p,
                                                           GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<ancestor_path, NEW_VER>(db);
...
void JitRunner_ancestor_path_trans_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_ancestor_path_trans_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_ancestor_path_trans_D0::launch_materialize(DB& db, LaunchParams& p,
...
void JitRunner_ancestor_path_trans_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_ancestor_path_trans_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_ancestor_path_trans_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("ancestor_path_trans_D0");
...
void JitRunner_ancestor_path_trans_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
