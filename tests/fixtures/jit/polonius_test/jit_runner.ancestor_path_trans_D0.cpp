...
struct JitRunner_ancestor_path_trans_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_ancestor_path_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_ancestor_path_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_ancestor_path_0_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_ancestor_path_0_1_DELTA_VER.num_rows_;
          auto h_ancestor_path_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_ancestor_path_0_1_DELTA_VER);
          if (!h_ancestor_path_0_root.valid()) continue;
          auto h_child_path_1_root = HandleType(0, view_child_path_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_child_path_0_1_FULL_VER);
          if (!h_child_path_1_root.valid()) continue;
          auto parent = root_val_2;
        // Nested CartesianJoin: bind child, grandparent from 2 source(s)
        // MIR: (cartesian-join :vars (child grandparent) :sources ((ancestor_path :handle 2 :prefix (parent)) (child_path :handle 3 :prefix (parent)) ))
        ...
        if (!h_ancestor_path_2_4.valid() || !h_child_path_3_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_ancestor_path_2_5.valid() || !h_child_path_3_7.valid()) continue;
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_ancestor_path_0_1_DELTA_VER.get_value(1, h_ancestor_path_2_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_child_path_0_1_FULL_VER.get_value(1, h_child_path_3_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto child = tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
                auto grandparent = tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
        // Emit: ancestor_path(grandparent, child)
        {
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto child = view_ancestor_path_0_1_DELTA_VER.get_value(1, h_ancestor_path_2_5.begin() + idx0_18);
            auto grandparent = view_child_path_0_1_FULL_VER.get_value(1, h_child_path_3_7.begin() + idx1_19);
        // Emit: ancestor_path(grandparent, child)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
          uint32_t _tc_active = __popc(_tc_ballot);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_fused(
    ...
    SpecCtx_0 output_ctx_0{output_data_0, atomic_write_pos_0, overflow_flag,
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto child = view_ancestor_path_0_1_DELTA_VER.get_value(1, h_ancestor_path_2_4.begin() + idx0_9);
          auto grandparent = view_child_path_0_1_FULL_VER.get_value(1, h_child_path_3_6.begin() + idx1_10);
...
JitRunner_ancestor_path_trans_D0::LaunchParams JitRunner_ancestor_path_trans_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: ancestor_path version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<ancestor_path, FULL_VER>(db) : get_relation_by_schema<ancestor_path, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<ancestor_path, FULL_VER>(db) : get_relation_by_schema<ancestor_path, DELTA_VER>(db);
...
uint32_t JitRunner_ancestor_path_trans_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ancestor_path_trans_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
