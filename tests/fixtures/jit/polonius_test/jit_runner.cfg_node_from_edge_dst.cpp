...
struct JitRunner_cfg_node_from_edge_dst {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: cfg_node(point2)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(point2);
    }
  }
...
JitRunner_cfg_node_from_edge_dst::LaunchParams JitRunner_cfg_node_from_edge_dst::setup(
...
uint32_t JitRunner_cfg_node_from_edge_dst::scan_and_resize(DB& db, LaunchParams& p,
                                                           GPU_STREAM_T stream) {
...
void JitRunner_cfg_node_from_edge_dst::launch_materialize(DB& db, LaunchParams& p,
