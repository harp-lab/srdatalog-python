// =============================================================
// JIT-Generated Complete Runner: cfg_node_from_edge_src
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_cfg_node_from_edge_src {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: cfg_edge binding point1, _gen1
        // MIR: (scan :rel cfg_edge :vars (point1 _gen1) :handle 0)
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto point1 = view_cfg_edge_0_1_FULL_VER.get_value(0, idx_3);
        // Emit: cfg_node(point1)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto point1 = view_cfg_edge_0_1_FULL_VER.get_value(0, idx_3);
          auto _gen1 = view_cfg_edge_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: cfg_node(point1)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(point1);
        }
  }
...
JitRunner_cfg_node_from_edge_src::LaunchParams JitRunner_cfg_node_from_edge_src::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_cfg_node_from_edge_src::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_cfg_node_from_edge_src::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_cfg_node_from_edge_src::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_cfg_node_from_edge_src::read_total(LaunchParams& p) {
...
void JitRunner_cfg_node_from_edge_src::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_cfg_node_from_edge_src::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_cfg_node_from_edge_src::read_fused_result(LaunchParams& p) {
...
void JitRunner_cfg_node_from_edge_src::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("cfg_node_from_edge_src");
...
void JitRunner_cfg_node_from_edge_src::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
