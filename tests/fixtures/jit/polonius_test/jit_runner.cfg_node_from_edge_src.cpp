...
struct JitRunner_cfg_node_from_edge_src {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: cfg_node(point1)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(point1);
    }
  }
...
JitRunner_cfg_node_from_edge_src::LaunchParams JitRunner_cfg_node_from_edge_src::setup(
...
uint32_t JitRunner_cfg_node_from_edge_src::scan_and_resize(DB& db, LaunchParams& p,
                                                           GPU_STREAM_T stream) {
...
void JitRunner_cfg_node_from_edge_src::launch_materialize(DB& db, LaunchParams& p,
