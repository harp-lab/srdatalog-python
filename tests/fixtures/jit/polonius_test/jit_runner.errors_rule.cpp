// =============================================================
// JIT-Generated Complete Runner: errors_rule
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_errors_rule {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = PoloniusRules_DB_DeviceDB;
  using FirstSchema = loan_invalidated_at;
  using DestSchema = errors;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
          ...
          auto ch_loan_live_at_3_point = h_loan_live_at_3_2.child_range(positions[1], point, tile, view_loan_live_at_0_1_FULL_VER);
        // Emit: errors(loan, point)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(loan, point);
        }
        }
  }
...
JitRunner_errors_rule::LaunchParams JitRunner_errors_rule::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: loan_invalidated_at version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<loan_invalidated_at, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: loan_live_at version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<loan_live_at, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<loan_invalidated_at, FULL_VER>(db);
...
void JitRunner_errors_rule::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_errors_rule::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<errors, NEW_VER>(db);
...
void JitRunner_errors_rule::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_errors_rule::read_total(LaunchParams& p) {
...
void JitRunner_errors_rule::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_errors_rule::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_errors_rule::read_fused_result(LaunchParams& p) {
...
void JitRunner_errors_rule::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("errors_rule");
...
void JitRunner_errors_rule::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
