...
struct JitRunner_kps_seed {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto root_handle_1 =
        HandleType(0, view_known_placeholder_subset_input_0_1_FULL_VER.num_rows_, 0);
...
JitRunner_kps_seed::LaunchParams JitRunner_kps_seed::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_kps_seed::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
