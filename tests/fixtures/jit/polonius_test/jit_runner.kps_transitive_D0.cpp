// =============================================================
// JIT-Generated Complete Runner: kps_transitive_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_kps_transitive_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_known_placeholder_subset_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_known_placeholder_subset_1_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_known_placeholder_subset_1_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_known_placeholder_subset_1_0_DELTA_VER.num_rows_;
          auto h_known_placeholder_subset_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_known_placeholder_subset_1_0_DELTA_VER);
          if (!h_known_placeholder_subset_0_root.valid()) continue;
          auto h_known_placeholder_subset_1_root = HandleType(0, view_known_placeholder_subset_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_known_placeholder_subset_0_1_FULL_VER);
          if (!h_known_placeholder_subset_1_root.valid()) continue;
          auto y = root_val_2;
        // Nested CartesianJoin: bind x, z from 2 source(s)
        // MIR: (cartesian-join :vars (x z) :sources ((known_placeholder_subset :handle 2 :prefix (y)) (known_placeholder_subset :handle 3 :prefix (y)) ))
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto x = view_known_placeholder_subset_1_0_DELTA_VER.get_value(1, h_known_placeholder_subset_2_4.begin() + idx0_9);
          auto z = view_known_placeholder_subset_0_1_FULL_VER.get_value(1, h_known_placeholder_subset_3_6.begin() + idx1_10);
...
JitRunner_kps_transitive_D0::LaunchParams JitRunner_kps_transitive_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_kps_transitive_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_kps_transitive_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_kps_transitive_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_kps_transitive_D0::read_total(LaunchParams& p) {
...
void JitRunner_kps_transitive_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_kps_transitive_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_kps_transitive_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_kps_transitive_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("kps_transitive_D0");
...
void JitRunner_kps_transitive_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
