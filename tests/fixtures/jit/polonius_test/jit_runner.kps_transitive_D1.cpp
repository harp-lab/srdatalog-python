...
struct JitRunner_kps_transitive_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 = view_known_placeholder_subset_0_1_DELTA_VER.num_rows_ -
                           (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_known_placeholder_subset_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_known_placeholder_subset_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4
                                          : view_known_placeholder_subset_0_1_DELTA_VER.num_rows_;
      auto h_known_placeholder_subset_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_known_placeholder_subset_0_1_DELTA_VER);
      if (!h_known_placeholder_subset_0_root.valid())
        continue;
      auto h_known_placeholder_subset_1_root =
          HandleType(0, view_known_placeholder_subset_1_0_FULL_VER.num_rows_, 0)
              .prefix(root_val_2, tile, view_known_placeholder_subset_1_0_FULL_VER);
      if (!h_known_placeholder_subset_1_root.valid())
        continue;
      auto y = root_val_2;
      // Nested CartesianJoin: bind z, x from 2 source(s)
      // MIR: (cartesian-join :vars (z x) :sources ((known_placeholder_subset :handle 2 :prefix (y))
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto z = view_known_placeholder_subset_0_1_DELTA_VER.get_value(
            1, h_known_placeholder_subset_2_4.begin() + idx0_9);
        auto x = view_known_placeholder_subset_1_0_FULL_VER.get_value(
...
JitRunner_kps_transitive_D1::LaunchParams JitRunner_kps_transitive_D1::setup(DB& db,
                                                                             uint32_t iteration,
                                                                             GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 0: known_placeholder_subset version DELTA_VER
  {
    auto& rel_0 = (iteration == 0)
                      ? get_relation_by_schema<known_placeholder_subset, FULL_VER>(db)
                      : get_relation_by_schema<known_placeholder_subset, DELTA_VER>(db);
...
uint32_t JitRunner_kps_transitive_D1::scan_and_resize(DB& db, LaunchParams& p,
                                                      GPU_STREAM_T stream) {
...
void JitRunner_kps_transitive_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                     GPU_STREAM_T stream) {
