// =============================================================
// JIT-Generated Complete Runner: kps_transitive_D1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_kps_transitive_D1 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = PoloniusRules_DB_DeviceDB;
  using FirstSchema = known_placeholder_subset;
  using DestSchema = known_placeholder_subset;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_known_placeholder_subset_0_1_DELTA_VER = views[0];
        auto view_known_placeholder_subset_1_0_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'y' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var y :sources ((known_placeholder_subset :handle 0) (known_placeholder_subset :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_known_placeholder_subset_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_known_placeholder_subset_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_known_placeholder_subset_0_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_known_placeholder_subset_0_1_DELTA_VER.num_rows_;
          auto h_known_placeholder_subset_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_known_placeholder_subset_0_1_DELTA_VER);
          if (!h_known_placeholder_subset_0_root.valid()) continue;
          auto h_known_placeholder_subset_1_root = HandleType(0, view_known_placeholder_subset_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_known_placeholder_subset_1_0_FULL_VER);
          if (!h_known_placeholder_subset_1_root.valid()) continue;
          auto y = root_val_2;
        // Nested CartesianJoin: bind z, x from 2 source(s)
        // MIR: (cartesian-join :vars (z x) :sources ((known_placeholder_subset :handle 2 :prefix (y)) (known_placeholder_subset :handle 3 :prefix (y)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_known_placeholder_subset_2_4 = h_known_placeholder_subset_0_root;  // reusing narrowed handle
        auto h_known_placeholder_subset_3_6 = h_known_placeholder_subset_1_root;  // reusing narrowed handle

        if (!h_known_placeholder_subset_2_4.valid() || !h_known_placeholder_subset_3_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto z = view_known_placeholder_subset_0_1_DELTA_VER.get_value(1, h_known_placeholder_subset_2_4.begin() + idx0_9);
          auto x = view_known_placeholder_subset_1_0_FULL_VER.get_value(1, h_known_placeholder_subset_3_6.begin() + idx1_10);
...
JitRunner_kps_transitive_D1::LaunchParams JitRunner_kps_transitive_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: known_placeholder_subset version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<known_placeholder_subset, FULL_VER>(db) : get_relation_by_schema<known_placeholder_subset, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: known_placeholder_subset version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<known_placeholder_subset, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{1, 0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<known_placeholder_subset, FULL_VER>(db) : get_relation_by_schema<known_placeholder_subset, DELTA_VER>(db);
...
void JitRunner_kps_transitive_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_kps_transitive_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<known_placeholder_subset, NEW_VER>(db);
...
void JitRunner_kps_transitive_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_kps_transitive_D1::read_total(LaunchParams& p) {
...
void JitRunner_kps_transitive_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_kps_transitive_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_kps_transitive_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_kps_transitive_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("kps_transitive_D1");
...
void JitRunner_kps_transitive_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
