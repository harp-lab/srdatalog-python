...
struct JitRunner_loan_live_at_rule {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_origin_live_on_entry_0_1_FULL_VER = views[2];
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          for (int _seg_1 = 0; _seg_1 < 2; _seg_1++) {
            auto view_origin_live_on_entry_1 = views[2 + _seg_1];
        ...
        auto intersect_8 = intersect_handles(tile, h_origin_contains_loan_on_entry_2_6.iterators(view_origin_contains_loan_on_entry_0_2_1_FULL_VER), h_origin_live_on_entry_3_7.iterators(view_origin_live_on_entry_0_1_FULL_VER));
        for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
          auto point = it_9.value();
          auto positions = it_9.positions();
          auto ch_origin_contains_loan_on_entry_2_point = h_origin_contains_loan_on_entry_2_6.child_range(positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_FULL_VER);
          auto ch_origin_live_on_entry_3_point = h_origin_live_on_entry_3_7.child_range(positions[1], point, tile, view_origin_live_on_entry_0_1_FULL_VER);
...
JitRunner_loan_live_at_rule::LaunchParams JitRunner_loan_live_at_rule::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
uint32_t JitRunner_loan_live_at_rule::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_loan_live_at_rule::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
