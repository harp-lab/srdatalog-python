// =============================================================
// JIT-Generated Complete Runner: loan_live_at_rule
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_loan_live_at_rule {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var origin :sources ((origin_contains_loan_on_entry :handle 0)
    // (origin_live_on_entry :handle 1) )) WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 = view_origin_contains_loan_on_entry_0_2_1_FULL_VER.num_rows_ -
                           (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_origin_contains_loan_on_entry_0_2_1_FULL_VER.num_rows_)
                      ? hint_hi_4
                      : view_origin_contains_loan_on_entry_0_2_1_FULL_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3)
                      ? hint_hi_4
                      : view_origin_contains_loan_on_entry_0_2_1_FULL_VER.num_rows_;
      auto h_origin_contains_loan_on_entry_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_origin_contains_loan_on_entry_0_2_1_FULL_VER);
      if (!h_origin_contains_loan_on_entry_0_root.valid())
        continue;
      auto h_origin_live_on_entry_1_root =
          HandleType(0, view_origin_live_on_entry_0_1_FULL_VER.num_rows_, 0)
              .prefix(root_val_2, tile, view_origin_live_on_entry_0_1_FULL_VER);
      if (!h_origin_live_on_entry_1_root.valid())
        continue;
      ...
      // (origin)) (origin_live_on_entry :handle 3 :prefix (origin)) ))
      auto h_origin_contains_loan_on_entry_2_6 = h_origin_contains_loan_on_entry_0_root;
      auto h_origin_live_on_entry_3_7 = h_origin_live_on_entry_1_root;
      auto intersect_8 = intersect_handles(
          tile,
          h_origin_contains_loan_on_entry_2_6.iterators(
              view_origin_contains_loan_on_entry_0_2_1_FULL_VER),
          h_origin_live_on_entry_3_7.iterators(view_origin_live_on_entry_0_1_FULL_VER));
      for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
        auto point = it_9.value();
        auto positions = it_9.positions();
        auto ch_origin_contains_loan_on_entry_2_point =
            h_origin_contains_loan_on_entry_2_6.child_range(
                positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_FULL_VER);
        auto ch_origin_live_on_entry_3_point = h_origin_live_on_entry_3_7.child_range(
            positions[1], point, tile, view_origin_live_on_entry_0_1_FULL_VER);
        // Nested CartesianJoin: bind loan from 1 source(s)
        // MIR: (cartesian-join :vars (loan) :sources ((origin_contains_loan_on_entry :handle 4
        // :prefix (origin point)) ))
        ...
        if (!h_origin_contains_loan_on_entry_4_4.valid())
          continue;
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_10 = intersect_handles(
          ...
          h_origin_contains_loan_on_entry_2_8.iterators(
              view_origin_contains_loan_on_entry_0_2_1_FULL_VER),
          h_origin_live_on_entry_3_9.iterators(view_origin_live_on_entry_0_1_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        ...
        auto ch_origin_contains_loan_on_entry_2_point =
            h_origin_contains_loan_on_entry_2_8.child_range(
                positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_FULL_VER);
        auto ch_origin_live_on_entry_3_point = h_origin_live_on_entry_3_9.child_range(
            positions[1], point, tile, view_origin_live_on_entry_0_1_FULL_VER);
        // Nested CartesianJoin: bind loan from 1 source(s)
        // MIR: (cartesian-join :vars (loan) :sources ((origin_contains_loan_on_entry :handle 4
        // :prefix (origin point)) ))
        ...
        for (uint32_t flat_idx_6 = lane_1; flat_idx_6 < total_5; flat_idx_6 += group_size_2) {
          ...
          auto loan = view_origin_contains_loan_on_entry_0_2_1_FULL_VER.get_value(
              2, h_origin_contains_loan_on_entry_4_4.begin() + idx0_7);

          // Emit: loan_live_at(loan, point)
          output_ctx_0.emit_direct(loan, point);
        }
      }
    }
  }
...
JitRunner_loan_live_at_rule::LaunchParams JitRunner_loan_live_at_rule::setup(DB& db,
                                                                             uint32_t iteration,
                                                                             GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: origin_contains_loan_on_entry version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<origin_contains_loan_on_entry, FULL_VER>(db);
  ...
  auto& first_rel = get_relation_by_schema<origin_contains_loan_on_entry, FULL_VER>(db);
...
void JitRunner_loan_live_at_rule::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_loan_live_at_rule::scan_and_resize(DB& db, LaunchParams& p,
                                                      GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<loan_live_at, NEW_VER>(db);
...
void JitRunner_loan_live_at_rule::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_loan_live_at_rule::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_loan_live_at_rule::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                     GPU_STREAM_T stream) {
...
void JitRunner_loan_live_at_rule::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_loan_live_at_rule::read_fused_result(LaunchParams& p) {
...
void JitRunner_loan_live_at_rule::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("loan_live_at_rule");
...
void JitRunner_loan_live_at_rule::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
