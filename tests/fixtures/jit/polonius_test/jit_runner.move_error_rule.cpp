...
struct JitRunner_move_error_rule {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var src :sources ((path_maybe_uninitialized_on_exit :handle 0) (cfg_edge
    // :handle 1) )) WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 = view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.num_rows_ -
                           (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.num_rows_)
                      ? hint_hi_4
                      : view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3)
                      ? hint_hi_4
                      : view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.num_rows_;
      auto h_path_maybe_uninitialized_on_exit_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_path_maybe_uninitialized_on_exit_1_0_FULL_VER);
      ...
      // MIR: (cartesian-join :vars (path tgt) :sources ((path_maybe_uninitialized_on_exit :handle 2
      // :prefix (src)) (cfg_edge :handle 3 :prefix (src)) ))
      uint32_t lane_1 = tile.thread_rank();
      uint32_t group_size_2 = tile.size();

      auto h_path_maybe_uninitialized_on_exit_2_4 =
      ...
      if (!h_path_maybe_uninitialized_on_exit_2_4.valid() || !h_cfg_edge_3_6.valid())
        continue;
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (total_8 > 32) {
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] =
                view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.get_value(
        ...
        for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
          ...
          auto path = view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.get_value(
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto path = view_path_maybe_uninitialized_on_exit_1_0_FULL_VER.get_value(
            1, h_path_maybe_uninitialized_on_exit_2_4.begin() + idx0_9);
...
JitRunner_move_error_rule::LaunchParams JitRunner_move_error_rule::setup(DB& db, uint32_t iteration,
                                                                         GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_move_error_rule::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                   GPU_STREAM_T stream) {
