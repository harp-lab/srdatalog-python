...
struct JitRunner_ocle_cfg_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      // MIR: (column-join :var point1 :sources ((origin_contains_loan_on_entry :handle 2 :prefix
      // (origin)) (cfg_edge :handle 3 :prefix ()) ))
      ...
      auto intersect_16 = intersect_handles(tile,
                                            h_origin_contains_loan_on_entry_2_14.iterators(
                                                view_origin_contains_loan_on_entry_0_2_1_DELTA_VER),
                                            h_cfg_edge_3_15.iterators(view_cfg_edge_0_1_FULL_VER));
      for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
        auto point1 = it_17.value();
        auto positions = it_17.positions();
        auto ch_origin_contains_loan_on_entry_2_point1 =
            h_origin_contains_loan_on_entry_2_14.child_range(
                positions[0], point1, tile, view_origin_contains_loan_on_entry_0_2_1_DELTA_VER);
        auto ch_cfg_edge_3_point1 =
            h_cfg_edge_3_15.child_range(positions[1], point1, tile, view_cfg_edge_0_1_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'point2' from 2 sources
        // MIR: (column-join :var point2 :sources ((cfg_edge :handle 4 :prefix (point1))
        // (origin_live_on_entry :handle 5 :prefix (origin)) ))
        ...
        auto intersect_12 = intersect_handles(
            tile, h_cfg_edge_4_10.iterators(view_cfg_edge_0_1_FULL_VER),
            h_origin_live_on_entry_5_11.iterators(view_origin_live_on_entry_0_1_FULL_VER));
        for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
          auto point2 = it_13.value();
          auto positions = it_13.positions();
          auto ch_cfg_edge_4_point2 =
              h_cfg_edge_4_10.child_range(positions[0], point2, tile, view_cfg_edge_0_1_FULL_VER);
          auto ch_origin_live_on_entry_5_point2 = h_origin_live_on_entry_5_11.child_range(
              positions[1], point2, tile, view_origin_live_on_entry_0_1_FULL_VER);
          // Nested CartesianJoin: bind loan from 1 source(s)
          // MIR: (cartesian-join :vars (loan) :sources ((origin_contains_loan_on_entry :handle 6
          // :prefix (origin point1)) ))
          uint32_t lane_3 = tile.thread_rank();
          uint32_t group_size_4 = tile.size();

          auto h_origin_contains_loan_on_entry_6_6 =
              ch_origin_contains_loan_on_entry_2_point1;  // reusing narrowed handle

          if (!h_origin_contains_loan_on_entry_6_6.valid())
            continue;
          ...
          auto h_loan_killed_at_neg_pre_1 =
              HandleType(0, view_loan_killed_at_1_0_FULL_VER.num_rows_, 0)
                  .prefix(point1, tile, view_loan_killed_at_1_0_FULL_VER);
          ...
          for (uint32_t flat_idx_8 = lane_3; flat_idx_8 < total_7; flat_idx_8 += group_size_4) {
            ...
            auto loan = view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.get_value(
                2, h_origin_contains_loan_on_entry_6_6.begin() + idx0_9);
            ...
            auto h_loan_killed_at_neg_7_2 =
                h_loan_killed_at_neg_pre_1.prefix_seq(loan, view_loan_killed_at_1_0_FULL_VER);
...
JitRunner_ocle_cfg_D0::LaunchParams JitRunner_ocle_cfg_D0::setup(DB& db, uint32_t iteration,
                                                                 GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_ocle_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                               GPU_STREAM_T stream) {
