...
struct JitRunner_ocle_issued {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: origin_contains_loan_on_entry(origin, loan, point)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(origin, loan, point);
    }
  }
...
JitRunner_ocle_issued::LaunchParams JitRunner_ocle_issued::setup(DB& db, uint32_t iteration,
                                                                 GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_ocle_issued::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                               GPU_STREAM_T stream) {
