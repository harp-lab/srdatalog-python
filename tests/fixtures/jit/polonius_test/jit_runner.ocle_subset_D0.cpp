...
struct JitRunner_ocle_subset_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var origin1 :sources ((origin_contains_loan_on_entry :handle 0) (subset :handle 1) ))
        ...
        auto intersect_10 = intersect_handles(tile, h_origin_contains_loan_on_entry_2_8.iterators(view_origin_contains_loan_on_entry_0_2_1_DELTA_VER), h_subset_3_9.iterators(view_subset_0_2_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto point = it_11.value();
          auto positions = it_11.positions();
          auto ch_origin_contains_loan_on_entry_2_point = h_origin_contains_loan_on_entry_2_8.child_range(positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_DELTA_VER);
          auto ch_subset_3_point = h_subset_3_9.child_range(positions[1], point, tile, view_subset_0_2_1_FULL_VER);
        // Nested CartesianJoin: bind loan, origin2 from 2 source(s)
        // MIR: (cartesian-join :vars (loan origin2) :sources ((origin_contains_loan_on_entry :handle 4 :prefix (origin1 point)) (subset :handle 5 :prefix (origin1 point)) ))
        ...
        if (!h_origin_contains_loan_on_entry_4_4.valid() || !h_subset_5_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto intersect_14 = intersect_handles(tile, h_origin_contains_loan_on_entry_2_12.iterators(view_origin_contains_loan_on_entry_0_2_1_DELTA_VER), h_subset_3_13.iterators(view_subset_0_2_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto point = it_15.value();
          auto positions = it_15.positions();
          auto ch_origin_contains_loan_on_entry_2_point = h_origin_contains_loan_on_entry_2_12.child_range(positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_DELTA_VER);
          auto ch_subset_3_point = h_subset_3_13.child_range(positions[1], point, tile, view_subset_0_2_1_FULL_VER);
        // Nested CartesianJoin: bind loan, origin2 from 2 source(s)
        // MIR: (cartesian-join :vars (loan origin2) :sources ((origin_contains_loan_on_entry :handle 4 :prefix (origin1 point)) (subset :handle 5 :prefix (origin1 point)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto loan = view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.get_value(2, h_origin_contains_loan_on_entry_4_4.begin() + idx0_9);
...
JitRunner_ocle_subset_D0::LaunchParams JitRunner_ocle_subset_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_ocle_subset_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
