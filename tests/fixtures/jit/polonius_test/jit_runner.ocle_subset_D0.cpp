// =============================================================
// JIT-Generated Complete Runner: ocle_subset_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_ocle_subset_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_subset_0_2_1_FULL_VER = views[1];

    // Root ColumnJoin (multi-source intersection): bind 'origin1' from 2 sources
    // Uses root_unique_values + prefix() pattern (like TMP)
    // MIR: (column-join :var origin1 :sources ((origin_contains_loan_on_entry :handle 0) (subset
    // :handle 1) )) WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 = view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.num_rows_ -
                           (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3)
                      ? hint_hi_4
                      : view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.num_rows_;
      auto h_origin_contains_loan_on_entry_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_origin_contains_loan_on_entry_0_2_1_DELTA_VER);
      if (!h_origin_contains_loan_on_entry_0_root.valid())
        continue;
      auto h_subset_1_root = HandleType(0, view_subset_0_2_1_FULL_VER.num_rows_, 0)
                                 .prefix(root_val_2, tile, view_subset_0_2_1_FULL_VER);
      if (!h_subset_1_root.valid())
        continue;
      auto origin1 = root_val_2;
      // Nested ColumnJoin (intersection): bind 'point' from 2 sources
      // MIR: (column-join :var point :sources ((origin_contains_loan_on_entry :handle 2 :prefix
      // (origin1)) (subset :handle 3 :prefix (origin1)) ))
      auto h_origin_contains_loan_on_entry_2_8 = h_origin_contains_loan_on_entry_0_root;
      auto h_subset_3_9 = h_subset_1_root;
      auto intersect_10 = intersect_handles(tile,
                                            h_origin_contains_loan_on_entry_2_8.iterators(
                                                view_origin_contains_loan_on_entry_0_2_1_DELTA_VER),
                                            h_subset_3_9.iterators(view_subset_0_2_1_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        auto point = it_11.value();
        auto positions = it_11.positions();
        auto ch_origin_contains_loan_on_entry_2_point =
            h_origin_contains_loan_on_entry_2_8.child_range(
                positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_DELTA_VER);
        auto ch_subset_3_point =
            h_subset_3_9.child_range(positions[1], point, tile, view_subset_0_2_1_FULL_VER);
        // Nested CartesianJoin: bind loan, origin2 from 2 source(s)
        // MIR: (cartesian-join :vars (loan origin2) :sources ((origin_contains_loan_on_entry
        // :handle 4 :prefix (origin1 point)) (subset :handle 5 :prefix (origin1 point)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_origin_contains_loan_on_entry_4_4 =
            ch_origin_contains_loan_on_entry_2_point;  // reusing narrowed handle
        auto h_subset_5_6 = ch_subset_3_point;         // reusing narrowed handle

        if (!h_origin_contains_loan_on_entry_4_4.valid() || !h_subset_5_6.valid())
          continue;

        uint32_t degree_3 = h_origin_contains_loan_on_entry_4_4.degree();
        uint32_t degree_5 = h_subset_5_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto h_subset_3_13 = h_subset_1_root;
      auto intersect_14 = intersect_handles(tile,
                                            h_origin_contains_loan_on_entry_2_12.iterators(
                                                view_origin_contains_loan_on_entry_0_2_1_DELTA_VER),
                                            h_subset_3_13.iterators(view_subset_0_2_1_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto point = it_15.value();
        auto positions = it_15.positions();
        auto ch_origin_contains_loan_on_entry_2_point =
            h_origin_contains_loan_on_entry_2_12.child_range(
                positions[0], point, tile, view_origin_contains_loan_on_entry_0_2_1_DELTA_VER);
        auto ch_subset_3_point =
            h_subset_3_13.child_range(positions[1], point, tile, view_subset_0_2_1_FULL_VER);
        // Nested CartesianJoin: bind loan, origin2 from 2 source(s)
        // MIR: (cartesian-join :vars (loan origin2) :sources ((origin_contains_loan_on_entry
        // :handle 4 :prefix (origin1 point)) (subset :handle 5 :prefix (origin1 point)) ))
        uint32_t lane_1 = tile.thread_rank();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto loan = view_origin_contains_loan_on_entry_0_2_1_DELTA_VER.get_value(
              2, h_origin_contains_loan_on_entry_4_4.begin() + idx0_9);
          auto origin2 = view_subset_0_2_1_FULL_VER.get_value(2, h_subset_5_6.begin() + idx1_10);

          // Emit: origin_contains_loan_on_entry(origin2, loan, point)
...
JitRunner_ocle_subset_D0::LaunchParams JitRunner_ocle_subset_D0::setup(DB& db, uint32_t iteration,
                                                                       GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 0: origin_contains_loan_on_entry version DELTA_VER
  {
    auto& rel_0 = (iteration == 0)
                      ? get_relation_by_schema<origin_contains_loan_on_entry, FULL_VER>(db)
                      : get_relation_by_schema<origin_contains_loan_on_entry, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 2, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: subset version FULL_VER
  {
...
void JitRunner_ocle_subset_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_ocle_subset_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ocle_subset_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_ocle_subset_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_ocle_subset_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                  GPU_STREAM_T stream) {
...
void JitRunner_ocle_subset_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_ocle_subset_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_ocle_subset_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("ocle_subset_D0");
...
void JitRunner_ocle_subset_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
