...
struct JitRunner_ole_drop {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var vr :sources ((var_drop_live_on_entry :handle 0) (drop_of_var_derefs_origin :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_;
          auto h_var_drop_live_on_entry_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_var_drop_live_on_entry_0_1_FULL_VER);
          if (!h_var_drop_live_on_entry_0_root.valid()) continue;
          auto h_drop_of_var_derefs_origin_1_root = HandleType(0, view_drop_of_var_derefs_origin_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_drop_of_var_derefs_origin_0_1_FULL_VER);
          if (!h_drop_of_var_derefs_origin_1_root.valid()) continue;
          auto vr = root_val_2;
        // Nested CartesianJoin: bind point, origin from 2 source(s)
        // MIR: (cartesian-join :vars (point origin) :sources ((var_drop_live_on_entry :handle 2 :prefix (vr)) (drop_of_var_derefs_origin :handle 3 :prefix (vr)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_var_drop_live_on_entry_2_4 = h_var_drop_live_on_entry_0_root;  // reusing narrowed handle
        auto h_drop_of_var_derefs_origin_3_6 = h_drop_of_var_derefs_origin_1_root;  // reusing narrowed handle

        if (!h_var_drop_live_on_entry_2_4.valid() || !h_drop_of_var_derefs_origin_3_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto point = view_var_drop_live_on_entry_0_1_FULL_VER.get_value(1, h_var_drop_live_on_entry_2_4.begin() + idx0_9);
          auto origin = view_drop_of_var_derefs_origin_0_1_FULL_VER.get_value(1, h_drop_of_var_derefs_origin_3_6.begin() + idx1_10);
...
JitRunner_ole_drop::LaunchParams JitRunner_ole_drop::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_ole_drop::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
