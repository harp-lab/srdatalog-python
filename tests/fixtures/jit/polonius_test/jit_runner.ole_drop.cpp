// =============================================================
// JIT-Generated Complete Runner: ole_drop
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_ole_drop {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var vr :sources ((var_drop_live_on_entry :handle 0)
    // (drop_of_var_derefs_origin :handle 1) )) WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 =
          view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_)
                      ? hint_hi_4
                      : view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_;
      hint_hi_4 =
          (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_var_drop_live_on_entry_0_1_FULL_VER.num_rows_;
      auto h_var_drop_live_on_entry_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_var_drop_live_on_entry_0_1_FULL_VER);
      if (!h_var_drop_live_on_entry_0_root.valid())
        continue;
      auto h_drop_of_var_derefs_origin_1_root =
          HandleType(0, view_drop_of_var_derefs_origin_0_1_FULL_VER.num_rows_, 0)
              .prefix(root_val_2, tile, view_drop_of_var_derefs_origin_0_1_FULL_VER);
      if (!h_drop_of_var_derefs_origin_1_root.valid())
        continue;
      auto vr = root_val_2;
      // Nested CartesianJoin: bind point, origin from 2 source(s)
      // MIR: (cartesian-join :vars (point origin) :sources ((var_drop_live_on_entry :handle 2
      // :prefix (vr)) (drop_of_var_derefs_origin :handle 3 :prefix (vr)) ))
      uint32_t lane_1 = tile.thread_rank();
      uint32_t group_size_2 = tile.size();

      auto h_var_drop_live_on_entry_2_4 =
          h_var_drop_live_on_entry_0_root;  // reusing narrowed handle
      auto h_drop_of_var_derefs_origin_3_6 =
          h_drop_of_var_derefs_origin_1_root;  // reusing narrowed handle

      if (!h_var_drop_live_on_entry_2_4.valid() || !h_drop_of_var_derefs_origin_3_6.valid())
        continue;

      uint32_t degree_3 = h_var_drop_live_on_entry_2_4.degree();
      uint32_t degree_5 = h_drop_of_var_derefs_origin_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto point = view_var_drop_live_on_entry_0_1_FULL_VER.get_value(
            1, h_var_drop_live_on_entry_2_4.begin() + idx0_9);
        auto origin = view_drop_of_var_derefs_origin_0_1_FULL_VER.get_value(
            1, h_drop_of_var_derefs_origin_3_6.begin() + idx1_10);
...
JitRunner_ole_drop::LaunchParams JitRunner_ole_drop::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: var_drop_live_on_entry version FULL_VER
  {
  ...
  {
    auto& rel_1 = get_relation_by_schema<drop_of_var_derefs_origin, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<var_drop_live_on_entry, FULL_VER>(db);
...
void JitRunner_ole_drop::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_ole_drop::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ole_drop::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_ole_drop::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_ole_drop::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
...
void JitRunner_ole_drop::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_ole_drop::read_fused_result(LaunchParams& p) {
...
void JitRunner_ole_drop::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("ole_drop");
...
void JitRunner_ole_drop::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
