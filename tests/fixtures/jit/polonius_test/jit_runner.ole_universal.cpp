...
struct JitRunner_ole_universal {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (cartesian-join :vars (point origin) :sources ((cfg_node :handle 0) (universal_region
    // :handle 1) ))
    ...
    if (!h_cfg_node_0_1.valid() || !h_universal_region_1_3.valid())
      return;
...
JitRunner_ole_universal::LaunchParams JitRunner_ole_universal::setup(DB& db, uint32_t iteration,
                                                                     GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: cfg_node version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<cfg_node, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: universal_region version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<universal_region, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<cfg_node, FULL_VER>(db);
...
void JitRunner_ole_universal::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_ole_universal::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_ole_universal::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_ole_universal::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_ole_universal::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                 GPU_STREAM_T stream) {
...
void JitRunner_ole_universal::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_ole_universal::read_fused_result(LaunchParams& p) {
...
void JitRunner_ole_universal::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("ole_universal");
...
void JitRunner_ole_universal::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
