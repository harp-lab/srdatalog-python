// =============================================================
// JIT-Generated Complete Runner: path_assigned_at_ancestor_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_path_assigned_at_ancestor_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = PoloniusRules_DB_DeviceDB;
  using FirstSchema = path_assigned_at;
  using DestSchema = path_assigned_at;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_path_assigned_at_0_1_DELTA_VER = views[0];
        auto view_ancestor_path_0_1_FULL_VER = views[1];

        // Root ColumnJoin (multi-source intersection): bind 'parent' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var parent :sources ((path_assigned_at :handle 0) (ancestor_path :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_path_assigned_at_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_path_assigned_at_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_path_assigned_at_0_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_path_assigned_at_0_1_DELTA_VER.num_rows_;
          auto h_path_assigned_at_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_path_assigned_at_0_1_DELTA_VER);
          if (!h_path_assigned_at_0_root.valid()) continue;
          auto h_ancestor_path_1_root = HandleType(0, view_ancestor_path_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_ancestor_path_0_1_FULL_VER);
          if (!h_ancestor_path_1_root.valid()) continue;
          auto parent = root_val_2;
        // Nested CartesianJoin: bind point, child from 2 source(s)
        // MIR: (cartesian-join :vars (point child) :sources ((path_assigned_at :handle 2 :prefix (parent)) (ancestor_path :handle 3 :prefix (parent)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_path_assigned_at_2_4 = h_path_assigned_at_0_root;  // reusing narrowed handle
        auto h_ancestor_path_3_6 = h_ancestor_path_1_root;  // reusing narrowed handle

        if (!h_path_assigned_at_2_4.valid() || !h_ancestor_path_3_6.valid()) continue;

        uint32_t degree_3 = h_path_assigned_at_2_4.degree();
        uint32_t degree_5 = h_ancestor_path_3_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_path_assigned_at_2_5 = h_path_assigned_at_0_root;  // reusing narrowed handle
        auto h_ancestor_path_3_7 = h_ancestor_path_1_root;  // reusing narrowed handle

        if (!h_path_assigned_at_2_5.valid() || !h_ancestor_path_3_7.valid()) continue;

        uint32_t degree_4 = h_path_assigned_at_2_5.degree();
        uint32_t degree_6 = h_ancestor_path_3_7.degree();
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_path_assigned_at_0_1_DELTA_VER.get_value(1, h_path_assigned_at_2_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_ancestor_path_0_1_FULL_VER.get_value(1, h_ancestor_path_3_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto point = tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
                auto child = tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
        // Emit: path_assigned_at(child, point)
        {
          ...
          if (_tc_active > 0) {
            ...
            if (tc_valid_1) {
              ...
              output_data_0[0 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = child;
              output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = point;
            }
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto point = view_path_assigned_at_0_1_DELTA_VER.get_value(1, h_path_assigned_at_2_5.begin() + idx0_18);
            auto child = view_ancestor_path_0_1_FULL_VER.get_value(1, h_ancestor_path_3_7.begin() + idx1_19);
        // Emit: path_assigned_at(child, point)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
          uint32_t _tc_active = __popc(_tc_ballot);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_fused(
    ...
    SpecCtx_0 output_ctx_0{output_data_0, atomic_write_pos_0, overflow_flag,
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto point = view_path_assigned_at_0_1_DELTA_VER.get_value(1, h_path_assigned_at_2_4.begin() + idx0_9);
          auto child = view_ancestor_path_0_1_FULL_VER.get_value(1, h_ancestor_path_3_6.begin() + idx1_10);

        // Emit: path_assigned_at(child, point)
        output_ctx_0.emit_direct(child, point);
        }
        }
    output_ctx_0.flush();
  }
...
JitRunner_path_assigned_at_ancestor_D0::LaunchParams JitRunner_path_assigned_at_ancestor_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: path_assigned_at version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<path_assigned_at, FULL_VER>(db) : get_relation_by_schema<path_assigned_at, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: ancestor_path version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<ancestor_path, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<path_assigned_at, FULL_VER>(db) : get_relation_by_schema<path_assigned_at, DELTA_VER>(db);
...
void JitRunner_path_assigned_at_ancestor_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_path_assigned_at_ancestor_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<path_assigned_at, NEW_VER>(db);
...
void JitRunner_path_assigned_at_ancestor_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_path_assigned_at_ancestor_D0::read_total(LaunchParams& p) {
...
void JitRunner_path_assigned_at_ancestor_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_path_assigned_at_ancestor_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_path_assigned_at_ancestor_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_path_assigned_at_ancestor_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("path_assigned_at_ancestor_D0");
...
void JitRunner_path_assigned_at_ancestor_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
