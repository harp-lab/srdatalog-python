...
struct JitRunner_pbwv_ancestor_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var parent :sources ((path_begins_with_var :handle 0) (ancestor_path
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_path_begins_with_var_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_path_begins_with_var_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_path_begins_with_var_0_1_DELTA_VER.num_rows_;
      hint_hi_4 =
          (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_path_begins_with_var_0_1_DELTA_VER.num_rows_;
      auto h_path_begins_with_var_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_path_begins_with_var_0_1_DELTA_VER);
      if (!h_path_begins_with_var_0_root.valid())
        continue;
      ...
      // MIR: (cartesian-join :vars (v child) :sources ((path_begins_with_var :handle 2 :prefix
      ...
      if (!h_path_begins_with_var_2_4.valid() || !h_ancestor_path_3_6.valid())
        continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_path_begins_with_var_2_5.valid() || !h_ancestor_path_3_7.valid())
        continue;
      ...
      if (total_8 > 32) {
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] = view_path_begins_with_var_0_1_DELTA_VER.get_value(
                1, h_path_begins_with_var_2_5.begin() + t0_base_10 + _ti);
        ...
        for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
          ...
          auto v = view_path_begins_with_var_0_1_DELTA_VER.get_value(
              1, h_path_begins_with_var_2_5.begin() + idx0_18);
          auto child =
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto v = view_path_begins_with_var_0_1_DELTA_VER.get_value(
            1, h_path_begins_with_var_2_4.begin() + idx0_9);
        auto child =
...
JitRunner_pbwv_ancestor_D0::LaunchParams JitRunner_pbwv_ancestor_D0::setup(DB& db,
                                                                           uint32_t iteration,
                                                                           GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<path_begins_with_var, FULL_VER>(db)
                                   : get_relation_by_schema<path_begins_with_var, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<path_begins_with_var, FULL_VER>(db)
                                     : get_relation_by_schema<path_begins_with_var, DELTA_VER>(db);
...
void JitRunner_pbwv_ancestor_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                    GPU_STREAM_T stream) {
