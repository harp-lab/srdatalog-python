...
struct JitRunner_placeholder_origin_rule {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      // Emit: placeholder_origin(origin)
      if (tile.thread_rank() == 0)
        output_ctx_0.emit_direct(origin);
    }
  }
...
JitRunner_placeholder_origin_rule::LaunchParams JitRunner_placeholder_origin_rule::setup(
...
uint32_t JitRunner_placeholder_origin_rule::scan_and_resize(DB& db, LaunchParams& p,
                                                            GPU_STREAM_T stream) {
...
void JitRunner_placeholder_origin_rule::launch_materialize(DB& db, LaunchParams& p,
