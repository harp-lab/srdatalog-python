...
struct JitRunner_pmuoe_cfg_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var point1 :sources ((path_maybe_uninitialized_on_exit :handle 0)
    // (cfg_edge :handle 1) )) WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 = view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.num_rows_ -
                           (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3)
                      ? hint_hi_4
                      : view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.num_rows_;
      auto h_path_maybe_uninitialized_on_exit_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER);
      if (!h_path_maybe_uninitialized_on_exit_0_root.valid())
        continue;
      auto h_cfg_edge_1_root = HandleType(0, view_cfg_edge_0_1_FULL_VER.num_rows_, 0)
                                   .prefix(root_val_2, tile, view_cfg_edge_0_1_FULL_VER);
      if (!h_cfg_edge_1_root.valid())
        continue;
      auto point1 = root_val_2;
      // Nested CartesianJoin: bind point2, path from 2 source(s)
      // MIR: (cartesian-join :vars (point2 path) :sources ((path_maybe_uninitialized_on_exit
      // :handle 2 :prefix (point1)) (cfg_edge :handle 3 :prefix (point1)) ))
      uint32_t lane_2 = tile.thread_rank();
      uint32_t group_size_3 = tile.size();

      auto h_path_maybe_uninitialized_on_exit_2_5 =
          h_path_maybe_uninitialized_on_exit_0_root;  // reusing narrowed handle
      auto h_cfg_edge_3_7 = h_cfg_edge_1_root;        // reusing narrowed handle

      if (!h_path_maybe_uninitialized_on_exit_2_5.valid() || !h_cfg_edge_3_7.valid())
        continue;
      ...
      for (uint32_t flat_idx_9 = lane_2; flat_idx_9 < total_8; flat_idx_9 += group_size_3) {
        ...
        auto path = view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.get_value(
            1, h_path_maybe_uninitialized_on_exit_2_5.begin() + idx0_10);
        ...
        auto h_path_assigned_at_neg_4_1 =
            HandleType(0, view_path_assigned_at_1_0_FULL_VER.num_rows_, 0)
                .prefix_seq(point2, view_path_assigned_at_1_0_FULL_VER)
                .prefix_seq(path, view_path_assigned_at_1_0_FULL_VER);
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (total_8 > 32) {
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] =
                view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.get_value(
                    1, h_path_maybe_uninitialized_on_exit_2_5.begin() + t0_base_10 + _ti);
          for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
              s_cart[warp_in_block][1][_ti] = view_cfg_edge_0_1_FULL_VER.get_value(
                  1, h_cfg_edge_3_7.begin() + t1_base_11 + _ti);
            tile.sync();
        ...
        for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
          ...
          auto path = view_path_maybe_uninitialized_on_exit_1_0_DELTA_VER.get_value(
              1, h_path_maybe_uninitialized_on_exit_2_5.begin() + idx0_18);
...
JitRunner_pmuoe_cfg_D0::LaunchParams JitRunner_pmuoe_cfg_D0::setup(DB& db, uint32_t iteration,
                                                                   GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 0: path_maybe_uninitialized_on_exit version DELTA_VER
  {
    auto& rel_0 = (iteration == 0)
                      ? get_relation_by_schema<path_maybe_uninitialized_on_exit, FULL_VER>(db)
                      : get_relation_by_schema<path_maybe_uninitialized_on_exit, DELTA_VER>(db);
...
void JitRunner_pmuoe_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                GPU_STREAM_T stream) {
