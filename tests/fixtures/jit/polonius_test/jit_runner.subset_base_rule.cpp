...
JitRunner_subset_base_rule::LaunchParams JitRunner_subset_base_rule::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  ...
  // Source 0: subset_base version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<subset_base, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1, 2}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<subset_base, FULL_VER>(db);
...
void JitRunner_subset_base_rule::read_fused_result(LaunchParams& p) {
...
void JitRunner_subset_base_rule::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("subset_base_rule");
