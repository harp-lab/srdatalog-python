// =============================================================
// JIT-Generated Complete Runner: subset_cfg_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_subset_cfg_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_origin_live_on_entry_1_0_FULL_VER = views[2];
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_subset_2_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_subset_2_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_subset_2_0_1_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_subset_2_0_1_DELTA_VER.num_rows_;
      auto h_subset_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_subset_2_0_1_DELTA_VER);
      ...
      // MIR: (column-join :var point2 :sources ((cfg_edge :handle 2 :prefix (point1))
      // (origin_live_on_entry :handle 3 :prefix ()) (origin_live_on_entry :handle 4 :prefix ()) ))
      auto h_cfg_edge_2_9 = h_cfg_edge_1_root;
      auto h_origin_live_on_entry_3_10 =
          HandleType(0, view_origin_live_on_entry_1_0_FULL_VER.num_rows_, 0);
      auto h_origin_live_on_entry_4_11 =
          HandleType(0, view_origin_live_on_entry_1_0_FULL_VER.num_rows_, 0);
      auto intersect_12 = intersect_handles(
          tile, h_cfg_edge_2_9.iterators(view_cfg_edge_0_1_FULL_VER),
          h_origin_live_on_entry_3_10.iterators(view_origin_live_on_entry_1_0_FULL_VER),
          h_origin_live_on_entry_4_11.iterators(view_origin_live_on_entry_1_0_FULL_VER));
      for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
        auto point2 = it_13.value();
        auto positions = it_13.positions();
        auto ch_cfg_edge_2_point2 =
            h_cfg_edge_2_9.child_range(positions[0], point2, tile, view_cfg_edge_0_1_FULL_VER);
        auto ch_origin_live_on_entry_3_point2 = h_origin_live_on_entry_3_10.child_range(
            positions[1], point2, tile, view_origin_live_on_entry_1_0_FULL_VER);
        auto ch_origin_live_on_entry_4_point2 = h_origin_live_on_entry_4_11.child_range(
            positions[2], point2, tile, view_origin_live_on_entry_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'origin1' from 2 sources
        // MIR: (column-join :var origin1 :sources ((subset :handle 5 :prefix (point1))
        // (origin_live_on_entry :handle 6 :prefix (point2)) ))
        auto h_subset_5_5 = h_subset_0_root;
        auto h_origin_live_on_entry_6_6 = ch_origin_live_on_entry_4_point2;
        auto intersect_7 = intersect_handles(
            tile, h_subset_5_5.iterators(view_subset_2_0_1_DELTA_VER),
            h_origin_live_on_entry_6_6.iterators(view_origin_live_on_entry_1_0_FULL_VER));
        for (auto it_8 = intersect_7.begin(); it_8.valid(); it_8.next()) {
          auto origin1 = it_8.value();
          auto positions = it_8.positions();
          auto ch_subset_5_origin1 =
              h_subset_5_5.child_range(positions[0], origin1, tile, view_subset_2_0_1_DELTA_VER);
          auto ch_origin_live_on_entry_6_origin1 = h_origin_live_on_entry_6_6.child_range(
              positions[1], origin1, tile, view_origin_live_on_entry_1_0_FULL_VER);
          // Nested ColumnJoin (intersection): bind 'origin2' from 2 sources
          // MIR: (column-join :var origin2 :sources ((subset :handle 7 :prefix (point1 origin1))
          // (origin_live_on_entry :handle 8 :prefix (point2)) ))
          ...
          auto intersect_3 = intersect_handles(
              tile, h_subset_7_1.iterators(view_subset_2_0_1_DELTA_VER),
              h_origin_live_on_entry_8_2.iterators(view_origin_live_on_entry_1_0_FULL_VER));
          for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
            auto origin2 = it_4.value();
            auto positions = it_4.positions();
            auto ch_subset_7_origin2 =
                h_subset_7_1.child_range(positions[0], origin2, tile, view_subset_2_0_1_DELTA_VER);
            auto ch_origin_live_on_entry_8_origin2 = h_origin_live_on_entry_8_2.child_range(
                positions[1], origin2, tile, view_origin_live_on_entry_1_0_FULL_VER);
      ...
      for (auto it_13 = intersect_12.begin(); it_13.valid(); it_13.next()) {
        ...
        for (auto it_8 = intersect_7.begin(); it_8.valid(); it_8.next()) {
          ...
          for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
            ...
            // Emit: subset(origin1, origin2, point2)
            if (tile.thread_rank() == 0)
              output_ctx_0.emit_direct(origin1, origin2, point2);
          }
        }
      }
    }
...
JitRunner_subset_cfg_D0::LaunchParams JitRunner_subset_cfg_D0::setup(DB& db, uint32_t iteration,
                                                                     GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 3: origin_live_on_entry version FULL_VER
  {
...
void JitRunner_subset_cfg_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_subset_cfg_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_subset_cfg_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_subset_cfg_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_subset_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                 GPU_STREAM_T stream) {
...
void JitRunner_subset_cfg_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_subset_cfg_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_subset_cfg_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("subset_cfg_D0");
...
void JitRunner_subset_cfg_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
