  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_subset_0_1_2_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_subset_0_1_2_FULL_VER.num_rows_) ? hint_hi_4 : view_subset_0_1_2_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_subset_0_1_2_FULL_VER.num_rows_;
          auto h_subset_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_subset_0_1_2_FULL_VER);
          if (!h_subset_0_root.valid()) continue;
          auto h_placeholder_origin_1_root = HandleType(0, view_placeholder_origin_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_placeholder_origin_0_FULL_VER);
          if (!h_placeholder_origin_1_root.valid()) continue;
          auto origin1 = root_val_2;
        // Nested ColumnJoin (intersection): bind 'origin2' from 2 sources
        // MIR: (column-join :var origin2 :sources ((subset :handle 2 :prefix (origin1)) (placeholder_origin :handle 3 :prefix ()) ))
        auto h_subset_2_9 = h_subset_0_root;
        auto h_placeholder_origin_3_10 = HandleType(0, view_placeholder_origin_0_FULL_VER.num_rows_, 0);
        auto intersect_11 = intersect_handles(tile, h_subset_2_9.iterators(view_subset_0_1_2_FULL_VER), h_placeholder_origin_3_10.iterators(view_placeholder_origin_0_FULL_VER));
        for (auto it_12 = intersect_11.begin(); it_12.valid(); it_12.next()) {
          auto origin2 = it_12.value();
          auto positions = it_12.positions();
          auto ch_subset_2_origin2 = h_subset_2_9.child_range(positions[0], origin2, tile, view_subset_0_1_2_FULL_VER);
          auto ch_placeholder_origin_3_origin2 = h_placeholder_origin_3_10.child_range(positions[1], origin2, tile, view_placeholder_origin_0_FULL_VER);
        ...
        if (!h_subset_4_5.valid()) continue;

        uint32_t degree_4 = h_subset_4_5.degree();
        uint32_t total_6 = degree_4;
        if (total_6 == 0) continue;

        // Pre-narrow negation handle for known_placeholder_subset (pre-Cartesian vars: origin1, origin2)
        auto h_known_placeholder_subset_neg_pre_1 = HandleType(0, view_known_placeholder_subset_0_1_FULL_VER.num_rows_, 0).prefix(origin1, tile, view_known_placeholder_subset_0_1_FULL_VER).prefix(origin2, tile, view_known_placeholder_subset_0_1_FULL_VER);
...
JitRunner_subset_error_rule::LaunchParams JitRunner_subset_error_rule::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: subset version FULL_VER
  {
  ...
  {
    auto& rel_1 = get_relation_by_schema<placeholder_origin, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 5: known_placeholder_subset version FULL_VER
  {
    auto& rel_5 = get_relation_by_schema<known_placeholder_subset, FULL_VER>(db);
    auto& idx_5 = rel_5.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_5.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<subset, FULL_VER>(db);
...
void JitRunner_subset_error_rule::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_subset_error_rule::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<subset_error, NEW_VER>(db);
...
void JitRunner_subset_error_rule::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_subset_error_rule::read_total(LaunchParams& p) {
...
void JitRunner_subset_error_rule::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_subset_error_rule::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_subset_error_rule::read_fused_result(LaunchParams& p) {
...
void JitRunner_subset_error_rule::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("subset_error_rule");
...
void JitRunner_subset_error_rule::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
