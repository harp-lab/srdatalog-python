...
struct JitRunner_subset_trans_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          uint32_t hint_hi_4 = view_subset_1_2_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_subset_1_2_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_subset_1_2_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_subset_1_2_0_DELTA_VER.num_rows_;
          auto h_subset_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_subset_1_2_0_DELTA_VER);
          if (!h_subset_0_root.valid()) continue;
          auto h_subset_base_1_root = HandleType(0, view_subset_base_0_2_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_subset_base_0_2_1_FULL_VER);
          if (!h_subset_base_1_root.valid()) continue;
          auto origin2 = root_val_2;
        // Nested ColumnJoin (intersection): bind 'point' from 2 sources
        // MIR: (column-join :var point :sources ((subset :handle 2 :prefix (origin2)) (subset_base :handle 3 :prefix (origin2)) ))
        auto h_subset_2_12 = h_subset_0_root;
        auto h_subset_base_3_13 = h_subset_base_1_root;
        auto intersect_14 = intersect_handles(tile, h_subset_2_12.iterators(view_subset_1_2_0_DELTA_VER), h_subset_base_3_13.iterators(view_subset_base_0_2_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto point = it_15.value();
          auto positions = it_15.positions();
          auto ch_subset_2_point = h_subset_2_12.child_range(positions[0], point, tile, view_subset_1_2_0_DELTA_VER);
          auto ch_subset_base_3_point = h_subset_base_3_13.child_range(positions[1], point, tile, view_subset_base_0_2_1_FULL_VER);
        // Nested CartesianJoin: bind origin1, origin3 from 2 source(s)
        // MIR: (cartesian-join :vars (origin1 origin3) :sources ((subset :handle 4 :prefix (origin2 point)) (subset_base :handle 5 :prefix (origin2 point)) ))
        ...
        if (!h_subset_4_4.valid() || !h_subset_base_5_6.valid()) continue;
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto origin3 = view_subset_base_0_2_1_FULL_VER.get_value(2, h_subset_base_5_6.begin() + idx1_10);
        ...
        if (h_subset_2_12.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
          ...
          if (_ws_K > 0) {
            ...
            for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
              ...
              if (_ws_ce > h_subset_2_12.end()) _ws_ce = h_subset_2_12.end();
            ...
            h_subset_2_12 = HandleType(h_subset_2_12.begin(), h_subset_2_12.begin() + ws::WCOJ_SPLIT_THRESHOLD, h_subset_2_12.depth());
          }
        }
        auto intersect_14 = intersect_handles(tile, h_subset_2_12.iterators(view_subset_1_2_0_DELTA_VER), h_subset_base_3_13.iterators(view_subset_base_0_2_1_FULL_VER));
    ...
    while (true) {
      ...
      if (has_work) {
        ...
        if (ws_task.split_handle_idx == 0) {
          {
        auto h_subset_2_2 = HandleType(ws_task.handle_begin[0], ws_task.handle_end[0], ws_task.handle_depth[0]);
        auto h_subset_base_3_2 = HandleType(ws_task.handle_begin[1], ws_task.handle_end[1], ws_task.handle_depth[1]);
        auto h_subset_0_root = HandleType(ws_task.handle_begin[2], ws_task.handle_end[2], ws_task.handle_depth[2]);
        auto h_subset_base_1_root = HandleType(ws_task.handle_begin[3], ws_task.handle_end[3], ws_task.handle_depth[3]);
        ...
        auto origin2 = view_subset_1_2_0_DELTA_VER.get_value(0, h_subset_2_2.begin());
                ...
                if (h_subset_2_29.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
                  ...
                  if (_ws_K > 0) {
                    ...
                    for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
                      ...
                      uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
                      if (_ws_ce > h_subset_2_29.end()) _ws_ce = h_subset_2_29.end();
                      ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
                      task.split_level = 2;
                      task.num_handles = 4;
                      task.handle_begin[0] = _ws_cb;
                    ...
                    h_subset_2_29 = HandleType(h_subset_2_29.begin(), h_subset_2_29.begin() + ws::WCOJ_SPLIT_THRESHOLD, h_subset_2_29.depth());
                  }
                }
                auto intersect_31 = intersect_handles(tile, h_subset_2_29.iterators(view_subset_1_2_0_DELTA_VER), h_subset_base_3_30.iterators(view_subset_base_0_2_1_FULL_VER));
                uint32_t ws_cumulative_tuples = 0;
                constexpr uint32_t WS_CUM_WORK_THRESHOLD = 4096;
                for (auto it_32 = intersect_31.begin(); it_32.valid(); it_32.next()) {
                  auto point = it_32.value();
                  auto positions = it_32.positions();
                  auto ch_subset_2_point = h_subset_2_29.child_range(positions[0], point, tile, view_subset_1_2_0_DELTA_VER);
                  auto ch_subset_base_3_point = h_subset_base_3_30.child_range(positions[1], point, tile, view_subset_base_0_2_1_FULL_VER);
                // Nested CartesianJoin: bind origin1, origin3 from 2 source(s)
                // MIR: (cartesian-join :vars (origin1 origin3) :sources ((subset :handle 4 :prefix (origin2 point)) (subset_base :handle 5 :prefix (origin2 point)) ))
                ...
                if (!h_subset_4_21.valid() || !h_subset_base_5_23.valid()) continue;
                ...
                for (uint32_t flat_idx_25 = lane_18; flat_idx_25 < total_24; flat_idx_25 += group_size_19) {
                  ...
                  auto origin1 = view_subset_1_2_0_DELTA_VER.get_value(2, h_subset_4_21.begin() + idx0_26);
                  auto origin3 = view_subset_base_0_2_1_FULL_VER.get_value(2, h_subset_base_5_23.begin() + idx1_27);
        ...
        for (uint32_t ws_ct_flat = ws_ct_flat_start + ws_ct_lane; ws_ct_flat < ws_ct_flat_end; ws_ct_flat += ws_ct_gs) {
          ...
          auto origin1 = view_subset_1_2_0_DELTA_VER.get_value(2, ws_ct_h0.begin() + ws_ct_idx0);
          auto origin3 = view_subset_base_0_2_1_FULL_VER.get_value(2, ws_ct_h1.begin() + ws_ct_idx1);
          if (origin1 != origin3) {
            local_count++;
          }
        }
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize_ws(
    ...
    SRDatalog::GPU::JIT::WS::ChunkedOutputContext<ValueType, 3, 16> output_ctx_0{output_data_0, global_write_pos, static_cast<uint32_t>(output_stride_0), old_size_0};
    ...
    // Phase 1: Main traversal with inline WS splitting
        ...
        if (h_subset_2_14.degree() > ws::WCOJ_SPLIT_THRESHOLD) {
          ...
          if (_ws_K > 0) {
            ...
            for (uint32_t _ws_ti = tile.thread_rank(); _ws_ti < _ws_K; _ws_ti += tile.size()) {
              ...
              uint32_t _ws_ce = _ws_cb + ws::WCOJ_SPLIT_THRESHOLD;
              if (_ws_ce > h_subset_2_14.end()) _ws_ce = h_subset_2_14.end();
              ws::WCOJTask& task = wcoj_queue->tasks[_ws_slot];
              task.split_level = 2;
              task.num_handles = 4;
              task.handle_begin[0] = _ws_cb;
            ...
            h_subset_2_14 = HandleType(h_subset_2_14.begin(), h_subset_2_14.begin() + ws::WCOJ_SPLIT_THRESHOLD, h_subset_2_14.depth());
          }
        }
        auto intersect_16 = intersect_handles(tile, h_subset_2_14.iterators(view_subset_1_2_0_DELTA_VER), h_subset_base_3_15.iterators(view_subset_base_0_2_1_FULL_VER));
        uint32_t ws_cumulative_tuples = 0;
        constexpr uint32_t WS_CUM_WORK_THRESHOLD = 4096;
        for (auto it_17 = intersect_16.begin(); it_17.valid(); it_17.next()) {
          auto point = it_17.value();
          auto positions = it_17.positions();
          auto ch_subset_2_point = h_subset_2_14.child_range(positions[0], point, tile, view_subset_1_2_0_DELTA_VER);
          auto ch_subset_base_3_point = h_subset_base_3_15.child_range(positions[1], point, tile, view_subset_base_0_2_1_FULL_VER);
        // Nested CartesianJoin: bind origin1, origin3 from 2 source(s)
        // MIR: (cartesian-join :vars (origin1 origin3) :sources ((subset :handle 4 :prefix (origin2 point)) (subset_base :handle 5 :prefix (origin2 point)) ))
        ...
        if (!h_subset_4_5.valid() || !h_subset_base_5_7.valid()) continue;
        ...
        for (uint32_t ws_batch_10 = 0; ws_batch_10 < total_8; ws_batch_10 += group_size_3) {
          ...
          auto origin1 = ws_valid_1 ? view_subset_1_2_0_DELTA_VER.get_value(2, h_subset_4_5.begin() + idx0_11) : ValueType{0};
          auto origin3 = ws_valid_1 ? view_subset_base_0_2_1_FULL_VER.get_value(2, h_subset_base_5_7.begin() + idx1_12) : ValueType{0};
...
JitRunner_subset_trans_D0::LaunchParams JitRunner_subset_trans_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: subset version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<subset, FULL_VER>(db) : get_relation_by_schema<subset, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<subset, FULL_VER>(db) : get_relation_by_schema<subset, DELTA_VER>(db);
...
void JitRunner_subset_trans_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
