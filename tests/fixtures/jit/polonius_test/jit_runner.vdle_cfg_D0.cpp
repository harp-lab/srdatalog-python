// =============================================================
// JIT-Generated Complete Runner: vdle_cfg_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_vdle_cfg_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = PoloniusRules_DB_DeviceDB;
  using FirstSchema = var_drop_live_on_entry;
  using DestSchema = var_drop_live_on_entry;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var v :sources ((var_drop_live_on_entry :handle 0)
    // (var_maybe_partly_initialized_on_exit :handle 1) )) WARP MODE: 32 threads cooperatively
    // handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 = view_var_drop_live_on_entry_0_1_DELTA_VER.num_rows_ -
                           (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_var_drop_live_on_entry_0_1_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_var_drop_live_on_entry_0_1_DELTA_VER.num_rows_;
      hint_hi_4 =
          (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_var_drop_live_on_entry_0_1_DELTA_VER.num_rows_;
      auto h_var_drop_live_on_entry_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_var_drop_live_on_entry_0_1_DELTA_VER);
      if (!h_var_drop_live_on_entry_0_root.valid())
        continue;
      auto h_var_maybe_partly_initialized_on_exit_1_root =
          HandleType(0, view_var_maybe_partly_initialized_on_exit_0_1_FULL_VER.num_rows_, 0)
              .prefix(root_val_2, tile, view_var_maybe_partly_initialized_on_exit_0_1_FULL_VER);
      if (!h_var_maybe_partly_initialized_on_exit_1_root.valid())
        continue;
      auto v = root_val_2;
      // Nested ColumnJoin (intersection): bind 'tgt' from 2 sources
      // MIR: (column-join :var tgt :sources ((var_drop_live_on_entry :handle 2 :prefix (v))
      // (cfg_edge :handle 3 :prefix ()) ))
      ...
      auto intersect_8 = intersect_handles(
          tile, h_var_drop_live_on_entry_2_6.iterators(view_var_drop_live_on_entry_0_1_DELTA_VER),
          h_cfg_edge_3_7.iterators(view_cfg_edge_1_0_FULL_VER));
      for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
        auto tgt = it_9.value();
        auto positions = it_9.positions();
        auto ch_var_drop_live_on_entry_2_tgt = h_var_drop_live_on_entry_2_6.child_range(
            positions[0], tgt, tile, view_var_drop_live_on_entry_0_1_DELTA_VER);
        auto ch_cfg_edge_3_tgt =
            h_cfg_edge_3_7.child_range(positions[1], tgt, tile, view_cfg_edge_1_0_FULL_VER);
        // Nested ColumnJoin (intersection): bind 'src' from 2 sources
        // MIR: (column-join :var src :sources ((cfg_edge :handle 4 :prefix (tgt))
        // (var_maybe_partly_initialized_on_exit :handle 5 :prefix (v)) ))
        auto h_cfg_edge_4_2 = ch_cfg_edge_3_tgt;
        auto h_var_maybe_partly_initialized_on_exit_5_3 =
            h_var_maybe_partly_initialized_on_exit_1_root;
        auto intersect_4 =
            intersect_handles(tile, h_cfg_edge_4_2.iterators(view_cfg_edge_1_0_FULL_VER),
                              h_var_maybe_partly_initialized_on_exit_5_3.iterators(
                                  view_var_maybe_partly_initialized_on_exit_0_1_FULL_VER));
        for (auto it_5 = intersect_4.begin(); it_5.valid(); it_5.next()) {
          auto src = it_5.value();
          auto positions = it_5.positions();
          auto ch_cfg_edge_4_src =
              h_cfg_edge_4_2.child_range(positions[0], src, tile, view_cfg_edge_1_0_FULL_VER);
          auto ch_var_maybe_partly_initialized_on_exit_5_src =
              h_var_maybe_partly_initialized_on_exit_5_3.child_range(
                  positions[1], src, tile, view_var_maybe_partly_initialized_on_exit_0_1_FULL_VER);
          // Negation: NOT EXISTS in var_defined_at
          // MIR: (negation :rel var_defined_at :prefix (v src) :handle 6)
          auto h_var_defined_at_neg_6_1 =
              HandleType(0, view_var_defined_at_0_1_FULL_VER.num_rows_, 0)
                  .prefix(v, tile, view_var_defined_at_0_1_FULL_VER)
                  .prefix(src, tile, view_var_defined_at_0_1_FULL_VER);
      ...
      for (auto it_9 = intersect_8.begin(); it_9.valid(); it_9.next()) {
        ...
        for (auto it_5 = intersect_4.begin(); it_5.valid(); it_5.next()) {
          ...
          if (!h_var_defined_at_neg_6_1.valid()) {
            // Emit: var_drop_live_on_entry(v, src)
            if (tile.thread_rank() == 0)
              output_ctx_0.emit_direct(v, src);
          }
        }
      }
    }
...
JitRunner_vdle_cfg_D0::LaunchParams JitRunner_vdle_cfg_D0::setup(DB& db, uint32_t iteration,
                                                                 GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: var_drop_live_on_entry version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<var_drop_live_on_entry, FULL_VER>(db)
                                   : get_relation_by_schema<var_drop_live_on_entry, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: var_maybe_partly_initialized_on_exit version FULL_VER
  {
  ...
  // Source 6: var_defined_at version FULL_VER
  {
    auto& rel_6 = get_relation_by_schema<var_defined_at, FULL_VER>(db);
  ...
  auto& first_rel = (iteration == 0)
                        ? get_relation_by_schema<var_drop_live_on_entry, FULL_VER>(db)
                        : get_relation_by_schema<var_drop_live_on_entry, DELTA_VER>(db);
...
void JitRunner_vdle_cfg_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_vdle_cfg_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<var_drop_live_on_entry, NEW_VER>(db);
...
void JitRunner_vdle_cfg_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_vdle_cfg_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_vdle_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                               GPU_STREAM_T stream) {
...
void JitRunner_vdle_cfg_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_vdle_cfg_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_vdle_cfg_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("vdle_cfg_D0");
...
void JitRunner_vdle_cfg_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
