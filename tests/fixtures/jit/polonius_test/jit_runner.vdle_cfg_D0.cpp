...
struct JitRunner_vdle_cfg_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_var_defined_at_neg_6_1.valid()) {
        // Emit: var_drop_live_on_entry(v, src)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(v, src);
        }
        }
        }
        }
...
JitRunner_vdle_cfg_D0::LaunchParams JitRunner_vdle_cfg_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: var_drop_live_on_entry version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<var_drop_live_on_entry, FULL_VER>(db) : get_relation_by_schema<var_drop_live_on_entry, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<var_drop_live_on_entry, FULL_VER>(db) : get_relation_by_schema<var_drop_live_on_entry, DELTA_VER>(db);
...
void JitRunner_vdle_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
