...
struct JitRunner_vdle_dropped {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    // MIR: (column-join :var vr :sources ((var_dropped_at :handle 0)
    // (var_maybe_partly_initialized_on_entry :handle 1) )) WARP MODE: 32 threads cooperatively
    // handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_var_dropped_at_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_var_dropped_at_0_1_FULL_VER.num_rows_)
                      ? hint_hi_4
                      : view_var_dropped_at_0_1_FULL_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_var_dropped_at_0_1_FULL_VER.num_rows_;
      auto h_var_dropped_at_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_var_dropped_at_0_1_FULL_VER);
      if (!h_var_dropped_at_0_root.valid())
        continue;
      auto h_var_maybe_partly_initialized_on_entry_1_root =
          HandleType(0, view_var_maybe_partly_initialized_on_entry_0_1_FULL_VER.num_rows_, 0)
              .prefix(root_val_2, tile, view_var_maybe_partly_initialized_on_entry_0_1_FULL_VER);
      if (!h_var_maybe_partly_initialized_on_entry_1_root.valid())
        continue;
      auto vr = root_val_2;
      // Nested ColumnJoin (intersection): bind 'point' from 2 sources
      // MIR: (column-join :var point :sources ((var_dropped_at :handle 2 :prefix (vr))
      // (var_maybe_partly_initialized_on_entry :handle 3 :prefix (vr)) ))
      auto h_var_dropped_at_2_1 = h_var_dropped_at_0_root;
      auto h_var_maybe_partly_initialized_on_entry_3_2 =
          h_var_maybe_partly_initialized_on_entry_1_root;
      auto intersect_3 =
          intersect_handles(tile, h_var_dropped_at_2_1.iterators(view_var_dropped_at_0_1_FULL_VER),
                            h_var_maybe_partly_initialized_on_entry_3_2.iterators(
                                view_var_maybe_partly_initialized_on_entry_0_1_FULL_VER));
      for (auto it_4 = intersect_3.begin(); it_4.valid(); it_4.next()) {
        auto point = it_4.value();
        auto positions = it_4.positions();
        auto ch_var_dropped_at_2_point = h_var_dropped_at_2_1.child_range(
            positions[0], point, tile, view_var_dropped_at_0_1_FULL_VER);
        auto ch_var_maybe_partly_initialized_on_entry_3_point =
            h_var_maybe_partly_initialized_on_entry_3_2.child_range(
                positions[1], point, tile, view_var_maybe_partly_initialized_on_entry_0_1_FULL_VER);
...
JitRunner_vdle_dropped::LaunchParams JitRunner_vdle_dropped::setup(DB& db, uint32_t iteration,
                                                                   GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: var_dropped_at version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<var_dropped_at, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: var_maybe_partly_initialized_on_entry version FULL_VER
  {
  ...
  auto& first_rel = get_relation_by_schema<var_dropped_at, FULL_VER>(db);
...
void JitRunner_vdle_dropped::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_vdle_dropped::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_vdle_dropped::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_vdle_dropped::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_vdle_dropped::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                GPU_STREAM_T stream) {
...
void JitRunner_vdle_dropped::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_vdle_dropped::read_fused_result(LaunchParams& p) {
...
void JitRunner_vdle_dropped::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("vdle_dropped");
...
void JitRunner_vdle_dropped::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
