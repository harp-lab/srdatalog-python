    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_var_live_on_entry_1_0_DELTA_VER.get_value(1, h_var_live_on_entry_2_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_cfg_edge_1_0_FULL_VER.get_value(1, h_cfg_edge_3_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto point1 = tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
        // Negation: NOT EXISTS in var_defined_at
        // MIR: (negation :rel var_defined_at :prefix (vr point1) :handle 4)
        auto h_var_defined_at_neg_4_1 = HandleType(0, view_var_defined_at_0_1_FULL_VER.num_rows_, 0).prefix_seq(vr, view_var_defined_at_0_1_FULL_VER).prefix_seq(point1, view_var_defined_at_0_1_FULL_VER);
        ...
        // Emit: var_live_on_entry(vr, point1)
        {
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto vr = view_var_live_on_entry_1_0_DELTA_VER.get_value(1, h_var_live_on_entry_2_5.begin() + idx0_18);
...
JitRunner_vle_cfg_D0::LaunchParams JitRunner_vle_cfg_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: var_live_on_entry version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<var_live_on_entry, FULL_VER>(db) : get_relation_by_schema<var_live_on_entry, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<var_live_on_entry, FULL_VER>(db) : get_relation_by_schema<var_live_on_entry, DELTA_VER>(db);
...
void JitRunner_vle_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
