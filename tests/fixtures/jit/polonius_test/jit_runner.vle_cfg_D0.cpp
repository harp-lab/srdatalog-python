// =============================================================
// JIT-Generated Complete Runner: vle_cfg_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_vle_cfg_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = PoloniusRules_DB_DeviceDB;
  using FirstSchema = var_live_on_entry;
  using DestSchema = var_live_on_entry;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_var_defined_at_0_1_FULL_VER = views[2];
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_var_live_on_entry_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_var_live_on_entry_1_0_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_var_live_on_entry_1_0_DELTA_VER.num_rows_;
      hint_hi_4 =
          (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_var_live_on_entry_1_0_DELTA_VER.num_rows_;
      auto h_var_live_on_entry_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_var_live_on_entry_1_0_DELTA_VER);
      if (!h_var_live_on_entry_0_root.valid())
        continue;
      auto h_cfg_edge_1_root = HandleType(0, view_cfg_edge_1_0_FULL_VER.num_rows_, 0)
                                   .prefix(root_val_2, tile, view_cfg_edge_1_0_FULL_VER);
      if (!h_cfg_edge_1_root.valid())
        continue;
      auto point2 = root_val_2;
      // Nested CartesianJoin: bind vr, point1 from 2 source(s)
      // MIR: (cartesian-join :vars (vr point1) :sources ((var_live_on_entry :handle 2 :prefix
      // (point2)) (cfg_edge :handle 3 :prefix (point2)) ))
      ...
      if (!h_var_live_on_entry_2_5.valid() || !h_cfg_edge_3_7.valid())
        continue;
      ...
      for (uint32_t flat_idx_9 = lane_2; flat_idx_9 < total_8; flat_idx_9 += group_size_3) {
        ...
        auto vr = view_var_live_on_entry_1_0_DELTA_VER.get_value(
            1, h_var_live_on_entry_2_5.begin() + idx0_10);
        ...
        auto h_var_defined_at_neg_4_1 = HandleType(0, view_var_defined_at_0_1_FULL_VER.num_rows_, 0)
                                            .prefix_seq(vr, view_var_defined_at_0_1_FULL_VER)
                                            .prefix_seq(point1, view_var_defined_at_0_1_FULL_VER);
      ...
      if (total_8 > 32) {
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] = view_var_live_on_entry_1_0_DELTA_VER.get_value(
                1, h_var_live_on_entry_2_5.begin() + t0_base_10 + _ti);
          for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
              s_cart[warp_in_block][1][_ti] = view_cfg_edge_1_0_FULL_VER.get_value(
            ...
            for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14;
              ...
              // Negation: NOT EXISTS in var_defined_at
              // MIR: (negation :rel var_defined_at :prefix (vr point1) :handle 4)
              auto h_var_defined_at_neg_4_1 =
                  HandleType(0, view_var_defined_at_0_1_FULL_VER.num_rows_, 0)
                      .prefix_seq(vr, view_var_defined_at_0_1_FULL_VER)
                      .prefix_seq(point1, view_var_defined_at_0_1_FULL_VER);
              tc_valid_1 = tc_valid_1 && (!h_var_defined_at_neg_4_1.valid());
              // Emit: var_live_on_entry(vr, point1)
              {
                ...
                if (_tc_active > 0) {
                  ...
                  if (tc_valid_1) {
                    ...
                    output_data_0[1 * static_cast<uint32_t>(output_stride_0) + _tc_pos_0] = point1;
                  }
        ...
        for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
          ...
          auto vr = view_var_live_on_entry_1_0_DELTA_VER.get_value(
              1, h_var_live_on_entry_2_5.begin() + idx0_18);
          auto point1 = view_cfg_edge_1_0_FULL_VER.get_value(1, h_cfg_edge_3_7.begin() + idx1_19);
          // Negation: NOT EXISTS in var_defined_at
          // MIR: (negation :rel var_defined_at :prefix (vr point1) :handle 4)
          auto h_var_defined_at_neg_4_1 =
              HandleType(0, view_var_defined_at_0_1_FULL_VER.num_rows_, 0)
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_9 = lane_2; flat_idx_9 < total_8; flat_idx_9 += group_size_3) {
        ...
        if (!h_var_defined_at_neg_4_1.valid()) {
          // Emit: var_live_on_entry(vr, point1)
          output_ctx_0.emit_direct(vr, point1);
        }
      }
    }
    output_ctx_0.flush();
...
JitRunner_vle_cfg_D0::LaunchParams JitRunner_vle_cfg_D0::setup(DB& db, uint32_t iteration,
                                                               GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: var_live_on_entry version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<var_live_on_entry, FULL_VER>(db)
                                   : get_relation_by_schema<var_live_on_entry, DELTA_VER>(db);
  ...
  // Source 4: var_defined_at version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<var_defined_at, FULL_VER>(db);
    auto& idx_4 = rel_4.ensure_index(SRDatalog::IndexSpec{{0, 1}}, true);
    p.views_vec.push_back(idx_4.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<var_live_on_entry, FULL_VER>(db)
                                     : get_relation_by_schema<var_live_on_entry, DELTA_VER>(db);
...
void JitRunner_vle_cfg_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
...
uint32_t JitRunner_vle_cfg_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<var_live_on_entry, NEW_VER>(db);
...
void JitRunner_vle_cfg_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_vle_cfg_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_vle_cfg_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                              GPU_STREAM_T stream) {
...
void JitRunner_vle_cfg_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
...
void JitRunner_vle_cfg_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_vle_cfg_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("vle_cfg_D0");
...
void JitRunner_vle_cfg_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
