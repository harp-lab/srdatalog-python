...
struct JitRunner_vmpie_from_exit {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // MIR: (column-join :var point1 :sources ((var_maybe_partly_initialized_on_exit :handle 0) (cfg_edge :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.num_rows_) ? hint_hi_4 : view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.num_rows_;
          auto h_var_maybe_partly_initialized_on_exit_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER);
          if (!h_var_maybe_partly_initialized_on_exit_0_root.valid()) continue;
        ...
        // MIR: (cartesian-join :vars (vr point2) :sources ((var_maybe_partly_initialized_on_exit :handle 2 :prefix (point1)) (cfg_edge :handle 3 :prefix (point1)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_var_maybe_partly_initialized_on_exit_2_4 = h_var_maybe_partly_initialized_on_exit_0_root;  // reusing narrowed handle
        auto h_cfg_edge_3_6 = h_cfg_edge_1_root;  // reusing narrowed handle

        if (!h_var_maybe_partly_initialized_on_exit_2_4.valid() || !h_cfg_edge_3_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_var_maybe_partly_initialized_on_exit_2_5 = h_var_maybe_partly_initialized_on_exit_0_root;  // reusing narrowed handle
        auto h_cfg_edge_3_7 = h_cfg_edge_1_root;  // reusing narrowed handle

        if (!h_var_maybe_partly_initialized_on_exit_2_5.valid() || !h_cfg_edge_3_7.valid()) continue;
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.get_value(1, h_var_maybe_partly_initialized_on_exit_2_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto vr = tc_valid_1 ? s_cart[warp_in_block][0][flat_idx_9 / t1_len_13] : ValueType{0};
        ...
        // Emit: var_maybe_partly_initialized_on_entry(vr, point2)
        {
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto vr = view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.get_value(1, h_var_maybe_partly_initialized_on_exit_2_5.begin() + idx0_18);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_fused(
    ...
    SpecCtx_0 output_ctx_0{output_data_0, atomic_write_pos_0, overflow_flag,
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto vr = view_var_maybe_partly_initialized_on_exit_1_0_FULL_VER.get_value(1, h_var_maybe_partly_initialized_on_exit_2_4.begin() + idx0_9);
...
JitRunner_vmpie_from_exit::LaunchParams JitRunner_vmpie_from_exit::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_vmpie_from_exit::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
