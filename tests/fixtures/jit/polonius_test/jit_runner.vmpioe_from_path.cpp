...
struct JitRunner_vmpioe_from_path {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_path_maybe_initialized_on_exit_2_5.valid() || !h_path_begins_with_var_3_7.valid())
        continue;
      ...
      if (total_8 > 32) {
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] =
                view_path_maybe_initialized_on_exit_0_1_FULL_VER.get_value(
          ...
          for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
              s_cart[warp_in_block][1][_ti] = view_path_begins_with_var_0_1_FULL_VER.get_value(
                  1, h_path_begins_with_var_3_7.begin() + t1_base_11 + _ti);
            tile.sync();
        ...
        for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
          ...
          auto vr = view_path_begins_with_var_0_1_FULL_VER.get_value(
              1, h_path_begins_with_var_3_7.begin() + idx1_19);
          // Emit: var_maybe_partly_initialized_on_exit(vr, point)
          {
            uint32_t _tc_ballot = tile.ballot(tc_valid_1);
            uint32_t _tc_active = __popc(_tc_ballot);
...
JitRunner_vmpioe_from_path::LaunchParams JitRunner_vmpioe_from_path::setup(DB& db,
                                                                           uint32_t iteration,
                                                                           GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_vmpioe_from_path::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                    GPU_STREAM_T stream) {
