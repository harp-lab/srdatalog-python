...
struct JitRunner_Assign_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      // MIR: (cartesian-join :vars (x y) :sources ((PointsTo :handle 2 :prefix (z)) (Assign :handle
      // 3 :prefix (z)) ))
      ...
      if (!h_PointsTo_2_4.valid() || !h_Assign_3_6.valid())
        continue;
...
JitRunner_Assign_D0::LaunchParams JitRunner_Assign_D0::setup(DB& db, uint32_t iteration,
                                                             GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Assign_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                             GPU_STREAM_T stream) {
