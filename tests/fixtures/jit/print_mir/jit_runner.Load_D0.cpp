// =============================================================
// JIT-Generated Complete Runner: Load_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Load_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_Load_1_root = HandleType(0, view_Load_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Load_1_0_FULL_VER);
          if (!h_Load_1_root.valid()) continue;
          auto x = root_val_2;
        // Nested ColumnJoin (intersection): bind 'z' from 2 sources
        // MIR: (column-join :var z :sources ((PointsTo :handle 2 :prefix (x)) (PointsTo :handle 3 :prefix ()) ))
        auto h_PointsTo_2_8 = h_PointsTo_0_root;
        auto h_PointsTo_3_9 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_PointsTo_2_8.iterators(view_PointsTo_0_1_DELTA_VER), h_PointsTo_3_9.iterators(view_PointsTo_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto z = it_11.value();
          auto positions = it_11.positions();
          auto ch_PointsTo_2_z = h_PointsTo_2_8.child_range(positions[0], z, tile, view_PointsTo_0_1_DELTA_VER);
          auto ch_PointsTo_3_z = h_PointsTo_3_9.child_range(positions[1], z, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind y, w from 2 source(s)
        // MIR: (cartesian-join :vars (y w) :sources ((Load :handle 4 :prefix (x)) (PointsTo :handle 5 :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_Load_4_4 = h_Load_1_root;  // reusing narrowed handle
        auto h_PointsTo_5_6 = ch_PointsTo_3_z;  // reusing narrowed handle

        if (!h_Load_4_4.valid() || !h_PointsTo_5_6.valid()) continue;

        uint32_t degree_3 = h_Load_4_4.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'z' from 2 sources
        // MIR: (column-join :var z :sources ((PointsTo :handle 2 :prefix (x)) (PointsTo :handle 3 :prefix ()) ))
        auto h_PointsTo_2_12 = h_PointsTo_0_root;
        auto h_PointsTo_3_13 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_PointsTo_2_12.iterators(view_PointsTo_0_1_DELTA_VER), h_PointsTo_3_13.iterators(view_PointsTo_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto z = it_15.value();
          auto positions = it_15.positions();
          auto ch_PointsTo_2_z = h_PointsTo_2_12.child_range(positions[0], z, tile, view_PointsTo_0_1_DELTA_VER);
          auto ch_PointsTo_3_z = h_PointsTo_3_13.child_range(positions[1], z, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind y, w from 2 source(s)
        // MIR: (cartesian-join :vars (y w) :sources ((Load :handle 4 :prefix (x)) (PointsTo :handle 5 :prefix (z)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto y = view_Load_1_0_FULL_VER.get_value(1, h_Load_4_4.begin() + idx0_9);
...
JitRunner_Load_D0::LaunchParams JitRunner_Load_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: Load version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Load, FULL_VER>(db);
...
void JitRunner_Load_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Load_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Load_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Load_D0::read_total(LaunchParams& p) {
...
void JitRunner_Load_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Load_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Load_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Load_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Load_D0");
...
void JitRunner_Load_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
