// =============================================================
// JIT-Generated Complete Runner: Store_D0
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Store_D0 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = AndersenRules_DB_DeviceDB;
  using FirstSchema = PointsTo;
  using DestSchema = PointsTo;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_PointsTo_0_1_DELTA_VER = views[0];
        auto view_Store_0_1_FULL_VER = views[1];
        auto view_PointsTo_0_1_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'y' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var y :sources ((PointsTo :handle 0) (Store :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_PointsTo_0_1_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_PointsTo_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_PointsTo_0_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_PointsTo_0_1_DELTA_VER.num_rows_;
          auto h_PointsTo_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_PointsTo_0_1_DELTA_VER);
          if (!h_PointsTo_0_root.valid()) continue;
          auto h_Store_1_root = HandleType(0, view_Store_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Store_0_1_FULL_VER);
          if (!h_Store_1_root.valid()) continue;
          auto y = root_val_2;
        // Nested ColumnJoin (intersection): bind 'x' from 2 sources
        // MIR: (column-join :var x :sources ((Store :handle 2 :prefix (y)) (PointsTo :handle 3 :prefix ()) ))
        auto h_Store_2_8 = h_Store_1_root;
        auto h_PointsTo_3_9 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_Store_2_8.iterators(view_Store_0_1_FULL_VER), h_PointsTo_3_9.iterators(view_PointsTo_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto x = it_11.value();
          auto positions = it_11.positions();
          auto ch_Store_2_x = h_Store_2_8.child_range(positions[0], x, tile, view_Store_0_1_FULL_VER);
          auto ch_PointsTo_3_x = h_PointsTo_3_9.child_range(positions[1], x, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind z, w from 2 source(s)
        // MIR: (cartesian-join :vars (z w) :sources ((PointsTo :handle 4 :prefix (y)) (PointsTo :handle 5 :prefix (x)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_PointsTo_4_4 = h_PointsTo_0_root;  // reusing narrowed handle
        auto h_PointsTo_5_6 = ch_PointsTo_3_x;  // reusing narrowed handle

        if (!h_PointsTo_4_4.valid() || !h_PointsTo_5_6.valid()) continue;

        uint32_t degree_3 = h_PointsTo_4_4.degree();
        uint32_t degree_5 = h_PointsTo_5_6.degree();
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'x' from 2 sources
        // MIR: (column-join :var x :sources ((Store :handle 2 :prefix (y)) (PointsTo :handle 3 :prefix ()) ))
        auto h_Store_2_12 = h_Store_1_root;
        auto h_PointsTo_3_13 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
        auto intersect_14 = intersect_handles(tile, h_Store_2_12.iterators(view_Store_0_1_FULL_VER), h_PointsTo_3_13.iterators(view_PointsTo_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto x = it_15.value();
          auto positions = it_15.positions();
          auto ch_Store_2_x = h_Store_2_12.child_range(positions[0], x, tile, view_Store_0_1_FULL_VER);
          auto ch_PointsTo_3_x = h_PointsTo_3_13.child_range(positions[1], x, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind z, w from 2 source(s)
        // MIR: (cartesian-join :vars (z w) :sources ((PointsTo :handle 4 :prefix (y)) (PointsTo :handle 5 :prefix (x)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto z = view_PointsTo_0_1_DELTA_VER.get_value(1, h_PointsTo_4_4.begin() + idx0_9);
          auto w = view_PointsTo_0_1_FULL_VER.get_value(1, h_PointsTo_5_6.begin() + idx1_10);

        // Emit: PointsTo(z, w)
        output_ctx_0.emit_direct(z, w);
        }
        }
        }
  }
...
JitRunner_Store_D0::LaunchParams JitRunner_Store_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: PointsTo version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<PointsTo, FULL_VER>(db) : get_relation_by_schema<PointsTo, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: Store version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Store, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 3: PointsTo version FULL_VER
  {
    auto& rel_3 = get_relation_by_schema<PointsTo, FULL_VER>(db);
    auto& idx_3 = rel_3.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_3.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<PointsTo, FULL_VER>(db) : get_relation_by_schema<PointsTo, DELTA_VER>(db);
...
void JitRunner_Store_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Store_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<PointsTo, NEW_VER>(db);
...
void JitRunner_Store_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Store_D0::read_total(LaunchParams& p) {
...
void JitRunner_Store_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Store_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Store_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_Store_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Store_D0");
...
void JitRunner_Store_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
