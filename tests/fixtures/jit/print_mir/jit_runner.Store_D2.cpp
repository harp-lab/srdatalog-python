// =============================================================
// JIT-Generated Complete Runner: Store_D2
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_Store_D2 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_Store_1_root = HandleType(0, view_Store_1_0_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Store_1_0_FULL_VER);
          if (!h_Store_1_root.valid()) continue;
          auto x = root_val_2;
        // Nested ColumnJoin (intersection): bind 'y' from 2 sources
        // MIR: (column-join :var y :sources ((Store :handle 2 :prefix (x)) (PointsTo :handle 3 :prefix ()) ))
        auto h_Store_2_8 = h_Store_1_root;
        auto h_PointsTo_3_9 = HandleType(0, view_PointsTo_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_Store_2_8.iterators(view_Store_1_0_FULL_VER), h_PointsTo_3_9.iterators(view_PointsTo_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto y = it_11.value();
          auto positions = it_11.positions();
          auto ch_Store_2_y = h_Store_2_8.child_range(positions[0], y, tile, view_Store_1_0_FULL_VER);
          auto ch_PointsTo_3_y = h_PointsTo_3_9.child_range(positions[1], y, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind w, z from 2 source(s)
        // MIR: (cartesian-join :vars (w z) :sources ((PointsTo :handle 4 :prefix (x)) (PointsTo :handle 5 :prefix (y)) ))
        ...
        auto h_PointsTo_5_6 = ch_PointsTo_3_y;  // reusing narrowed handle
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto intersect_14 = intersect_handles(tile, h_Store_2_12.iterators(view_Store_1_0_FULL_VER), h_PointsTo_3_13.iterators(view_PointsTo_0_1_FULL_VER));
        for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
          auto y = it_15.value();
          auto positions = it_15.positions();
          auto ch_Store_2_y = h_Store_2_12.child_range(positions[0], y, tile, view_Store_1_0_FULL_VER);
          auto ch_PointsTo_3_y = h_PointsTo_3_13.child_range(positions[1], y, tile, view_PointsTo_0_1_FULL_VER);
        // Nested CartesianJoin: bind w, z from 2 source(s)
        // MIR: (cartesian-join :vars (w z) :sources ((PointsTo :handle 4 :prefix (x)) (PointsTo :handle 5 :prefix (y)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto z = view_PointsTo_0_1_FULL_VER.get_value(1, h_PointsTo_5_6.begin() + idx1_10);
...
JitRunner_Store_D2::LaunchParams JitRunner_Store_D2::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_Store_D2::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_Store_D2::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Store_D2::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_Store_D2::read_total(LaunchParams& p) {
...
void JitRunner_Store_D2::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_Store_D2::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_Store_D2::read_fused_result(LaunchParams& p) {
...
void JitRunner_Store_D2::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("Store_D2");
...
void JitRunner_Store_D2::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
