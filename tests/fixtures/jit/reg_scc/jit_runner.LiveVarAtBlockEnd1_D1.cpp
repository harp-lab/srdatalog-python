...
struct JitRunner_LiveVarAtBlockEnd1_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_RegDefUseFlowDef_2_1_0_3_FULL_VER = views[2];

        // Root ColumnJoin (multi-source intersection): bind 'blk' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var blk :sources ((RegDefUseLiveVarUsed :handle 0) (BlockNext :handle 1) ))
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          auto h_BlockNext_1_root = HandleType(0, view_BlockNext_2_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_BlockNext_2_0_1_FULL_VER);
          if (!h_BlockNext_1_root.valid()) continue;
          auto blk = root_val_2;
        // Nested CartesianJoin: bind mvar, _gen10, _gen11, prevBlock, prevBlockEnd from 2 source(s)
        // MIR: (cartesian-join :vars (mvar _gen10 _gen11 prevBlock prevBlockEnd) :sources ((RegDefUseLiveVarUsed :handle 2 :prefix (blk)) (BlockNext :handle 3 :prefix (blk)) ))
        uint32_t lane_3 = tile.thread_rank();
        uint32_t group_size_4 = tile.size();

        auto h_RegDefUseLiveVarUsed_2_6 = h_RegDefUseLiveVarUsed_0_root;  // reusing narrowed handle
        auto h_BlockNext_3_8 = h_BlockNext_1_root;  // reusing narrowed handle

        if (!h_RegDefUseLiveVarUsed_2_6.valid() || !h_BlockNext_3_8.valid()) continue;

        uint32_t degree_5 = h_RegDefUseLiveVarUsed_2_6.degree();
        uint32_t degree_7 = h_BlockNext_3_8.degree();
        uint32_t total_9 = degree_5 * degree_7;
        if (total_9 == 0) continue;

        // Pre-narrow negation handle for RegDefUseFlowDef (pre-Cartesian vars: blk)
        auto h_RegDefUseFlowDef_neg_pre_1 = HandleType(0, view_RegDefUseFlowDef_2_1_0_3_FULL_VER.num_rows_, 0).prefix(blk, tile, view_RegDefUseFlowDef_2_1_0_3_FULL_VER);
        ...
        for (uint32_t flat_idx_10 = lane_3; flat_idx_10 < total_9; flat_idx_10 += group_size_4) {
          ...
          auto mvar = view_RegDefUseLiveVarUsed_0_1_2_3_DELTA_VER.get_value(1, h_RegDefUseLiveVarUsed_2_6.begin() + idx0_11);
          auto prevBlock = view_BlockNext_2_0_1_FULL_VER.get_value(1, h_BlockNext_3_8.begin() + idx1_12);
          auto prevBlockEnd = view_BlockNext_2_0_1_FULL_VER.get_value(2, h_BlockNext_3_8.begin() + idx1_12);

        // Negation: NOT EXISTS in RegDefUseFlowDef
        // MIR: (negation :rel RegDefUseFlowDef :prefix (blk mvar prevBlockEnd) :handle 4)
        // Using pre-narrowed handle (pre-Cartesian vars: blk)
        auto h_RegDefUseFlowDef_neg_4_2 = h_RegDefUseFlowDef_neg_pre_1.prefix_seq(mvar, view_RegDefUseFlowDef_2_1_0_3_FULL_VER).prefix_seq(prevBlockEnd, view_RegDefUseFlowDef_2_1_0_3_FULL_VER);
        if (!h_RegDefUseFlowDef_neg_4_2.valid()) {
        // Emit: RegDefUseLiveVarAtBlockEnd(prevBlock, blk, mvar)
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        for (uint32_t flat_idx_10 = lane_3; flat_idx_10 < total_9; flat_idx_10 += group_size_4) {
          ...
          auto mvar = view_RegDefUseLiveVarUsed_0_1_2_3_DELTA_VER.get_value(1, h_RegDefUseLiveVarUsed_2_6.begin() + idx0_11);
          auto _gen10 = view_RegDefUseLiveVarUsed_0_1_2_3_DELTA_VER.get_value(2, h_RegDefUseLiveVarUsed_2_6.begin() + idx0_11);
          auto _gen11 = view_RegDefUseLiveVarUsed_0_1_2_3_DELTA_VER.get_value(3, h_RegDefUseLiveVarUsed_2_6.begin() + idx0_11);
          auto prevBlock = view_BlockNext_2_0_1_FULL_VER.get_value(1, h_BlockNext_3_8.begin() + idx1_12);
          auto prevBlockEnd = view_BlockNext_2_0_1_FULL_VER.get_value(2, h_BlockNext_3_8.begin() + idx1_12);
        ...
        if (!h_RegDefUseFlowDef_neg_4_2.valid()) {
        // Emit: RegDefUseLiveVarAtBlockEnd(prevBlock, blk, mvar)
        output_ctx_0.emit_direct(prevBlock, blk, mvar);
        }
        }
        }
  }
...
JitRunner_LiveVarAtBlockEnd1_D1::LaunchParams JitRunner_LiveVarAtBlockEnd1_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  // Source 1: BlockNext version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<BlockNext, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{2, 0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // Source 4: RegDefUseFlowDef version FULL_VER
  {
    auto& rel_4 = get_relation_by_schema<RegDefUseFlowDef, FULL_VER>(db);
    auto& idx_4 = rel_4.ensure_index(SRDatalog::IndexSpec{{2, 1, 0, 3}}, true);
...
void JitRunner_LiveVarAtBlockEnd1_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LiveVarAtBlockEnd1_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<RegDefUseLiveVarAtBlockEnd, NEW_VER>(db);
...
void JitRunner_LiveVarAtBlockEnd1_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LiveVarAtBlockEnd1_D1::read_total(LaunchParams& p) {
...
void JitRunner_LiveVarAtBlockEnd1_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtBlockEnd1_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarAtBlockEnd1_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_LiveVarAtBlockEnd1_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LiveVarAtBlockEnd1_D1");
...
void JitRunner_LiveVarAtBlockEnd1_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
