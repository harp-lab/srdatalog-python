...
struct JitRunner_LiveVarUsed1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto h_RegDefUseBlockLastDef_neg_1_1 = HandleType(0, view_RegDefUseBlockLastDef_0_2_1_FULL_VER.num_rows_, 0).prefix(eaUsed, tile, view_RegDefUseBlockLastDef_0_2_1_FULL_VER).prefix(mvar, tile, view_RegDefUseBlockLastDef_0_2_1_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        if (!h_RegDefUseBlockLastDef_neg_1_1.valid()) {
        // Emit: RegDefUseLiveVarUsed(blk, mvar, eaUsed, index)
        if (tile.thread_rank() == 0) output_ctx_0.emit_direct(blk, mvar, eaUsed, index);
        }
        }
  }
...
JitRunner_LiveVarUsed1::LaunchParams JitRunner_LiveVarUsed1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: RegDefUseUsedInBlock version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<RegDefUseUsedInBlock, FULL_VER>(db);
  ...
  // Source 1: RegDefUseBlockLastDef version FULL_VER
  {
    ...
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 2, 1}}, true);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<RegDefUseUsedInBlock, FULL_VER>(db);
...
void JitRunner_LiveVarUsed1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_LiveVarUsed1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarUsed1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_LiveVarUsed1::read_total(LaunchParams& p) {
...
void JitRunner_LiveVarUsed1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarUsed1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_LiveVarUsed1::read_fused_result(LaunchParams& p) {
...
void JitRunner_LiveVarUsed1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("LiveVarUsed1");
...
void JitRunner_LiveVarUsed1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
