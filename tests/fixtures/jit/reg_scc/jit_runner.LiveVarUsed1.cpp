  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
      ...
      if (!h_RegDefUseBlockLastDef_neg_1_1.valid()) {
        // Emit: RegDefUseLiveVarUsed(blk, mvar, eaUsed, index)
        if (tile.thread_rank() == 0)
          output_ctx_0.emit_direct(blk, mvar, eaUsed, index);
      }
    }
  }
...
JitRunner_LiveVarUsed1::LaunchParams JitRunner_LiveVarUsed1::setup(DB& db, uint32_t iteration,
                                                                   GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_LiveVarUsed1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                GPU_STREAM_T stream) {
