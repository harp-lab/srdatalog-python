...
struct JitRunner_RetValUsed_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      uint32_t hint_hi_4 =
          view_RegDefUseDefUsed_0_1_2_3_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_RegDefUseDefUsed_0_1_2_3_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_RegDefUseDefUsed_0_1_2_3_DELTA_VER.num_rows_;
      hint_hi_4 =
          (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_RegDefUseDefUsed_0_1_2_3_DELTA_VER.num_rows_;
      auto h_RegDefUseDefUsed_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0)
              .prefix(root_val_2, tile, view_RegDefUseDefUsed_0_1_2_3_DELTA_VER);
      if (!h_RegDefUseDefUsed_0_root.valid())
        continue;
      auto h_DirectCall_1_root = HandleType(0, view_DirectCall_0_1_FULL_VER.num_rows_, 0)
                                     .prefix(root_val_2, tile, view_DirectCall_0_1_FULL_VER);
      if (!h_DirectCall_1_root.valid())
        continue;
      auto eaCall = root_val_2;
      // Nested ColumnJoin (intersection): bind 'reg' from 2 sources
      // MIR: (column-join :var reg :sources ((RegDefUseDefUsed :handle 2 :prefix (eaCall))
      // (ArchReturnReg :handle 3 :prefix ()) ))
      ...
      auto intersect_10 = intersect_handles(
          tile, h_RegDefUseDefUsed_2_8.iterators(view_RegDefUseDefUsed_0_1_2_3_DELTA_VER),
          h_ArchReturnReg_3_9.iterators(view_ArchReturnReg_0_FULL_VER));
      for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
        auto reg = it_11.value();
        auto positions = it_11.positions();
        auto ch_RegDefUseDefUsed_2_reg = h_RegDefUseDefUsed_2_8.child_range(
            positions[0], reg, tile, view_RegDefUseDefUsed_0_1_2_3_DELTA_VER);
        auto ch_ArchReturnReg_3_reg =
            h_ArchReturnReg_3_9.child_range(positions[1], reg, tile, view_ArchReturnReg_0_FULL_VER);
        // Nested CartesianJoin: bind eaUsed, indexUsed, callee from 2 source(s)
        // MIR: (cartesian-join :vars (eaUsed indexUsed callee) :sources ((RegDefUseDefUsed :handle
        // 4 :prefix (eaCall reg)) (DirectCall :handle 5 :prefix (eaCall)) ))
        ...
        if (!h_RegDefUseDefUsed_4_4.valid() || !h_DirectCall_5_6.valid())
          continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 = intersect_handles(
          tile, h_RegDefUseDefUsed_2_12.iterators(view_RegDefUseDefUsed_0_1_2_3_DELTA_VER),
          h_ArchReturnReg_3_13.iterators(view_ArchReturnReg_0_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        auto reg = it_15.value();
        auto positions = it_15.positions();
        auto ch_RegDefUseDefUsed_2_reg = h_RegDefUseDefUsed_2_12.child_range(
            positions[0], reg, tile, view_RegDefUseDefUsed_0_1_2_3_DELTA_VER);
        auto ch_ArchReturnReg_3_reg = h_ArchReturnReg_3_13.child_range(
            positions[1], reg, tile, view_ArchReturnReg_0_FULL_VER);
        // Nested CartesianJoin: bind eaUsed, indexUsed, callee from 2 source(s)
        // MIR: (cartesian-join :vars (eaUsed indexUsed callee) :sources ((RegDefUseDefUsed :handle
        // 4 :prefix (eaCall reg)) (DirectCall :handle 5 :prefix (eaCall)) ))
        uint32_t lane_1 = tile.thread_rank();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto eaUsed = view_RegDefUseDefUsed_0_1_2_3_DELTA_VER.get_value(
              2, h_RegDefUseDefUsed_4_4.begin() + idx0_9);
          auto indexUsed = view_RegDefUseDefUsed_0_1_2_3_DELTA_VER.get_value(
              3, h_RegDefUseDefUsed_4_4.begin() + idx0_9);
          auto callee =
              view_DirectCall_0_1_FULL_VER.get_value(1, h_DirectCall_5_6.begin() + idx1_10);
...
JitRunner_RetValUsed_D1::LaunchParams JitRunner_RetValUsed_D1::setup(DB& db, uint32_t iteration,
                                                                     GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_RetValUsed_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                                 GPU_STREAM_T stream) {
