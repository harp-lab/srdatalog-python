...
struct JitRunner_SGBase {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        auto view_Arc_0_1_FULL_VER = views[0];

        // Root ColumnJoin (multi-source intersection): bind 'p' from 2 sources
        // Uses root_unique_values + prefix() pattern (like TMP)
        // MIR: (column-join :var p :sources ((Arc :handle 0) (Arc :handle 1) ))
        // WARP MODE: 32 threads cooperatively handle one row
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          auto root_val_2 = root_unique_values[y_idx_1];

          uint32_t hint_lo_3 = y_idx_1;
          uint32_t hint_hi_4 = view_Arc_0_1_FULL_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_Arc_0_1_FULL_VER.num_rows_) ? hint_hi_4 : view_Arc_0_1_FULL_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Arc_0_1_FULL_VER.num_rows_;
          auto h_Arc_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Arc_0_1_FULL_VER);
          if (!h_Arc_0_root.valid()) continue;
        ...
        // MIR: (cartesian-join :vars (x y) :sources ((Arc :handle 2 :prefix (p)) (Arc :handle 3 :prefix (p)) ))
        uint32_t lane_1 = tile.thread_rank();
        uint32_t group_size_2 = tile.size();

        auto h_Arc_2_4 = h_Arc_1_root;  // reusing narrowed handle
        auto h_Arc_3_6 = h_Arc_1_root;  // reusing narrowed handle

        if (!h_Arc_2_4.valid() || !h_Arc_3_6.valid()) continue;

        uint32_t degree_3 = h_Arc_2_4.degree();
        uint32_t degree_5 = h_Arc_3_6.degree();
        ...
        for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
          ...
          auto x = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_2_4.begin() + idx0_9);
          auto y = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_3_6.begin() + idx1_10);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        auto h_Arc_2_5 = h_Arc_1_root;  // reusing narrowed handle
        auto h_Arc_3_7 = h_Arc_1_root;  // reusing narrowed handle

        if (!h_Arc_2_5.valid() || !h_Arc_3_7.valid()) continue;

        uint32_t degree_4 = h_Arc_2_5.degree();
        uint32_t degree_6 = h_Arc_3_7.degree();
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_2_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_3_7.begin() + t1_base_11 + _ti);
              tile.sync();
        ...
        // Emit: Sg(x, y)
        {
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto x = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_2_5.begin() + idx0_18);
            auto y = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_3_7.begin() + idx1_19);
        tc_valid_1 = tc_valid_1 && (x != y);
        // Emit: Sg(x, y)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
...
JitRunner_SGBase::LaunchParams JitRunner_SGBase::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Arc version FULL_VER
  {
    auto& rel_0 = get_relation_by_schema<Arc, FULL_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // First source for root keys
  auto& first_rel = get_relation_by_schema<Arc, FULL_VER>(db);
...
void JitRunner_SGBase::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SGBase::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SGBase::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SGBase::read_total(LaunchParams& p) {
...
void JitRunner_SGBase::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SGBase::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SGBase::read_fused_result(LaunchParams& p) {
...
void JitRunner_SGBase::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SGBase");
...
void JitRunner_SGBase::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
