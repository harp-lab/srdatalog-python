...
struct JitRunner_SGBase {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      hint_hi_4 = (hint_hi_4 <= view_Arc_0_1_FULL_VER.num_rows_) ? hint_hi_4
                                                                 : view_Arc_0_1_FULL_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Arc_0_1_FULL_VER.num_rows_;
      auto h_Arc_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Arc_0_1_FULL_VER);
      if (!h_Arc_0_root.valid())
        continue;
      ...
      // MIR: (cartesian-join :vars (x y) :sources ((Arc :handle 2 :prefix (p)) (Arc :handle 3
      // :prefix (p)) ))
      ...
      if (!h_Arc_2_4.valid() || !h_Arc_3_6.valid())
        continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      if (!h_Arc_2_5.valid() || !h_Arc_3_7.valid())
        continue;
        ...
        for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
          ...
          for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
            s_cart[warp_in_block][0][_ti] =
                view_Arc_0_1_FULL_VER.get_value(1, h_Arc_2_5.begin() + t0_base_10 + _ti);
          for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
              s_cart[warp_in_block][1][_ti] =
                  view_Arc_0_1_FULL_VER.get_value(1, h_Arc_3_7.begin() + t1_base_11 + _ti);
            tile.sync();
...
JitRunner_SGBase::LaunchParams JitRunner_SGBase::setup(DB& db, uint32_t iteration,
                                                       GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_SGBase::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                          GPU_STREAM_T stream) {
