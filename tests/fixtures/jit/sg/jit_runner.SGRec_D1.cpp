// =============================================================
// JIT-Generated Complete Runner: SGRec_D1
// Fully concrete - NO C++ templates/metaprogramming
// =============================================================

struct JitRunner_SGRec_D1 {
  // Type aliases - all concrete, resolved at Nim JIT time
  using DB = SGRules_DB_DeviceDB;
  using FirstSchema = Sg;
  using DestSchema = Sg;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          hint_hi_4 = (hint_hi_4 <= view_Sg_0_1_DELTA_VER.num_rows_) ? hint_hi_4 : view_Sg_0_1_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Sg_0_1_DELTA_VER.num_rows_;
          auto h_Sg_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Sg_0_1_DELTA_VER);
          if (!h_Sg_0_root.valid()) continue;
          auto h_Arc_1_root = HandleType(0, view_Arc_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Arc_0_1_FULL_VER);
          if (!h_Arc_1_root.valid()) continue;
          auto p = root_val_2;
        ...
        auto h_Sg_2_8 = h_Sg_0_root;
        auto h_Arc_3_9 = HandleType(0, view_Arc_0_1_FULL_VER.num_rows_, 0);
        auto intersect_10 = intersect_handles(tile, h_Sg_2_8.iterators(view_Sg_0_1_DELTA_VER), h_Arc_3_9.iterators(view_Arc_0_1_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto q = it_11.value();
          auto positions = it_11.positions();
          auto ch_Sg_2_q = h_Sg_2_8.child_range(positions[0], q, tile, view_Sg_0_1_DELTA_VER);
          auto ch_Arc_3_q = h_Arc_3_9.child_range(positions[1], q, tile, view_Arc_0_1_FULL_VER);
        // Nested CartesianJoin: bind x, y from 2 source(s)
        // MIR: (cartesian-join :vars (x y) :sources ((Arc :handle 4 :prefix (p)) (Arc :handle 5 :prefix (q)) ))
        ...
        if (!h_Arc_4_4.valid() || !h_Arc_5_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_materialize(
    ...
    OutputCtx_0 output_ctx_0{output_data_0, output_prov_0, output_stride_0, old_size_0 + thread_offset};
        ...
        // Nested ColumnJoin (intersection): bind 'q' from 2 sources
        // MIR: (column-join :var q :sources ((Sg :handle 2 :prefix (p)) (Arc :handle 3 :prefix ()) ))
        auto h_Sg_2_20 = h_Sg_0_root;
        auto h_Arc_3_21 = HandleType(0, view_Arc_0_1_FULL_VER.num_rows_, 0);
        auto intersect_22 = intersect_handles(tile, h_Sg_2_20.iterators(view_Sg_0_1_DELTA_VER), h_Arc_3_21.iterators(view_Arc_0_1_FULL_VER));
        for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
          auto q = it_23.value();
          auto positions = it_23.positions();
          auto ch_Sg_2_q = h_Sg_2_20.child_range(positions[0], q, tile, view_Sg_0_1_DELTA_VER);
          auto ch_Arc_3_q = h_Arc_3_21.child_range(positions[1], q, tile, view_Arc_0_1_FULL_VER);
        // Nested CartesianJoin: bind x, y from 2 source(s)
        // MIR: (cartesian-join :vars (x y) :sources ((Arc :handle 4 :prefix (p)) (Arc :handle 5 :prefix (q)) ))
        uint32_t lane_2 = tile.thread_rank();
        uint32_t group_size_3 = tile.size();

        auto h_Arc_4_5 = h_Arc_1_root;  // reusing narrowed handle
        auto h_Arc_5_7 = ch_Arc_3_q;  // reusing narrowed handle

        if (!h_Arc_4_5.valid() || !h_Arc_5_7.valid()) continue;

        uint32_t degree_4 = h_Arc_4_5.degree();
        uint32_t degree_6 = h_Arc_5_7.degree();
        ...
        if (total_8 > 32) {
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_4_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_5_7.begin() + t1_base_11 + _ti);
              tile.sync();
              ...
              for (uint32_t tc_batch_15 = 0; tc_batch_15 < tile_total_14; tc_batch_15 += group_size_3) {
                ...
                auto y = tc_valid_1 ? s_cart[warp_in_block][1][flat_idx_9 % t1_len_13] : ValueType{0};
        // Emit: Sg(x, y)
        {
          ...
          for (uint32_t fb_batch_16 = 0; fb_batch_16 < total_8; fb_batch_16 += group_size_3) {
            ...
            auto x = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_4_5.begin() + idx0_18);
            auto y = view_Arc_0_1_FULL_VER.get_value(1, h_Arc_5_7.begin() + idx1_19);
        // Emit: Sg(x, y)
        {
          uint32_t _tc_ballot = tile.ballot(tc_valid_1);
          uint32_t _tc_active = __popc(_tc_ballot);
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_fused(
    ...
    SpecCtx_0 output_ctx_0{output_data_0, atomic_write_pos_0, overflow_flag,
        ...
        auto intersect_14 = intersect_handles(tile, h_Sg_2_12.iterators(view_Sg_0_1_DELTA_VER), h_Arc_3_13.iterators(view_Arc_0_1_FULL_VER));
...
JitRunner_SGRec_D1::LaunchParams JitRunner_SGRec_D1::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Sg version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<Sg, FULL_VER>(db) : get_relation_by_schema<Sg, DELTA_VER>(db);
    auto& idx_0 = rel_0.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_0.view());
  }

  // Source 1: Arc version FULL_VER
  {
    auto& rel_1 = get_relation_by_schema<Arc, FULL_VER>(db);
    auto& idx_1 = rel_1.ensure_index(SRDatalog::IndexSpec{{0, 1}}, false);
    p.views_vec.push_back(idx_1.view());
  }

  // First source for root keys
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<Sg, FULL_VER>(db) : get_relation_by_schema<Sg, DELTA_VER>(db);
...
void JitRunner_SGRec_D1::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_SGRec_D1::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  ...
  auto& dest_rel_0 = get_relation_by_schema<Sg, NEW_VER>(db);
...
void JitRunner_SGRec_D1::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_SGRec_D1::read_total(LaunchParams& p) {
...
void JitRunner_SGRec_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_SGRec_D1::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_SGRec_D1::read_fused_result(LaunchParams& p) {
...
void JitRunner_SGRec_D1::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("SGRec_D1");
...
void JitRunner_SGRec_D1::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
