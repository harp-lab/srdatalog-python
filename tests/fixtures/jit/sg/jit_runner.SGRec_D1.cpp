...
struct JitRunner_SGRec_D1 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_10 = intersect_handles(tile, h_Sg_2_8.iterators(view_Sg_0_1_DELTA_VER),
                                            h_Arc_3_9.iterators(view_Arc_0_1_FULL_VER));
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_22 = intersect_handles(tile, h_Sg_2_20.iterators(view_Sg_0_1_DELTA_VER),
                                            h_Arc_3_21.iterators(view_Arc_0_1_FULL_VER));
      for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
        ...
        if (!h_Arc_4_5.valid() || !h_Arc_5_7.valid())
          continue;
          ...
          for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
            ...
            for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
              s_cart[warp_in_block][0][_ti] =
                  view_Arc_0_1_FULL_VER.get_value(1, h_Arc_4_5.begin() + t0_base_10 + _ti);
            for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                s_cart[warp_in_block][1][_ti] =
                    view_Arc_0_1_FULL_VER.get_value(1, h_Arc_5_7.begin() + t1_base_11 + _ti);
              tile.sync();
...
JitRunner_SGRec_D1::LaunchParams JitRunner_SGRec_D1::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
  ...
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<Sg, FULL_VER>(db)
                                   : get_relation_by_schema<Sg, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<Sg, FULL_VER>(db)
                                     : get_relation_by_schema<Sg, DELTA_VER>(db);
...
void JitRunner_SGRec_D1::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
