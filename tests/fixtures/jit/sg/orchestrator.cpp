...
static void step_0(DB& db, std::size_t max_iterations) {
  ...
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, NEW_VER>>(db, 0);

  using Sg_canonical_spec_t =
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, FULL_VER>;
  bool _tail_mode = false;

  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Arc, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Arc, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Arc, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Arc, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  if (_tail_mode)
    JitRunner_SGBase::execute_fused(db, 0);
  else
    JitRunner_SGBase::execute(db, 0);
  SRDatalog::GPU::mir_helpers::rebuild_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, NEW_VER>>(db);
  ...
  SRDatalog::GPU::mir_helpers::compute_delta_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, NEW_VER>,
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, FULL_VER>,
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, DELTA_VER>>(db);
  nvtxRangePop();  // merge
  SRDatalog::GPU::mir_helpers::clear_relation_fn<Sg, NEW_VER>(db);
  nvtxRangePushA("merge");
  SRDatalog::GPU::mir_helpers::merge_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, FULL_VER>>(db);
  nvtxRangePop();  // merge
}
template <typename DB>
...
static void step_2(DB& db, std::size_t max_iterations) {
  ...
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, DELTA_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Arc, std::integer_sequence<int, 0, 1>, FULL_VER>>(db, 0);
  mir_helpers::create_index_fn<
      SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, DELTA_VER>>(db, 0);
  ...
  for (std::size_t iter = 0; iter < max_iterations; ++iter) {
    ...
    if (_tail_mode)
      JitRunner_SGRec_D1::execute_fused(db, static_cast<uint32_t>(iter));
    else
      JitRunner_SGRec_D1::execute(db, static_cast<uint32_t>(iter));
    SRDatalog::GPU::mir_helpers::rebuild_index_fn<
        SRDatalog::mir::IndexSpecT<Sg, std::integer_sequence<int, 0, 1>, NEW_VER>>(db);
