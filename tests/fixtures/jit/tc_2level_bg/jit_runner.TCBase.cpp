// =============================================================
// JIT-Generated Complete Runner: TCBase
...
struct JitRunner_TCBase {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        // Root Scan: Edge binding x, y
        // MIR: (scan :rel Edge :vars (x y) :handle 0)
        auto root_handle_1 = HandleType(0, view_Edge_0_1_FULL_VER.num_rows_, 0);
        ...
        for (uint32_t idx_3 = warp_id; idx_3 < degree_2; idx_3 += num_warps) {
          auto x = view_Edge_0_1_FULL_VER.get_value(0, idx_3);
          auto y = view_Edge_0_1_FULL_VER.get_value(1, idx_3);
        // Emit: Path(x, y)
...
JitRunner_TCBase::LaunchParams JitRunner_TCBase::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
...
void JitRunner_TCBase::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
...
uint32_t JitRunner_TCBase::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_TCBase::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0) return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr, p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0, thrust::plus<uint32_t>());
}

// Phase 3b: Read total count (call after device sync)
uint32_t JitRunner_TCBase::read_total(LaunchParams& p) {
...
void JitRunner_TCBase::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
...
void JitRunner_TCBase::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_TCBase::read_fused_result(LaunchParams& p) {
...
void JitRunner_TCBase::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("TCBase");
...
void JitRunner_TCBase::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0) return;
