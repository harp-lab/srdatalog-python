...
struct JitRunner_TCRec_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    auto view_Path_1_0_DELTA_VER = views[0];
    auto view_Edge_0_1_FULL_VER = views[1];

    // Root ColumnJoin (multi-source intersection): bind 'y' from 2 sources
    // Uses root_unique_values + prefix() pattern (like TMP)
    // MIR: (column-join :var y :sources ((Path :handle 0) (Edge :handle 1) ))
    // WARP MODE: 32 threads cooperatively handle one row
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      auto root_val_2 = root_unique_values[y_idx_1];

      uint32_t hint_lo_3 = y_idx_1;
      uint32_t hint_hi_4 = view_Path_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - y_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_Path_1_0_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_Path_1_0_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Path_1_0_DELTA_VER.num_rows_;
      auto h_Path_0_root =
          HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Path_1_0_DELTA_VER);
      if (!h_Path_0_root.valid())
        continue;
      auto h_Edge_1_root = HandleType(0, view_Edge_0_1_FULL_VER.num_rows_, 0)
                               .prefix(root_val_2, tile, view_Edge_0_1_FULL_VER);
      if (!h_Edge_1_root.valid())
        continue;
      auto y = root_val_2;
      // Nested CartesianJoin: bind x, z from 2 source(s)
      // MIR: (cartesian-join :vars (x z) :sources ((Path :handle 2 :prefix (y)) (Edge :handle 3
      // :prefix (y)) ))
      uint32_t lane_1 = tile.thread_rank();
      uint32_t group_size_2 = tile.size();

      auto h_Path_2_4 = h_Path_0_root;  // reusing narrowed handle
      auto h_Edge_3_6 = h_Edge_1_root;  // reusing narrowed handle

      if (!h_Path_2_4.valid() || !h_Edge_3_6.valid())
        continue;

      uint32_t degree_3 = h_Path_2_4.degree();
      uint32_t degree_5 = h_Edge_3_6.degree();
      ...
      // Count-as-product: per-lane share without inner loop
      {
        ...
        uint32_t lane_share =
            (lane_1 < lane_total) ? ((lane_total - lane_1 + group_size_2 - 1) / group_size_2) : 0;
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      for (uint32_t flat_idx_8 = lane_1; flat_idx_8 < total_7; flat_idx_8 += group_size_2) {
        ...
        auto x = view_Path_1_0_DELTA_VER.get_value(1, h_Path_2_4.begin() + idx0_9);
        auto z = view_Edge_0_1_FULL_VER.get_value(1, h_Edge_3_6.begin() + idx1_10);

        // Emit: Path(x, z)
        output_ctx_0.emit_direct(x, z);
      }
    }
  }
  ...
  static __global__ void __launch_bounds__(kBlockSize)
      kernel_bg_histogram(const ViewType* __restrict__ views,
                          const ValueType* __restrict__ root_unique_values,
                          const ValueType* __restrict__ head_root_unique_values,
                          uint32_t num_unique_root_keys, uint32_t num_full_unique_root_keys,
                          uint32_t num_root_keys, uint64_t* __restrict__ bg_work_per_key) {
    auto block = cg::this_thread_block();
    auto tile = cg::tiled_partition<kGroupSize>(block);
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys;
         bg_hist_key += num_warps) {
      ...
      uint32_t bg_hhi =
          view_Path_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_Path_1_0_DELTA_VER.num_rows_) ? bg_hhi
                                                             : view_Path_1_0_DELTA_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_Path_1_0_DELTA_VER.num_rows_;
      auto h_Path_0_root =
          HandleType(bg_hlo, bg_hhi, 0).prefix(bg_hist_root_val, tile, view_Path_1_0_DELTA_VER);
      if (!h_Path_0_root.valid()) {
        bg_work_per_key[bg_hist_key] = 0;
        continue;
      }
      auto h_Edge_1_root = HandleType(0, view_Edge_0_1_FULL_VER.num_rows_, 0)
                               .prefix(bg_hist_root_val, tile, view_Edge_0_1_FULL_VER);
      if (!h_Edge_1_root.valid()) {
        bg_work_per_key[bg_hist_key] = 0;
        continue;
      }
      uint64_t bg_deg = 1;
      bg_deg *= h_Path_0_root.degree();
      bg_deg *= h_Edge_1_root.degree();
      if (tile.thread_rank() == 0)
        bg_work_per_key[bg_hist_key] = bg_deg;
    }
  }

  // Block-group partitioned count kernel
  static __global__ void __launch_bounds__(kBlockSize)
      kernel_bg_count(const ViewType* __restrict__ views,
                      const ValueType* __restrict__ root_unique_values,
                      const ValueType* __restrict__ head_root_unique_values,
                      uint32_t num_unique_root_keys, uint32_t num_full_unique_root_keys,
                      uint32_t num_root_keys, const uint64_t* __restrict__ bg_cumulative_work,
                      uint64_t bg_total_work, uint32_t* __restrict__ thread_counts) {
    auto block = cg::this_thread_block();
    auto tile = cg::tiled_partition<kGroupSize>(block);
    auto single_thread = cg::tiled_partition<1>(block);
    __shared__ char s_views_buf[NumSources * sizeof(ViewType)];
    ...
    // Root ColumnJoin (BLOCK-GROUP): bind 'y' from 2 sources
    ...
    if (bg_block_end > bg_total_work)
      bg_block_end = bg_total_work;
    ...
    while (bg_key_lo < bg_key_hi) {
      ...
      if (bg_cumulative_work[bg_mid] <= (uint64_t)bg_block_begin)
        bg_key_lo = bg_mid + 1;
      else
        bg_key_hi = bg_mid;
    }
    ...
    for (uint32_t bg_key_idx_1 = bg_key_lo;
         bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end;
         bg_key_idx_1++) {
      ...
      if (bg_key_work_end <= bg_remaining_begin)
        continue;
      if (bg_key_work_start >= bg_remaining_end)
        break;

      uint64_t bg_my_begin_in_key =
          (bg_remaining_begin > bg_key_work_start) ? (bg_remaining_begin - bg_key_work_start) : 0;
      uint64_t bg_my_end_in_key = (bg_remaining_end < bg_key_work_end)
                                      ? (bg_remaining_end - bg_key_work_start)
                                      : (bg_key_work_end - bg_key_work_start);

      uint32_t hint_lo_3 = bg_key_idx_1;
      uint32_t hint_hi_4 =
          view_Path_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
      hint_hi_4 = (hint_hi_4 <= view_Path_1_0_DELTA_VER.num_rows_)
                      ? hint_hi_4
                      : view_Path_1_0_DELTA_VER.num_rows_;
      hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Path_1_0_DELTA_VER.num_rows_;
      ...
      uint32_t bg_deg_first = (uint32_t)(h_Path_0_root.end() - h_Path_0_root.begin());
      uint32_t bg_block_row_begin =
          (uint32_t)((bg_my_begin_in_key * (uint64_t)bg_deg_first) / bg_key_total_work);
      uint32_t bg_block_row_end =
          (uint32_t)((bg_my_end_in_key * (uint64_t)bg_deg_first) / bg_key_total_work);
      if (bg_my_end_in_key >= bg_key_total_work)
        bg_block_row_end = bg_deg_first;
      if (bg_block_row_begin >= bg_block_row_end) {
        bg_remaining_begin = bg_key_work_end;
        continue;
      }
      ...
      if (bg_warp_row_end > bg_block_row_end)
        bg_warp_row_end = bg_block_row_end;
      if (bg_warp_row_begin >= bg_warp_row_end) {
        bg_remaining_begin = bg_key_work_end;
        continue;
      }
      ...
      {
        auto bg_narrow_begin = h_Path_0_root.begin() + bg_warp_row_begin;
        auto bg_narrow_end = h_Path_0_root.begin() + bg_warp_row_end;
        h_Path_0_root = HandleType(bg_narrow_begin, bg_narrow_end, h_Path_0_root.depth());
      }
  ...
  static __global__ void __launch_bounds__(kBlockSize)
      kernel_bg_materialize(const ViewType* __restrict__ views,
                            const ValueType* __restrict__ root_unique_values,
                            const ValueType* __restrict__ head_root_unique_values,
                            uint32_t num_unique_root_keys, uint32_t num_full_unique_root_keys,
                            uint32_t num_root_keys, const uint64_t* __restrict__ bg_cumulative_work,
                            uint64_t bg_total_work, const uint32_t* __restrict__ thread_offsets,
                            ValueType* __restrict__ output_data_0,
                            semiring_value_t<SR>* __restrict__ output_prov_0,
                            std::size_t output_stride_0, uint32_t old_size_0) {
    auto block = cg::this_thread_block();
  ...
  static __global__ void __launch_bounds__(kBlockSize)
      kernel_bg_fused(const ViewType* __restrict__ views,
                      const ValueType* __restrict__ root_unique_values,
                      const ValueType* __restrict__ head_root_unique_values,
                      uint32_t num_unique_root_keys, uint32_t num_full_unique_root_keys,
                      uint32_t num_root_keys, const uint64_t* __restrict__ bg_cumulative_work,
                      uint64_t bg_total_work, ValueType* __restrict__ output_data_0,
                      std::size_t output_stride_0, uint32_t old_size_0,
...
JitRunner_TCRec_D0::LaunchParams JitRunner_TCRec_D0::setup(DB& db, uint32_t iteration,
                                                           GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Path version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<Path, FULL_VER>(db)
                                   : get_relation_by_schema<Path, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<Path, FULL_VER>(db)
                                     : get_relation_by_schema<Path, DELTA_VER>(db);
  ...
  if (p.num_root_keys >= 256 && p.num_unique_root_keys >= 32) {
    ...
    uint32_t hist_blocks =
        std::max((uint32_t)((p.num_unique_root_keys + (kBlockSize / kGroupSize) - 1) /
                            (kBlockSize / kGroupSize)),
                 (uint32_t)(bg_num_sms * 4));
    kernel_bg_histogram<<<hist_blocks, kBlockSize, 0, stream>>>(
        p.d_views.data(), p.root_unique_values_ptr, p.head_root_unique_values_ptr,
        p.num_unique_root_keys, p.num_full_unique_root_keys, p.num_root_keys, bg_wk_ptr);
    thrust::inclusive_scan(rmm::exec_policy(stream), bg_wk_ptr, bg_wk_ptr + p.num_unique_root_keys,
                           p.bg_cumulative_work_ptr);
    GPU_MEMCPY_ASYNC(&p.bg_total_work, p.bg_cumulative_work_ptr + p.num_unique_root_keys - 1,
                     sizeof(uint64_t), GPU_DEVICE_TO_HOST, stream);
    // Decide: is BG worth it? Compare max per-key work vs fair share.
    auto bg_max_it = thrust::max_element(rmm::exec_policy(stream), bg_wk_ptr,
                                         bg_wk_ptr + p.num_unique_root_keys);
    uint64_t bg_max_work = 0;
    GPU_MEMCPY_ASYNC(&bg_max_work, thrust::raw_pointer_cast(bg_max_it), sizeof(uint64_t),
...
void JitRunner_TCRec_D0::launch_count(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  ...
  if (p.bg_total_work > 0) {
    kernel_bg_count<<<p.num_blocks, kBlockSize, 0, stream>>>(
        p.d_views.data(), p.root_unique_values_ptr, p.head_root_unique_values_ptr,
        p.num_unique_root_keys, p.num_full_unique_root_keys, p.num_root_keys,
...
uint32_t JitRunner_TCRec_D0::scan_and_resize(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
...
void JitRunner_TCRec_D0::scan_only(LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_threads == 0)
    return;
  thrust::exclusive_scan(rmm::exec_policy(stream), p.thread_counts_ptr,
                         p.thread_counts_ptr + p.num_threads + 1, p.thread_counts_ptr, 0,
...
uint32_t JitRunner_TCRec_D0::read_total(LaunchParams& p) {
  if (p.num_threads == 0)
    return 0;
  uint32_t total_count = 0;
...
void JitRunner_TCRec_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count,
                                            GPU_STREAM_T stream) {
...
void JitRunner_TCRec_D0::launch_fused(DB& db, LaunchParams& p, GPU_STREAM_T stream) {
  if (p.num_unique_root_keys == 0)
    return;
  ...
  thrust::inclusive_scan(rmm::exec_policy(stream), bg_work_per_key.data(),
                         bg_work_per_key.data() + p.num_unique_root_keys,
                         bg_cumulative_work.data());
  uint64_t bg_total_work = 0;
  cudaStreamSynchronize(stream);
  GPU_MEMCPY(&bg_total_work, bg_cumulative_work.data() + p.num_unique_root_keys - 1,
             sizeof(uint64_t), GPU_DEVICE_TO_HOST);
  if (bg_total_work == 0)
    return;
  ...
  kernel_bg_fused<<<bg_num_blocks, kBlockSize, 0, stream>>>(
      p.d_views.data(), p.root_unique_values_ptr, p.head_root_unique_values_ptr,
      p.num_unique_root_keys, p.num_full_unique_root_keys, p.num_root_keys,
      bg_cumulative_work.data(), bg_total_work, dest_rel_0.template interned_column<0>(),
      dest_rel_0.interned_stride(), p.old_size_0, p.fused_wp_ptr_0, p.fused_capacity,
      p.fused_of_ptr);
}

// read_fused_result: readback fused write counts (call after device sync)
void JitRunner_TCRec_D0::read_fused_result(LaunchParams& p) {
...
void JitRunner_TCRec_D0::execute(DB& db, uint32_t iteration) {
  nvtxRangePushA("TCRec_D0");
  ...
  thrust::inclusive_scan(thrust::device, bg_work_per_key.data(),
                         ...
                         bg_cumulative_work.data());
  cudaDeviceSynchronize();
  GPU_MEMCPY(&p.bg_total_work, bg_cumulative_work.data() + p.num_unique_root_keys - 1,
             sizeof(uint64_t), GPU_DEVICE_TO_HOST);
  if (p.bg_total_work == 0) {
    nvtxRangePop();
    return;
  ...
  kernel_bg_count<<<p.bg_num_blocks, kBlockSize>>>(
      p.d_views.data(), p.root_unique_values_ptr, p.head_root_unique_values_ptr,
      p.num_unique_root_keys, p.num_full_unique_root_keys, p.num_root_keys,
      bg_cumulative_work.data(), p.bg_total_work, p.thread_counts_ptr);
  cudaDeviceSynchronize();

  // BG Step 4: Prefix sum + resize
  thrust::exclusive_scan(thrust::device, p.thread_counts_ptr,
                         p.thread_counts_ptr + p.bg_num_threads + 1, p.thread_counts_ptr, 0,
                         thrust::plus<uint32_t>());
  uint32_t total_count = 0;
  GPU_MEMCPY(&total_count, p.thread_counts_ptr + p.bg_num_threads, sizeof(uint32_t),
             GPU_DEVICE_TO_HOST);
  if (total_count == 0) {
    nvtxRangePop();
    return;
  ...
  auto& bg_dest_0 = get_relation_by_schema<Path, NEW_VER>(db);
  ...
  kernel_bg_materialize<<<p.bg_num_blocks, kBlockSize>>>(
      ...
      p.num_unique_root_keys, p.num_full_unique_root_keys, p.num_root_keys,
      bg_cumulative_work.data(), p.bg_total_work, p.thread_counts_ptr,
      bg_dest_0.template interned_column<0>(), static_cast<semiring_value_t<SR>*>(nullptr),
      bg_dest_0.interned_stride(), bg_old_size_0);
  cudaDeviceSynchronize();
  nvtxRangePop();
}

// Tail-mode fused execution: single kernel, no count/scan phase
void JitRunner_TCRec_D0::execute_fused(DB& db, uint32_t iteration) {
  auto p = setup(db, iteration);
  if (p.num_unique_root_keys == 0)
    return;
  ...
  kernel_bg_fused<<<bg_num_blocks, kBlockSize>>>(
      ...
      p.num_unique_root_keys, p.num_full_unique_root_keys, p.num_root_keys,
      bg_cumulative_work.data(), bg_total_work, dest_rel_0.template interned_column<0>(),
      dest_rel_0.interned_stride(), old_size_0, s_wp_0.data(), capacity, s_of.data());
