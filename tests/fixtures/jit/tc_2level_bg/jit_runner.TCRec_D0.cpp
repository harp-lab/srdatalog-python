...
struct JitRunner_TCRec_D0 {
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
          ...
          hint_hi_4 = (hint_hi_4 <= view_Path_1_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_Path_1_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Path_1_0_DELTA_VER.num_rows_;
          auto h_Path_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Path_1_0_DELTA_VER);
          if (!h_Path_0_root.valid()) continue;
          auto h_Edge_1_root = HandleType(0, view_Edge_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Edge_0_1_FULL_VER);
          if (!h_Edge_1_root.valid()) continue;
          auto y = root_val_2;
        // Nested CartesianJoin: bind x, z from 2 source(s)
        // MIR: (cartesian-join :vars (x z) :sources ((Path :handle 2 :prefix (y)) (Edge :handle 3 :prefix (y)) ))
        ...
        if (!h_Path_2_4.valid() || !h_Edge_3_6.valid()) continue;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_histogram(
    ...
    for (uint32_t bg_hist_key = warp_id; bg_hist_key < num_unique_root_keys; bg_hist_key += num_warps) {
      ...
      uint32_t bg_hhi = view_Path_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - bg_hist_key - 1);
      bg_hhi = (bg_hhi <= view_Path_1_0_DELTA_VER.num_rows_) ? bg_hhi : view_Path_1_0_DELTA_VER.num_rows_;
      bg_hhi = (bg_hhi > bg_hlo) ? bg_hhi : view_Path_1_0_DELTA_VER.num_rows_;
      auto h_Path_0_root = HandleType(bg_hlo, bg_hhi, 0).prefix(bg_hist_root_val, tile, view_Path_1_0_DELTA_VER);
      if (!h_Path_0_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      auto h_Edge_1_root = HandleType(0, view_Edge_0_1_FULL_VER.num_rows_, 0).prefix(bg_hist_root_val, tile, view_Edge_0_1_FULL_VER);
      if (!h_Edge_1_root.valid()) { bg_work_per_key[bg_hist_key] = 0; continue; }
      uint64_t bg_deg = 1;
  ...
  static __global__ void __launch_bounds__(kBlockSize) kernel_bg_count(
    ...
    OutputCtx output_ctx{nullptr, nullptr, 0, 0};
        ...
        for (uint32_t bg_key_idx_1 = bg_key_lo; bg_key_idx_1 < num_unique_root_keys && bg_remaining_begin < bg_remaining_end; bg_key_idx_1++) {
          ...
          uint32_t hint_hi_4 = view_Path_1_0_DELTA_VER.num_rows_ - (num_unique_root_keys - bg_key_idx_1 - 1);
          hint_hi_4 = (hint_hi_4 <= view_Path_1_0_DELTA_VER.num_rows_) ? hint_hi_4 : view_Path_1_0_DELTA_VER.num_rows_;
          hint_hi_4 = (hint_hi_4 > hint_lo_3) ? hint_hi_4 : view_Path_1_0_DELTA_VER.num_rows_;
          auto h_Path_0_root = HandleType(hint_lo_3, hint_hi_4, 0).prefix(root_val_2, tile, view_Path_1_0_DELTA_VER);
          if (!h_Path_0_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
          auto h_Edge_1_root = HandleType(0, view_Edge_0_1_FULL_VER.num_rows_, 0).prefix(root_val_2, tile, view_Edge_0_1_FULL_VER);
          if (!h_Edge_1_root.valid()) { bg_remaining_begin = bg_key_work_end; continue; }
...
JitRunner_TCRec_D0::LaunchParams JitRunner_TCRec_D0::setup(DB& db, uint32_t iteration, GPU_STREAM_T stream) {
  LaunchParams p;
  p.views_vec.reserve(NumSources);

  // Source 0: Path version DELTA_VER
  {
    auto& rel_0 = (iteration == 0) ? get_relation_by_schema<Path, FULL_VER>(db) : get_relation_by_schema<Path, DELTA_VER>(db);
  ...
  auto& first_rel = (iteration == 0) ? get_relation_by_schema<Path, FULL_VER>(db) : get_relation_by_schema<Path, DELTA_VER>(db);
...
void JitRunner_TCRec_D0::launch_materialize(DB& db, LaunchParams& p, uint32_t total_count, GPU_STREAM_T stream) {
