...
struct JitRunner_Triangle {
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_14 = intersect_handles(tile, h_RRel_2_12.iterators(view_RRel_0_1_FULL_VER),
                                            h_SRel_3_13.iterators(view_SRel_0_1_2_FULL_VER));
      for (auto it_15 = intersect_14.begin(); it_15.valid(); it_15.next()) {
        ...
        auto intersect_10 = intersect_handles(tile, h_SRel_4_8.iterators(view_SRel_0_1_2_FULL_VER),
                                              h_TRel_5_9.iterators(view_TRel_1_0_2_FULL_VER));
        for (auto it_11 = intersect_10.begin(); it_11.valid(); it_11.next()) {
          auto z = it_11.value();
          auto positions = it_11.positions();
          auto ch_SRel_4_z =
              h_SRel_4_8.child_range(positions[0], z, tile, view_SRel_0_1_2_FULL_VER);
          auto ch_TRel_5_z =
              h_TRel_5_9.child_range(positions[1], z, tile, view_TRel_1_0_2_FULL_VER);
  ...
  static __global__ void __launch_bounds__(kBlockSize)
    ...
    for (uint32_t y_idx_1 = warp_id; y_idx_1 < num_unique_root_keys; y_idx_1 += num_warps) {
      ...
      auto intersect_26 = intersect_handles(tile, h_RRel_2_24.iterators(view_RRel_0_1_FULL_VER),
                                            h_SRel_3_25.iterators(view_SRel_0_1_2_FULL_VER));
      for (auto it_27 = intersect_26.begin(); it_27.valid(); it_27.next()) {
        ...
        auto intersect_22 = intersect_handles(tile, h_SRel_4_20.iterators(view_SRel_0_1_2_FULL_VER),
                                              h_TRel_5_21.iterators(view_TRel_1_0_2_FULL_VER));
        for (auto it_23 = intersect_22.begin(); it_23.valid(); it_23.next()) {
          auto z = it_23.value();
          auto positions = it_23.positions();
          auto ch_SRel_4_z =
              h_SRel_4_20.child_range(positions[0], z, tile, view_SRel_0_1_2_FULL_VER);
          auto ch_TRel_5_z =
              h_TRel_5_21.child_range(positions[1], z, tile, view_TRel_1_0_2_FULL_VER);
          // Nested CartesianJoin: bind h, f from 2 source(s)
          // MIR: (cartesian-join :vars (h f) :sources ((SRel :handle 6 :prefix (y z)) (TRel :handle
          // 7 :prefix (x z)) ))
          ...
          if (!h_SRel_6_5.valid() || !h_TRel_7_7.valid())
            continue;
            ...
            for (uint32_t t0_base_10 = 0; t0_base_10 < degree_4; t0_base_10 += kCartTileSize) {
              ...
              for (uint32_t _ti = lane_2; _ti < t0_len_12; _ti += group_size_3)
                s_cart[warp_in_block][0][_ti] =
                    view_SRel_0_1_2_FULL_VER.get_value(2, h_SRel_6_5.begin() + t0_base_10 + _ti);
              for (uint32_t t1_base_11 = 0; t1_base_11 < degree_6; t1_base_11 += kCartTileSize) {
                uint32_t t1_len_13 =
                    min(t1_base_11 + (uint32_t)kCartTileSize, degree_6) - t1_base_11;
                for (uint32_t _ti = lane_2; _ti < t1_len_13; _ti += group_size_3)
                  s_cart[warp_in_block][1][_ti] =
                      view_TRel_1_0_2_FULL_VER.get_value(2, h_TRel_7_7.begin() + t1_base_11 + _ti);
                tile.sync();
