...
def build_lsqb_q7_optional() -> Program:
  ...
  M, T_, C, CMT, LKR = (
    Var("m"), Var("t"), Var("c"), Var("comment"), Var("liker"),
  )
  ...
  return Program(
    relations=[has_tag, has_creator, reply_of, likes, has_reply, has_liker,
    ...
    rules=[
      ...
      ).named("Case1Both").with_plan(
        var_order=["m", "comment", "liker", "t", "c"],
      ).with_count(),
      (
        case2(M, T_, C, CMT) <=
        has_tag(M, T_) & has_creator(M, C) & reply_of(CMT, M) & ~has_liker(M)
      ).named("Case2ReplyOnly").with_plan(
        var_order=["m", "comment", "t", "c"],
      ).with_count(),
      (
        case3(M, T_, C, LKR) <=
        has_tag(M, T_) & has_creator(M, C) & likes(LKR, M) & ~has_reply(M)
      ).named("Case3LikeOnly").with_plan(
        var_order=["m", "liker", "t", "c"],
      ).with_count(),
      (
        case4(M, T_, C) <=
        has_tag(M, T_) & has_creator(M, C) & ~has_reply(M) & ~has_liker(M)
      ).named("Case4Neither").with_plan(
        var_order=["m", "t", "c"],
      ).with_count(),
    ],
  )
