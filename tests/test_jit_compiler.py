...
def test_detect_cxx_falls_back_to_clangpp():
  with mock.patch.dict(os.environ, {}, clear=False):
    ...
    with mock.patch("srdatalog.dialects.target.cuda.build.compiler.shutil.which") as w:
...
def test_detect_cxx_raises_when_none_available():
  with mock.patch.dict(os.environ, {}, clear=False):
    os.environ.pop("CXX", None)
    with mock.patch("srdatalog.dialects.target.cuda.build.compiler.shutil.which", return_value=None):
      try:
        _detect_cxx()
...
def test_compile_cpp_cached_path_doesnt_spawn_subprocess():
  with tempfile.TemporaryDirectory() as td:
    ...
    with mock.patch(
      "srdatalog.dialects.target.cuda.build.compiler.subprocess.run",
    ) as run_mock:
