...
def test_prefix_reorder_moves_prefixed_to_front():
  ...
  s1 = mir.ColumnSource(
    rel_name="B", version=Version.FULL, index=[0], prefix_vars=["x"], clause_idx=1
  )
...
def test_prefix_reorder_noop_when_first_already_prefixed():
  ...
  s0 = mir.ColumnSource(
    rel_name="A", version=Version.FULL, index=[0], prefix_vars=["x"], clause_idx=0
  )
...
def test_balanced_scan_pass_converts_column_join_to_positioned_extract():
  ...
  cj_src = mir.ColumnSource(rel_name="C", version=Version.FULL, index=[0], clause_idx=2)
