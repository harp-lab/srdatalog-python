'''Phase E discipline test — `sorted_array` is shipped as a
...
and §5 (built-ins ARE plugins). Companion to `tests/test_core_plugin.py`,
which exercises the F4 framework with synthetic plugins; this file
exercises the framework against the first real built-in dialect to ship
as a plugin.

Three things asserted:

  1. `Compiler.with_default_plugins()` discovers `sorted_array`
  ...
  2. Calling `compiler.register_plugin(register)` twice is a no-op the
     second time (F4 idempotency, exercised against the real
     register callable).
...
from srdatalog.ir.dialects.relation.sorted_array import (
  DIALECT as SORTED_ARRAY_DIALECT,
)
from srdatalog.ir.dialects.relation.sorted_array import (
  register as register_sorted_array,
)
...
def _tc_program() -> Program:
  ...
  `tests/test_default_pipelines_shims.py::_tc_program`. Two rules give
...
def _find_execute_pipelines(node: object) -> list[mir.ExecutePipeline]:
  ...
  `_find_execute_pipelines` in `test_default_pipelines_shims.py`.
  '''
...
# -----------------------------------------------------------------------------
# 1. Plugin discovery — `with_default_plugins()` picks up sorted_array
# -----------------------------------------------------------------------------


def test_sorted_array_is_discovered_by_with_default_plugins() -> None:
  '''`Compiler.with_default_plugins()` walks the
  `srdatalog.dialects` entry-point group; the `sorted_array` entry
  point shipped in this repo's `pyproject.toml` is loaded and
  registers the `relation.sorted_array` dialect.
  ...
  # (`[project.entry-points."srdatalog.dialects"] sorted_array = "..."`)
  # — F4's `_resolve_plugin` records that name in `_plugins_loaded`.
  assert 'sorted_array' in compiler._plugins_loaded, (
    f'expected sorted_array in loaded plugins; got {list(compiler._plugins_loaded)!r}. '
  ...
  dialect = compiler.get_dialect('relation.sorted_array')
  assert dialect is SORTED_ARRAY_DIALECT, (
  ...
  assert compiler._dialects_by_plugin['relation.sorted_array'] == 'sorted_array'
...
def test_register_sorted_array_plugin_idempotent() -> None:
  '''Calling `register_plugin(register_sorted_array)` twice on the
  ...
  (`register.plugin_name = 'sorted_array'`) — this test verifies the
  short-circuit fires for the real register callable, not just the
  synthetic ones in `test_core_plugin.py`.
  '''
  compiler = Compiler()
  compiler.register_plugin(register_sorted_array)
  ...
  assert compiler.get_dialect('relation.sorted_array') is SORTED_ARRAY_DIALECT


def test_register_sorted_array_metadata_matches_entry_point() -> None:
  ...
  '''
  assert register_sorted_array.plugin_name == 'sorted_array'  # type: ignore[attr-defined]
  assert register_sorted_array.provides == ('relation.sorted_array',)  # type: ignore[attr-defined]
  assert register_sorted_array.requires == ()  # type: ignore[attr-defined]
...
def test_compile_kernel_body_byte_equivalent_across_plugin_paths() -> None:
  ...
  compiler_explicit.register_plugin(register_sorted_array)
